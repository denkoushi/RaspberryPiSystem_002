...
- **メイン背景**: `bg-slate-800`（中程度のダーク）
- **カード背景**: `bg-white`（純白、最大コントラスト）
- **セカンダリ背景**: `bg-slate-100`（明るいグレー）

#### アイテム種別の背景色

- **工具（TOOL）**: `bg-blue-500` + `text-white` + `border-blue-700`（2px以上）+ 🔧 アイコン
- **計測機器（MEASURING_INSTRUMENT）**: `bg-purple-600` + `text-white` + `border-purple-800`（2px以上）+ 📏 アイコン
- **吊具（RIGGING_GEAR）**: `bg-orange-500` + `text-white` + `border-orange-700`（2px以上）+ ⚙️ アイコン
...
- **キオスク持出画面** (`/kiosk/tag`): アイテム種別の背景色を適用
- **キオスク返却画面** (`/kiosk`): 持出一覧のアイテムカードに背景色を適用
