...
- 同じ端末が接続リースで採用した同一物理レンチ・同一条件・同一最新設定の確認は、別の丸数字だけでなく別作業ID・別ロットでも引き継ぐ
- 確認を引き継いでもBluetoothは自動接続せず、「同じ締付条件の現物確認を引継ぎ済み・使用開始が必要です」を表示して、作業ごとに`このレンチを使用開始`を必須とする
- 別端末へレンチを移動した場合は移動先で製造番号と現物設定を再確認する。使用中なら約1.2秒の待機、現物チェック、別位置の最終ボタンによる二段階引継ぎを行う
- 接続状態は、確認候補の取得中を「確認状態を読込中」、現物未確認を「現物確認待ち」、確認済みかつ明示開始前を「使用開始待ち」、締付箇所なしを「待機中」と表示する。ローカルagentの未取得状態をPi5全体の空きと誤認させる「使用可能」は表示しない
- 現物確認前は副作用のないローカルagent healthを確認し、実際に到達できない場合だけ「通信断」と表示する。healthのローカルlease snapshotは他端末を含むPi5全体の所有状態として扱わない
- 操作結果と接続メッセージはヘッダー直下の常設固定高領域で差し替え、表示・消去によって手順書と右ペインを上下移動させない
...
- **持ち出し画面**: `/kiosk/instruments/borrow`
  ...
  - 点検画像はジャンル共通で1〜2枚を同時表示（2枚時は並列表示）
  - ジャンル未設定またはジャンル画像未設定の計測機器は持出不可（明示エラー）
  ...
  - OKの場合: 氏名タグUID入力またはNFCスキャンで自動送信（500msデバウンス、ボタン不要）、全項目PASSとしてInspectionRecordを作成  
    ...
    - **2枚目の氏名NFC（2026-04-16 修正）**: `setEmployeeTagUid()` の反映完了を待たず、**読み取った `nfcEvent.uid` をそのまま `handleSubmit` に渡して送信**する。これにより、画面遷移後に氏名タグをスキャンしても React の state 更新タイミングに依存せず持出登録と戻り遷移が発火する。回帰テスト: `KioskInstrumentBorrowPage.nfc.test.tsx`
...
- **返却機能**: ✅ **統合完了**（2025-12-11）。専用タブを削除し、持出画面の持出一覧から返却可能に:
  ...
  - 工具は従来背景を維持。写真持出は **`photoToolDisplayName` があればその値、未付与時は「撮影mode」** を表示（フォールバック定数は `PHOTO_LOAN_CARD_PRIMARY_LABEL`）。端末の設置場所は **値のみ**（`端末場所:` ラベルなし）。「アイテム情報なし」は非表示
...
- **背景色**: `bg-purple-600`（鮮やかな紫、不透明度なし）
- **文字色**: `text-white`（純白、太字推奨）
- **ボーダー**: `border-purple-800`（太め、2px以上）
- **アイコン**: 📏 計測機器アイコンを追加（色だけでなく形状でも識別）

### 適用範囲

- **管理コンソール**: 計測機器一覧ページ (`/admin/tools/measuring-instruments`)
- **キオスク**: 計測機器持出画面 (`/kiosk/instruments/borrow`)
- **キオスク返却画面**: 持出一覧の計測機器カード
- **サイネージ**: 計測機器表示（現状の藍系背景は維持）
