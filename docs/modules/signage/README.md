...
最終更新: 2025-12-24（カードレイアウト最適化、タイトル変更、フォントサイズ調整）
...
- 左右タイトルは共通オフセットで揃える（titleOffsetY目安: 約22px * scale）。
  - 左ペインタイトル: 「持出中アイテム」（2025-12-24修正）
  - 右ペインタイトル: 「ドキュメント」（2025-12-24修正）
  - 右ペインのファイル名: タイトルの右横、右端揃えで表示（2025-12-24修正）
...
#### 工具カード
- **表示項目**（統一順序）:
  1. アイテム名（primaryText、9px、太字、白、左揃え）+ 管理番号/アイテムコード（14px、等幅フォント、白、右揃え、同一行）
  2. 従業員名（secondary、16px、白、左揃え）+ 借出日 + 借出時刻（14px、白、右揃え、スペース区切り、同一行）
  3. 警告（期限超過の場合、「⚠ 期限超過」、14px、白）
- **レイアウト**: 管理番号はアイテム名の右横に表示（2025-12-24修正）

#### 計測機器カード
- **表示項目**（統一順序）:
  1. 名称（primaryText、9px、太字、白、左揃え）+ 管理番号（14px、等幅フォント、白、右揃え、同一行）
  2. 従業員名（secondary、16px、白、左揃え）+ 借出日 + 借出時刻（14px、白、右揃え、スペース区切り、同一行）
  3. 警告（期限超過の場合、「⚠ 期限超過」、14px、白）
- **レイアウト**: 管理番号は名称の右横に表示（2025-12-24修正）

#### 吊具カード
- **表示項目**（統一順序）:
  1. 名称（primaryText、9px、太字、白、左揃え）+ 管理番号/アイテムコード（14px、等幅フォント、白、右揃え、同一行）
  2. 従業員名（secondary、16px、白、左揃え）+ 借出日 + 借出時刻（14px、白、右揃え、スペース区切り、同一行）
  3. 警告（期限超過の場合、「⚠ 期限超過」、14px、白）
- **レイアウト**: 管理番号は名称の右横に表示（2025-12-24修正）

#### 日付と時刻の配置
- **配置**: 従業員名の右横、右揃えで表示（2025-12-24修正）
  - 日付と時刻はスペース区切りで連続表示（例: "12/24 17:30"）
  - 従業員名は左揃え、日時は右揃えで同一行に配置
- **視認性**: 情報密度を向上し、レイアウトを最適化

#### 行間設定
- **フォントサイズに応じた行間**: フォントサイズの1.6-2倍を確保
  - primaryText (9px) → 行間14px
  - secondary (16px) → 行間20px
  - date/time (14px) → 行間20px
- **文字同士が重ならないように調整**
