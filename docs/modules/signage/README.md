...
- **スケジュール画面での選択（2026-04-23）**: `/admin/signage/schedules` の **可視化**スロットでは、ダッシュボードを **`パレット可視化`（`pallet_visualization_board`）と `その他の可視化`** に `<optgroup>` 分類し、ラベルに種別タグ（例: **`パレット可視化`・`計測機器点検`・`renderer:`**）を付与。**無効**なダッシュボードも一覧に出し **`（無効）`** と表示。パレット可視化が **1 件も無い**ときは **`/admin/visualization-dashboards` へのリンク**でプリセット作成を案内（取得中・エラー時は案内を出さない）。実装・デプロイ記録は [KB-355](../../knowledge-base/api.md)・[deployment.md](../../guides/deployment.md) 冒頭。
