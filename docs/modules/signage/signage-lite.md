...
#### 左ペイン（Items On Loan）の変更

1. **レイアウト**:
   - SPLITモードの左ペインを3列から2列に変更
   - サイネージモニタのアスペクト比に合わせてレイアウト調整

2. **タイトル**:
   - 「Tool Management Data」→「Items On Loan」に変更

3. **各アイテムの表示**:
   - **アイテム名**: 左揃え、フォントサイズを半分に（既存仕様に準拠）
   - **管理番号**: アイテム名の右側に表示（スペース付き）
   - **従業員名**: 左揃え
   - **日時**: 従業員名の右側に表示（スペース付き）、日時を連続表示（空白なし）

4. **表示情報の修正**:
   - 各アイテムの表示情報を既存仕様に合わせて修正
...
- **フロントエンド**: `apps/web/src/pages/SignageDisplayPage.tsx`
- **バックエンドレンダラー**: `apps/api/src/services/signage/signage.renderer.ts`

### 実機検証手順

1. **Pi3サイネージクライアントで確認**:
   - SPLITモードで左ペインが2列表示されることを確認
   - 左ペインのタイトルが「Items On Loan」であることを確認
   - 各アイテムの表示が仕様通りであることを確認（アイテム名・管理番号・従業員名・日時の位置とフォントサイズ）
   - 右ペインのタイトルが「Document」で、ファイル名が右側に表示されることを確認
...
- [x] **サイネージUIブラッシュアップ**: 表示領域最大化と既存仕様準拠のUI改善 ✅
