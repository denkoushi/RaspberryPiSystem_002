...
- **検証項目**:
  ...
  - ✅ エラーハンドリングが正常に動作するか（詳細確認完了 - 2025-12-06）
    - ER-001: 無効なJSON入力の処理 → ✅ パス（適切なエラーメッセージを出力）
    - ER-002: 無効なdeploy_targets形式の処理 → ✅ パス（適切なエラーメッセージを出力）
    - ER-003: 未知のターゲットの処理 → ⏳ 実機検証で確認（DEPLOY_EXECUTOR_ENABLE=1が必要）
    - ER-004: デプロイコマンド実行失敗時の処理 → ⏳ 実機検証で確認（実際のデプロイ実行が必要）
