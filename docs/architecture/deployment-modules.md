...
### 7.3 次のステップ
1. **運用パイロットを実施（deploy-all.sh標準手順の試行）**
   - 影響が極小な変更（例: kioskの文言微修正）を用意
   - Pi5で `DEPLOY_EXECUTOR_ENABLE=1 DEPLOY_VERIFIER_ENABLE=1 ./scripts/deploy/deploy-all.sh` を実行
   - ログ: `logs/deploy/*.jsonl` を確認、失敗時は自動ロールバック（`ROLLBACK_ON_FAIL=1`）
2. **Runbook化とドキュメント反映**
   - `deployment.md` に運用チェックリストを追加（環境確認→サービス停止→デプロイ→検証→サービス起動→ログ保管）
   - 異常時ハンドリングを1ページに集約（ロック解除・ロールバック手順）
3. **ブランチ運用ルール**
