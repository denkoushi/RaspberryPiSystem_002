...
- **✅ Location Scope 安全実装フォローアップ（Phase0-4）反映**: 用語固定（scope ownership matrix）を文書化し、`kiosk/production-schedule` の依存注入を `resolveLocationScopeContext` 単一入口へ整理。`due-management-query` の policy 呼び出しを scope 形式へ変更し、`resource-category-policy` に site 解決経路（`siteKey/deviceScopeKey/legacyString/default`）の監視ログを追加。Runbook に fallback 監視項目を追加し、Phase4 の DB 物理分離判定は **No-Go**（即時移行見送り）を ADR で確定。詳細は [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md#location-scope-安全実装フォローアップphase0-42026-03-15) / [ADR-20260315 Phase4 DB Go-No-Go](./decisions/ADR-20260315-location-scope-phase4-db-go-no-go.md) / [location-scope-phase1-audit.md](./plans/location-scope-phase1-audit.md) / [deploy-status-recovery.md](./runbooks/deploy-status-recovery.md) を参照。
