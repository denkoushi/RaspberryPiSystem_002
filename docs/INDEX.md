...
- **✅ 納期管理トリアージ（B第1段階）実装・デプロイ完了・実機検証完了**: 納期管理画面に「今日判断候補（危険/注意/余裕）」を追加し、リーダーが候補を選ぶだけで計画対象を絞れるように改善。拠点共有の選択保存として `ProductionScheduleTriageSelection` を追加。`GET /api/kiosk/production-schedule/due-management/triage` と `PUT /api/kiosk/production-schedule/due-management/triage/selection` を実装。**デプロイ**: Run ID `20260307-154428-18368`、Pi5+Pi4×2（`--limit "server:kiosk"`）。**実機検証**: APIヘルス、deploy-status、トリアージAPI、マイグレーション39件、ProductionScheduleTriageSelection テーブル、backup.json、Pi4サービス稼働を確認。詳細は [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md) / [production-schedule-kiosk-execplan.md](./plans/production-schedule-kiosk-execplan.md) / [EXEC_PLAN.md](../EXEC_PLAN.md) を参照。
