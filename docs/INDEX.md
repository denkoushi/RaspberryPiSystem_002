...
- **✅ 納期管理 B第3次段（全体ランキング可視化・閲覧専用）実装完了**: 左ペインに「全体ランキング（親）」セクションを追加し、`GET /api/kiosk/production-schedule/due-management/global-rank` の結果を可視化。既存「今日の計画順」は「子（全体ランキングから切り出し）」として意味を明確化し、カードに「今日対象 / 対象外 / 引継ぎ」バッジを表示。今回は閲覧専用で、全体ランキングの編集（`PUT /global-rank`）は次段。検証は web unit test・web lint・既存API統合テスト（41件）で回帰なしを確認。詳細は [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md#b第3次段全体ランキング可視化閲覧専用2026-03-07) を参照。
