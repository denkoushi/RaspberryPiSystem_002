...
- **🛠️ P2-4 Web Split (ProductionSchedulePage Part 2) 実装進行中**: `ProductionSchedulePage` から mutation と副作用を分離。`useProductionScheduleMutations`（API mutation実行・pending集約・書き込みクールダウン）と `useMutationFeedback`（備考/納期モーダル状態遷移）を追加し、ページ本体を query + イベント委譲中心へ縮退。`apps/web` の lint/test/build は通過。`pnpm test:e2e:smoke` はローカル PostgreSQL 未起動（`localhost:5432` 到達不可）により環境要因で失敗し、DB起動後に再実行が必要。詳細は [phase2-safe-refactor-backlog.md](./plans/phase2-safe-refactor-backlog.md) / [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md#p2-4-web-split-part-2mutation副作用分離2026-03-13) / [EXEC_PLAN.md](../EXEC_PLAN.md) を参照。
