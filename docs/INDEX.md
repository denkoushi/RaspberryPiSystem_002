...
### 最新アップデート（2026-06-26 · Pi4 5台目 Sessaku-01 追加 · `main@20e70bfa`）

- **第2工場キオスク `raspi4-sessaku-01` を追加**: host/user `raspi4-sessaku-01`、Tailscale `100.115.109.18`、`tag:kiosk`、Expiry disabled、client/status/NFC ID `raspi4-sessaku-01-kiosk1`。Pi5 から Ansible 初回デプロイ成功、`kiosk-browser.service` / `status-agent.timer` / Docker NFC agent を確認済み。USB 接続後は **バーコードリーダー `TMC HIDKeyBoard`** と **NFC `SONY FeliCa RC-S300/P`** を確認し、移動票の **製造 order 番号バーコードの実スキャン成功**、nfc-agent `readerConnected: true` まで確認済み。sudoers は bootstrap `/etc/sudoers.d/900-<host>-ansible` と role 管理 `/etc/sudoers.d/<user>-client-services` を分離。**記録**: [KB-393](./knowledge-base/infrastructure/ansible-deployment.md#kb-393-pi4-sessaku-fifth-kiosk) · [client setup](./guides/client-initial-setup.md) · [deployment](./guides/deployment.md#raspi4-sessaku-01-kiosk-addition-2026-06-26)。

### 最新アップデート（2026-06-26 · キオスク自主検査 HID 移動票スキャン導線 · 全台デプロイ済）

- **自主検査一覧から移動票スキャンで検査記録呼び出し**: Pi4 有線 HID スキャナを Web keyboard wedge として受け、`productNos` 完全一致 + `selfInspectionEligibleOnly=true` で候補検索。資源CD入力済みで候補 1 件なら **検査方法を選択** を自動表示、未指定/複数なら候補選択。順位ボード **検** と共通 `SelfInspectionWorkflowModal`。PR **#626** · merge **`4fd96bcf`** · CI all success · deploy/health `success=true`（6 hosts）。**記録**: [KB-320](./knowledge-base/KB-320-kiosk-part-measurement.md#自主検査-hid-移動票スキャン-2026-06-26) · [KB-390](./knowledge-base/KB-390-kiosk-leaderboard-inspection-workflow.md#shared-workflow-modal-2026-06-26--hid-scan-entry) · [Runbook](./runbooks/kiosk-part-measurement.md#自主検査-hid-移動票スキャン導線-2026-06-26) · [deployment](./guides/deployment.md#kiosk-self-inspection-hid-scan-entry-2026-06-26)。
...
- **生産スケジュール 表示用納期 `effectiveDueDate`・計画列 UI 正本**: [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md#表示用納期-effectiveduedate計画列-ui2026-04-01) · [verification](./guides/verification-checklist.md) · [deployment](./guides/deployment.md)。
...
- **Pi4 5台目（第2工場 Sessaku-01・Ansible `raspi4-sessaku-01`）追加・Tailscale `100.115.109.18` 反映・固定 client key / NFC secret・`deploy-staged --limit` 成功**: 初回 Tailscale apt timeout、bootstrap sudoers 上書きによる `Missing sudo password`、locale warning を切り分け、sudoers を `900-<host>-ansible` と `<user>-client-services` に分離。最終的に `deploy-status` は `isMaintenance=false`、`kiosk-browser.service` / `status-agent.timer` active、`docker-nfc-agent-1` Up、kiosk/NFC HTTP 200 を確認。USB 接続後は `TMC HIDKeyBoard`（HID キーボード型バーコードリーダー）と `SONY FeliCa RC-S300/P` を確認し、移動票の製造 order 番号バーコードの実スキャンも成功。**参照**: [KB-393](./knowledge-base/infrastructure/ansible-deployment.md#kb-393-pi4-sessaku-fifth-kiosk) / [client-initial-setup.md](./guides/client-initial-setup.md) / [deployment.md](./guides/deployment.md#raspi4-sessaku-01-kiosk-addition-2026-06-26)。
