...
- **✅ Location Scope Phase3（scope契約統一 + Flag段階切替）実装・デプロイ・実機検証完了**: `feat/location-scope-phase3-completion` ブランチで、`production-schedule` 系ルートの `resolveLocationScopeContext()` 統一、`due-management-location-scope-adapter.service.ts` 追加、`LOCATION_SCOPE_PHASE3_ENABLED` による段階切替経路を実装。**デプロイ**: Pi5 → raspberrypi4 → raspi4-robodrill01 の順に1台ずつ実行、約45分。**実機検証**: リモート自動チェック全項目合格。`location_scope_phase3_enabled` は初期 `false` のため Phase3新経路は未有効。詳細は [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md#location-scope-phase3scope契約統一--flag段階切替2026-03-15) / [ADR-20260315-Phase3](./decisions/ADR-20260315-location-scope-phase3-flagged-scope-contract.md) / [deploy-status-recovery.md](./runbooks/deploy-status-recovery.md) を参照。
