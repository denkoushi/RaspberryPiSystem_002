...
- **✅ 全端末共有優先順位（Mac対象ロケーション指定）基盤を追加**: 納期管理 `global-rank` API に `targetLocation` / `rankingScope`（`globalShared` / `locationScoped` / `localTemporary`）を拡張し、Mac から対象拠点を明示指定できるように変更。基準順位は `shared-global-rank` に統合し、必要時は TTL 付き一時上書き（localTemporary）を適用可能。移行手順は [mac-target-location-migration.md](./runbooks/mac-target-location-migration.md) を参照。
