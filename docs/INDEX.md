...
- **✅ 納期管理 B第4段階（全体ランキング自動生成・根拠表示）実装完了**: `global-rank` に自動生成エンジンを追加。`GET /proposal` で提案順位とスコア根拠（reasons）を返し、`PUT /auto-generate` で安全ガード（最小件数・差分率・既存尾部保持）付き保存を実行。`GET /explanation/:fseiban` で個別製番の説明を取得可能。UIには「自動生成して保存」ボタンと score/reasons 表示を追加。**検証**: API統合テスト、api/web lint、web test を通過。詳細は [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md#b第4段階全体ランキング自動生成根拠表示2026-03-07) を参照。
