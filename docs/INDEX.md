...
- **✅ Location Scope Phase9（compat呼び出し棚卸し・公開面縮小）実装・品質ゲート・受け入れ検証完了**: `feat/location-scope-phase9-compat-callsite-audit` で `kiosk/shared.ts` の未使用互換公開（`resolveLocationKey` / `resolveCompatLocationScopeContext`）を整理し、標準契約中心へ集約。`csv-import-execution.service.ts` のローカル関数名を `resolveImportMetadataLocationKey` へ改名して責務を明確化。API/DB契約は維持。**検証**: `@raspi-system/api` lint・対象テスト・build、`@raspi-system/web` lint・build を通過。**受け入れ確認**: health / deploy-status（両Pi4）/ 納期管理API群 / Mac向け `targetLocation`（URLエンコード） / signage API / backup.json / migration status / Pi3/Pi4サービスを確認。詳細は [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md#location-scope-phase9compat呼び出し棚卸し公開面縮小2026-03-15) / [EXEC_PLAN.md](../EXEC_PLAN.md) / [deploy-status-recovery.md](./runbooks/deploy-status-recovery.md) を参照。
