...
- **管理コンソール LocalLLM（`/admin/local-llm`）・Runbook トークンローテーション・順次デプロイ・実機検証（2026-03-28）**: `feat/admin-local-llm-ui-and-runbook`。Pi5→Pi4×4 を **`--limit` 1 台ずつ**（Pi3 除外）。**実機**: Tailscale 経由 `health` / 未認証 `local-llm/status` **401** / upstream **`/healthz` 200** / **`/admin/local-llm` 200**。`AdminLayout` の localhost デバッグ `fetch` を除去。**参照**: [local-llm-tailscale-sidecar.md](./runbooks/local-llm-tailscale-sidecar.md)（実機検証メモ）/ [EXEC_PLAN.md](../EXEC_PLAN.md)。
