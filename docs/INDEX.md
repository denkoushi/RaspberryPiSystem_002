...
- **✅ 納期管理UI Phase2（開閉アイコン化・デフォルト閉じ・状態記憶・最下段カード削除）実装完了**: 左ペインの開閉トグルをアイコン化し、3セクションをデフォルト閉じへ変更。開閉状態は `localStorage`（`due-management-section-open`）で永続化。製番登録/削除機能は維持したまま、左ペイン最下段の重複サマリカードを削除。`resolveNextSelectedFseiban` は `sharedHistory` 基準へ更新し、表示構造変更への耐性を強化。詳細は [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md#納期管理ui-phase2開閉アイコン化デフォルト閉じ状態記憶最下段カード削除2026-03-13) を参照。
