...
- **✅ 進捗一覧画面（閲覧専用）を追加**: 登録製番のみを対象に、1画面で `品名 / 納期 / 工程進捗（資源CDのみ）` を可視化するキオスク画面を追加。ルートは `/kiosk/production-schedule/progress-overview`、APIは `GET /api/kiosk/production-schedule/progress-overview`。並び順は納期昇順（同一納期は登録順）、納期未設定は別セクションで表示。更新は5分自動 + 手動更新。運用詳細は [production-schedule-progress-overview.md](./guides/production-schedule-progress-overview.md) を参照。
