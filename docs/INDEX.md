...
- **キオスク沉浸式ヘッダー allowlist 拡張・手動順番上ペイン行（品名を工順直後）**: `usesKioskImmersiveLayout`（[`kioskImmersiveLayoutPolicy.ts`](./knowledge-base/KB-311-kiosk-immersive-header-allowlist.md)）で持出・計測/吊具持出・生産スケジュール本体・進捗一覧・手動順番を対象化。手動順番カードは `presentManualOrderRow` で Row A に品名を含める。**参照**: [KB-311](./knowledge-base/KB-311-kiosk-immersive-header-allowlist.md)。
