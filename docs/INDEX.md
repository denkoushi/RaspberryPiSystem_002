...
- **✅ 手動順番 上ペイン 工順(FKOJUN)のみ表示・Pi5+Pi4×2 順次デプロイ・Phase12 合格・ドキュメント反映・main マージ**: API に `processOrderLabel`（FKOJUN のみ）を追加し、`processingType` との混在を解消。上ペインカードの「工順」列は下ペインと一貫して FKOJUN のみ表示。**ブランチ**: `feat/manual-order-overview-fkojun-display-only`。**デプロイ**: Pi5 → raspberrypi4 → raspi4-robodrill01 のみ（Pi3 除外）・1台ずつ、`--foreground`。**実機検証**: `./scripts/deploy/verify-phase12-real.sh` **PASS 28 / WARN 0 / FAIL 0**（2026-03-23）。**参照**: [KB-297 工順FKOJUN節](./knowledge-base/KB-297-kiosk-due-management-workflow.md#manual-order-overview-fkojun-display-only-2026-03-23) / [deploy-status-recovery.md](./runbooks/deploy-status-recovery.md) / [EXEC_PLAN.md](../EXEC_PLAN.md)。
