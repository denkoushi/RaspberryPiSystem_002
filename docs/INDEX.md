...
- **✅ 納期管理 B第6段階（行単位全体順位スナップショット）Phase 1 実装完了**: 全体ランキングを「製番単位の親順位」から「行単位の通し順位」へ投影する `ProductionScheduleGlobalRowRank` を追加。`global-rank`（manual/auto）および `daily-plan` 保存後に行単位順位を再生成し、`GET /api/kiosk/production-schedule` に `globalRank` を返却。生産スケジュール画面に `全体順位` 列を新設し、既存 `順番` は `資源順番` として維持（編集可能）。**方針**: 順位は先に全体計算し、製番・資源CDは表示時に絞り込み（再ランキングしない）。**検証**: APIユニットテスト、kiosk統合テスト、api lint/build、web lint 通過。詳細は [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md#b第6段階行単位全体順位スナップショット導入phase-12026-03-09) / [ADR-20260308](./decisions/ADR-20260308-due-management-offline-learning-events.md) を参照。
