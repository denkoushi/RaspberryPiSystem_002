...
- **キオスク要領書: 文書番号（候補/確定）・要約候補3・確定要約・フリーワード検索 OR 拡張・管理一覧列・キオスク左一覧表示（`feat/kiosk-documents-doc-number-summary`）・Pi5+Pi4×2 順次デプロイ（Pi3 除外）・Phase12 実機検証・ドキュメント反映・`main` マージ**: Prisma `20260327120000_add_kiosk_document_number_summary`。OCR は候補のみ自動更新、確定は人手。**実機検証**: `./scripts/deploy/verify-phase12-real.sh` **PASS 30 / WARN 0 / FAIL 0**（約22s・2026-03-27）。**参照**: [KB-313](./knowledge-base/KB-313-kiosk-documents.md) / [kiosk-documents.md](./runbooks/kiosk-documents.md) / [knowledge-base/index.md](./knowledge-base/index.md) / [EXEC_PLAN.md](../EXEC_PLAN.md)。
