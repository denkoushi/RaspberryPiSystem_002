...
- **✅ 実績工数列の整合化（推定工数列廃止 + 資源CD手動マッピング）実装完了**: 生産日程CSVに `FSEZOSIJISU` が存在しないため、生産スケジュール/納期管理の `実績推定工数(分)` 表示を廃止し、`実績基準時間(分/個)` + `実績カバー率(%)` に統一。`ActualHoursFeatureResolver` を導入して `strict一致 -> 手動マッピング一致` の順で実績特徴量を探索。`ProductionScheduleResourceCodeMapping` と管理コンソール設定API/UI（`/api/production-schedule-settings/resource-code-mappings`）を追加。詳細は [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md#実績工数列の整合化2026-03-11-追補) / [EXEC_PLAN.md](../EXEC_PLAN.md) を参照。
