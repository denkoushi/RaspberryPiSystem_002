...
- **✅ P2-5 Boundary Guard 実装完了（境界ルール段階強化）**: API/Web に `import/no-restricted-paths` を段階導入し、**APIでは `routes/system <-> routes/kiosk` の横断依存**、**Webでは `features/components/hooks/lib/api/layouts/utils -> pages` の逆依存**を lint で検知可能化。あわせて `apps/api/src/lib/client-key.ts` を新設し、`normalizeClientKey` を routes/plugins 間で共通化。**検証**: `apps/api` / `apps/web` の lint・test・build を通過。詳細は [phase2-safe-refactor-backlog.md](./plans/phase2-safe-refactor-backlog.md) / [EXEC_PLAN.md](../EXEC_PLAN.md) を参照。
