...
- **⏳ 4件の未実施実機検証**: 以下の機能は実装完了しているが、実機検証が未実施です。詳細な検証手順は [guides/verification-checklist.md](./guides/verification-checklist.md) を参照してください。
  - **デプロイプロセス改善機能**: 自動デプロイ前チェック、構造化ログ生成、エラーハンドリング強化を実装完了。**✅ 実機検証成功**（2025-12-27）。試行錯誤の詳細（ansible pingタイムアウト、Pi3メモリ不足、Pi5のGit状態問題など）を標準ドキュメントに記録済み。詳細は [guides/verification-checklist.md#4-デプロイプロセス改善機能の検証](./guides/verification-checklist.md#4-デプロイプロセス改善機能の検証) / [guides/deployment.md](./guides/deployment.md) / [guides/deployment-troubleshooting.md](./guides/deployment-troubleshooting.md) / [knowledge-base/infrastructure.md#kb-098](./knowledge-base/infrastructure.md#kb-098-ansible_ssh_common_argsのrequestttyforceによるansible-pingタイムアウト) を参照。
  - **CSVバックアップ（バックアップ対象管理UI）**: 管理コンソールからバックアップ対象を管理する機能を実装完了。実機検証未実施。詳細は [guides/backup-target-management-verification.md](./guides/backup-target-management-verification.md) / [requirements/backup-target-management-ui.md](./requirements/backup-target-management-ui.md) を参照。
  - **サイネージUIブラッシュアップ**: サイネージモニタの表示領域最大化と既存仕様準拠のUI改善を実装完了。実機検証未実施。詳細は [guides/verification-checklist.md#5-サイネージuiブラッシュアップの検証未実施](./guides/verification-checklist.md#5-サイネージuiブラッシュアップの検証未実施) / [modules/signage/signage-lite.md](./modules/signage/signage-lite.md) を参照。
  - **Gmail添付ファイル連携機能**: PowerAutomateからGmailにCSVファイルを添付ファイルとして送信し、Pi5が自動的にGmailから添付ファイルを取得してインポートする機能を実装完了。実機検証未実施。詳細は [guides/verification-checklist.md#6-gmail添付ファイル連携機能の検証未実施](./guides/verification-checklist.md#6-gmail添付ファイル連携機能の検証未実施) / [guides/gmail-attachment-integration.md](./guides/gmail-attachment-integration.md) / [guides/powerautomate-gmail-integration.md](./guides/powerautomate-gmail-integration.md) を参照。

### 🆕 最新アップデート（2025-12-21）

- **✅ Gmail添付ファイル統合機能実装完了**: PowerAutomateからGmailにCSVファイルを添付ファイルとして送信し、Pi5が自動的にGmailから添付ファイルを取得してインポートする機能を実装完了。Gmail OAuth 2.0認証フロー、Gmail APIクライアント、Gmailストレージプロバイダー、Gmailインポートエンドポイントを実装。CIテストも追加済み（gmail-oauth, gmail-api, gmail-storage, imports-gmail）。ドキュメント作成も完了（Gmail連携ガイド、PowerAutomate側仕様）。**実機検証**: ⏳ 未実施（2025-12-27時点）。詳細は [guides/gmail-attachment-integration.md](./guides/gmail-attachment-integration.md) / [guides/powerautomate-gmail-integration.md](./guides/powerautomate-gmail-integration.md) / [guides/verification-checklist.md#6-gmail添付ファイル連携機能の検証未実施](./guides/verification-checklist.md#6-gmail添付ファイル連携機能の検証未実施) を参照。
...
- **✅ バックアップ対象管理UI実装完了**: 管理コンソールの「バックアップ」タブからバックアップ対象（`targets`）を管理できる機能を実装完了。バックアップ対象の追加・編集・削除、有効/無効切り替え、手動バックアップ実行が可能。`backup.sh`スクリプトと管理コンソールの機能が整合性を保ち、設定ファイル（`backup.json`）の変更が即座に反映される。統合テスト・E2Eテストも実装済み。**実機検証**: ⏳ 未実施（2025-12-27時点）。詳細は [requirements/backup-target-management-ui.md](./requirements/backup-target-management-ui.md) / [guides/backup-target-management-verification.md](./guides/backup-target-management-verification.md) / [guides/backup-and-restore.md](./guides/backup-and-restore.md) / [guides/backup-configuration.md](./guides/backup-configuration.md) / [guides/verification-checklist.md#4-デプロイプロセス改善機能の検証未実施](./guides/verification-checklist.md#4-デプロイプロセス改善機能の検証未実施) を参照。
