...
- **✅ manual-order-overview 割当資源の siteKey 正本優先（旧 deviceScopeKey 行混在時の上ペイン表示欠落を修正）・Pi5+Pi4×2 順次デプロイ・Phase12 合格・ドキュメント反映**: [`due-management-manual-order-overview.service.ts`](../apps/api/src/services/production-schedule/due-management-manual-order-overview.service.ts) に `resolveManualOrderOverviewResourcesForAssignedDevice` を追加し、割当順ごとに **site 正本 → 無ければ端末 slice**。Vitest [`merge-manual-order-resource-assignments.test.ts`](../apps/api/src/services/production-schedule/__tests__/merge-manual-order-resource-assignments.test.ts)。**デプロイ（Pi3 除外）**: ブランチ `feat/sitekey-shared-manual-rank-sync`。**Ansible ログ timestamp**: `20260323-113105`（Pi5）/ `20260323-113715`（raspberrypi4）/ `20260323-114137`（raspi4-robodrill01）。**実機検証**: `./scripts/deploy/verify-phase12-real.sh` **PASS 28 / WARN 0 / FAIL 0**（2026-03-23）。**参照**: [KB-297 manual-order-overview siteKey 優先節](./knowledge-base/KB-297-kiosk-due-management-workflow.md#manual-order-overview-assigned-resource-sitekey-priority-2026-03-23) / [deploy-status-recovery.md](./runbooks/deploy-status-recovery.md) / [EXEC_PLAN.md](../EXEC_PLAN.md)。
