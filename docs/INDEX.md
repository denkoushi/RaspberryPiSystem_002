...
- **✅ 進捗一覧 5列・「納期」「実」ラベル削除・presentation 分割・デプロイ（Pi5・raspberrypi4）・実機検証（部分）・ドキュメント反映・main マージ**: [`progressOverviewPresentation.ts`](../apps/web/src/features/kiosk/productionSchedule/progressOverviewPresentation.ts)、[`ProgressOverviewSeibanCard`](../apps/web/src/components/kiosk/progressOverview/ProgressOverviewSeibanCard.tsx)、[`ProgressOverviewPartRow`](../apps/web/src/components/kiosk/progressOverview/ProgressOverviewPartRow.tsx)。ブランチ `feat/kiosk-progress-overview-five-cols-layout`。**デプロイ Run ID**: `20260322-084633-25240`（Pi5）/ `20260322-085040-1342`（raspberrypi4）。**raspi4-robodrill01** は SSH 未到達のため未デプロイ（復旧後に再実行）。**Phase12**: PASS 26 / WARN 1 / FAIL 1（2026-03-22）。**ローカル**: `NODE_ENV=test POSTGRES_PORT=5432 pnpm test:api` 推奨。**参照**: [KB-297 進捗一覧5列節](./knowledge-base/KB-297-kiosk-due-management-workflow.md#progress-overview-five-cols-layout-2026-03-22) / [deploy-status-recovery.md](./runbooks/deploy-status-recovery.md) / [EXEC_PLAN.md](../EXEC_PLAN.md)。
