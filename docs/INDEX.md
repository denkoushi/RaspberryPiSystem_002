...
- **✅ Location Scope Phase3（scope契約統一 + Flag段階切替）実装完了（未デプロイ）**: `feat/location-scope-phase3-completion` ブランチで、`production-schedule` 系ルートの `resolveLocationScopeContext()` 統一、`due-management-location-scope-adapter.service.ts` 追加、`LOCATION_SCOPE_PHASE3_ENABLED` による段階切替経路を実装。`api/web lint/build` と対象回帰テストを通過。**注意**: フル `api test` はローカルDB未起動時に backup系が失敗するため、DB起動後の再実行が必要。詳細は [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md#location-scope-phase3scope契約統一--flag段階切替2026-03-15) / [ADR-20260315-Phase3](./decisions/ADR-20260315-location-scope-phase3-flagged-scope-contract.md) を参照。
