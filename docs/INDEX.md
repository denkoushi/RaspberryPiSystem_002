...
- **✅ 手動順番 上ペイン 資源CD割り当て（DB・GET/PUT API・モーダル・overview 統合）**: テーブル `ProductionScheduleManualOrderResourceAssignment`、エンドポイント `GET|PUT /api/kiosk/production-schedule/manual-order-resource-assignments`（v2 時のみ・409 `RESOURCE_ALREADY_ASSIGNED`）、`manual-order-overview` で登録端末全件＋割当順マージ。Web はカード **「資源」** と `ManualOrderResourceAssignmentModal`。Phase12 に assignments 検証を追加。ブランチ `feat/manual-order-resource-assignment-ui`。**参照**: [KB-297 資源割り当て節](./knowledge-base/KB-297-kiosk-due-management-workflow.md#manual-order-resource-assignment-2026-03-20) / [deploy-status-recovery.md](./runbooks/deploy-status-recovery.md) / [EXEC_PLAN.md](../EXEC_PLAN.md)。
