...
- **✅ 手動順番の端末単位正規化（deviceScopeKey + siteKey 集計 + Mac targetDeviceScopeKey 必須）**: `ProductionScheduleOrderAssignment.siteKey` を追加し、v2 有効時は order 更新で Mac は `targetDeviceScopeKey` 必須、キオスクは自端末のみ。overview は `siteKey` 必須で `devices[]` 返却、旧サイト単位行は `__legacy_site__`。ロールバックは `KIOSK_MANUAL_ORDER_DEVICE_SCOPE_V2_ENABLED=false` / `VITE_KIOSK_MANUAL_ORDER_DEVICE_SCOPE_V2_ENABLED=false`。詳細は [ADR-20260319-manual-order-device-scope-v2](./decisions/ADR-20260319-manual-order-device-scope-v2.md) を参照。
