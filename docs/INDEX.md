...
- **✅ 納期管理トリアージ B第2最小（今日の計画順）実装**: `ProductionScheduleDailyPlan` / `ProductionScheduleDailyPlanItem` を追加し、`GET|PUT /api/kiosk/production-schedule/due-management/daily-plan` で当日計画順の保存・再表示を実装。納期管理左ペインに「今日の計画順」UI（上下移動・保存）を追加。あわせて、トリアージカード選択が右ペインへ反映されない不具合を修正。API統合テスト（39件）と api/web lint・build を通過。詳細は [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md) / [production-schedule-kiosk-execplan.md](./plans/production-schedule-kiosk-execplan.md) / [EXEC_PLAN.md](../EXEC_PLAN.md) を参照。
