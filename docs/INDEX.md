...
- **✅ FSIGENマスタ導入（資源CD→資源名）とホバー表示対応**: `ProductionScheduleResourceMaster` を追加し、`FSIGENCD + FSIGENMEI` ユニークで資源名称をDB管理。`FSIGENCLASSCD` / `FSIGENGRPCD` も保持。`GET /api/kiosk/production-schedule/resources` は後方互換で `resources: string[]` を維持しつつ `resourceNameMap` を追加。納期管理詳細 `parts[].processes[]` には `resourceNames` を追加。UIは既存備考ホバー方式を流用し、生産スケジュール資源CDボタンと納期管理工程バッジで `title + aria-label` 表示に対応。初回投入は `apps/api/prisma/seeds/dataSIGEN.csv` と `prisma/seed.ts` の upsert で実装。詳細は [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md#資源cd名称マスタ導入とホバー表示2026-03-11) / [EXEC_PLAN.md](../EXEC_PLAN.md) を参照。
