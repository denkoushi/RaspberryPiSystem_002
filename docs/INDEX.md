...
- **✅ P2-3 Web Split (ProductionSchedulePage Part 1) ローカル検証完了**: `ProductionSchedulePage` から派生計算・検索同期・表示を分離し、`displayRowDerivation` / `useProductionScheduleDerivedRows` / `useProductionScheduleQueryParams` / `useSharedSearchHistory` を追加。表示は `ProductionScheduleResourceFilters` / `ProductionScheduleHistoryStrip` / `ProductionScheduleTable` に分割し、Container責務を縮小。**ローカル検証**: `pnpm --filter @raspi-system/web lint/test/build` 通過。**未実施**: デプロイ・実機検証。詳細は [phase2-safe-refactor-backlog.md](./plans/phase2-safe-refactor-backlog.md) / [EXEC_PLAN.md](../EXEC_PLAN.md) を参照。
