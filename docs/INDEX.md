...
- **✅ ロケーション間同期共有化（納期・備考・表面処理）実装完了**: `ProductionScheduleRowNote` / `ProductionScheduleSeibanDueDate` / `ProductionSchedulePartProcessingType` を location 非依存へ移行し、Mac と第2工場で `納期` / `備考` / `表面処理` を同一値として参照できるように変更。競合解決は Last-Write-Wins（`updatedAt` 優先）。API契約は維持し、内部実装のみ shared repository 化。**検証**: API統合テストにロケーション間共有回帰を追加して `49 passed`、`apps/api` / `apps/web` lint 成功。詳細は [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md#ロケーション間同期共有化納期備考表面処理2026-03-11) / [EXEC_PLAN.md](../EXEC_PLAN.md) を参照。
