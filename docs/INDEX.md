...
- **📝 手動順番 上ペイン overview は割当CDのみ・カード Location は site 重複除去・下ペインは絞り込み無し時ホバー展開**: v2 `mergeManualOrderOverviewResourcesWithAssignmentOrder` は割当外集計を載せない。実装は [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md#手動順番-専用ページキオスク追加2026-03-20)（専用ページ節・資源割り当て節）。Runbook: [deploy-status-recovery.md](./runbooks/deploy-status-recovery.md)（「overview 割当のみ・…」行）。
