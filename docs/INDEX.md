...
- **✅ 納期管理 B第6段階（行単位全体順位スナップショット）Phase 1 実装・デプロイ・実機検証完了**: 全体ランキングを「製番単位の親順位」から「行単位の通し順位」へ投影する `ProductionScheduleGlobalRowRank` を追加。`global-rank`（manual/auto）および `daily-plan` 保存後に行単位順位を再生成し、`GET /api/kiosk/production-schedule` に `globalRank` を返却。生産スケジュール画面に `全体順位` 列を新設し、既存 `順番` は `資源順番` として維持（編集可能）。**デプロイ**: 初回 `--limit "server:kiosk"` で Pi5 フェーズ完了後に Pi4 キオスクフェーズでハング（[KB-300](./knowledge-base/infrastructure/ansible-deployment.md#kb-300-pi4デプロイ時のキオスクフェーズハングserverkiosk-並列実行時)）。ハングプロセス停止・ロック解除後、Pi4 を単体で `--limit "raspberrypi4"` / `--limit "raspi4-robodrill01"` により再デプロイし、両端末とも成功（Run ID `20260309-165843-1497`、`20260309-170927-5242`）。**実機検証**: APIヘルス、deploy-status（両Pi4で `isMaintenance: false`）、キオスクAPI、納期管理API（triage・daily-plan・global-rank・global-rank/proposal・global-rank/learning-report）、サイネージAPI、backup.json、マイグレーション（43件）、Pi4/Pi3サービス稼働を確認。詳細は [KB-297](./knowledge-base/KB-297-kiosk-due-management-workflow.md#b第6段階行単位全体順位スナップショット導入phase-12026-03-09) / [deploy-status-recovery.md](./runbooks/deploy-status-recovery.md) を参照。
