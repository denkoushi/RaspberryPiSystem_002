...
- **サイネージAPIがSPLITを返さない現象を追加調査中**: 管理コンソールで左右2ペインを指定しても `/api/signage/content` が常に `contentType: "TOOLS"` を返し、物理サイネージが単一ペインのままになることを確認。セキュリティ実装ではなく `SignageService` のスケジュール判定が要因とみられ、Phase 8タスクに追加しました。詳細は [plans/security-hardening-execplan.md](./plans/security-hardening-execplan.md) および [KB-082](./knowledge-base/infrastructure.md#kb-082-管理コンソールでsplitを指定してもサイネージapiが常にtoolsを返す) を参照してください。
