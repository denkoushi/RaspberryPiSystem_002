...
- Decision: Pre-transaction item discovery for due/resource/rank saves reads only winner rows whose `COALESCE(rowData->>'FSEIBAN', '')` equals the leading logical-key part of the requested row items, instead of every winner row. Rationale: the logical key already starts with the raw FSEIBAN and the predicate matches `csv_dashboard_row_prod_schedule_logical_key_idx`; the in-memory logical-key match is unchanged, so a missed row still fails closed with `STALE_PLANNING_BOARD_ITEM`. Date/author: 2026-09-25, perf/planning-board-save-scope.
