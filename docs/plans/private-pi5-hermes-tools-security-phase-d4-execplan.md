...
- [x] 実機デプロイ（私用 Pi5 のみ）— 2026-05-25 · [KB Phase D4 本番](../knowledge-base/KB-private-pi5-hermes-phase-d4-production.md)
- [x] `HERMES_TOOLS_PHASE=d4` 検証 · browser smoke
- [ ] Discord 回帰（任意）
...
2. `./scripts/private-pi5-hermes/deploy-private-pi5-hermes.sh` → **ok=78**
3. `HERMES_TOOLS_PHASE=d4` で `verify-tools-profile-deploy.sh`（ansible: `copy` + `shell`）→ **OK**
4. `verify-tools-browser-smoke.sh`（`REPO_ROOT` に lib 配置）→ **OK**
5. D3/D2 回帰（任意）— profile deploy で chat/tools Bearer **200** 確認済
