...
- [x] (2026-03-07) **納期管理トリアージ B第2最小（今日の計画順）実装・デプロイ・実機検証完了**: トリアージで選んだ製番を当日の実行順として並べ替え・保存・再表示できるようにした。**実装内容**: `ProductionScheduleDailyPlan` / `ProductionScheduleDailyPlanItem` を追加、`GET|PUT /api/kiosk/production-schedule/due-management/daily-plan` を実装。納期管理左ペインに「今日の計画順（選択済み製番）」UI（上下移動・保存）を追加。トリアージカード選択が右ペインへ反映されない不具合を修正。**デプロイ**: Run ID `20260307-171320-24942`、`state: success`、約12分（Pi5+Pi4×2、`--limit "server:kiosk"`）。**実機検証**: APIヘルス、マイグレーション（40件）、deploy-status、キオスクAPI、納期管理API（triage・daily-plan）、Pi4サービス稼働を確認。操作説明は [KB-297](../knowledge-base/KB-297-kiosk-due-management-workflow.md#b第2最小の操作説明) を参照。
- [x] (2026-03-07) **納期管理 B第3段階（全体ランキング・引継ぎ）実装・デプロイ・実機検証完了**: 日次計画の順序を拠点全体で共有し、前日未完了製番を「引継ぎ」として後段に配置する機能を実装。**実装内容**: `ProductionScheduleGlobalRank` テーブル追加、`GET|PUT /api/kiosk/production-schedule/due-management/global-rank` を実装。未保存時は前日計画＋全体ランキング＋当日トリアージで初期順を生成、トリアージ外は「引継ぎ」として後段に配置。日次計画保存時に global rank へマージ。「今日の計画順」で引継ぎ製番に「引継ぎ」バッジを表示。**デプロイ**: Run ID `20260307-182958-1095`、`state: success`、約11分（Pi5+Pi4×2、`--limit "server:kiosk"`）。**実機検証**: APIヘルス、マイグレーション（41件）、deploy-status、キオスクAPI、納期管理API（triage・daily-plan・global-rank）、Pi4サービス（Pi5経由SSHで確認）を確認。詳細は [KB-297](../knowledge-base/KB-297-kiosk-due-management-workflow.md#b第3段階全体ランキング引継ぎ2026-03-07) を参照。
...
**概要**: 納期管理画面 A修正（左ペインchip化、ソフトキーボード、MH/SH除外、ProductNo列、備考同期、遷移認証、ヘッダ発色修正）は 2026-03-07 にデプロイ・実機検証完了。次の改善候補は運用観察後に検討。

**候補タスク**:
1. **納期管理の実運用継続観察**（優先度: 高）: 現場での利用状況を確認し、パスワード変更・備考同期・工程進捗表示のフィードバックを収集
2. **低レイヤー観測強化のカナリア検証**（優先度: 高）: Pi5 1台カナリアの7日連続合格後に次フェーズ移行（[operation-manual.md](../guides/operation-manual.md) 参照）
3. **納期管理のパスワード管理強化**（優先度: 低）: 現状は shared 単一パスワード。将来的にロケーション別・ロール別の検討
4. **キオスク画面のUI改善**（優先度: 中）: 計測機器持出画面への情報表示追加など（[EXEC_PLAN.md](../../EXEC_PLAN.md) Next Steps 参照）

**関連**: [KB-297](../knowledge-base/KB-297-kiosk-due-management-workflow.md) / [ADR-20260307](../decisions/ADR-20260307-kiosk-due-management-model.md)
