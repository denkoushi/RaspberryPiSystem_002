...
- (2026-04-25 22:03 JST) repo 側で provider 別 runtime control 実装を追加した。`INFERENCE_PROVIDERS_JSON` の各 provider に `runtimeControl` を持てるようにし、Pi5 API の on-demand controller は `document_summary -> dgx_text`、`photo_label -> ubuntu_vlm`、`admin_console_chat -> primary provider` の対応で start/stop を解決するよう更新した。関連 unit test 30 件は成功した。
...
- Observation: Pi5 API の管理 Chat / status の primary provider は、`INFERENCE_PROVIDERS_JSON` の先頭 provider に従う。今回 `dgx_text` を先頭に置いたことで、`LOCAL_LLM_*` と整合した管理系の既定経路を DGX に寄せつつ、`photo_label` だけを Ubuntu へ残せた。
...
1. 今回の議論を正本ドキュメントへ反映した前提で、Pi5 例外反映（`.env` 更新 + `apps/api` 差分同期）を commit / push 後に Ansible 正規経路へ収束させる。
2. Ubuntu 退役判断のため、`photo_label` を Spark 側へ寄せる代表ケース検証（品質・ジョブ統合・Phase12 回帰確認）を最優先の技術検証として進める。`current payload` と Pi5 synthetic はすでに Spark 単一 endpoint で成功したため、次は **代表画像セット** と **ジョブ統合** に主眼を移す。詳細は `docs/plans/dgx-spark-photo-label-validation-plan.md` を正本とする。
3. `vault_api_local_llm_ubuntu_runtime_control_token` を恒久運用に載せるか判断する。Spark への一本化が短期で見込めるなら、Ubuntu 専用 token の正本化は最低限に留める。
4. localhost smoke の Python gateway を、このまま host service で固定するか、後で `tailscale + nginx` 構成へ寄せ直すか判断する。
5. GB10 対応済み SGLang image/build の調査を、Qwen3.5 GGUF ベースライン結果と Spark VLM 検証結果を見て再開するか判断する。
