...
P2-5（Boundary Guard）はデプロイ完了・実機検証完了。`import/no-restricted-paths` を段階強化し、境界違反の新規混入を lint で検知可能にした。**API**: `routes/system <-> routes/kiosk` の横断依存を禁止。**Web**: `features/components/hooks/lib/api/layouts/utils -> pages` の逆依存を禁止。`normalizeClientKey` を `apps/api/src/lib/client-key.ts` へ集約。**デプロイ**: ブランチ `feat/p2-5-boundary-guard`、Pi5 → raspberrypi4 → raspi4-robodrill01 の順に1台ずつ実行、Pi3除外、約20分。**実機検証**: [deploy-status-recovery.md](../runbooks/deploy-status-recovery.md) のチェックリスト全項目合格。**知見**: 境界ルールは `target/from` の向きを誤ると大量誤検知になるため、段階導入時は小さく追加して即lintで確認する。
