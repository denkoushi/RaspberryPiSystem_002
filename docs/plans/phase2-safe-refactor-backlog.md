...
P2-5（Boundary Guard）は実装完了。`import/no-restricted-paths` を段階強化し、境界違反の新規混入を lint で検知可能にした。**API**: `routes/system <-> routes/kiosk` の横断依存を禁止。**Web**: `features/components/hooks/lib/api/layouts/utils -> pages` の逆依存を禁止。あわせて `normalizeClientKey` の重複実装を `apps/api/src/lib/client-key.ts` へ集約し、`routes`/`plugins` の共通利用へ変更。**検証**: `apps/api` / `apps/web` の lint・test・build を通過。**知見**: 境界ルールは `target/from` の向きを誤ると大量誤検知になるため、段階導入時は小さく追加して即lintで確認する。
