...
## P2-2 実装完了（2026-03-12, ローカル検証）

P2-2（auth Route Thin化）は実装完了。`AuthRoleAdminService` / `role-change-policy` / `role-change-alert.service` を新設し、`POST /auth/users/:id/role` の通知理由判定・通知副作用をサービス層へ移譲。ルート層は認可・入力検証・HTTP整形に限定。API契約/認可仕様は不変。

- 追加テスト: `auth-role-admin.service.test.ts` / `role-change-policy.test.ts`（7件）
- 検証: `pnpm --filter @raspi-system/api lint` / `pnpm --filter @raspi-system/api test -- src/services/auth/__tests__/role-change-policy.test.ts src/services/auth/__tests__/auth-role-admin.service.test.ts` / `pnpm --filter @raspi-system/api build` を通過
- トラブルシュート: `pnpm --filter @raspi-system/api test` 全体実行はローカルDB (`localhost:5432`) 未起動で失敗。P2-2範囲外の既存DB依存テスト起因で、変更範囲の回帰は対象テストで確認済み
