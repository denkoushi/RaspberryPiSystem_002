...
- 2026-05-10 late: bridge 実装を **`stackchan_chat_core.py`**（ユースケース）と **`bridge_server.py`**（HTTP I/O）に分離し、**`unittest`** で **`ChatCompletionWorkflow`** の再試行契約を固定。同日、私用 Pi5 へ playbook を再適用し **`stackchan_chat_core.py` の実配備**と **事後ヘルス確認**まで実施（詳細は [private-pi5-stackchan-bridge-deploy.md](../runbooks/private-pi5-stackchan-bridge-deploy.md)）
