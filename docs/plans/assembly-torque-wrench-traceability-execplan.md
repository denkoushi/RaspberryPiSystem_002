...
validation: preview approved; traceability DB/API/agent/infrastructure and Draft PR CI contracts pass through commit d606006d; three normal and five rapid-consecutive CEM3-BTLA frames are sanitized; an external RTL8761BU controller established a stable bond and survived wrench power cycling; the strict parser is production-registered behind exact controller/HID identity, startup-wait, and reconnect contracts; 38 agent and 626 deploy tests, Ruff, root lint, Ansible syntax, disposable Linux image runtime, document audit, and diff checks pass; the expand-only remediation additionally passes all 149 migrations, NOT NULL inspection, REQUIRED session/lot integration, API build, root lint, 626 deploy tests, and document audit in disposable resources; Draft PR #1040 passed Secret scan, CodeQL, API, Web, DB, E2E, both Docker-security jobs, deploy contracts, and aggregate ci-required at commit 033fd760; Milestone 4A CSS-pixel callout and marker-nudge validation passes
open_items: record firmware when available; deploy only under separate authorization; after deployment verify reboot, exact external-controller selection, stable HID link, first-frame capture, rapid input, and the production work screen
---
...
Add `templateId`, structured fastener fields, and capability-group reference to `AssemblyTemplateBolt`. Backfill the template ID through its area and add a template-wide marker unique constraint only after an explicit duplicate check. The migration must abort with an actionable error when duplicates exist and must never renumber historical markers. Keep the legacy lot/session wrench strings NOT NULL; REQUIRED rows use an empty compatibility value while physical identity comes from the new relations. Add confirmation and torque-record audit columns additively so existing rows remain valid.
