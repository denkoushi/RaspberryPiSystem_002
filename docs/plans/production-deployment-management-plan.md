...
1. ✅ **完了**: Macから `./scripts/update-all-clients.sh` を実行すると、全クライアントの更新可否が一覧表示される
2. ✅ **完了**: どのクライアントで更新/エラーが発生したかを `apps/web` もしくは `curl` で即座に追跡できる（Phase 2で実装）
3. ✅ **完了**: ネットワーク断でもクライアントは最後に取得したサイネージ/設定を維持し、復旧後に自動同期する（既存機能）
4. ✅ **完了**: 更新フローとデバッグ手順が `docs/guides/` に手順化され、非専門家でも再現できる

**Phase 1完了日**: 2025-12-01  
**実機テスト**: ✅ 成功（Macからの実行テスト含む）
...
### Phase 1: 一括更新システム（SSH + Ansible） ✅ **完了**（2025-12-01）

**所要時間**: 約2-3日  
**前提条件**: なし  
**実装状況**: ✅ **完了**（2025-12-01）
- Step 1.1: SSH鍵の設定 ✅ 完了
- Step 1.2: Ansibleの導入 ✅ 完了
- Step 1.3: 更新Playbookの作成 ✅ 完了
- Step 1.4: 一括更新スクリプト ✅ 完了
- 実機テスト: ✅ 成功（Macからの実行テスト含む）
...
### Phase 1 テスト ✅ **完了**（2025-12-01）

| テスト項目 | 確認内容 | 合格基準 | 結果 |
|-----------|---------|---------|------|
| SSH接続 | サーバー→クライアント接続 | パスワードなしで接続成功 | ✅ 成功 |
| Ansible ping | 全クライアントへのping | 全台成功 | ✅ 成功 |
| 更新Playbook | git pull、サービス再起動 | エラーなく完了 | ✅ 成功 |
| 一括更新 | Mac→全クライアント更新 | 全台更新成功、結果表示 | ✅ 成功 |
