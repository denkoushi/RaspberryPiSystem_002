...
```text
/memo <life note>
/inbox [list|memo N|remind N|done N|delete N]
/digest [focus]
...
- `life_discord_inbox.py`: Android 共有メニュー等から Discord に届いたリンク/添付名を Life Pilot inbox として保存し、一覧・状態更新・明示参照 context を提供
- `life_interest_digest.py`: NVIDIA DGX Spark Forum / Hermes GitHub / Discord共有Xリンクの安全な日次ダイジェストと好みフィードバック
- `discord_task_bridge_plugin.py`: `life-pilot.policy.yaml` 配備時のみ `/memo` `/inbox` `/interest` `/digest` `/remind` `/recommend` 登録、Life Pilot 単体でも送信先 context を取得
...
2026-06-07 D13.1 reply routing で入口を分離した。Discord 画面からの普通の自由文 DM は Life Pilot が捕捉せず Hermes 通常会話へ流す。Life Pilot が直接テキストで捕捉するのは `1` / `2` / `3`（全角含む）だけ。自由文の Life 返信は `/life-reply <文章>` に限定する。URL/画像/添付/本文なし共有は引き続き inbox 保存 + ack で通常会話へ渡さない。

## D14-D18-life Discord inbox triage / butler hints（2026-06-07 · repo）

ユーザー指定の5項目を一括で実装する。目的は、Discord shared inbox を保存箱から「AI執事が日々扱える材料」にすること。

1. **inbox を朝晩確認へ強く接続**: 朝 check-in は `status=new` / `snoozed` の Discord inbox だけを候補化し、候補に出した item へ `lastSuggestedAt` / `suggestedCount` / `lastSuggestedCheckinId` を保存する。
2. **通常会話との分離を維持**: 通常DMは Hermes chat へ流す。`さっき共有したURL`、`送った画像`、`添付` など明示参照がある時だけ、直近の未処理 inbox context を通常会話プロンプトへ添える。
3. **`/inbox` triage command**: `/inbox` で一覧、`memo N` で Life memo 化、`remind N <日時>` で reminder 化、`done N` / `dismiss N` / `delete N` で未処理から外す。
4. **retention / delete**: `status=deleted` による論理削除と `/inbox prune` による古い行整理を用意する。朝 check-in と通常会話 context は deleted/done/memoed/reminded を読まない。
5. **軽い personalization**: 同じ共有を何度も出し続けないよう、提案回数と最終提案時刻を保存し、未提案 item を優先する。

引き続き、URL を開かない、添付をダウンロードしない、OCR/画像認識をしない、Codex/Cursor・git・deploy・terminal・外部Web・HA へ接続しない。

## D19-life Daily Interest Digest（2026-06-07 · repo）

D19 は「ユーザーが興味ある外部投稿を毎日1回出す」AI執事機能の最小実装。Hermes標準の Skill / Memory / Cron を使えるようにしつつ、実行系は開けない。

対象:

- NVIDIA公式 DGX Spark / GB10 Forum / Announcements RSS
- NousResearch/hermes-agent GitHub releases / issues Atom
- 既存 Discord shared inbox に保存された X リンク

X の自動検索は正規API/OAuthが用意できるまで未接続。ブラウザ巡回やログイン裏スクレイピングは採用しない。

保存先:

```text
/home/hermes/.hermes-life/interest/
  items.jsonl
  feedback.jsonl
  seen.jsonl
  profile.json
...
Local validation / production（2026-06-27 · D19 editorial）:

- focused unittest: **28 OK**
- full Hermes unittest: **260 OK**
- py_compile（D19 digest/editorial/CLI）: OK
- Ansible syntax-check: OK
- PR #862 GitHub Actions / CodeQL / gitleaks / e2e: **pass**
- Production deploy: `PLAY RECAP ok=188 changed=9 failed=0`
- Pi5 dry render: `今日見るなら` / `主筋` / `最新` / 元URL / feedback 行を生成
- Discord `/interest`: editorial 版配信 OK
...
2026-06-07 D13.1-life Discord reply routing 修正後:

- branch: `feat/hermes-life-pilot-reply-routing`
- commit: `c28cc370` `fix(hermes): keep regular Discord chat out of Life replies`
- CI [27076969918](https://github.com/denkoushi/RaspberryPiSystem_002/actions/runs/27076969918): **success**
- `python3 -m pytest scripts/private-pi5-hermes/tests -q`: **210 passed**
- `python3 scripts/private-pi5-hermes/validate_boundary_policy.py --validate-life-pilot`: OK
- `git diff --check`: OK
- Pi5 deploy: `PLAY RECAP ok=177 changed=4 failed=0`
- services: `hermes-life-discord-ui.service` / `hermes-gateway.service` active after restart（2026-06-07 08:53:19 JST）
- 実機E2E: 通常 DM `おはよう` は通常会話、`1` は返信待ち時だけ Life 返信、`/life-reply 今日は眠い` は自由文 Life 返信、画像/URL 共有は `受け取り箱に保存しました。`

2026-06-07 D14-D18-life Discord inbox triage repo実装後:
...
- `git diff --check`: OK
- 追加 focused: `/inbox` slash payload、inbox item status 更新、memo/remind triage、明示参照時のみ通常会話 context 添付、朝 check-in の suggested tracking
