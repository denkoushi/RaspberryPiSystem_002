...
**実装内容**:
1. ✅ Ansibleロールへの分割（`common`, `server`, `client`, `kiosk`, `signage`の5ロール）
2. ✅ 共通タスクのロール化（リポジトリ同期、バックアップ、サービス管理）
3. ✅ ロールの再利用（新メインプレイブック`deploy.yml`の作成）
4. ✅ ロール開発ガイドの作成
...
**実装内容**:
- ✅ 5つのロール（`common`, `server`, `client`, `kiosk`, `signage`）を作成
- ✅ 新メインプレイブック`playbooks/deploy.yml`を作成
- ✅ 既存プレイブック`update-clients.yml`を`deploy.yml`への委譲に変更
- ✅ テンプレートパスの修正（`playbook_dir`使用、`remote_src`追加）
- ✅ `status-agent.conf.j2`を`infrastructure/ansible/templates/`に移動
- ✅ ロール開発ガイド（`docs/guides/ansible-role-development.md`）を作成

**検証状況**:
- ✅ Raspberry Pi 4（キオスク）での実機テスト成功（全タスク成功、エラーなし）
- ✅ Raspberry Pi 3（サイネージ）での実機テスト成功（サイネージ停止状態で実行、全タスク成功）
- ✅ 構文チェック（`--syntax-check`）成功
- ✅ タスク一覧確認（`--list-tasks`）成功

**効果**:
- ✅ コードの再利用性向上（ロール単位で管理可能）
- ✅ メンテナンスの容易化（責務が明確に分離）
- ✅ 拡張性の向上（新規ロール追加が容易）
- ✅ 10台規模への対応準備完了
