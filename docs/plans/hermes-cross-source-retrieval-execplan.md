...
- [x] (2026-09-25 18:55 JST) `HERMES_RETRIEVAL_DENSE_PROVIDER=dgx` is on in Pi5 (maintenance run `20260925-095113-364be0` on release `bb1e5feb`, after fresh release `20260925-094326-703b27`). The Chat is not yet in shop-floor use. A first maintenance-only attempt (`20260925-092153-236514`) stopped at the same-release precondition because main had moved past the running release, and its rescue then failed on an undefined `release_pi5_compose_environment`. No maintenance mutation had run.
...
- Observation (2026-09-25): on Pi5, the follow-ups "テーブルの不適合" and "塗装不良の不適合" after "機械課の最近の不適合２件" returned the same department's newest records. The first suspicion was the dense content gate (`RECENT_CONTENT_DENSE_FRACTION`), but a replay with dense off reproduced the same plans. The planner had appended the previous plan to the JEV request text, so JEV judged content and limit from the previous plan. PR #1490 sends the question alone and passes the previous plan as `relatedHistory`.
  Evidence: a private replay through `createRetrievalAnswering`. Before the fix, both follow-ups planned 機械課 with no content. After it, they plan 機械課 plus テーブル or plus content, and return the no-result sentence. The refine cases "3件にして", "そのうち寸法不良のもの", and "組立課は？" are unchanged. Evaluation gates did not catch this because the stage sets have no multi-turn cases. Milestone 5 needs dialogue cases.
