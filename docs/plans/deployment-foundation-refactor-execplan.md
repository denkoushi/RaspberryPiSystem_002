...
- [x] (2026-07-18 13:31Z) Published the aggregate-preflight and bounded manual Secret-scan corrections at exact head `2f749bb4`. Hosted CI run `29645981896`, CodeQL `29645982881`, and Secret scan `29645984536` all passed. The approved real Pi5 plus StoneBase `--preflight-only` then stopped before release-unit submission with one issue: the current StoneBase checkout lacks the candidate-owned `clients/torque-agent` tree. Independent read-only confirmation found no transient unit, release state, maintenance, checkout, or service mutation.
...
- [x] (2026-07-18 13:31Z) Published the Secret scan range correction and obtained fresh successful `ci-required`, `codeql`, and `gitleaks` evidence at exact head `2f749bb4` before the live read-only preflight.
- [x] (2026-07-18 14:20Z) Replaced per-symptom retry with a full agent-release lifecycle audit. Candidate-owned source trees and playbook artifacts are now validated from the immutable candidate commit on Pi5, while current terminals are checked only for pre-existing host resources. The same audit found and removed the stale final `pcscd.service` health assertion, aligning preflight, Ansible lifecycle, and final health proof on `pcscd.socket` plus `/run/pcscd/pcscd.comm`. Twenty-three focused tests, all 647 deploy Python tests, the client lifecycle and deploy safety contracts, zero-warning root lint, deployment playbook syntax, document audit, Python compilation, shell syntax, and diff hygiene pass. Remaining gates are exact-head hosted checks, one clean real preflight, and the approved standard release.
...
- Observation: repository-owned component trees cannot be a prerequisite of the terminal's outgoing checkout.
  Evidence: real preflight `20260718-133143-5ac407` rejected StoneBase only because `/opt/RaspberryPiSystem_002/clients/torque-agent` is absent at outgoing SHA `3e52d7bb`, while release-only resets the repository to the immutable candidate before the client role checks and writes that directory. Requiring the outgoing tree makes every first release of a new agent impossible even when the candidate is complete.
- Observation: correcting an early lifecycle assertion did not correct the independent final health helper.
  Evidence: `nfc-agent-lifecycle.yml` and aggregate preflight used the accepted socket-activation contract, but `terminal-agent-health-probe.py` still required `pcscd.service` enabled and active. That final probe would have reproduced the same late false rejection after checkout and container startup.
...
- Decision: Separate immutable-candidate artifacts from pre-existing terminal provisioning in the aggregate preflight.
  Rationale: Pi5 already fetches and verifies the exact candidate before terminal probing, so Git `cat-file` can prove every selected profile and enabled agent's source tree, lifecycle task, environment template, Compose file, and Dockerfile without checkout. The terminal probe must instead validate only host-owned prerequisites that the candidate checkout cannot create, such as packages, systemd sockets, Docker, NetworkManager policy, helper parent directories, memory, and disk. Post-checkout release-only assertions remain as defense in depth.
  Date/Author: 2026-07-18 / Codex, required by the user's no-individual-repair rule.

- Decision: Make the final NFC agent proof use the same PC/SC socket authority as preflight and Ansible lifecycle.
  Rationale: Three different release phases must not encode different meanings of healthy. A static safety contract and executable unit tests now reject any reintroduction of `pcscd.service` into the normal release proof.
  Date/Author: 2026-07-18 / Codex.
...
Revision note (2026-07-18): Exact-head hosted CI, CodeQL, and bounded Secret scan succeeded at `2f749bb4`. The first real read-only aggregate preflight then exposed a lifecycle-domain error rather than a missing manual repair: it treated the outgoing terminal checkout as the owner of a new candidate source tree. A complete agent-release audit moved repository artifacts to immutable-candidate Git-object validation, retained only host-owned prerequisites on terminals, and found the independent stale final `pcscd.service` assertion. The final health helper now shares the socket-activation contract with preflight and Ansible. No release retry or direct terminal repair occurred during this correction.
