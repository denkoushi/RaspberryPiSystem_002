...
- [ ] (2026-07-16 06:58Z) Complete the final Pi3 correction from `fix/signage-prestage-race`. The last full-fleet target stopped before checkout when the periodic signage updater consumed the shared maintenance `current.tmp.jpg`; rollback then incorrectly required the never-fetched candidate object even though the repository remained clean at the sealed prior SHA. Run-scoped maintenance image/seal paths are now manifest-owned, and clean pre-checkout rollback no longer requires that absent object. All 557 deploy Python tests and focused signage, lifecycle, entrypoint, and safety contracts pass locally; hosted validation and a minimized recovery run remain.
