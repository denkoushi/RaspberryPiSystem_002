...
- [ ] (2026-07-16 05:01Z) Complete Draft PR #1023 from `fix/interrupted-recovery-preflight`. A read-only aggregate preflight checks every affected host's file, repository, systemd, Docker, receipt, retained-image, and ready-SHA authority before the first host mutation. Repeated runtime restore reconciles actual drift even when a valid restored receipt already exists, while retaining that receipt unchanged. Isolated tests reproduce the real receipt-plus-file-replay sequence and prove same-host, multi-host, and multi-prerequisite reporting. All 555 deploy Python tests and focused shell contracts pass locally; CI run `29472631700`, CodeQL `29472631695`, and gitleaks `29472631718` succeeded, including a 2m17s deploy-contract job. Final-head validation, merge, and one approved production retry remain.
