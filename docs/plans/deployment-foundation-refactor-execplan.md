...
- [ ] Complete and publish PR 6: replace terminal rollback with exact run-manifest ownership, then consolidate the Pi5 migration guard, load evidence, and executor stages.
...
- Observation: Pi5 and terminal rollback still have three owners and no exact run manifest.
  Evidence: coordinator rollback, Ansible `rescue`, and Pi5 lifecycle shell rollback can each mutate state independently. Manifest-only coordinator rollback remains an unfinished PR 6 slice.
...
Local PR 6 currently has four accepted implementation slices. Client lifecycle decisions use the real pre-sync Git revision and choose build, force-recreate without build, or no-build. Deploy-status preserves notice and maintenance behavior while binding ready to the active verification ID and immutable release SHA. Pi5 host configuration is a separate fail-closed stage with exact live environment evidence and safe prior-release recovery. Terminal maintenance now remains until the device proves its rendered artifact, authenticated identity, exact HEAD, display manager, required services, oneshot result, and timers. The accepted slices pass focused and full tests and commit lint hooks; publication remains blocked on exact manifest-only terminal rollback and the consolidated Pi5 executor.
...
Current replacement map at this revision:
    ...
    PR 6:
      ...
      accepted commits -> client lifecycle selection 629933e1; release-bound ready acknowledgement 6a1869c1; Pi5 host convergence and live config evidence b1c98f32; terminal readiness and authenticated health 6e9ad694
      local evidence -> 363 deploy Python tests, 283 Web files / 1,430 tests, isolated PostgreSQL deploy-status 20/20, Blue/Green and deployment shell contracts, Ansible/Python/Bash syntax, lint/type checks, diff check, and post-fix reviews with no slice-local P1/P2
      accepted host and terminal slices -> Pi5 host-config-only convergence, stopped-container genre rescue, exact live API environment/unset evidence, active-run-bound structural expired-handoff recovery, cycle-bound Kiosk/signage readiness, authenticated identity, display/service/timer evidence, expanded failure/cancel contracts
      remaining -> exact run-manifest-only terminal rollback with controller-owned legacy-signage readiness, consolidated migration/load/Pi5 executor behavior
