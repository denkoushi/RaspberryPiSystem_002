...
Pre-deployment rehearsal completed (PR #1400): the actual source readers, existing DGX business model and Docker service processed six candidate reviews, all passing. The catalogue grew from eight to nine canonical answers. Protected results stayed at 19/26 correct outcomes and zero wrong offers (missing answers are separate); this run added coverage outside the fixed sample rather than proving an accuracy increase. All 8,611 sources refreshed. The first full cycle peaked at 1,350,647,808 bytes under the 1400 MiB limit. A deliberately invalid candidate produced nine wrong protected answers and was rejected, retaining the current catalogue. A separate sealed seed combines the previously validated aliases with that new answer: nine answers, 21 reviewed wordings, 22/26 correct outcomes (12/16 eligible questions plus 10/10 out-of-scope guards), zero wrong offers. No protected questions were sent to the nightly generator.

The first hosted CI pass identified only new-component preparation issues in the completed failing jobs: a nonempty disabled-token placeholder, rollback environment/call-order contracts, and fixable vulnerabilities in the Python base image/build tools. The placeholder and rollback implementation were aligned with existing contracts; the image now upgrades the four affected Debian packages and removes pip/setuptools/wheel using the existing Python-agent runtime pattern. The two failing release-contract checks and production-secret structure check passed locally after those changes. Broad CI and actual Pi rollout remain the next evidence boundary.
