...
The runtime rehearsal storage allowlist was extended with both new mounts. Container path provisioning, Mac development bind mounts and the existing encrypted DR source policy now include the same two directories; a focused storage contract checks these boundaries together.
