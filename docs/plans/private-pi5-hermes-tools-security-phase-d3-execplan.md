...
- [x] 実機デプロイ（私用 Pi5 のみ）— 2026-05-25 · [KB Phase D3 本番](../knowledge-base/KB-private-pi5-hermes-phase-d3-production.md)
- [x] 実機検証 `HERMES_TOOLS_PHASE=d3` PASS
- [ ] Discord 回帰（任意）
...
2. `./scripts/private-pi5-hermes/deploy-private-pi5-hermes.sh` — **ok=67**
3. `HERMES_TOOLS_PHASE=d3` で `verify-tools-profile-deploy.sh`（ansible: `copy` + `shell`）→ **OK**
4. `verify-tools-web-smoke.sh`（任意）— Pi5 curl **OK** · `validate_url` は repo 上で実施
5. `verify-tools-file-smoke.sh`（D2 回帰）→ **OK**

詳細: [KB Phase D3 本番](../knowledge-base/KB-private-pi5-hermes-phase-d3-production.md)。
