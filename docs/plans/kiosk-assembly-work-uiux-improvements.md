...
2026-07-27に、本承認版がfeature branchから配備された一方で`main`へ未統合だったため、
後続のmain配備で旧UIへ戻ったことが判明した。本計画の完了履歴は変更せず、履歴統合、
現行矩形／共通マーカー／NFC／新viewerとの競合解消、main-only再配備は
[`assembly-approved-uiux-regression-recovery-execplan.md`](./assembly-approved-uiux-regression-recovery-execplan.md)
を後継正本とする。原因と再発防止は
[`KB-402`](../knowledge-base/KB-402-assembly-approved-uiux-feature-branch-overwrite.md)
に記録する。
