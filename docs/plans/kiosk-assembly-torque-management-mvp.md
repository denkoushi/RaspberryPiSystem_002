...
validation: prior deployed MVP evidence below; 2026-07-17 traceability preview and disposable-Postgres/API/agent/infrastructure checks passed; 2026-07-18 assembly callout parity plus torque-input presentation unit/E2E/browser checks passed without DB changes; CEM3-BTLA parser promotion remains gated
...
- The 2026-07-18 Milestone 4B Web refinement gives the torque operator pane compact two-line conditions, one-row LEGACY entry or REQUIRED agent readiness, content-width workflow controls, and a larger scrollable recent history. A pure chronological presentation selector keeps each marker's number visible and distinguishes waiting (ring only), complete (`✓`), NG retry (red `×`), and rejected agent input (amber `×` / `未受付`) without changing event audit meaning or session transitions.
