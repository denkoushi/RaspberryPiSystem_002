...
Migration `20260717070000_assembly_torque_wrench_traceability` adds the torque-wrench domain, template traceability mode and structured fields, template-wide `(templateId, markerNo)` uniqueness, confirmations, and nullable audit snapshots. It backfills existing templates as `LEGACY`, preserves the NOT NULL legacy free-text wrench columns (REQUIRED rows store an empty compatibility value), preserves old record values, and aborts rather than renumbering when historical cross-area marker duplicates exist.
