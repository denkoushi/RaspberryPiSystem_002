...
- 2026-06-29 local investigation follow-up: branch `chore/leaderboard-labor-metadata-perf-split` adds `laborMetadata` `processChangeResidualContext` subphases (`generationTokenInitial`, `residualMaterialization`, `generationTokenRefresh`) so the remaining `requestTotal` fixed cost can be separated before choosing an optimization. Local validation: focused API/Web tests, API lint/build, temporary Docker `pgvector/pgvector:pg15` migration, labor-metadata integration, and read-only EXPLAIN PASS. The temporary container was removed.

**API follow-up: laborMetadata residual timing split（2026-06-29 · PR #868 deployed）**:

- Implementation branch: `chore/leaderboard-labor-metadata-perf-split`; commit `7ab36259` (`perf(kiosk): instrument labor metadata context timing`); PR #868; merge commit `75a49e3c`. This is measurement-only: no optimization, SQL branch, cache/in-flight dedupe, Web behavior change, or API shape change.
- Diagnostics added: when `LEADERBOARD_BOARD_PERF_LOG=true`, `endpoint: "laborMetadata"` now emits `processChangeResidualContext` plus `generationTokenInitial`, `residualMaterialization`, and revision-only `generationTokenRefresh` subphases. `attachLabor` uses the refreshed generation token only when materialization observes a newer mail revision. Sink failures remain non-fatal.
- CI/deploy: PR #868 CI, CodeQL, and gitleaks succeeded. Deploy run `20260629-094618-6253` succeeded for Pi5 plus all five Pi4 kiosk clients with `failed=0`, `unreachable=0`, summary success, and exit `0`. Phase12 real verification after deploy passed (`PASS=45`, `WARN=0`, `FAIL=0`). Perf logging was enabled temporarily on Pi5 for measurement and then restored OFF; Pi5 `.env` and API container env no longer contain `LEADERBOARD_BOARD_PERF_LOG`, API health is `status: ok`, and `api`/`db` are healthy with `web` up.
- Real Pi4 remeasurement used physical Pi4 clients via Pi5 SSH. Each client performed `leaderboard-board includeLabor=false -> labor-metadata POST`, kept first board labor at `0`, sent `80` displayed row ids, returned HTTP 200 and `80` metadata rows. Client-side samples: kensakuMain metadata `3.078s` then `1.956s` (`77/80` non-zero, board rows `517`; first board GET had an outlier `23.790s`, second `3.662s`); RoboDrill01 metadata `2.997s` then `1.873s` (`80/80` non-zero, board rows `266`); FJV60/80 metadata `3.390s` then `1.825s` (`77/80` non-zero, board rows `278`).
