...
### Phase 8（モジュール間契約テストと依存検証）【これから】

**目的**: API/Web 間の契約ずれや依存崩れを早期検知し、統合を安全にする。

**タスク**:
1. 契約テストの追加  
   - shared-types を用いた API/Web のスナップショット型検証または zod スキーマ一致チェックを追加  
   - API レスポンスのキー/型が shared-types と一致することをテスト
2. 依存・互換性チェック  
   - API 公開インターフェースの破壊的変更を検出する backward-compat チェックを追加（型差分の検出）
3. E2Eスモーク拡張  
   - `e2e/smoke` に契約整合性チェックを1ケース追加（基本レスポンスのキー/型の存在確認）
4. CI確認  
   - 上記テストをCIに組み込み、安定通過を確認
