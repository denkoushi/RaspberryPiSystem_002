...
## 9. Lint準拠の自動強制（2025-12-07追加）

### 実装内容

**仕組み上、必ずlint準拠のコードが生成される**ようになっています：
...
2. **CI/CD** (`.github/workflows/ci.yml`):
   ...
   - `pnpm lint --max-warnings=0`で実行される
   - 失敗すると後続のジョブ（`e2e-smoke`、`docker-build`）は実行されない
   - lint違反があればCIが失敗し、マージできない

3. **開発ガイド** (`docs/guides/development.md`):
   - lint準拠の自動強制について明記
   - 開発フローにlintチェック手順を追加

### 効果

- **CIでは必ずlint準拠が強制される**: lint違反があればマージできない（仕組み上、忘れることができない）
- **ローカル開発時にも自動チェック**: pre-commitフックにより、コミット前に自動的にlintチェックが実行される
- **デプロイ標準手順のように忘れられることがない**: 仕組み上、lint違反があればコミット・マージできないため、「気をつける」だけの状態ではない