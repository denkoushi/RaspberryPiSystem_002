...
- [ ] (新規) インターネット公開時のDDoS/ブルートフォース緩和  
  - Fastifyレイヤーでレート制限を再導入（管理画面/APIのデフォルト制限、ログイン系はより厳格）  
  - Caddyレイヤーでの簡易レート制限/同時接続上限を検討（可能なら適用）  
  - テスト: 許可IPから通常操作が429にならないこと、連続アクセスで429が返ることを確認  
- [ ] (新規) ログ長期保持/外部保存の検討  
  - セキュリティログの保持期間を延長（例: 12週→52週）し、容量影響を確認  
  - 必要に応じて外部ストレージ/リモート転送を検討  
  - テスト: ログローテーション後も新規ログが書けること、容量逼迫がないことを確認  
- [ ] (新規) インシデント対応手順の明文化・演習  
  - 侵入/マルウェア検知時の初動・封じ込め・復旧の手順を整理し、運用手順に追加  
  - 最低年1回の机上演習/手順レビューを計画  
  - テスト: 手順通りにアラート→封じ込め→復旧まで模擬実行し、所要時間を記録  
