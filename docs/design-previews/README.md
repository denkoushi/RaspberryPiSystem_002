...
| [signage-leader-order-cards-kiosk-aligned-preview.html](./signage-leader-order-cards-kiosk-aligned-preview.html) | **サイネージ FULL: 順位ボード資源CDカード（提案）** — **キオスク最新**（`LeaderOrderResourceCard` / 製番左縁24色・クラスタ行・行下チップ・閲覧専用）に合わせた 4×2 静的モック。**実装前** |
