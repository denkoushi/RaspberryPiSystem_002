...
| [kiosk-inspection-drawing-library-retire-and-keypad-preview.html](./kiosk-inspection-drawing-library-retire-and-keypad-preview.html) | **検査図面ライブラリ v2 初期案（旧）** — 上バーの数字テンキーと行操作のBefore/After。現行repo実装は**図面名の全件サーバー数字検索**・40件超過案内・履歴直後の**無効ON/OFF**（無効化済み項目の表示切替、行の無効操作は常時表示）へ更新中。正確な操作確認は本番共通DEVプレビューを使用。[Plan](../plans/kiosk-inspection-drawing-server-digit-search-retire-mode.md) |
