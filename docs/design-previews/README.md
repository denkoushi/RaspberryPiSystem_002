...
原則として本番アプリに接続していない **モック画面** です。例外として、生成済みキオスク取説は本番Reactルートの決定的キャプチャーを内包します。
...
| [signage-leader-order-cards-kiosk-aligned-preview.html](./signage-leader-order-cards-kiosk-aligned-preview.html) | **サイネージ FULL: 順位ボード資源CDカード** — **キオスク最新＋コンパクト＋未完のみ**（製番左縁24色・クラスタ行・行下チップ・工順/顧客名なし・機種名10字・完了行非表示）静的モック（レイアウトは **4×2 見本**・本番は **5×2・max10**）。**本番**: コンパクト `20260521-114344-436`・**5×2** `20260521-131417-25249`（Pi5 のみ・[KB-335](../knowledge-base/infrastructure/signage.md#kb-335-キオスク順位ボード資源cdカードkiosk_leader_order_cardsサイネージ-jpeg)） |
...
| [kiosk-assembly-work-session-operator-layout-preview.html](./kiosk-assembly-work-session-operator-layout-preview.html) | **組立作業セッション（オペレータ）** — Before/After。タイトル「組立作業」・1行ヘッダー・テンプレ/Excel 削除・左見出し統合で手順書拡大。[ADR-20260709](../decisions/ADR-20260709-assembly-work-session-operator-layout.md) |
| [kiosk-assembly-home-table-layout-preview.html](./kiosk-assembly-home-table-layout-preview.html) | **組立トップ（/kiosk/assembly）3ペイン** — Before=カード / After=検査図面ライブラリ同系のコンパクト表。登録済みロット・仕掛中・完了を高密度化。右のロット登録は現状維持。**実装反映済（Web）** · HEAD **`a78a36d5`** · Pi5+StoneBase 反映済 · [Plan](../plans/kiosk-assembly-home-table-layout.md) · [ADR Decision 6](../decisions/ADR-20260707-assembly-kiosk-record-approval-and-ui-consistency.md) · [Deployment](../archive/deployments/2026-07.md#kiosk-assembly-home-table-layout-2026-07-09) |
| [kiosk-assembly-home-table-dense-3col-preview.html](./kiosk-assembly-home-table-dense-3col-preview.html) | **組立トップ密度改善案** — 左を**横3列**・行高≈**0.75**（操作は min-h-11）・ロット／仕掛／完了は **▸/▾ 開閉（既定閉じ・案A）**。Before=現行全幅表 / After=クリック可。**実装対象の静的モック** |
| [kiosk-assembly-home-unit-cards-preview.html](./kiosk-assembly-home-unit-cards-preview.html) | **組立トップ（/kiosk/assembly）個体別3ペイン** — 上部KPI、`着手前`/`仕掛中`/`完了・承認` のS/N単位カード。閉じたカードは製番・S/N・進捗+機種名の1.5行、詳細・操作は開いたときだけ。カード内状態ラベルなし。右のロット登録は現状維持。 |
| [kiosk-inspection-drawing-library-retire-and-keypad-preview.html](./kiosk-inspection-drawing-library-retire-and-keypad-preview.html) | **検査図面ライブラリ v2 初期案（旧）** — 上バーの数字テンキーと行操作のBefore/After。現行repo実装は**図面名の全件サーバー数字検索**・40件超過案内・履歴直後の**無効ON/OFF**（無効化済み項目の表示切替、行の無効操作は常時表示）へ更新済み。正確な操作確認は本番共通DEVプレビューを使用。[Plan](../plans/kiosk-inspection-drawing-server-digit-search-retire-mode.md) |
...
| [kiosk-self-inspection-record-approval-dense-preview.html](./kiosk-self-inspection-record-approval-dense-preview.html) | **検査記録確認・高密度レイアウト案** — 説明文削除、左カードは製番＋半角機種名を主表示、詳細メタを2行化、重複する汎用検査員ボタンを削除、点検2列化・測定値2倍・鮮明な規格外行・横スクロールなし。**実装前・静的** |
...
| [kiosk-inspection-drawing-edit-existing-sop.html](./kiosk-inspection-drawing-edit-existing-sop.html) | **生成済み・直接編集禁止** — 本番Reactルートから固定Playwrightで生成した検査図面取説。10シート・45操作をシート別の実画面状態で生成し、丸数字は対象DOMの右下角へ結び付ける。選択したカードだけを必須は橙実線、任意は控えめなスレート破線で対象へ接続する。`pnpm kiosk-sop:generate` で画像・HTML・manifestを同時更新する。**正本**: [semantic target correction ExecPlan](../plans/kiosk-sop-semantic-target-correction-execplan.md) · [ADR](../decisions/ADR-20260801-kiosk-sop-generated-from-production-ui.md) |
| [kiosk-inspection-drawing-dimension-tolerance-sop-1page.html](./kiosk-inspection-drawing-dimension-tolerance-sop-1page.html) | **検査図面 寸法・公差 SOP（A4横1枚・レイアウト見本）** — 6手順のコアのみ。見た目/step-railの型。現場の既存編集正本ではない。**正本**: [Plan](../plans/kiosk-inspection-drawing-dimension-tolerance-sop-1page.md) · [ADR](../decisions/ADR-20260728-inspection-drawing-sop-step-rail.md) |
...
| [kiosk-shelf-master-edit-dialog-compact-preview.html](./kiosk-shelf-master-edit-dialog-compact-preview.html) | **棚マスタ・区画「編集」Dialog（コンパクト案）** — モーダル上限 920px / 92dvh、map 26rem 正方形・スクロールなし、dock 縦スクロールで棚番パイ・保存まで到達。**本番反映済み**（`fix/kiosk-shelf-master-zone-dialog-compact` · **`2e73aeed`**） |
| [kiosk-shelf-master-9grid-view-edit-popup-preview.html](./kiosk-shelf-master-9grid-view-edit-popup-preview.html) | **棚マスタ — 9マス俯瞰＋編集ポップアップ（B採用・旧案）** — 本番は Dialog + 常時 9 マス俯瞰（[KB-382](../knowledge-base/KB-382-mobile-placement-shelf-layout-master.md)） |
| [kiosk-shelf-master-9grid-edit-popup-ux-preview.html](./kiosk-shelf-master-9grid-edit-popup-ux-preview.html) | **棚マスタ — 9マス編集 Dialog UX 改善案** — 4 列ドック・**確定**（保存統合）・**リセット**・**選択解除**（区画のみ）。**本番反映済み（Web）**: **`ca45c479`**·**Pi5 先行** Detach **`20260524-123432-32158`**·Pi4×4 は未（2026-05-24）·[KB-382 §ドック UX](../knowledge-base/KB-382-mobile-placement-shelf-layout-master.md#layout-editor-dock-confirm-reset-2026-05)·[deployment §ドック](../archive/deployments/2026-05.md#kiosk-shelf-layout-editor-dock-confirm-reset-2026-05-24) |
| [kiosk-shelf-master-9grid-fullscreen-preview.html](./kiosk-shelf-master-9grid-fullscreen-preview.html) | **棚マスタ — 9マス全面（工場全体）** — ミニ 3×3 常時表示の俯瞰 UI 参考 |
| [kiosk-shelf-master-cell-flow-preview.html](./kiosk-shelf-master-cell-flow-preview.html) | **棚マスタ・編集 Dialog（マス中心フロー案）** — 左右並列・タブなし。空マス→部品置き場→Piチップを**同一マス選択でドック入替**。**未採用案の静的モック（クリック可）**（本番は Dialog + 9マス俯瞰・[KB-382](../knowledge-base/KB-382-mobile-placement-shelf-layout-master.md)） |
| [kiosk-shelf-master-zero2w-dialog-preview.html](./kiosk-shelf-master-zero2w-dialog-preview.html) | **Zero2W 担当棚（Dialog 統合前の検討）** — 本番は編集 Dialog 右の棚番パイ（`9a1af348` 以降） |
| [kiosk-shelf-master-zero2w-dialog-a-chips-preview.html](./kiosk-shelf-master-zero2w-dialog-a-chips-preview.html) | Zero2W 割当 UI 案 A（チップ列）·検討用 |
| [kiosk-shelf-master-zero2w-dialog-b-map-preview.html](./kiosk-shelf-master-zero2w-dialog-b-map-preview.html) | Zero2W 割当 UI 案 B（地図主導）·検討用 |
