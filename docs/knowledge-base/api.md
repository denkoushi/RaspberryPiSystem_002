...
- Control Target に **`experiment-lab`** が追加された
- `private-comfyui` と `experiment-lab` は、`DGX_RESOURCE_*_RUNTIME_*` が揃っているときのみ **`EXECUTE_TARGET_ACTION`** で POST 起停
- **`SET_POLICY`** に **`applyWorkloadChanges`** があり、GUI チェックで **ワークロード調停**が走る
...
- **Control Targets / `EXECUTE_TARGET_ACTION`**: 実行可否は `overview.targets[].capabilities`。補助起停・モード自動調停は [KB-365](./KB-365-dgx-resource-phase3-workload-orchestration.md)・[ADR-20260502](../decisions/ADR-20260502-dgx-resource-control-targets.md)・[dgx-system-prod-local-llm.md](../runbooks/dgx-system-prod-local-llm.md) を参照（旧本番のみ gateway 書込だった経緯あり）
...
- **自動表示**（可視化データソースで `date` を指定しない／加工機で `findDailyInspectionSummaries` に `date` なし）のときのみ:
  ...
  - ~~集計窓は半開区間 **`[業務日 JST 09:00, 翌業務日 JST 09:00)`**~~（**2026-04-17 追補**: 下記のとおり **暦日窓 + 前日スナップショット上限**へ変更。旧 9:00〜9:00 窓は早朝のイベントで「前日」表示が崩れるため廃止）。
- **明示的な `date`（YYYY-MM-DD）指定**がある場合は従来どおり **暦日 0:00〜24:00（JST）** の窓（互換維持）。

**追補（2026-04-17・本番反映済み）**:
- **問題**: 業務日ラベルは前日でも、集計が **9:00〜9:00** のままだと **当日 0:00〜9:00 のイベント**が前日カードに混ざる。加工機点検は **点検結果「未使用」** が誤って異常扱いになることがあった。
- **修正**（ブランチ **`feat/signage-snapshot-fix`**・コミット **`e1025de2`**）:
  - **加工機 `findDailyInspectionSummaries`（`date` 未指定）**: `resolveJstSignageBusinessDate` でラベル日を決め、CSV/点検の集計窓は **`resolveTokyoDayRange(そのラベル日)`**（暦日 0:00〜24:00 JST）。
  - **計測機器持出 `today_jst`**: 同じく暦日窓＋**前日ラベル表示中は集計上限を前日 24:00 JST（`snapshotUpperBoundUtc`）**に固定し、当日早朝の持出・返却で前日表示が変わらないようにする。
  - **分類**: 点検結果に **`未使用`** が含まれる場合は **異常に加算しない**（`classifyInspectionResult`）。
- **本番デプロイ**: `raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01` → **`raspberrypi3`（最後に単独・プレフライト準拠）**・Detach `20260417-092831-28647` → `20260417-094134-12068` → `20260417-094603-4958` → `20260417-094927-30738` → `20260417-100845-9831` → `20260417-101235-32147`・Phase12 **PASS 43 / WARN 0 / FAIL 0**。
...
- **仕様**: レンダラー `measuring_instrument_loan_inspection` で画像上の表題行を **1行（タイトル＋日付）**に統合し、カード本体の描画高を拡大。行の並びは `sortRowsForDisplay`（`row-priority.ts`）：**貸出中がありを上**、同帯は **貸出件数の降順**、同数は **従業員氏名の昇順**。グリッドは **4列固定**、カード高は内容に応じて **可変**（`card-layout.ts`）。1ページ（想定 1080p キャンバス）に収まらない場合は **警告テキスト**を同画像内に描画。機名トークンは `instrument-name-text.ts`（長い名称の折り返しと省略）で整形。
...
- **仕様**: シングルマシン領域で、**左ペイン**は `illustrationUrl` 解決成功時 **`<image>`**（`preserveAspectRatio="xMidYMid meet"`）・未登録/失敗時は従来の **「機械イラスト」** テキスト。各スロット左は **同梱 PNG**（`assets/pallet-card-thumbnail.png`）を data URI 化して表示、**読み込み失敗**時は **線画 `palletBoardFixtureInnerSvg`**。全スロット **`clipPath`** 内にカード地・サムネ・**`<text>`** を収め、長文は **概算文字数＋`…`**（`pallet-board-svg-text.ts`）。
- **実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（本セッション **約 58s**）。**手動目視（残）**: 管理 **`/admin/signage/preview`** で当該スケジュールの **1 台専用レイアウト**（パレット可視化）を確認（イラスト・サムネ・長い FHIN/機種名の省略）。
- **トラブルシュート**: 左イラストが常にプレースホルダのまま → `illustrationUrl` 欠如、または `readIllustration` 失敗（**未マウント再作成で実ファイル喪失**した場合は **再アップロード**（上記 **「追補（2026-04-22・`pallet-machine-illustrations` ストレージ永続化 / Pi5 `api`）」**）と同系）。**固定サムネが出ない** → `getPalletCardThumbnailDataUri()` が **`dist/.../pallet-board/assets` を解決**できていない（`build` の **assets コピー**要確認）。隣と文字が重なる → **本変更前**の SVG を疑い、**`d01eb79c` 以降の `api` イメージ**か確認。

**追補（2026-04-25・パレット可視化ボード スロット幾何・下段4明細 全幅・API のみ・Pi5 のみ）**:
- ブランチ **`feat/pallet-board-slot-geometry-and-fullwidth-details`**・代表 **`b67476da`**（`computePalletSlotCardLayout`・`pallet-board-slot-card-layout.ts`・`pallet-board-single-layout.ts`・Vitest）。
- **本番**: **`raspberrypi5` のみ**・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/pallet-board-slot-geometry-and-fullwidth-details infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260425-133302-18334`**（**`failed=0` / `unreachable=0` / exit `0`**）。**Pi3 不要**（JPEG は Pi5 `api`）。
- **仕様**: カード内の **機種1行**は従来どおり右ペイン基準の `bodyX`／`machineBaselineY`。**下段4明細**（製番・品番・品名・着手日）は **`fullWidthX`** 起点で **カード内全幅**に配置し、長文は **`maxFullWidthChars` 相当の `ellipsize`**。幾何は **`pallet-board-slot-card-layout.ts`** に集約し、`renderSlot` は純粋関数の出力を `<text>` に流し込むだけに薄い。
- **実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **59s**）。**手動目視（推奨）**: 管理 **`/admin/signage/preview`** で **パレット可視化**・1台専用レイアウトの **下段4行の左揃え全幅**。
- **知見（テスト）**: フィクスチャ SVG に **ネストした `</g>`** があるため、スロット全体を `<g>…</g>` で切り出すと **先に誤マッチ**し得る。下段4行の **`x` 位置の回帰**は **出力 SVG 末尾4つの `<text x="…">`** で検証（単一1スロット想定。`pallet-board-single-layout.test.ts`）。
- **トラブルシュート**: デプロイ前 **未追跡ファイル**は [KB-200](./infrastructure/ansible-deployment.md#kb-200-デプロイ標準手順のfail-fastチェック追加とデタッチ実行ログ追尾機能) どおり **commit** か **`git stash push -u`**。
...
### [KB-262] 加工機点検状況サイネージのカードレイアウト変更と背景色改善

**日付**: 2026-02-14

**事象**:
- テーブルレイアウトでは49件の加工機を表示できない（フォントサイズを大きくすると全件が表示されない）
- 異常・正常の件数バッジの背景色がカードの背景色からはみ出る
- 49件すべてを表示する必要があった

**要因**:
- テーブルレイアウトでは行高さとフォントサイズの制約により、49件を1画面に収めることが困難だった
- バッジの背景色（`rect`要素）がカードの境界を超えて描画されていた
- カードレイアウトへの変更により、より多くのアイテムを表示できるようになったが、バッジの背景色設定に問題があった

**有効だった対策**:
- ✅ テーブルレイアウトからカードレイアウト（4列グリッド）へ変更
- ✅ カード背景色の条件分岐を実装:
  - 未使用: 全体背景色 + 枠線のみ表示
  - 正常1件以上・異常0件: 青（`#2d4a7a`）
  - 異常1件以上: 赤（`#8c1d18`）
- ✅ 異常・正常の件数バッジの背景色を`transparent`に変更（はみ出し対策）
- ✅ 管理番号40033をフィルタリングして非表示
- ✅ 正常/異常ラベルと件数の間にスペース（4px*scale）を追加
- ✅ 青色を明るく変更（`#1e3a5f` → `#2d4a7a`）

**トラブルシューティング**:
- カードレイアウトで49件が表示されない問題: 4列固定グリッドと高さ計算を調整し、最大表示件数を動的に計算
- バッジ背景色のはみ出し: `rect`要素の`fill`を`transparent`に変更して解決
- カード内の要素バランス: 垂直方向のセンタリングと`dominant-baseline="middle"`を適用
- 正常/異常の順序: 正常を上、異常を下に変更

**検証**:
- ローカル: `pnpm test` 成功（`apps/api`: 543 passed / 7 skipped）
- CI:
  - Run `aceb900` 成功（カードレイアウト変更）
  - Run `2d3a462` 成功（青色変更・スペース追加）
  - Run `cc6241d` 成功（背景色透明化）
- デプロイ:
  - runId `20260214-192352-16908` 成功（カードレイアウト、ok=120, changed=4, failed=0）
  - runId `20260214-205412-4936` 成功（青色変更・スペース追加、ok=120, changed=4, failed=0）
  - runId `20260214-213624-19294` 成功（背景色透明化、ok=120, changed=4, failed=0）
- 実機: 49件すべてが4列グリッドで表示され、カード背景色が状態に応じて正しく発色することを確認。バッジの背景色がはみ出さないことを確認
...
2. **Pi5側**: `downloadAllWithMetadata`メソッドで180件のメールを一度に処理する際、短時間で大量のAPI呼び出しが発生
   ...
   - **比喩**: 180件のメールを処理するのは、180人の人に同時に電話をかけるようなもの。Gmail APIは「1秒あたり250回まで」という制限があるため、短時間で大量のリクエストを送ると制限に達する
3. **Gmail APIのレート制限**: User-rate limitは250 queries per second per user
   - **重要なポイント**: レート制限は「1秒あたりのリクエスト数」だけでなく、「短時間での大量リクエスト」でも発生する
...
**解決状況**: ✅ **実装完了・復旧完了**（2026-02-15〜2026-02-16）、✅ **フェーズ1実機検証完了**（2026-02-24）
...
**フェーズ1実機検証完了（2026-02-24）**:
- **429再突入防止機能**: 手動実行時に「Gmail API is rate limited; deferred until 2026-02-23T23:03:49.435Z」が返され、クールダウン処理が正常に動作することを確認
- **GmailRateLimitState**: 429発生時にクールダウン状態が記録され、再突入が防止されることを確認
- **単一オーケストレータ**: 3つのGmailスケジュールが同じ時刻（`*/10 * * * 0,1,2,3,4,5,6`）に発火する設定だが、システムは正常に動作
- **PROCESSING自動解消**: PROCESSING状態の履歴が0件で、古いPROCESSING状態は解消済み
- **検証方法**: Tailscale経由でAPI接続し、ヘルスチェック・スケジュール設定・インポート履歴・手動実行を確認
- **検証結果**: フェーズ1の主要機能が正常に動作していることを確認

**復旧作業（2026-02-16）**:
- **事象**: CSV自動取得が停止し、管理コンソールから手動実行しても「既に取得済み」と表示される
- **原因分析（連鎖的な問題の流れ）**:
  1. **第1段階: Gmail 429エラーの発生**
     - Gmail 429エラーが発生すると、`Retry-After`ヘッダーに従って約15分待機する必要がある
     - CSVインポート1回の実行が長時間化（待機→再試行→失敗/再試行…）
     - **比喩**: レストランで「15分待ってください」と言われて待っている状態
  2. **第2段階: PROCESSING状態の残留**
     - その途中でAPI再起動/中断が入ると、DBの`CsvImportHistory`に`PROCESSING`（処理中）状態が残留
     - **比喩**: レストランで注文したが、途中で店が閉店してしまい、「注文中」のまま残ってしまった状態
  3. **第3段階: 重複実行防止機能によるブロック**
     - スケジューラーの重複実行防止機能（`runningImports` Set）により、`PROCESSING`状態のスケジュールは「既に実行中」としてスキップされる
     - **比喩**: 「まだ注文処理中です」と表示されているため、新しい注文を受け付けられない状態
  4. **第4段階: 同時発火によるバーストアクセス**
     - 複数のスケジュールが同時刻（`*/10`、つまり毎時0分、10分、20分、30分、40分、50分）に発火
     - 3つのスケジュールが同時にGmail APIを呼び出すため、バーストアクセスが発生
     - 429エラーが連鎖的に発生し、さらに多くの`PROCESSING`状態が残留
     - **比喩**: 3つのレストランが同時に同じ食材を大量に注文し、食材店が「在庫切れ、15分待ってください」と言い続ける状態
- **実施した復旧対策**:
  1. ✅ **cronスケジュールの分散**: `/opt/RaspberryPiSystem_002/config/backup.json`の該当3スケジュールを同時発火しないように分単位でずらし（例: `15,25,35,45,55` / `18,28,38,48,58` / `21,31,41,51`）
  2. ✅ **リトライ回数の抑制**: 各スケジュールに`retryConfig.maxRetries=0`を付与し、1回の実行が何時間もロックし続けないように抑制
  3. ✅ **PROCESSING履歴のクリーンアップ**: API再起動で中断された古い`PROCESSING`履歴を`FAILED`に確定（「実行中扱い」の詰まりを除去）
  4. ✅ **APIコンテナ再起動**: スケジュール再登録と`runningImports` Setのクリア
- **復旧確認（2026-02-16）**:
  - `csv-import-加工機_日常点検結果`: ✅ **COMPLETED**（実際に添付CSVのダウンロード処理が進行して完了するログを確認）
  - `csv-import-measuringinstrumentloans`: ✅ **COMPLETED**
  - `csv-import-productionschedule_mishima_grinding`: ✅ **COMPLETED**（大きい添付を複数処理して完了を確認）
  - **実機検証結果**: ✅ **正常動作**（Gmailのメールが正常に処理されゴミ箱へ移動している）

**学んだこと（復旧作業から）**:
- Gmail 429エラーが発生すると、`Retry-After`ヘッダーに従って約15分待機する必要がある
- 複数のスケジュールが同時刻に発火すると、Gmail APIへのバーストアクセスが発生し、429エラーが連鎖する
- `PROCESSING`状態の履歴がDBに残ると、スケジューラーの重複実行防止機能により、新しい実行がスキップされる
- cronスケジュールを分単位で分散させることで、同時発火を回避できる
- `retryConfig.maxRetries=0`により、1回の実行が長時間ロックし続けることを防げる

**今後の運用**:
- 未読メールが溜まっている場合は、次のスケジュールでも継続して取り込みが走り、`PROCESSING`が一定時間出る（バックログ解消中）
- 必要に応じてPowerAutomate側で「未読を溜めない（送信頻度/まとめ送信）」対策を実施

**新しいCSV取得スケジュールを作成する際のガイドライン**:
- ⚠️ **重要**: 既存のスケジュールと分単位でバッティングしないようにする必要がある
...
- **推奨パターン**:
  - **10分間隔の場合**: 既存の分（`15,18,21,25,28,31,35,38,41,45,48,51,55,58`）を避け、空いている分を選択
    ...
    - 例: `5,15,25,35,45,55`（⚠️ 既存の`15,25,35,45,55`とバッティングするため不可）
  - **5分間隔の場合**: 既存の分を避け、空いている分を選択
    - 例: `0,5,10,15,20,25,30,35,40,45,50,55`（⚠️ 既存の`15,25,35,45,55`とバッティングするため不可）
    - 例: `1,6,11,16,21,26,31,36,41,46,51,56`（⚠️ 既存の`21,31,41,51`とバッティングするため不可）
...
**UI改善（2026-02-16）**:
- ✅ **CSV自動取得スケジュールのcron表示UI改善**: 分のリスト形式（`15,25,35,45,55`等）を読みやすく表示
- ✅ **規則的な間隔の自動検出**: 10分間隔などの規則的な間隔は`intervalMinutes`モードとして扱い、UIで編集可能に
...
### [KB-269] 生産スケジュールprogress別テーブル化（CSV取り込み時の上書きリスク回避）
...
**Symptoms**:
- CSV取り込み実行後、キオスク画面で設定した完了状態が消える可能性がある
- 完了状態が`rowData.progress='完了'`として保存され、CSV取り込み時の`rowData`更新で上書きされる

**Investigation**:
- **仮説1**: CSV取り込み時に`rowData`全体を上書きしている（CONFIRMED）
  - `CsvDashboardIngestor`のDEDUP更新処理で`rowData`が完全置換される
- **仮説2**: 完了状態だけ別テーブルに分離すれば解決する（CONFIRMED）
  - 他のユーザー操作データと同様に、`ProductionScheduleProgress`テーブルを新設
  - `rowData.progress`はAPIレスポンスで合成し、フロントエンド互換性を維持

**Root cause**:
- 完了状態が`rowData`（JSONB）に保存されていたため、CSV取り込み時の`rowData`更新で上書きされる
- 他のユーザー操作データは既に別テーブル化されていたが、完了状態のみ`rowData`に残っていた

**Fix**:
- ✅ **`ProductionScheduleProgress`テーブルを新設**:
...
- ✅ **既存データの移行**:
  - マイグレーションSQLで`rowData.progress='完了'`の行を新テーブルへ移行
  - `ON CONFLICT DO UPDATE`で安全に適用
- ✅ **APIサービスの更新**:
  - `completeProductionScheduleRow`: 新テーブルへの`upsert`に変更、レスポンスで`rowData.progress`を合成
  - `listProductionScheduleRows`: `LEFT JOIN`で新テーブルから完了状態を取得、`rowData.progress`を合成
  - `fetchSeibanProgressRows`: `LEFT JOIN`で新テーブルから完了状態を集計
- ✅ **テスト更新**:
  - 統合テストの`beforeEach`/`afterAll`で新テーブルのクリーンアップを追加
  - ユニットテストのPrismaモックに`productionScheduleProgress`を追加

**Prevention**:
- CSV取り込み（DEDUP更新）が走っても完了状態が保持される
- ユーザー操作データは原則別テーブルで管理し、`rowData`への依存を避ける
- マイグレーションSQLで既存データの移行を確実に実行

**実装の詳細**:
```prisma
model ProductionScheduleProgress {
  csvDashboardRowId String         @id
  csvDashboardId    String
  isCompleted       Boolean        @default(false)
  createdAt         DateTime       @default(now())
  updatedAt         DateTime       @updatedAt

  csvDashboardRow   CsvDashboardRow @relation(fields: [csvDashboardRowId], references: [id], onDelete: Cascade)

  @@index([csvDashboardId, isCompleted])
}
```

**トラブルシューティング**:
- **マイグレーション未適用エラー**: `relation "ProductionScheduleProgress" does not exist`
  - 原因: マイグレーション適用前にテストを実行
  - 解決: `pnpm prisma migrate deploy`でマイグレーションを適用
- **テストアサーション失敗**: `rowData.progress`の期待値不一致
  - 原因: テストのseedで`rowData.progress`を直接設定していたが、新テーブルが真実になった
  - 解決: `beforeEach`で`ProductionScheduleProgress`レコードを`upsert`するように変更

**検証**:
- **ローカル検証**: `pnpm --filter @raspi-system/api test`成功、`pnpm lint`成功
- **CI実行**: GitHub Actions Run ID `22178638075`成功（全ジョブ成功）
- **デプロイ結果**: Pi5でデプロイ成功（runId `20260219-200946-13664`, `state: success`, `exitCode: 0`）
- **実機検証結果**（2026-02-19）:
  - ✅ APIヘルスチェック: `status: degraded`（メモリ高負荷）だがDBは`ok`、APIは正常稼働
  - ✅ マイグレーション状態: `Database schema is up to date!`（33 migrations found）
  - ✅ テーブル存在確認: `ProductionScheduleProgress`テーブルが存在（223件の完了状態レコード）
  - ✅ 完了トグル動作: `PUT /api/kiosk/production-schedule/:rowId/complete`を2回実行し、DB `isCompleted`が`true → false → true`に変化、API返却`rowData.progress`が`'' → '完了'`に変化することを確認
  - ✅ 一覧取得: `GET /api/kiosk/production-schedule?q=A`が正常応答し、`rowData.progress`が含まれることを確認
  - ✅ 履歴進捗: `GET /api/kiosk/production-schedule/history-progress`が正常応答し、`history`と`progressBySeiban`を返すことを確認

**学んだこと**:
- ユーザー操作データはCSVデータと分離し、別テーブルで管理することで、CSV取り込み時の上書きリスクを回避できる
- APIレスポンスで`rowData.progress`を合成することで、フロントエンドの互換性を維持しながら、バックエンドのデータ構造を改善できる
- マイグレーションSQLで既存データの移行を確実に実行することで、データ損失を防げる
- KB-268の対策（フロントエンドのrefetch抑止、バックエンドのworker分離）とは衝突しない

**関連ファイル**:
- `apps/api/prisma/schema.prisma`（`ProductionScheduleProgress`モデル追加）
- `apps/api/prisma/migrations/20260219120000_add_production_schedule_progress/migration.sql`（テーブル作成とデータ移行）
- `apps/api/src/services/production-schedule/production-schedule-command.service.ts`（完了トグルAPI更新）
- `apps/api/src/services/production-schedule/production-schedule-query.service.ts`（一覧取得API更新）
- `apps/api/src/services/production-schedule/seiban-progress.service.ts`（進捗集計API更新）
