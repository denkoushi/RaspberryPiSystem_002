...
- **Status**: active（2026-06-07 · D13 Discord shared inbox + reply routing **`c28cc370` Pi5 deploy / 実機 E2E 完了** · D14-D18 inbox triage **`f6917d16` Pi5 deploy 完了** · D19 Daily Interest Digest **Pi5 deploy 完了**）
- **Scope**: 私用 Pi5 Hermes · Discord Life Pilot のみ（業務 Pi5 / Pi4 / `update-all-clients.sh` 対象外）
- **Branch / HEAD（再開用）**: `feat/hermes-life-pilot-reply-routing` @ **working tree after `f6917d16`** — D19 Daily Interest Digest implementation
...
### 私用 Pi5 deploy（D19 · Daily Interest Digest · 2026-06-07）
...
| `hermes-life-discord-ui.service` | `active (running)` |
| deploy summary | `life_interest_digest_enabled=True` / `life_interest_digest_active=True` |
| 手動 `sudo systemctl start hermes-life-interest-digest.service` | service 自体は正常終了。ただし `sent=0 skipped_missing_channel=1` |

**今回の追加知見**:

- `private_pi5_hermes_life_interest_digest_enabled: true` を fragment に入れた状態で標準 deploy すれば、`hermes-life-interest-digest.timer` は自動で enable/start される。
- 日次 dispatch の即時送信先は、固定 `private_pi5_hermes_life_interest_digest_channel_id` または直近の Life Pilot Discord context が必要。どちらも無い状態では service は失敗せず、`skipped_missing_channel=1` で終了する。
...
1. **D19 client-side E2E** — `/interest` の Discord 実表示、`like/save/more` 反応、固定 channel または直近 context 解決後の日次自動配信は未確認
2. **D14-D18 Discord/Android client-side E2E** — `/inbox` / 通常DM `さっき共有したリンク...` / Android 画像共有 / URL共有を実端末で未確認
3. **D12 Obsidian Syncthing 運用は任意化** — 手数が重い場合は Discord shared inbox を主経路にする（D13 が主経路候補）
4. **follow-up 自由入力 modal の目視 E2E** — button `やる` は確認済み。同一 check-in が `answered` 後は再試行不可のため別セッションで確認
5. **retention / export / delete** — `~/.hermes-life` の保持・削除ポリシー未設計
6. **Codex/Cursor worker** — 1 task = 1 worktree/branch · 別 HOME/token · 承認境界（D6+、Life Pilot から直接解放しない）
7. **LLM ベース推薦** — 現状は deterministic suggestion のみ
8. **数日運用の体感評価** — proactive 朝晩・reminder/follow-up 通知の頻度/文言調整
