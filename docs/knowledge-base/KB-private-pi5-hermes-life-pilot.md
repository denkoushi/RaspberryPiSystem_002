...
`/interest` と `hermes-life-interest-digest.timer` を追加し、AI執事が毎日1回出す「気になる投稿」ダイジェストの土台を実装した。初期対象は、NVIDIA公式 DGX Spark / GB10 Forum/Announcements RSS、NousResearch/hermes-agent GitHub releases/issues Atom、既存 Discord shared inbox の X リンク。X 自動検索は正規認証が用意できるまで未接続。

| 操作 | 処理 |
|------|------|
| `/interest` / `/interest refresh` | フィードとDiscord共有Xリンクを収集し、上位5件を提示 |
...
保存先は `/home/hermes/.hermes-life/interest/`。`items.jsonl` は title/URL/短いsnippet/sourceのみ、`feedback.jsonl` は反応履歴、`seen.jsonl` は重複抑止、`profile.json` は短期/中期の重み。外部投稿は常に `untrusted=true` として扱い、本文全文保存・添付DL・OCR・コード実行・Codex/Cursor・git・deploy・terminalには接続しない。
