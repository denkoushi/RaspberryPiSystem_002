...
- **Status**: active（2026-06-07 · D13 Discord shared inbox + reply routing **`c28cc370` Pi5 deploy / 実機 E2E 完了** · D14-D18 inbox triage **`f6917d16` Pi5 deploy 完了** · D19 Daily Interest Digest **Pi5 deploy / 送信先固定化完了** · 日次 timer E2E は **2026-06-08 08:10 JST 待ち**）
- **Scope**: 私用 Pi5 Hermes · Discord Life Pilot のみ（業務 Pi5 / Pi4 / `update-all-clients.sh` 対象外）
- **Branch / HEAD（再開用）**: `main` @ **`1534bef6`** — PR [#414](https://github.com/denkoushi/RaspberryPiSystem_002/pull/414) `fix(hermes): preserve interest digest service arguments`
...
| 種類 | 内容 |
|------|------|
| slash | `/memo` `/inbox` `/interest` `/digest` `/remind` `/recommend` |
| slash（補助） | `/life-reply` — button/modal が使えないときの fallback |
| proactive check-in | 朝は `hermes-life-proactive-morning.timer` が有用候補ありの時だけ送信。夜は `private_pi5_hermes_life_proactive_evening_enabled: true` の時だけ opt-in |
| follow-up | 朝 `夕方にもう一度` → `followups.jsonl` に pending · `hermes-life-followup.timer`（既定 5 分）が due 時に **1回だけ** 再確認 |
| button | 朝 `これをやる` / `夕方にもう一度` / `今日は外す`、follow-up `やる` / `明日に回す` / `外す`、夜 opt-in 時は `終わった` / `明日に回す` / `メモだけ残す` + `自由入力` modal |
...
朝 check-in は、有用候補がある時だけ短く送る。最近の memo に疲れ・眠い・しんどい等の低エネルギー signal があれば軽めに聞き、未処理が多い場合は責めずに1つだけ聞く。直近3日で `selectedOption=2` になった候補は `carried_forward` として翌朝もう一度出す（今日までの日時つき reminder がある場合はそちらを優先）。空候補や単なる最近メモだけでは送らない。
...
Obsidian が有用候補として選ばれた時だけ、朝 check-in の `補足:` に snippet / 添付名を出す。Obsidian の新着メモに疲れ・眠い等の signal があれば D11 briefing の `lowEnergy` に反映する。
...
Discord 共有メモが有用候補として選ばれた時だけ、朝 check-in の `補足:` に共有メモを出す。候補優先順は、今日までの日時つき reminder → carried_forward → **Discord 共有メモ** → Obsidian 新着 → 日時なし reminder → 次の日時つき reminder → 最近の memo。最近の memo だけの場合は定期送信しない。
...
### D19 Daily Interest Digest（repo · 2026-06-07）

`/interest` と `hermes-life-interest-digest.timer` を追加し、AI執事が毎日1回出す「気になる投稿」ダイジェストの土台を実装した。初期対象は、NVIDIA公式 DGX Spark / GB10 Forum/Announcements RSS、NousResearch/hermes-agent GitHub releases/issues Atom、既存 Discord shared inbox の X リンク。追加 source として、環境変数で明示した場合だけ Open-Meteo の日次天気と Brave Search API のWeb検索結果を取り込める。X 自動検索は正規認証が用意できるまで未接続。

| 操作 | 処理 |
|------|------|
| `/interest` / `/interest refresh` | フィード、設定済み天気/検索、Discord共有Xリンクを収集し、上位5件を提示 |
| `/interest search <query>` | Brave Search API が有効な時だけ一回検索し、結果を番号付き feedback 対象にする |
| `/interest like N` / `save N` / `later N` | source/topicの優先度を上げる |
| `/interest dismiss N` | source/topicの優先度を下げる |
| `/interest more <topic>` / `less <topic>` | 明示的に増減したい話題を保存 |
| `/interest profile` | ローカル興味profileとMemory昇格候補を表示 |

保存先は `/home/hermes/.hermes-life/interest/`。`items.jsonl` は title/URL/短いsnippet/sourceのみ、`feedback.jsonl` は反応履歴、`seen.jsonl` は重複抑止、`profile.json` は短期/中期の重み。天気/API検索結果を含む外部入力は常に `untrusted=true` として扱い、本文全文保存・添付DL・OCR・コード実行・Codex/Cursor・git・deploy・terminalには接続しない。

Hermes標準機能として、`daily-interest-digest` skill を `~/.hermes/skills/research/daily-interest-digest/` へ配備する。`private_pi5_hermes_life_interest_digest_enabled: true` の時だけ chat profile の `memory` / `skills` / `cronjob` toolset を安全枠内で解放し、長期の好みだけを Memory / USER.md へ昇格できるようにする。raw投稿や一時URLはMemoryへ保存しない。日次配信は既定 `08:10:00` の `hermes-life-interest-digest.timer` で、候補がない日は Discord へ投稿せず `skipped_empty=1` を journal JSON に残す。同日重複送信は `interest/dispatch.jsonl` で抑止する。

fragment 例（ローカル inventory · 非追跡）:
...
/home/hermes/.hermes-life/
  ...
  interest/items.jsonl        # Daily Interest Digest candidates; title/URL/snippet only
  interest/feedback.jsonl     # Local feedback used for ranking
  interest/seen.jsonl         # Duplicate suppression
  interest/profile.json       # Short/mid-term preference weights
  interest/dispatch.jsonl     # Daily dispatch history and duplicate suppression
```
...
| `hermes-life-proactive-evening.timer` | 夜 check-in 送信（既定無効、`private_pi5_hermes_life_proactive_evening_enabled: true` で opt-in） |
...
# 既定 true: private_pi5_hermes_life_proactive_morning_enabled
# 既定 false: private_pi5_hermes_life_proactive_evening_enabled
...
# 既定 false: private_pi5_hermes_life_discord_debug_lines_enabled
```
...
| `life_proactive_loop.py` | 朝/follow-up check-in 構築・返信保存（夜は opt-in） |
...
### Local（D19 · Daily Interest Digest · 2026-06-07 repo）

- `python3 -m pytest scripts/private-pi5-hermes/tests -q`: **225 passed**
- focused: `test_life_interest_digest.py` · `test_discord_command_sync.py` · `test_discord_task_bridge_plugin_register.py`: **40 passed**
- `python3 scripts/private-pi5-hermes/validate_boundary_policy.py --validate-life-pilot`: OK
- `python3 scripts/private-pi5-hermes/validate_boundary_policy.py --validate-daily-pilot --validate-life-pilot`: OK
- `python3 -m py_compile`（Daily Interest Digest / plugin / command sync / CLI）: OK
- `ansible-playbook -i infrastructure/ansible/inventory-private-pi5-stackchan-bridge-fragment.sample.yml infrastructure/ansible/playbooks/private-pi5-hermes.yml --syntax-check`: OK
- `git diff --check`: OK

### 私用 Pi5 deploy（D19 · 初回 · PR #413 · 2026-06-07）

```
PLAY RECAP: ok=186 changed=13 failed=0
life_interest_digest_enabled=True
life_interest_digest_active=True
```

初回 deploy 時点では固定 channel 未設定のため、手動 dispatch は `skipped_missing_channel=1`。送信先固定化は後続 PR #414 + 再 deploy で解消。

### 私用 Pi5 deploy（D19 · 送信先固定化 · `1534bef6` · 2026-06-07）

```
PLAY RECAP: ok=185 changed=5 failed=0
life_interest_digest_enabled=True
life_interest_digest_active=True
```

PR [#414](https://github.com/denkoushi/RaspberryPiSystem_002/pull/414)（`4232ad41`）で service unit テンプレート修正後、fragment に `private_pi5_hermes_life_interest_digest_channel_id` を設定して標準 deploy 実施。

**Pi5 検証（Runbook 準拠）**:

| 確認 | 結果 |
|------|------|
| `hermes-life-interest-digest.timer` | `active (waiting)` · 次回 `2026-06-08 08:10:00 JST` |
| `systemctl cat hermes-life-interest-digest.service` | `ExecStart=/bin/sh -c ...` · `LIFE_PILOT_INTEREST_CHANNEL_ID` 展開 · `StandardOutput=journal` は別行 |
| 手動 `sudo systemctl start hermes-life-interest-digest.service` | `ok=true sent=0 skipped_duplicate=1 skipped_missing_channel=0`（本日送信済みのため duplicate guard） |

**追加知見**:

- 固定 channel + 修正版 service unit で「送信先未解決で日次配信されない」問題は解消済み。
- 本日すでに dispatch 済みの場合、手動起動は `skipped_duplicate=1` が正常。`sent=1` の確認は翌 timer 実行または dispatch 履歴削除後のテストで行う。
- D19 日次配信 E2E の完了条件: **2026-06-08 08:10 JST** の timer 実行で `sent=1` を確認。
...
| interest digest 手動起動が `skipped_missing_channel=1` | 固定 channel 未設定か、service unit が旧テンプレ（`ExecStart` と `StandardOutput` 連結） | fragment に `private_pi5_hermes_life_interest_digest_channel_id` を設定し、PR #414 以降を標準 deploy。`systemctl cat hermes-life-interest-digest.service` で `/bin/sh -c` と別行 `StandardOutput=journal` を確認 |
| interest digest service が起動直後に失敗 | Jinja テンプレで `ExecStart` 行末と `StandardOutput` が同一行に連結され、systemd が invalid unit 扱い | PR #414（`4232ad41`）以降のテンプレを deploy |

## Open Items

1. **D19 日次 timer E2E** — 2026-06-08 08:10 JST の `hermes-life-interest-digest.timer` 実行で `sent=1` を確認すれば日次配信 E2E 完了
2. **D19 `/interest` client-side E2E** — `/interest` の Discord 実表示、`like/save/more` 反応は未確認
3. **D14-D18 Discord/Android client-side E2E** — `/inbox` / 通常DM `さっき共有したリンク...` / Android 画像共有 / URL共有を実端末で未確認
4. **D12 Obsidian Syncthing 運用は任意化** — 手数が重い場合は Discord shared inbox を主経路にする（D13 が主経路候補）
5. **follow-up 自由入力 modal の目視 E2E** — button `やる` は確認済み。同一 check-in が `answered` 後は再試行不可のため別セッションで確認
6. **retention / export / delete** — `~/.hermes-life` の保持・削除ポリシー未設計
7. **Codex/Cursor worker** — 1 task = 1 worktree/branch · 別 HOME/token · 承認境界（D6+、Life Pilot から直接解放しない）
8. **LLM ベース推薦** — 現状は deterministic suggestion のみ
9. **数日運用の体感評価** — 朝 proactive・reminder/follow-up 通知の頻度/文言調整。夜 check-in は必要性が出た時だけ opt-in
