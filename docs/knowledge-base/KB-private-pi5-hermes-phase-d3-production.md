...
- PR [#336](https://github.com/denkoushi/RaspberryPiSystem_002/pull/336) → **`main` マージ**
- 実装: **`cfdae77a`** `feat(hermes): add Phase D3 file and web tools profile`
- 本番 KB + ansible verify 修正: **`6eb4a79c`** `fix(hermes): D3 deploy verify and record Phase D3 production`
- CI: **`26375912601`** success

## 製品方針（2026-05-25 議論）

- **北極星**: Discord から **AI執事**（裏で tools）。**いま Discord から file/web は不可**（chat ツール無効は意図的）。
- **次の開発優先**: D4 browser 隔離 → D5 Discord↔tools 橋。詳細: [執事ロードマップ](../plans/private-pi5-hermes-butler-vision-and-roadmap.md)。

## 次（技術）

- **browser** 有効化は Phase D4（sandbox / 隔離設計）
- **web-smoke** Pi5 完走: `scripts/private-pi5-hermes` ツリー copy または playbook 化

## References

- [Phase D3 ExecPlan](../plans/private-pi5-hermes-tools-security-phase-d3-execplan.md)
- [AI執事ビジョンとロードマップ](../plans/private-pi5-hermes-butler-vision-and-roadmap.md)
