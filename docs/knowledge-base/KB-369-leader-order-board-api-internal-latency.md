...
- **段階取得（API+Web）**: 初回のみ **複数 GET/POST**。挙動が古いときは Pi5 **`api` と `web` の両方**を確認し、同上 **強制リロード**。`bf9dea17` 以降は旧 bundle が `includeLabor=false` を送らなくても API default false で first usable 速度は救済する。`+人` ON の正確な人工数表示は bundle **`index-CtusrliU.js`** 以降が必要。**装飾欠落の切り分け**は上記 **hydrate raw SQL** 知見と Network 順序を参照。
