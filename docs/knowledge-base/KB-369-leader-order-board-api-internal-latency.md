...
- **追補（実装メモ・2026-05-07 · shell 初回高速化ブランチ）**: 初回 shell は **先頭 `pageSize` 件のマージのみ**確定（[`fetchLeaderboardShellMergedPrefixRows`](../../apps/api/src/services/production-schedule/leaderboard/leaderboard-row-selection.service.ts)）。snapshot は **`partialOrdering`** + **`appendSnapshotOrderingChunk`**（[`leaderboard-shell-snapshot.store.ts`](../../apps/api/src/services/production-schedule/leaderboard/leaderboard-shell-snapshot.store.ts)）で **lazy 拡張**。continue の **hasMore** は `orderedRowIds` 長だけでなく **`partialOrdering` が true の間は継続可能**とする（[`listLeaderboardShellContinuationProductionScheduleRows`](../../apps/api/src/services/production-schedule/production-schedule-query.service.ts)）。**手動 0 件**では manual 向け SELECT をスキップし、Fkojunst 可視と同じ **JOIN 付き EXISTS** でのみ判定（`commonWhere` 内の `fkmail` は **単一 `CsvDashboardRow` FROM には載せない**）。**Mac 検証**: `pnpm test -- --run src/routes/__tests__/kiosk-production-schedule.integration.test.ts`（71 passed）·prefix マージ単体テスト。
