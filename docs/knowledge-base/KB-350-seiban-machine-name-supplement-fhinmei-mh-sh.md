...
- **スケジュール**: `defaultBackupConfig.csvImports` に `csv-import-seiban-machine-name-supplement`（**`18 6 * * 0`**・KB-391 で `15 6 * * 0` から変更・**既定 disabled**）を追加済み。有効化するときは Gmail トークンと `targets` のダッシュボードIDを確認。**本番 `backup.json` はデプロイで自動変更されない**（衝突時は管理画面で `18 6 * * 0` 等へ手動修正 → [KB-391](../knowledge-base/KB-391-gmail-csv-import-reliability.md)）。
