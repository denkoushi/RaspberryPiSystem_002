...
| 最終目的 | — | **破綻防止だけでなく**、プロンプト追随・画質の **実用水準**（1 変更ずつ検証） |
...
## 今後の改善候補（優先順・2026-05-31）

| 優先 | 内容 |
|------|------|
| 1 | **基準線 `0531_…` を固定**し、1 変更ずつ検証 |
| 2 | 不足モデルの **復旧または再取得**（下表）— 導入後は基準線との **差分比較**のみ |
| 3 | 公式 **FLUX.2 Klein 9B base-edit** 相当サブグラフへ寄せる |
| 4 | ReferenceLatent / PuLID / SNOFS / Turbo LoRA の **役割分離**検証 |
| 5 | **プロンプト追随** — CFG/steps の微調整だけでなく、テキストエンコーダ・参照経路・LoRA・base-edit 構成として扱う |
...
基準線 workflow: 0531_flux2_klein_9b_DGXSpark_NEXT_standard_available_models.json
（リポジトリ scripts/dgx-private-comfyui/workflows/ に同梱）
UNET: flux-2-klein-base-9b-nvfp4.safetensors
CLIP: qwen_3_8b_bf16.safetensors（fp8mixed は現在未配置）
LoRA: r64 無効（未配置）, r128=0.45, SNOFS=0.35
Flux2KleinEnhancer は使わない（DGX で破綻原因）。
最終目的: 破綻防止ではなくプロンプト追随・画質の実用水準。
次の改善: base-edit 寄せ、不足モデル復旧後の A/B、参照画像の扱い。
Pi5 からは gateway /private-comfyui/health で疎通可。コンテナ内作業は Mac→DGX SSH。
```
