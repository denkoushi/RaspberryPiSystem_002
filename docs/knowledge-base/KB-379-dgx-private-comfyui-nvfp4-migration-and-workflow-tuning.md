...
| 最終目的 | — | **Phase 1（2026-06-13）**: 単一画像の **プロンプト編集**。**Phase 2（2026-06-14）**: body/pose 保持の **head swap**（BFS Head V5 二段構成・約80秒/warm） |
...
**結論**: 一旦成功。全面生成 20step（約15分台）から、**頭部 crop → Qwen+BFS → feather 合成**の二段構成 8step **warm 約80秒**まで短縮。

**正本 workflow**: [`scripts/dgx-private-comfyui/workflows/0613_phase2_bfs_head_v5_twostage_headcrop_8step_cfg4_d075.json`](../../scripts/dgx-private-comfyui/workflows/0613_phase2_bfs_head_v5_twostage_headcrop_8step_cfg4_d075.json)
...
| CLIP | `qwen_2.5_vl_7b_fp8_scaled.safetensors` |
| VAE | `qwen_image_vae.safetensors` |
| LoRA | `bfs_head_v5_2511_merged_version_rank_16_fp16.safetensors` strength **`1.2`** |
| Lightning / SNOFS | **使用しない**（Lightning 4step は速いが顔混ざりやすい） |
| 構成 | body から頭部 crop → crop を Image1・顔参照を Image2 で head swap → feather mask で元画像へ合成 |
| Crop | x/y **`40`** · width **`640`** · height **`768`** · feather **`32px`** |
| KSampler | steps **`8`** · cfg **`4.0`** · euler · simple · denoise **`0.75`** |

**入力契約（Image1/Image2 を逆にしない）**:

| 入力 | 役割 | 例 |
|------|------|-----|
| **Image1** | body / pose / background / lighting を保持するベース（crop 元） | `sample20260607_115425.jpg` |
| **Image2** | face / head identity 参照（顔ID・髪・眼鏡・顔形状） | `DSC07559_face_identity_noclothes_864.jpg` 等 |

**検証（2026-06-14）**:

| 構成 | 結果 | 備考 |
|------|------|------|
| Standard 20step 全面生成 | 約 **15分30秒** | 品質は良いが遅い |
| Lightning 4step 全面生成 | 約 **1分16秒** | 速いが顔混ざりやすい → 本命から外す |
| 二段 8step（低電力固着中） | 約 **231秒**（全面314秒より速い） | sampler **~25秒/step** · 体・背景保持良好 |
| `swapoff` + `drop_caches` 後 | **231→219秒**程度 | 主因ではないが swap 落ち回避に有効 |
| 二段 8step（電源完全リセット後 warm） | **約80秒** ×2回再現 | sampler **~8.5–8.8秒/step** · GPU **~2450MHz / 80–90W** |

**改善要因（確定）**:

1. **二段構成**: 全面生成ではなく頭部 crop のみ Qwen+BFS → 合成
2. **低電力固着の解消**: GPU が **611MHz / 11–14W** に張り付いていた状態を、シャットダウン + **電源ブリック完全抜き差し（60秒）** で **2400MHz / 80–90W** へ復帰

**低電力固着の切り分け（再発防止）**:

| 状態 | `nvidia-smi` の目安 | 対処 |
|------|---------------------|------|
| **正常**（生成中） | clock **~2400MHz** · power **~80W** · util 高 | workflow 調整を続行 |
| **異常**（低電力固着） | clock **~611MHz** · power **10–14W** · util は高い | **workflow/フラグを疑う前に** DGX シャットダウン → 電源ブリック両端抜き → 60秒 → 再起動 |

```bash
nvidia-smi --query-gpu=clocks.gr,power.draw,utilization.gpu --format=csv
```

管理コンソールの私用/業務切替がトリガーかは **未確認**。切替前後で上記を確認する。

**Compose / フラグ（2026-06-14 検証済み・現行維持）**:

- `CUDA_CACHE_MAXSIZE=4294967296` · `PYTORCH_NO_CUDA_MEMORY_CACHING=1`
- `--disable-dynamic-vram` · `--reserve-vram 8` · `--disable-pinned-memory` · `--disable-mmap`
- **`--disable-dynamic-vram` を外す** → `aimdo ... mmap reserve failed` で停止 → **不採用**
- **`PYTORCH_NO_CUDA_MEMORY_CACHING=1` を外す** → Qwen load 悪化 → **不採用**
- **fp8mixed UNET** → bf16 より遅い傾向 → 本命から外す
- CUDA cache は `/root/.nv/ComputeCache`（**コンテナ再作成で消失**）。永続化するなら `CUDA_CACHE_PATH=/opt/ComfyUI/user/cuda-cache`（未実施・速度主因ではない）

**Runtime（2026-06-14 確認）**: ComfyUI **0.22.0** · comfy-aimdo **0.4.5** · comfy-kitchen **0.2.8** · PyTorch **2.12.0+cu130** · Driver **580.142**

**品質上の未完了**: crop 境界の見え方 · face identity の完全固定 · 正面 Face 参照→斜め pose 時の生成寄り · feather/crop/mask 調整 · 軽い harmonize/upscale 後段

**環境 A/B 未実施**（低電力解消後・優先度低）: `--disable-mmap` なし · `--highvram` · ComfyUI 0.24.x / aimdo 0.4.8 別コンテナ · safetensors loader patch 等
...
## 今後の改善候補（優先順・2026-06-14）

| 優先 | 内容 |
|------|------|
| 1 | **Phase 2 正本固定**: `0613_phase2_bfs_head_v5_twostage_headcrop_8step_cfg4_d075.json` で生成継続 |
| 2 | **遅延時**: まず `nvidia-smi` で低電力固着（611MHz/10–14W）→ 電源完全リセット |
| 3 | **品質**: crop / feather / mask 調整 · face identity 固定 · harmonize/upscale 後段 |
| 4 | **運用**: CUDA cache を `/opt/ComfyUI/user/cuda-cache` へ永続化 · ベンチログ JSON/CSV 化 · 低電力検出スクリプト |
| 5 | **速度 A/B（必要時のみ）**: `--highvram` · mmap 無し · loader patch / ComfyUI 0.24.x 別コンテナ |
| 6 | Phase 1 `phase1_…` は単一画像プロンプト編集用。FLUX `0531_…` は txt2img+参照。Enhancer 禁止 |
...
- **速度低下時**: 解像度・量子化より先に **`nvidia-smi` で低電力固着**（611MHz/10–14W）を確認 → 電源完全リセット（[NVIDIA Forums #362735](https://forums.developer.nvidia.com/t/comfyui-performance-problem-related-to-power-usage/362735) と同型）。
...
【Phase 2 正本・head swap】
0613_phase2_bfs_head_v5_twostage_headcrop_8step_cfg4_d075.json
Qwen bf16 + BFS Head V5 LoRA 1.2、二段 crop→swap→合成。
Image1=body/pose、Image2=face identity（逆にしない）。
warm ~80秒（低電力固着解消後）。遅いときは nvidia-smi で 611MHz/10-14W を疑う。

【Phase 1・単一画像プロンプト編集】
phase1_qwen_edit_2511_dgx_flat.json

【FLUX txt2img+参照】0531_…（編集/head swap 用途ではない）
0612 tryon / candidate 系は失敗済み。Enhancer 禁止。
Pi5 gateway health OK。コンテナ作業は Mac→DGX SSH。workflow は docker cp。
```
...
- [NVIDIA Forums: ComfyUI performance / power usage (#362735)](https://forums.developer.nvidia.com/t/comfyui-performance-problem-related-to-power-usage/362735)
- [NVIDIA Forums: Unlocking the Power of the Spark In ComfyUI (#360336)](https://forums.developer.nvidia.com/t/unlocking-the-power-of-the-spark-in-comfyui-no-crashes/360336)
