...
- **参照**: [deployment.md §DGX 単一キュー 2026-05-10](../archive/deployments/2026-05.md#dgx-main-llm-single-queue-stop-policy-2026-05-10)·[dgx-system-prod-local-llm.md §管理コンソール](../runbooks/dgx-system-prod-local-llm.md#管理コンソール-dgx-リソースpi5-api-経由)·[`EXEC_PLAN.md`](../../EXEC_PLAN.md)。
...
- **参照**: [deployment.md §StackChan 本番](../archive/deployments/2026-05.md#stackchan-production-2026-05-10)·[dgx-system-prod-local-llm.md §管理コンソール](../runbooks/dgx-system-prod-local-llm.md#管理コンソール-dgx-リソースpi5-api-経由)·[`EXEC_PLAN.md`](../../EXEC_PLAN.md)。
...
- **ホスト（順序固定・Ansible スコープ外は SSH のみ）**:
  ...
  - **② DGX（Tailscale 例 `100.118.82.72`・ユーザー `ubudgxkoushi`）** — **`scp`** で **`gateway-server.py`** を **`/srv/dgx/system-prod/bin/`** へ配置し **ゲートウェイ再起動**（詳細は [deployment.md §AgentContainer](../archive/deployments/2026-05.md#dgx-agent-container-control-target-2026-05-10)）。
...
- **参照**: [deployment.md §AgentContainer 2026-05-10](../archive/deployments/2026-05.md#dgx-agent-container-control-target-2026-05-10)·[dgx-system-prod-local-llm.md §AgentContainer 本番反映](../runbooks/dgx-system-prod-local-llm.md)·[`gateway-server.py`](../../scripts/dgx-local-llm-system/gateway-server.py)。
...
- **参照**: [deployment.md §2026-05-25](../archive/deployments/2026-05.md#dgx-resource-private-ok-strong-stop-force-2026-05-25)·[Runbook §本番反映 2026-05-25](../runbooks/dgx-system-prod-local-llm.md)·[ADR-20260428](../decisions/ADR-20260428-dgx-active-backend-prod-default.md)·[`EXEC_PLAN.md`](../../EXEC_PLAN.md)。
...
- **参照**: [deployment.md §2026-05-29](../archive/deployments/2026-05.md#dgx-profile-capabilities-runtime-intent-2026-05-29) · [Runbook §本番](../runbooks/dgx-system-prod-local-llm.md#本番反映2026-05-29-dgx-profile-capabilities-runtime-intent)
...
- **参照**: [deployment §2026-05-29](../archive/deployments/2026-05.md#dgx-business-profile-intent-propagation-2026-05-29) · [Runbook §本番](../runbooks/dgx-system-prod-local-llm.md#本番反映2026-05-29-business-profile-intent-propagation) · [KB-366 §業務復帰と各機能](../knowledge-base/KB-366-dgx-spark-operational-understanding.md#2-写真ラベル要領書hermes-は別モデルか)
...
- **参照**: [deployment §2026-05-29 optin-ready](../archive/deployments/2026-05.md#dgx-business-profile-optin-ready-2026-05-29) · [Runbook §本番](../runbooks/dgx-system-prod-local-llm.md#本番反映2026-05-29-business-profile-optin-ready) · [KB-366 §35B 写真ラベル](../knowledge-base/KB-366-dgx-spark-operational-understanding.md#35b-photo-label-cold-start-runtime-ready-timeout)

## `qwen36_35b_uncensored` と `START_MODEL_PROFILE`（2026-05-29 · コード） {#dgx-uncensored-profile-2026-05-29}

- **用途**: 私用テキスト GGUF（green / text only）。**業務キオスク・`BusinessProfileIntent`・業務復帰 orchestration 非対象**。
- **DGX 契約**: registry manifest **`businessOrchestrationEligible: false`**。`model_profiles.py` が API JSON に同フィールドを載せる（**未送信は Pi5 側で eligible=true 扱い** — 混在デプロイ注意）。
- **Pi5 API**: action **`START_MODEL_PROFILE`**（`modelProfileId` 必須）— orchestration / Strict Ready を通さない。overview **`businessReturnSelectable`** は **`businessOrchestrationEligible !== false` かつ `available`** のみ。
- **Web**: `DgxResourceQuickProfileActions` + `dgxResourceQuickProfileConfig.ts`（固定ボタン）。業務復帰 UI（`DgxResourcePrimaryScenarioFlow`）は **`businessReturnSelectable`** を参照。

### 本番反映（2026-05-29 · uncensored profile + 固定ボタン · Pi5→DGX） {#production-2026-05-29-dgx-uncensored-profile-button}

- **ブランチ**: **`feat/dgx-uncensored-profile-button`** · **`83bb4fc0`**
- **CI**: **`26630868193`** — **success**
- **デプロイ対象（順序固定）**:
  - **① `raspberrypi5` のみ** — Detach **`20260529-191402-25013`**（**`ok=134` `changed=4` `failed=0`**）
  - **② DGX Spark** — `model_profiles.py` + manifest **`scp`** · **control + gateway 再起動**（Pi4/Pi3/Hermes **未実施**）
- **実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **43/0/0**（約 **109s**）
- **実機（機能）**:
  - DGX: **`qwen36_35b_uncensored`** · **`status: available`** · **`businessOrchestrationEligible: false`**
  - Pi5 api **`83bb4fc0`**: **`START_MODEL_PROFILE`** · **`filterBusinessReturnSelectableProfiles`**
  - Pi5 web: bundle に **`qwen36_35b_uncensored`**
- **トラブルシュート**:
  - **業務復帰ドロップダウンに uncensored** → DGX bin 未反映 or 再起動漏れ — `GET /system/model-profiles` で **`businessOrchestrationEligible` 欠落**を確認
  - **固定ボタンなし** → Pi5 web ref 未更新 or キャッシュ
  - **`unavailable`** → GGUF **43605014656** B とパスを `stat` で確認
- **参照**: [deployment §2026-05-29](../archive/deployments/2026-05.md#dgx-uncensored-profile-button-2026-05-29) · [Runbook §本番](../runbooks/dgx-system-prod-local-llm.md#本番反映2026-05-29-dgx-uncensored-profile-button) · [計画](../plans/dgx-uncensored-profile-button.md)
...
- **参照**: [deployment §Strict Ready profile](../archive/deployments/2026-05.md#dgx-strict-ready-profile-match-2026-05-28) · [Runbook §本番](../runbooks/dgx-system-prod-local-llm.md#本番反映2026-05-28-strict-ready-profile-match) · [KB-366 §KPI 不一致](./KB-366-dgx-spark-operational-understanding.md#dgx-strict-ready-model-profile-mismatch-2026-05-28)。
...
- **参照**: [deployment §2026-05-29](../archive/deployments/2026-05.md#dgx-runtime-state-alignment-2026-05-29) · [Runbook §本番 2026-05-29](../runbooks/dgx-system-prod-local-llm.md#本番反映2026-05-29-runtime-state-alignment) · [KB-366 §KPI 帯 2026-05-29](./KB-366-dgx-spark-operational-understanding.md#管理画面-kpi-帯2026-05-29-再設計--overviewruntimesummary)
...
- **参照**: [deployment §activeProfileId null](../archive/deployments/2026-05.md#dgx-active-profile-null-contract-2026-05-28) · [Runbook §本番 activeProfileId null](../runbooks/dgx-system-prod-local-llm.md#本番反映2026-05-28-activeprofileid-null-pi5-api) · [KB-366 §503 切り分け](./KB-366-dgx-spark-operational-understanding.md#production-2026-05-28-dgx-business-return-model-selection)。
...
- **参照**: [Runbook §本番 storage path](../runbooks/dgx-system-prod-local-llm.md#本番反映2026-05-28-27b-model-profile-storage-path)·[deployment.md](../archive/deployments/2026-05.md#dgx-model-profile-storage-path-2026-05-28)·[KB-366 §storage path](./KB-366-dgx-spark-operational-understanding.md#production-2026-05-28-dgx-model-profile-storage-path)。
...
- **参照**: [deployment.md §2026-05-28](../archive/deployments/2026-05.md#dgx-business-return-model-selection-2026-05-28)·[KB-366 §本番](./KB-366-dgx-spark-operational-understanding.md#production-2026-05-28-dgx-business-return-model-selection)·[Runbook §2026-05-28](../runbooks/dgx-system-prod-local-llm.md#本番反映2026-05-28-業務復帰モデル選択)·[`scripts/dgx-local-llm-system/README.md`](../../scripts/dgx-local-llm-system/README.md)。
