...
- **API**: `buildPostPolicyOrchestrationSteps` が **`business_to_private` + `comfyRuntimeConfigured`** のときのみ **`private-comfyui start`** を返す。プランの指紋に **`postPolicyPrivateComfyStart`** を反映。ワークロード遷移は **事前調停 → `setPolicyMode` → post-policy**。型は **post-policy ステップの `targetId` を `WorkloadAdjustmentStep['targetId']` と揃え**、`tsc -p tsconfig.build.json` と Vitest で整合させる。
- **本番反映（記録）**
