...
| 管理コンソール 貸出レポート（HTML・Gmail 下書き） | [KB-354-admin-loan-report-gmail-draft-deploy.md](./KB-354-admin-loan-report-gmail-draft-deploy.md) | 1件 | `/admin` 貸出レポート・`GET/POST /api/reports/loan-report/*`・`gmail.compose`・**2026-04-18 本番 `raspberrypi5` のみ**・Phase12 42/1/0・deploy 前 **`git stash push -u`** 知見 |
