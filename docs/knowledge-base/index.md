...
- 2026-04-08（サイネージ・順位ボード資源CDカード `kiosk_leader_order_cards`・readability / SOLID 分割）: [KB-335](./infrastructure/signage.md#kb-335-キオスク順位ボード資源cdカードkiosk_leader_order_cardsサイネージ-jpeg) — ブランチ **`feat/signage-leader-order-readability-solid`**・**Pi5 のみ** `--limit raspberrypi5`・Detach **`20260408-083856-28270`**（**`failed=0`**）・**Phase12 PASS 43/0/0**（約 **29s**・Mac / Tailscale）・**Pi3 は必須デプロイ対象外**。**`main` へマージ**: PR マージ後に本行へ PR リンクを追記可。
