...
| カテゴリ | ファイル | 件数 | 説明 |
|---------|---------|------|------|
| 生産スケジュール（負荷調整・山崩し支援） | [KB-362-kiosk-load-balancing.md](./KB-362-kiosk-load-balancing.md) | 1件 | `/kiosk/production-schedule/load-balancing`・月次負荷（`plannedEndDate`）・能力/分類/移管ルール・工程行サジェスト（自動適用なし）・[ガイド](../guides/kiosk-production-schedule-load-balancing.md) |
| キオスク順位ボード（API 内部レイテンシ） | [KB-369-leader-order-board-api-internal-latency.md](./KB-369-leader-order-board-api-internal-latency.md) | 1件 | `responseProfile=leaderboard` で **COUNT と行 SELECT を並列化**・**段階取得**（`leaderboard-shell` / `leaderboard-total` / `leaderboard-decorations`）オプション・Vitest [`production-schedule-query.service.test.ts`](../../apps/api/src/services/production-schedule/__tests__/production-schedule-query.service.test.ts) |
| 生産スケジュール・CSV外部完了（3系統OR） | [KB-370-production-schedule-external-completion-triple-source.md](./KB-370-production-schedule-external-completion-triple-source.md) | 1件 | **手動** OR **工順ST**（メール消滅・`C/P/X/O`） OR **生産日程CSV**（winner論理キー消滅）・`ProductionScheduleExternalCompletion` 由来別列・`ProductionScheduleCsvIngestLogicalKeySnapshot`・マイグレ `20260506150000_triple_source_external_completion` |
...
| キオスク沉浸式ヘッダー allowlist | [KB-311-kiosk-immersive-header-allowlist.md](./KB-311-kiosk-immersive-header-allowlist.md) | 1件 | `usesKioskImmersiveLayout`・上端リビール対象 URL の単一情報源（拡張時はポリシー＋Vitest）・デプロイ/実機検証/Troubleshooting（2026-03-21 追記）・**`/kiosk/pallet-visualization` 追補（2026-04-22・KB-355 連携）**・**同画面 UI コンポーネント分割の本番6台（2026-04-22・[deployment.md](../guides/deployment.md) 冒頭）** |
...
| Zero 2 W 配膳追跡（haizen・HID エージェント） | [KB-368-zero2w-haizen-placement-tracking.md](./KB-368-zero2w-haizen-placement-tracking.md) | 1件 | `HaizenScanEvent` / `HaizenCurrentPlacement`・`GET/PATCH …/haizen-preset-shelf`・`POST …/haizen-scans`・`GET …/haizen-current`・キオスク **棚番配膳パネル**・`clients/haizen-agent`（2026-05-04 E2E・**2026-05-05 hardening 本番 / Zero 復旧**・**2026-05-06 断片 `sudo_nopasswd_commands` + Phase12 再検証**） |
...
| DGXリソース Phase3–11（補助起停〜複合シナリオ／運用コンソール／目的別4操作／post-policy・最小 UI・gateway／Strict Ready／進行中表示持続化） | [KB-365-dgx-resource-phase3-workload-orchestration.md](./KB-365-dgx-resource-phase3-workload-orchestration.md) | 1件 | Phase3–10 に加え **Phase11**（タブ移動後も `進行中:` を維持・イベント判定 + `sessionStorage` pending・長時間切替の運用解釈） |
