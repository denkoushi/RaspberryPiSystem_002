...
| キオスクIME・電源（電源は原因確定） | [KB-investigation-kiosk-ime-and-power-regression.md](./KB-investigation-kiosk-ime-and-power-regression.md), [KB-investigation-kiosk-schedule-regression-20260301.md](./KB-investigation-kiosk-schedule-regression-20260301.md) | 2件 | 備考欄の日本語入力: 調査中。電源ボタン: **原因確定**（実機ログ解析）。raspi4-robodrill01から誤ってclient-key-raspberrypi4-kiosk1が送信→シャットダウン中のraspberrypi4をターゲットにしてSSHタイムアウト。現状はkiosk-launch.sh正しく設定済み。 |
