...
| Zero 2 W 配膳追跡（haizen・HID エージェント） | [KB-368-zero2w-haizen-placement-tracking.md](./KB-368-zero2w-haizen-placement-tracking.md) | 1件 | `HaizenScanEvent` / `HaizenCurrentPlacement`・`GET/PATCH …/haizen-preset-shelf`・`POST …/haizen-scans`・`GET …/haizen-current`・キオスク **棚番配膳パネル**・`clients/haizen-agent`（2026-05-04） |
