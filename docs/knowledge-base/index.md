...
- 2026-04-02（Gmail・部品納期個数・補助同期）: [KB-324](./KB-324-gmail-order-supplement-prisma-transaction.md) を追加（`ProductionScheduleOrderSupplement` の長いインタラクティブ `upsert` 列を **ソース単位の deleteMany + createMany** と **明示 `timeout` / `maxWait`** に変更。未読メールは同期失敗時は引き続き再試行用に残る）。`KB-297` 部品納期節・本索引カテゴリ表・`docs/INDEX.md` を整合。ブランチ **`fix/order-supplement-sync-transaction`**（コミット/プッシュ方針は作業計画に従う）。
