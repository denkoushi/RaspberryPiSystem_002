...
| キオスク順位ボード（API 内部レイテンシ） | [KB-369-leader-order-board-api-internal-latency.md](./KB-369-leader-order-board-api-internal-latency.md) | 1件 | `responseProfile=leaderboard` で **COUNT と行 SELECT を並列化**・COUNT を専用サービスへ分離（契約・SQL 意味は維持）・Vitest [`production-schedule-query.service.test.ts`](../../apps/api/src/services/production-schedule/__tests__/production-schedule-query.service.test.ts) |
