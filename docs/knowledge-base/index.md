...
| 吊具 持出・返却 可視化（キオスク API / `/kiosk/rigging-analytics`） | [KB-334-kiosk-rigging-loan-analytics-deploy.md](./KB-334-kiosk-rigging-loan-analytics-deploy.md) | 1件 | Pi5+キオスク Pi4×4 のみ順次デプロイ・`RASPI_SERVER_HOST`・Phase12 **PASS 43 / WARN 0 / FAIL 0**・Pi5 初回 Playwright Chromium ダウンロード知見（2026-04-07） |
...
| [KB-335](./infrastructure/signage.md#kb-335-キオスク順位ボード資源cdカードkiosk_leader_order_cardsサイネージ-jpeg) | キオスク順位ボード・資源CDカード（`kiosk_leader_order_cards`）JPEG・管理画面で資源CD複数指定・`cardsPerPage` 1〜8・SVG モジュール分割 | ✅ 解決済み（2026-04-08・Pi5 のみ追補デプロイ・Phase12 PASS 43/0/0） |
