...
| カテゴリ | ファイル | 件数 | 説明 |
|---------|---------|------|------|
| API関連 | [api.md](./api.md) | 38件 | APIエラー、レート制限、認証、履歴、サイネージ、キオスクサポート、温度表示、環境変数バリデーション、WebRTCシグナリング、CSVインポートエラーハンドリング、CSVインポートスケジュール間隔設定、FSEIBANバリデーション修正、生産スケジュール画面のパフォーマンス最適化と検索機能改善（API側）、生産スケジュールAPI拡張（資源CDフィルタ・加工順序割当・検索状態同期・AND検索）、生産スケジュール検索状態の全キオスク間共有化 |
...
- 2026-01-28: KB-199を追加（Dropbox証明書ピニング検証失敗によるバックアップ500エラー）→ 2026-01-28に解決完了（content.dropboxapi.comの新しい証明書フィンガープリント追加）
