...
| キオスクIME・電源（電源はSOLIDリファクタ完了・遅延原因特定・連打防止オーバーレイ実装完了） | [KB-investigation-kiosk-ime-and-power-regression.md](./KB-investigation-kiosk-ime-and-power-regression.md), [KB-investigation-kiosk-schedule-regression-20260301.md](./KB-investigation-kiosk-schedule-regression-20260301.md), [ansible-deployment.md#KB-285](./infrastructure/ansible-deployment.md#kb-285-電源操作再起動シャットダウンのボタン押下から発動まで約20秒かかる), [frontend.md#KB-286](./frontend.md#kb-286-電源操作の連打防止オーバーレイ実装react-portal-による表示失敗の解決) | 4件 | 備考欄の日本語入力: 調査中。電源ボタン: **SOLIDリファクタ完了**（ClientKeyResolverモジュール化）。電源操作遅延: **原因特定**（多段構成に起因、約20秒/85秒）。KB-285に記録。**連打防止オーバーレイ**: 実装完了（KB-286、React Portalで表示失敗を解決）。 |
