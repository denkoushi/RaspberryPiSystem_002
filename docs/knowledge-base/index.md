...
| 写真持出 VLM ラベル | [KB-319-photo-loan-vlm-tool-label.md](./KB-319-photo-loan-vlm-tool-label.md) | 1件 | 写真持出サムネイルを LocalLLM VLM へ非同期推論・`Loan.photoToolDisplayName`・`photoToolLabelRequested` / `photoToolLabelClaimedAt`・`PHOTO_TOOL_LABEL_*`・Pi5 実機確認（requested/labeled/claimed）・トラブルシューティング |
