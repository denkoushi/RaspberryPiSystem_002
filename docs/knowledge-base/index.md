...
| 生産スケジュール納期管理 | [KB-297-kiosk-due-management-workflow.md](./KB-297-kiosk-due-management-workflow.md) | 1件 | 製番納期・部品優先・トリアージ・日次計画・全体ランキング（B3）、手動順番 device-scope v2（Mac 登録端末の代理・Pi4 範囲・Phase12 `siteKey` 検証）、**手動順番専用ページ**（`/kiosk/production-schedule/manual-order`・デプロイ/実機検証）、Location Scope Phase0-12（命名規約・Runbook自動化・横展開監査）を含む運用フロー |
