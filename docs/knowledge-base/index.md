...
| Gmail・生産日程（製番→機種名補完 `FHINMEI_MH_SH`） | [KB-350-seiban-machine-name-supplement-fhinmei-mh-sh.md](./KB-350-seiban-machine-name-supplement-fhinmei-mh-sh.md) | 1件 | 専用 `CsvDashboard`・`ProductionScheduleSeibanMachineNameSupplement`・`POST …/seiban-machine-names` の優先順位（MH/SH → 補完 → `機種名未登録`）・重複は最新run内の `createdAt` / `id` 昇順で末尾行 |
