...
- **CONFIRMED（sudo・Pi4 準拠 2026-05-06 late）**: **`client` ロール**はホスト変数 **`sudo_nopasswd_commands`** を **`sudoers-client.j2`** で **`/etc/sudoers.d/…`** に書く。工場 **Pi4** と同趣旨に、Zero2W 断片へ **限定コマンド列**（**`status-agent` / `haizen-agent` の `systemctl`**・**reboot/poweroff**。キオスク無しのため **kiosk-browser 行は不要**）を載せる整理自体は有効。**ただしこれはサービス操作向けであり、playbook 全体の `become: true` を無対話化するものではない**。`zero2w-edge-setup.yml` は **`common`** を先に通るため、Zero が **`sudo -n true` 不可**の状態では **`ansible_become_password` なし**で最初の root タスクを突破できない。
- **CONFIRMED（Zero 本番是正 2026-05-06 late）**: 実機 `zero2w-tanaban01` は **Pi3/Pi4 と同じく `NOPASSWD: ALL`** にそろえたあと、Pi5 からの **`zero2w-edge-setup.yml`** が **`ok=82 changed=10 failed=0 unreachable=0`** で完了した。以後は **Pi3/Pi4 と同じ無人 Ansible 前提**でよい。**サンプル断片**（`inventory-zero2w-edge-fragment.sample.yml`）の `sudo_nopasswd_commands` は、引き続き **サービス再起動用の明示例**として維持する。
