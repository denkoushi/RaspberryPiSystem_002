...
- **Zero2W `zero2w-tanaban01`（`zero2w-edge-setup.yml`・Pi5 実行）**: **解消済み**。Zero の自動化ユーザーを **Pi3/Pi4 と同じく `NOPASSWD: ALL`** にそろえたあとで、Pi5 から **`ANSIBLE_REPO_VERSION=main ansible-playbook playbooks/zero2w-edge-setup.yml -i inventory.yml -i inventory-zero2w-edge-fragment.yml --limit zero2w-tanaban01`** を再実行し、**`ok=82 changed=10 failed=0 unreachable=0`** で完了。`status-agent.timer` / `haizen-agent.service` は **enabled + active**。  
- **Zero2W E2E（Pi5 実行・2026-05-06 late）**: `PATCH /api/mobile-placement/haizen-preset-shelf` → `POST /api/mobile-placement/haizen-scans` → `GET /api/mobile-placement/haizen-current?shelfCodeRaw=西-北-01&limit=5` を再確認し、**HTTP 200 / `found_in_rows=true`** を確認。`resolutionStatus=UNRESOLVED` は日程未一致時の契約どおり。

## 運用・ドキュメント整合（2026-05-06）

- **限定 NOPASSWD と全体 sudo の役割分離**: 断片の **`sudo_nopasswd_commands`** は **`status-agent` / `haizen-agent` の `systemctl`** などサービス操作向けで、サンプル断片に残す。**一方、Pi3/Pi4 と同じく playbook 全体を無人で通す前提**に合わせるには、**Zero の自動化ユーザー側で `sudo -n true` が通ること（実運用では `NOPASSWD: ALL`）** が必要だった。今回の `zero2w-tanaban01` はこの前提へそろえ、以後は Pi3/Pi4 と同じ感覚で `zero2w-edge-setup.yml` を再実行できる。
