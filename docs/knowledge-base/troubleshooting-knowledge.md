...
- **根本原因3**: テストデータのみをリストアしたいが、`pg_dump`はデータベース全体をバックアップするため、スキーマ定義を除外する必要がある

**試行した対策**: 
- [試行1] `docker exec`に`-i`オプションを追加して標準入力を受け取れるように修正（2025-11-26） → **部分的成功**（標準入力は受け取れるようになったが、スキーマ競合エラーが発生）
- [試行2] `pg_dump`に`--data-only`オプションを追加してデータのみをバックアップするように変更（2025-11-26） → **実装完了・検証待ち**

**有効だった対策**: 
- [試行1] `docker exec`に`-i`オプションを追加（2025-11-26）
- [試行2] `pg_dump`に`--data-only`オプションを追加（2025-11-26）

**学んだこと**: 
- `docker exec`で標準入力からデータを読み込むには、`-i`オプションが必要
- `pg_dump`で作成されたバックアップにはスキーマ定義も含まれるため、既存のデータベースに対してリストアする場合は、`--data-only`オプションを使用してデータのみをバックアップする必要がある
- `--data-only`オプションを使用すると、スキーマ定義（CREATE TYPE、CREATE TABLEなど）は除外され、データの挿入文（INSERT文）のみが含まれる
- `--inserts`オプションと組み合わせることで、COPY文ではなくINSERT文でデータを出力できる（より互換性が高い）
- CI環境では既存のマイグレーションやシードデータがあるため、完全にクリーンアップするのではなく、データのみをリストアする方法が適切
