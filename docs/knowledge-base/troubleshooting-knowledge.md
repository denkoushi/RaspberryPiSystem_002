...
**解決方法の詳細**:
- **外部レビュー提案の「標準モデル」**: テストスクリプトは「前提状態を作る」「本番スクリプトを呼ぶ」「結果を1〜2箇所だけ検証する」という3つの責務に絞る。本番スクリプト（`backup.sh`/`restore.sh`）は環境変数でDB名・ファイルパス・コンテナ名を切り替え可能にし、テストからも本番からも同じスクリプトを使えるようにする。
- **ADMIN_DB_URL/TEST_DB_URLの分離**: CREATE/DROP DATABASEは`postgres`データベースに接続（ADMIN_DB_URL）、migrate/INSERT/SELECTは`test_borrow_return`データベースに接続（TEST_DB_URL）。これにより、権限エラーや接続エラーを防ぐ。
- **docker exec -i フラグの重要性**: ヒアドキュメント（`<<'EOSQL'`）を使う場合、`-i`フラグがないと標準入力が閉じられてSQLが実行されない。CI環境では`docker exec -i`、ローカル環境では`docker compose exec -T`を使用。
- **環境変数による切り替え**: `backup.sh`/`restore.sh`を環境変数対応にすることで、本番環境とCI環境で同じスクリプトを使える。`DB_NAME`, `BACKUP_FILE`, `DB_CONTAINER`, `COMPOSE_FILE`を環境変数で指定可能に。
- **テストスクリプトの責務を最小化**: テストスクリプトは複雑なロジックを持たず、本番スクリプトを呼び出すだけにする。これにより、本番スクリプトの変更がテストに自動的に反映される。

**CIの継続的な成功確認**:
- CIの1回の成功だけでは不十分。複数回実行して安定性を確認する必要がある。
- **確認方法**:
  1. GitHub Actionsの実行履歴を確認（リポジトリの「Actions」タブ）
  2. `scripts/ci/get-logs.sh`を使用してCIの実行履歴を確認
  3. 小さな変更（コメント追加、ドキュメント更新など）を加えてCIを再実行
- **目標**: 連続5回以上の成功、または成功率90%以上（10回実行して9回以上成功）
- **検証結果（2025-11-26）**: CI #215〜#218で4回連続成功を確認
  - バックアップ/リストアテストが4回連続で成功
  - APIテスト（統合テスト）が4回連続で成功
  - E2Eテスト（Playwright）が4回連続で成功
  - 目標（連続5回以上）には未達だが、4回連続成功で一定の安定性を確認
- **確認項目**:
  - バックアップ/リストアテスト: PostgreSQLコンテナの起動、Prismaマイグレーション、テストデータの挿入、バックアップ/リストア、データの検証が安定しているか
  - APIテスト: 統合テストが毎回成功するか、タイムアウトエラーが発生していないか
  - E2Eテスト: Playwrightテストが安定して動作するか、タイミング問題が発生していないか
- **トラブルシューティング**: CIが失敗する場合は`./scripts/ci/get-logs.sh`でエラーを確認し、原因を特定して修正。不安定なテストは`test.skip(process.env.CI, '説明')`でスキップ、タイムアウト延長、リトライ追加を検討。

**E2Eテストの安定化（ステップ4完了）**:
- **現状**: E2Eテストは既に「CI環境で確実に動作する範囲に限定」されている
- **実行されるテスト**:
  - `auth.spec.ts`: ログイン画面表示、無効な認証情報でのログイン失敗、未認証リダイレクト
  - `kiosk.spec.ts`: キオスク持出/返却画面表示、ナビゲーション動作
  - `admin.spec.ts`: ダッシュボード表示、従業員/アイテム/履歴画面へのアクセス
- **CI環境でスキップされるテスト**:
  - 認証フローの詳細（タイミング問題）→ 統合テスト（`auth.integration.test.ts`）でカバー
  - NFCスキャン（物理デバイス必要）→ ユニットテスト（`borrowMachine.test.ts`）と統合テスト（`loans.integration.test.ts`）でカバー
- **設計方針**: CI環境では画面表示・ナビゲーション・フォーム表示の確認に限定。認証フローやNFCスキャンは統合テスト・ユニットテストでカバー。
- **今後の改善計画**: E2Eテストの安定性向上（タイムアウト最適化、待機条件改善）、テストカバレッジの可視化、CI環境での実行時間短縮、実機環境でのE2Eテスト、視覚的回帰テスト
