...
### [KB-267] 吊具持出画面に吊具情報表示を追加

**日付**: 2026-02-18

**事象**: 
- 吊具持出画面に遷移したとき、吊具タグのUIDだけが表示されており、吊具の詳細情報（名称、管理番号、保管場所、荷重、寸法）が表示されていなかった
- 点検見本の右側に余白があり、ここに情報を表示できるスペースがあった

**要因**: 
- 吊具タグUIDから吊具マスタを取得する処理が、貸出登録時のみ実行されていた
- 表示用の情報取得が実装されていなかった

**有効だった対策**: 
- ✅ **吊具情報表示機能の実装（2026-02-18）**:
  1. **データ取得ロジック**: `riggingTagUid`が設定された時点で`getRiggingGearByTagUid()`を呼び出し、吊具情報をstateに保持
  2. **UI表示**: 点検見本の右側余白に「吊具持出」情報ブロックを追加
     - タイトル: ページタイトルと同じフォントサイズ（`text-xl font-bold`）
     - 表示項目: 名称、管理番号、保管場所、荷重(t)、長さ/幅/厚み(mm)
  3. **状態管理**: loading/error/データ有無の3状態を管理し、適切なメッセージを表示
  4. **API二重呼び出し回避**: 貸出登録時の存在チェックで、既に取得済みの`riggingGear`を再利用（ref経由で最新state参照）

**実装の詳細**:
- **データ取得**: `useEffect`で`riggingTagUid`の変化を監視し、空でない場合にAPI呼び出しを実行
- **取消ガード**: コンポーネントアンマウント時やUID更新時の競合を防ぐため、`cancelled`フラグを使用
- **エラーハンドリング**: 404エラーは「タグ未登録（吊具）」、その他のエラーはAPIメッセージまたはエラーメッセージを表示
- **リセット処理**: リセットボタンで`riggingTagUid`と`riggingGear`の両方をクリア

**学んだこと**: 
- **useRefによる最新state参照**: NFCイベント処理の`useEffect`で`riggingGear`を参照する際、依存配列に含めると不要な再実行が発生するため、`useRef`で最新値を参照する方法を採用
- **API二重呼び出しの回避**: 表示用の取得と貸出登録時の存在チェックで同じAPIを呼び出す場合、取得済みデータを再利用することでパフォーマンスを向上
- **UIレイアウト**: 既存のグリッドレイアウト（`lg:grid-cols-[3fr,1fr]`）を活用し、右側余白に情報ブロックを追加することで、画面レイアウトを崩さずに情報を追加

**関連ファイル**: 
- `apps/web/src/pages/kiosk/KioskRiggingBorrowPage.tsx`（実装ファイル）
- `apps/web/src/api/client.ts`（`getRiggingGearByTagUid`関数）
...
- デプロイ実績・Pi5 経由疎通の切り分け・CSV は [KB-312](./KB-312-rigging-idnum-deploy-verification.md)（2026-03-25 時点 Pi5+Pi4×2 完了・Phase12 PASS）。

**解決状況**: ✅ **実装完了・CI成功・デプロイ完了・実機検証完了**（2026-02-18）

**実機検証結果（2026-02-18）**:
- ✅ **吊具情報表示**: 吊具タグをスキャンした時点で、右側余白に吊具情報（名称、管理番号、保管場所、荷重、寸法）が表示されることを確認
- ✅ **状態表示**: タグ未スキャン時は「吊具タグをスキャンしてください」、取得中は「吊具情報を取得中…」、エラー時は適切なエラーメッセージが表示されることを確認
...
**デプロイ・実機検証**:
- **デプロイ**: Run ID `20260306-200303-31051`、`state: success`、約10分（Pi5+Pi4×2、`--limit "server:kiosk"`）
- **実機検証**: 生産スケジュールページで登録製番ボタン（BA1S3352、BA1S3351）の並び替えUIを確認
  - 左右矢印ボタンがカード下辺に正しく配置されていること
  - 右矢印クリックで右に移動（BA1S3352 → BA1S3351 → BA1S3352 の順に変更）
  - 左矢印クリックで左に移動（BA1S3352 → BA1S3351 の順に戻る）
  - disabled状態が正しく切り替わること（先頭の左矢印・末尾の右矢印がdisabled）
  - カードサイズ（w-36 h-16）と×ボタン位置が変更されていないこと
...
**解決状況**: ✅ **実装完了・デプロイ完了・実機検証完了**（2026-03-06）

---

### [KB-303] 納期管理右ペイン 資源CDフィルタ（ドロップダウンのみ・研削/切削ボタン削除）

**実装日時**: 2026-03-17

**仕様**:
- 納期管理画面の右ペイン（詳細パネル）ヘッダーに「資源CD」ドロップダウンのみ表示。研削工程・切削工程のPillToggleボタンは廃止。
- ドロップダウンには研削・切削の資源CDのみ列挙（`getGrindingAndCuttingResourceCds`）。「全件」選択でフィルタ解除。
- 資源CDまたはドロップダウン選択でフィルタ有効時は、右ペインの優先順位保存・上下移動を無効化（`conflictDisabled`）し、フィルタ絞り込みと優先順位操作の衝突を防止。
- 左ペイン・右ペインの表示は `filterPartsByResourceFilter` / `hasDueManagementResourceFilter` に従い絞り込まれる。daily-plan の保存はフィルタに非干渉。

**知見**:
- ドロップダウンだけで資源CD指定が十分なため、研削/切削のカテゴリトグルはUIを冗長にしていた。削除により操作が単純化。
- フィルタ有効時は「見えている部品だけの順序」で保存すると他資源の順序が失われるため、保存・移動は無効化する設計とした。

**実機検証**: ローカルブラウザで資源CDドロップダウンのみ表示・研削/切削ボタンなしを確認。デプロイ後、Pi5 + Pi4×2 でデプロイ完了。実機でのドロップダウン検索動作はユーザー確認済み。

**関連**: [deploy-status-recovery.md](../runbooks/deploy-status-recovery.md) 実機検証チェックリスト「納期管理 資源CDフィルタ」、[KB-297](./KB-297-kiosk-due-management-workflow.md)
