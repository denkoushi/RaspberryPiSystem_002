...
- **スクロール安定化（2026-04-09）**: Pi4 での上下ガタつき対策として、`useKioskDocumentNearVisibleRows` は **その回に変化した entry だけでなく、観測中の全行の交差率**から active row を決めるよう修正し、同率時は現 active を優先して不要な切替を抑える。`KioskDocumentViewerPageRow` のプレースホルダは従来の **`min-h-[40vh]` 固定**をやめ、**実ページ画像と同じ A4 比率**（`1490 / 2108`）で確保して、近傍 `<img>` の mount/unmount 時に行高が大きく跳ねないようにした。検索・選択時の即表示（prefetch・`scrollTop = 0`）は維持。
...
- **バーコード／QR スキャン検索（2026-03-29・チューニング 2026-04-23）**: キオスク要領書の検索欄横 **スキャンボタン** でモーダルを開き、**カメラはセッション中のみ ON**（写真持出と同趣旨で Pi 負荷回避）。デコードは **`@zxing/library` を npm バンドル**（Firefox キオスク向けに **`BarcodeDetector` は前提にしない**）。汎用モジュールは `apps/web/src/features/barcode-scan/`（`formatPresets.ts`・`readerOptionPresets.ts`・`zxingVideoReader.ts`・`useBarcodeScanSession.ts`・`BarcodeScanModal.tsx`）。**要領書画面**では `BARCODE_FORMAT_PRESET_ONE_DIMENSIONAL`（主要な一次元形式。QR・DataMatrix 等は除外）のまま、**`BARCODE_READER_OPTIONS_KIOSK_CONSERVATIVE`（`timeBetweenScansMillis` 400 / `timeBetweenDecodingAttempts` 200ms＝`zxingVideoReader` 既定に近い）**で、配膳等より **積極的な 220/120ms** 読取（`KIOSK_DEFAULT`）と **同時多タブ負荷**を避ける（[KB-339](./KB-339-mobile-placement-barcode-survey.md) V24 と合わせて参照）。**成功時**: 読取文字列を **trim** し検索欄へ反映、`search` / `debouncedSearch` を同時更新して **即検索**、直後にモーダルを閉じてカメラ停止。**キャンセル・タイムアウト（30 秒未検出）・起動失敗後の閉じる**: 検索欄は **空にクリア**（スキャン開始前の手入力も含め）。カメラ不可時のユーザー向け表示は **短文のみ**。判断の根拠は [ADR-20260329](../decisions/ADR-20260329-kiosk-document-barcode-scan-zxing.md)。**購買照会（2026-04-22 追記）**: `/kiosk/purchase-order-lookup` は **`BARCODE_FORMAT_PRESET_PURCHASE_ORDER`** と任意 **`stabilityConfig`**（短時間 **同一値2連続**で確定）を指定（[KB-297 §FKOBAINO](./KB-297-kiosk-due-management-workflow.md#fkobaino-purchase-order-lookup-from-gmail-csv-2026-04-20)）。

## 実機検証

- **本番データ（2026-04-08 JST 夜〜09・Gmail HTML・同名・別メール上書き）**: Pi5 **`docker-db-1`**（DB `borrow_return`）と **`docker-api-1`**（`/app/storage/pdf-pages`）で確認。**前提**: `gmailLogicalKey` / `gmailInternalDateMs` マイグレーション済み・API は論理キー upsert 実装版。**手順例**（SSH 後、**秘密情報をログに残さない**こと）:

```bash
docker exec docker-db-1 psql -U postgres -d borrow_return -c \
  "SELECT id, enabled, \"gmailLogicalKey\", \"gmailInternalDateMs\"::text, \"gmailMessageId\", \"updatedAt\" FROM \"KioskDocument\" WHERE title LIKE '%SD000032603%研削%OP-01%' ORDER BY \"updatedAt\" DESC;"
docker exec docker-api-1 ls -la /app/storage/pdf-pages/eecbd587-cdca-45c3-b178-c5073569f0c0/
```

**結果（代表）**:
  - **有効**行: `gmailLogicalKey`=`sd000032603_研削_op-01.html`、`gmailInternalDateMs`=`1775654906000`（**2026-04-08 22:28:26 JST**）、`gmailMessageId` **新メール**（先頭 `19d6d47e…`）、`updatedAt` **2026-04-08 17:30:26 UTC** 前後。
  - **無効**行: `enabled=false`、論理キーなし、旧 `gmailMessageId`（`19d6c661…`）。
  - **1 ページ目 JPEG**: 上記有効行の UUID 配下で mtime **2026-04-08 13:30 UTC** 前後・**1490×2108**（180dpi 相当）。

**解釈**: **別メール・同一添付名**は **`internalDate` が新しい方**が既存 **`id` を維持したまま**更新し、ページ画像が張り替わる。運用の「解像度が上がったように見える」と一致。**参照**: [kiosk-documents Runbook](../runbooks/kiosk-documents.md)、[検証計画](../plans/kiosk-html-gmail-ingest-verification.md)。

- **デプロイ（キオスク バーコード読取チューニング・Web のみ・Pi5 のみ・2026-04-23）**: ブランチ **`feat/kiosk-barcode-reader-tuning`**（代表 **`70cb9e09`**）。`readerOptionPresets`・`BARCODE_FORMAT_PRESET_ONE_DIMENSIONAL_CORE`（他画面用）・`KioskDocumentsPage` は **`BARCODE_READER_OPTIONS_KIOSK_CONSERVATIVE`**。API/DB 契約不変。**手順**: [deployment.md](../guides/deployment.md)・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/kiosk-barcode-reader-tuning infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**: `20260423-211624-9136`（**`failed=0` / `unreachable=0`**）。**Phase12**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **52s**）。**Pi4 Ansible 不要**（キオスク SPA は Pi5 `web` 配信。Pi4 Firefox は同 URL 参照で追随）。**手動（推奨）**: `/kiosk/documents` でスキャンが従来どおり動作し、他タブで配膳等を開いたとき **Pi4 CPU** が上がりすぎないか目視。
- **デプロイ（要領書ビューア・スクロール安定化・Web+CI・Pi5 のみ・2026-04-09）**: ブランチ `fix/kiosk-documents-viewer-scroll-stability`。`useKioskDocumentNearVisibleRows`（観測中の**全行の交差率**から active を決定・stale マップの範囲外除外と `totalRows` クランプ・`setRowElement` unobserve 時に `visibilityRatiosRef` を掃除）・`KioskDocumentViewerPageRow`（**A4 比率**プレースホルダ `1490/2108`）・`kioskDocumentViewerVisibility.ts` / Vitest。CI: `.trivyignore` に **CVE-2026-39883**（Caddy 由来・[ci-troubleshooting](../guides/ci-troubleshooting.md) の Trivy 暫定と同趣旨）。**手順**: [deployment.md](../guides/deployment.md)・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh fix/kiosk-documents-viewer-scroll-stability infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**: `20260409-185355-5342`（**`PLAY RECAP` `failed=0` / `unreachable=0`**・所要 **約 7 分**）。**Phase12**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **120s**・Mac / Tailscale）。**Pi4 Ansible**: 不要（SPA は Pi5 `web` 配信。[deployment.md §Web のみ](../guides/deployment.md) と整合）。**手動（推奨）**: 各 Pi4 Firefox で `/kiosk/documents` を開き、**長い PDF を縦スクロール**して **上下のガタつき・行高の跳ね**が改善しているか目視確認（自動スクリプトは DOM スクロール挙動を検証しない）。
- **デプロイ（要領書 PDF ページ画像配信ルートの保存先整合・API のみ・Pi5 のみ・2026-04-08）**: ブランチ `fix/kiosk-pdf-pages-route-storage-path`。`routes/storage/pdf-pages.ts` が個別に持っていた保存先既定値をやめ、**変換側と同じ `PDF_PAGES_DIR`** を参照するよう修正。**背景**: Pi5 API コンテナでは実際の変換先が **`/app/storage/pdf-pages`** だった一方、配信ルートは **`/opt/RaspberryPiSystem_002/storage/pdf-pages`** を読んでおり、**180 DPI / 88 品質で再生成した JPEG が 404** になっていた。**手順**: [deployment.md](../guides/deployment.md)・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh fix/kiosk-pdf-pages-route-storage-path infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**: `20260408-204640-13669`（**`PLAY RECAP` `failed=0` / `unreachable=0`**）。**Phase12**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**。**実測**: 対象 HTML 要領書 `SD000032603_研削_OP-01` のページ画像 URL が **HTTP 200** に復帰し、取得 JPEG は **`1490 x 2108`**（A4・180dpi 相当）を確認。
...
- **デプロイ（要領書 HTML Gmail 取り込み・API・Pi5 のみ・2026-04-08）**: 運用指示により **HTML 添付を Playwright で PDF 化する API** を **Pi5 だけ**反映（[deployment.md](../guides/deployment.md)・**`--limit raspberrypi5`**・`RASPI_SERVER_HOST`・`--detach --follow`）。**Pi3 / Pi4 への Ansible デプロイは不要**（Pi3 のメモリ制約向け **単独・プレフライト手順**も今回は未使用）。**Ansible サマリ**: `ansible-update-20260408-154206-25754`（**`failed=0` / `unreachable=0`**）。**追従 `main` 例**: `895a1060`。**Phase12**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 42 / WARN 1 / FAIL 0**（**WARN**: auto-tuning ログ件数 0）。**手動 E2E（2026-04-08/09 本番確認済）**: 件名「要領書HTML研削」系・**未読**・HTML 添付および **同一 HTML の別メール再送**で、`gmailLogicalKey` 維持・`gmailMessageId` 更新・`pdf-pages` 再生成を DB/ストレージで確認（本節 **本番データ（2026-04-08 JST 夜〜09）**）。詳細は [kiosk-html-gmail-ingest-verification.md](../plans/kiosk-html-gmail-ingest-verification.md)。
...
- **バーコードが読めない／遅い（Pi4 Firefox）**: 照明・距離・ピント・ラベル汚れの影響が大きい。**一次元のみ** preset でも ZXing の連続デコードは負荷があるため、実装は **解像度・デコード間引き**を Pi 向けに調整済み（`zxingVideoReader.ts`・**`readerOptionPresets.ts`**。要領書は **400/200ms**、配膳等は **220/120ms**＋**コア一次元**が既定）。改善の手戻りは **カメラ解像度制約**・**間引き間隔**・必要なら **対象形式の絞り込み**（`formatPresets`）で切り分ける。読取速度の **1 秒以内 SLA** は現場実測で確認すること（ブラウザ・CPU・コード種別で変動）。
...
- **ページ画像 URL が 404 なのに `GET /api/kiosk-documents/:id` の `pageUrls` は返る**: まず **配信ルートと変換ルートが同じ保存先を見ているか**を確認する。2026-04-08 の Pi5 実機では、変換先が **`/app/storage/pdf-pages`**、配信ルートが **`/opt/RaspberryPiSystem_002/storage/pdf-pages`** を参照しており不整合だった。恒久対策として、`routes/storage/pdf-pages.ts` は **`lib/pdf-storage.ts` の `PDF_PAGES_DIR` を共用**する形に統一した。切り分けは **(1) 対象 URL の HTTP ステータス**、**(2) コンテナ内の `pdf-pages/{id}` 実在**、**(3) `pdfinfo` と `pdftoppm -r 180` 手動実行結果** の順が速い。
...
- **スクロール中に上下へ揺れる／行が跳ねる**: `KioskDocumentViewerPageRow` のプレースホルダ高さが実画像と大きくずれていると、近傍 `<img>` の mount/unmount たびに総高さが変わって見える。現行は **A4 比率プレースホルダ**へ修正済み。再発時は `useKioskDocumentNearVisibleRows` の active row 判定が **観測中の全行 ratio** を見ているか、`KIOSK_DOCUMENT_PDF_DPI` / `KIOSK_DOCUMENT_JPEG_QUALITY` が Pi4 に対して過大でないかを確認する。
