...
| **実施済み（2026-07-07）** | UI で非一致を明示 — 資源CD俯瞰・機種別月次タブに常時注記（`LoadBalancingProductionSystemNote` · `f91fa79a`）· [KB-362 §画面構成](./KB-362-kiosk-load-balancing.md#画面構成3タブ) |
| **未実施（任意）** | 生産 KPI との **別系統参考表示**（EXEC_PLAN タスク3の残） |
