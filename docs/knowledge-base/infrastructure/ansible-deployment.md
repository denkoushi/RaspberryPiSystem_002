...
**Troubleshooting**:
- Pi5 デプロイ前に **`ssh "denkon5sd02@100.106.158.2" 'ssh -o BatchMode=yes denkon5sd02@100.106.158.2 "echo SELF_OK"'"`** で self-SSH を確認すると、同症状を先に潰せる。
- lock を手で消す前に、**`runPid` が無い / 対応する deploy artifact が無い / `ps -ef` に `ansible-playbook` や `update-all-clients` が無い**ことを確認する。
- ローカル作業ツリーが dirty だと fail-fast で止まるため、**commit するか stash**してから再試行する（[KB-315](#kb-315-pi4-fjv-third-kiosk) と同系）。

**References**:
- `scripts/update-all-clients.sh`（`run_preflight_remotely` / `acquire_remote_lock`）
