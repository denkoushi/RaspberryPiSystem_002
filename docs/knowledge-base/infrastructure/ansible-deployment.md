...
5. NFC リーダー USB 接続直後、host `pcsc_scan` では見えているのに nfc-agent が `readerConnected:false` / `lastError:"no-readers"` のままになった。
...
- host PC/SC では NFC リーダーを検出できるのに nfc-agent が `no-readers` のままなら、PC/SC ソケットの接続状態を取り直すため **`docker restart docker-nfc-agent-1`** を実行する。
...
- USB: `lsusb` で **`TMC HIDKeyBoard`** と **`Sony Corp. FeliCa Port/PaSoRi 4.0`** を確認。
- バーコード: `/dev/input/by-id/usb-TMC_HIDKeyBoard_1234567890abcd-event-kbd -> ../event8`、`/proc/bus/input/devices` は `Name="TMC HIDKeyBoard"` / `Handlers=sysrq kbd leds event8`。移動票の **製造order番号バーコード**は実機で正常スキャン済み。
- NFC: `pcsc_scan` とコンテナ内 `pyscard` の両方で **`SONY FeliCa RC-S300/P (0461583) 00 00`** を確認。`docker restart docker-nfc-agent-1` 後、`GET http://localhost:7071/api/agent/status` は **`readerConnected:true`** / `readerName="SONY FeliCa RC-S300/P (0461583) 00 00"`。
...
<a id="kb-366-pi5-self-ssh-preflight-and-orphan-lock"></a>

### [KB-366] `update-all-clients.sh` の Pi5 preflight が self-SSH で失敗し orphan lock が残る

**記録日**: 2026-05-03  
**Status**: ✅ 復旧済み・Pi5 本番デプロイ / 実機検証済み

**Context / Symptoms**:
- `./scripts/update-all-clients.sh <branch> infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow` の preflight で、Pi5 対象なのに **`raspberrypi5 | UNREACHABLE! Permission denied (publickey)`** が出て停止した。
- その直後、Pi5 に **`/opt/RaspberryPiSystem_002/logs/.update-all-clients.lock`** が残り、次回実行で **`LOCKED: age=... timeout=2400s`** になった。
- lock の JSON は **`runner=bootstrap` / `runPid=null`**、対応する **`.status.json` / `.pid` / `.exit` / `.log`** が存在しなかった。

**Investigation**:
- **CONFIRMED**: `scripts/update-all-clients.sh` の preflight は **Mac からの Ansible**ではなく、**Pi5 上で `ANSIBLE_BECOME=false ansible -i inventory.yml all -m ping --limit raspberrypi5`** を実行する。
- **CONFIRMED**: Mac からの `ssh denkon5sd02@100.106.158.2` とローカル `ansible raspberrypi5 -m ping` は成功した一方、**Pi5 上での `ssh -o BatchMode=yes denkon5sd02@100.106.158.2`** は **`Permission denied (publickey)`** で失敗した。
- **CONFIRMED**: Pi5 の **`~/.ssh/id_ed25519.pub`** は存在したが、**同じ内容が `~/.ssh/authorized_keys` に入っていなかった**。
- **CONFIRMED**: preflight 失敗時に cleanup まで到達せず、**bootstrap 直後の lock** が残り得る。

**Root cause**:
- Pi5 標準デプロイの preflight は **Pi5→Pi5 self-SSH** を前提にしているが、Pi5 自身の公開鍵が `authorized_keys` に登録されておらず、preflight が失敗した。
- その失敗パスで **orphan lock** が残り、再試行がロックで止まった。

**Fix（最小変更）**:
- Pi5 の **`~/.ssh/authorized_keys`** をバックアップし、**`~/.ssh/id_ed25519.pub`** を追記。
- 権限を **`~/.ssh 700`**、**`authorized_keys 600`** に再設定。
- orphan lock は **実行中プロセス・deploy artifact 不在**を確認したうえで、バックアップを取ってから削除。
- **恒久（再発防止）**: **server** ロール [`ssh-loopback-authorized.yml`](../../infrastructure/ansible/roles/server/tasks/ssh-loopback-authorized.yml) が上記 **`authorized_keys` 冪等管理**を行う。`update-all-clients.sh` は **preflight 前**に **self-SSH チェック**を行い、**preflight 失敗時はリモート bootstrap lock を EXIT トラップで返却**する。次回ロック取得時は **`REMOTE_BOOTSTRAP_ORPHAN_SECONDS`（既定 180s）** で **status artifact 無しの bootstrap lock** を掃除し得る。

**Verification（2026-05-03 本番・当初障害復旧時）**:
- self-SSH: `ssh -o BatchMode=yes denkon5sd02@100.106.158.2 "echo SELF_OK"` → **成功**
- **ブランチ**: `feat/dgx-resource-dashboard-ui-phase8`
- **Detach Run ID**: **`20260503-181600-946`**（`PLAY RECAP` **`ok=130` `changed=4` `failed=0` / `unreachable=0`**・exit **`0`**）
- **実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**

**Verification（恒久対策デプロイ後・2026-05-03）**:
- **ブランチ**: `feat/deploy-preflight-selfssh-lock-guard`（**`main` マージ後は標準運用どおりブランチ引数を `main`**）
- **Detach Run ID**: **`20260503-190010-8105`**（`PLAY RECAP` **`ok=131` `changed=3` `failed=0` / `unreachable=0`**・exit **`0`**・`--follow` 約 **194s**）
- **実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **103s**）

**Troubleshooting**:
- Pi5 デプロイ前に **`ssh "denkon5sd02@100.106.158.2" 'ssh -o BatchMode=yes denkon5sd02@100.106.158.2 "echo SELF_OK"'"`** で self-SSH を確認すると、同症状を先に潰せる（**スクリプトも preflight 直前に `UPDATE_ALL_CLIENTS_SELF_SSH_OK` で同等チェック**する）。
- lock を手で消す前に、**`runPid` が無い / 対応する deploy artifact が無い / `ps -ef` に `ansible-playbook` や `ansible … -m ping` が無い**ことを確認する。

**Prevention**:
- 本番での再発防止は **Ansible server ロール**（[`ssh-loopback-authorized.yml`](../../infrastructure/ansible/roles/server/tasks/ssh-loopback-authorized.yml)）と **`update-all-clients.sh` の self-SSH 事前チェック／lock EXIT 解放／orphan bootstrap 掃除**に任せる。
- ローカル作業ツリーが dirty だと fail-fast で止まるため、**commit するか stash**してから再試行する（[KB-315](#kb-315-pi4-fjv-third-kiosk) と同系）。

**References**:
- `scripts/update-all-clients.sh`（`check_pi5_loopback_ssh_preflight` / `run_preflight_remotely` / `acquire_remote_lock` / EXIT trap）
- [deployment.md](../../guides/deployment.md)
- [KB-365](../KB-365-dgx-resource-phase3-workload-orchestration.md)

---

<a id="kb-367-zero2w-edge-status-agent-tailscale"></a>

### [KB-367] Zero 2 W 棚番エッジ — Tailscale・Ansible・status-agent（TLS・`git`）

**記録日**: 2026-05-04

**要約**: **Mac（admin）→ Zero（signage）直 SSH**は ACL 次第で **不通**。**Pi 5 踏み台**が正。**Raspberry Pi OS Lite** では **`git` 未導入**で Ansible の repo 同期が失敗しうる。Pi 5 の **自己署名 TLS** では **`status_agent_tls_skip_verify: "1"`**（`/etc/raspi-status-agent.conf` の **`TLS_SKIP_VERIFY`**）が初期導入で必要。`POST /api/clients` は **JSON の多重量子化に注意**。

**本文・Runbook**: [KB-367-zero2w-tanaban-edge-tailscale-ansible.md](../KB-367-zero2w-tanaban-edge-tailscale-ansible.md) · [zero2w-tanaban-edge-setup.md](../../runbooks/zero2w-tanaban-edge-setup.md)
...
<a id="kb-356-local-llm--inference_providers_json-の整合検証とデプロイ時-assert"></a>

### [KB-356] Local LLM / `INFERENCE_PROVIDERS_JSON` の整合検証とデプロイ時 assert

**記録日**: 2026-04-26  
**Status**: ✅ 実装反映済み・Pi5 本番デプロイ・Phase12 実機検証済み

**Context / Symptoms**:
- Pi5 の **`LOCAL_LLM_*`**（ベース URL・共有トークン・モデル・runtime control）と **`INFERENCE_PROVIDERS_JSON`** の primary プロバイダが **ずれる**と、推論や on-demand の **`/start` / `/stop`** が **401 / 接続失敗**になり得る（過去に vault と DGX 実 secret の **ドリフト**が障害化した事例あり）。

**Root cause**:
- 設定が **複数箇所**（vault → Ansible → `docker.env` → API）に分散し、**単一の契約**で早期検証されていなかった。

**Fix（最小変更）**:
- **API 起動時**: `apps/api/src/config/env.ts` で `INFERENCE_PROVIDERS_JSON` を静かにパースし、**`collectLocalLlmProviderAlignmentIssues`** により不整合を Zod エラー化（logger 非依存の schema 層を分離）。
- **デプロイ時**: `infrastructure/ansible/playbooks/manage-app-configs.yml` に **assert** を追加し、inventory から組み立てた JSON と **`LOCAL_LLM_*`** の **provider id / base URL / token 系 / on_demand URL** を突合（失敗時は playbook が **fail-fast**）。

**運用**:
- **対象は Pi5 のみ**（`--limit raspberrypi5`）で API と Ansible 経路が更新される。DGX 側の **systemd 導入**は [dgx-system-prod-local-llm.md](../../runbooks/dgx-system-prod-local-llm.md)（inventory 外）。

**Verification（2026-04-26 本番）**:
- **ブランチ**: `feat/dgx-ops-hardening`・**Detach Run ID**: `20260426-135827-7914`（`failed=0` / `unreachable=0` / exit `0`）。
- **実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**。

**Troubleshooting**:
- **assert でデプロイが止まる** → Ansible のエラーメッセージに従い、`vault_api_local_llm_shared_token` / `vault_api_local_llm_runtime_control_token` と DGX 実設定、および `INFERENCE_PROVIDERS_JSON` 内の **`auth.sharedToken` / runtime control の token・URL** を揃える（[KB-318](#kb-318-pi5-local-llm-via-docker-env)）。
- **API が起動しない（Zod）** → 本番ログの **path**（例: `INFERENCE_PROVIDERS_JSON` / `LOCAL_LLM_RUNTIME_CONTROL_START_URL`）を確認。`PHOTO_TOOL_EMBEDDING_ENABLED=true` のとき **`PHOTO_TOOL_EMBEDDING_URL` / `PHOTO_TOOL_EMBEDDING_MODEL_ID` 必須**。

**References**:
- 実装: `apps/api/src/services/inference/config/local-llm-env-alignment.ts`、`inference-providers-json.schema.ts`、`infrastructure/ansible/playbooks/manage-app-configs.yml`
- デプロイ記録: [deployment.md](../../guides/deployment.md) 冒頭（2026-04-26 補足）
...
- **Phase12 自動検証（2026-03-30）**: `deploy-status`（FJV 用 `x-client-key`）が PASS でも Pi5→FJV の SSH がタイムアウトする場合がある（端末オフ・Tailscale 未接続等）。`verify-phase12-real.sh` は当該項目を **WARN** とし全体は **FAIL 0** で完走する。`systemctl` まで必須なら端末復帰後に再実行する（[deploy-status-recovery.md](../../runbooks/deploy-status-recovery.md) 注記）。
...
<a id="kb-316-pi4-stonebase-fourth-kiosk"></a>

### [KB-316] Pi4 4台目（Kensaku StoneBase01・`raspi4-kensaku-stonebase01`）追加時の Docker 未導入失敗と復旧

**発生日 / 記録日**: 2026-03-28  
**Status**: ✅ 復旧済み（実機 deploy 成功）

**Context（何をしたか）**:
- 第2工場キオスク用 Pi4 を **4台目**として追加（StoneBase01）。
- `inventory.yml` に `raspi4-kensaku-stonebase01` を追加、`group_vars/all.yml` に LAN (`192.168.10.238`) と Tailscale (`100.101.113.95`) を反映。
- `status_agent_client_id`: `raspi4-kensaku-stonebase01-kiosk1`、`status_agent_client_key` / NFC: `client-key-raspi4-kensaku-stonebase01-kiosk1`（固定文字列）。
- Pi5 で `register-clients.sh` 実行後、`deploy-staged.yml --limit raspi4-kensaku-stonebase01` を実施。

**Symptoms（症状）**:
1. 新 Pi4 で `tailscale` コマンドが存在せず、初期時点は Tailscale 未参加。
2. Pi5 から新 Pi4 へ SSH は通るが、初回 Ansible で `nfc-agent には Docker が必要です` で fail-fast。
3. Docker 導入後の再実行で `Ensure nfc-agent container is up` が長時間停止して見える（初回 build）。

**Investigation**:
- `register-clients.sh` は固定キー端末として `raspi4-kensaku-stonebase01` を正常登録できる。**CONFIRMED**。
- 失敗原因は `roles/client/tasks/nfc-agent-lifecycle.yml` の Docker 前提チェック。`docker --version` 非0で停止。**CONFIRMED**。
- `curl -fsSL https://get.docker.com | sudo sh` 実施後は Docker 前提チェックを通過し、`nfc-agent` 起動と readiness が成功。**CONFIRMED**。

**Fix / 運用手順（要約）**:
- 新 Pi4 で Tailscale 参加後、`sudo tailscale set --ssh=false` を設定し、Pi5 公開鍵を `authorized_keys` に投入。
- Docker 未導入端末では、先に次を実施:
  - `curl -fsSL https://get.docker.com | sudo sh`
  - `sudo usermod -aG docker <user>`
  - `sudo systemctl enable --now docker`
- その後、Pi5 で
  - `cd /opt/RaspberryPiSystem_002/infrastructure/ansible`
  - `ANSIBLE_CONFIG="$PWD/ansible.cfg" ANSIBLE_REPO_VERSION=main ansible-playbook playbooks/deploy-staged.yml --limit raspi4-kensaku-stonebase01`
- 実機確認:
  - `GET /api/system/deploy-status`（StoneBase client key）で `"isMaintenance": false`
  - `kiosk-browser.service` / `status-agent.timer` が `active`
  - `docker-nfc-agent-1` が `Up`

**Prevention**:
- 新規 Pi4 追加時は初期セットアップで **Docker を必須前提**として先に入れる。
- `tailscale_network` の `100.x` を確定後すぐ `group_vars/all.yml` に反映し、`local_network` フォールバック依存を残さない。
...
<a id="kb-318-pi5-local-llm-via-docker-env"></a>

### [KB-318] Pi5 LocalLLM: API コンテナへは `docker.env` 経由（`apps/api/.env` のみでは届かない）

**発生日 / 記録日**: 2026-03-28  
**Status**: ✅ 解決済み（Ansible・実機デプロイ・疎通確認）

**Context**:
- Pi5 API に `GET /api/system/local-llm/status` / `POST /api/system/local-llm/chat/completions` を追加し、Ubuntu LocalLLM（[KB-317](./security.md#kb-317-ubuntu-localllm-を-tailscale-sidecar--tagllm-で分離公開する)）へ代理する。
- Ansible の `api.env.j2` や手作業で **`apps/api/.env` にだけ** `LOCAL_LLM_*` を書いていた。

**Symptoms**:
- `GET /api/system/local-llm/status` が `configured=false`、または upstream 未到達に見える。
- Pi5 ホストで `apps/api/.env` に `LOCAL_LLM_*` がある一方、`docker compose ... exec api printenv` では **空**。

**Investigation**:
- `docker-compose.server.yml` の `api` が **`env_file` に `apps/api/.env` を含めない**ため、コンテナプロセスに変数が渡らない。**CONFIRMED**。
- JWT 弱値問題（[KB-260](#kb-260-デプロイ後にapiが再起動ループするjwt秘密鍵が弱い値で上書きされる)）と同系で、**本番 API の実効 env は `infrastructure/docker/.env` 側**が正本になりうる。

**Fix**:
1. `infrastructure/ansible/templates/docker.env.j2` に `LOCAL_LLM_BASE_URL` / `LOCAL_LLM_SHARED_TOKEN` / `LOCAL_LLM_MODEL` を出力する。
2. （オンデマンド運用時）同テンプレートに `LOCAL_LLM_RUNTIME_*`（`MODE` / `CONTROL_*` / `HEALTH_BASE_URL` / タイムアウト類 / **任意で `WARM_WINDOW_*`**）を出力する。値は `inventory` / `host_vars` で渡す。
3. `inventory.yml` と `host_vars`（例: `vault_api_local_llm_shared_token`）でテンプレート参照を整合させる。
4. `roles/server/tasks/main.yml` のデプロイ後検証で、利用時は `LOCAL_LLM_*` の存在を確認する（既存 env 検証に準拠）。

**確認コマンド（例・Pi5 上）**:
```bash
docker compose -f infrastructure/docker/docker-compose.server.yml exec api sh -lc 'printenv | grep LOCAL_LLM_ || true'
```
...
**参照**: [KB-317](./security.md#kb-317-ubuntu-localllm-を-tailscale-sidecar--tagllm-で分離公開する) / [local-llm-tailscale-sidecar.md](../../runbooks/local-llm-tailscale-sidecar.md)
...
2. **ロック（並行実行防止・2026-03-30 拡張）**:
   - **ローカル（Mac 実行端末）**: `logs/.update-all-clients.local.lock` を `mkdir` で取得し、同一 Mac での `update-all-clients.sh` 多重起動を即時拒否（exit 3）
   - **リモート（Pi5）**: `/opt/RaspberryPiSystem_002/logs/.update-all-clients.lock` に JSON（`runId` / `runPid` / `state` / `runner` 等）を書き込み、`acquire_remote_lock` で **PID 生存・`ansible-playbook … update-clients.yml` 稼働・経過時間（既定 `REMOTE_LOCK_TIMEOUT_SECONDS`＝2400 秒）** を組み合わせて stale を判定（旧来の単純 mtime のみから強化）
   - **解放**: フォアグラウンド完了時は Mac 側 `release_remote_lock` が同一 `runId` のロックのみ削除。デタッチ実行中の Pi5 側 `cleanup` も `runId` 整合で削除
   - ロック取得失敗時はデプロイを中断（エラーコード3）
   - `scripts/update-all-clients.sh` の `acquire_local_lock` / `acquire_remote_lock` / `release_remote_lock` / `update_remote_lock_metadata` で実装
   - **運用**: 同一 `RASPI_SERVER_HOST` への並列起動は引き続き禁止（[deployment.md](../../guides/deployment.md)）。復旧時の手動削除は [deploy-status-recovery.md](../../runbooks/deploy-status-recovery.md)（`runPid` 非生存を確認してから）
...
- ✅ **2026-03-30**: `update-all-clients` のローカル + Pi5 JSON ロック・stale 判定強化を Pi5 のみデプロイで踏襲。Phase12 実測 **PASS 35 / WARN 2 / FAIL 0**（[deploy-status-recovery.md](../../runbooks/deploy-status-recovery.md) の FJV 注記と整合）
