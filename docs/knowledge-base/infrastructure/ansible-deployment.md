...
2. `inventory.yml` と `host_vars`（例: `vault_api_local_llm_shared_token`）でテンプレート参照を整合させる。
3. `roles/server/tasks/main.yml` のデプロイ後検証で、利用時は `LOCAL_LLM_*` の存在を確認する（既存 env 検証に準拠）。
