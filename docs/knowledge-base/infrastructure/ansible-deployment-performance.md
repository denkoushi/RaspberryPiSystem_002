...
**改善後（メンテナンスフラグ早期解除の実機検証）**:
- **カナリア（Pi5 + Pi4 1台）**
  - **runId**: `20260207-212512-5840`
  - **結果**: **success（exit=0）**
  - **所要時間**: **2分56秒**
  - **確認**: `Clear kiosk maintenance flag if server deployment completed` が **ok**（Pi5完了後にPi4側でフラグ解除）
- **全台並行（Pi5 + Pi4 + Pi3）**
  - **runId**: `20260207-212941-25124`
  - **結果**: **success（exit=0）**
  - **所要時間**: **13分03秒**
  - **確認**: Pi4プレイ内で `Clear kiosk maintenance flag if server deployment completed` が **changed**（21:33:43）。Pi3プレイ開始前に解除され、Pi4のメンテナンス表示が先に終了
- **Pi5のみ（server限定）**
  - **runId**: `20260207-214349-26376`
  - **結果**: **success（exit=0）**
  - **所要時間**: **1分14秒**
  - **確認**: kioskメンテナンスフラグは設定されず、従来通りの挙動
