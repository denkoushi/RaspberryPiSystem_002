...
**追記（2026-03-28）**: Pi4 3台目（FJV60/80・`raspi4-fjv60-80`）でも **Docker 前提・`nfc_agent_*` 定義・初回 `docker compose` ビルドの長時間**は RoboDrill01 追加時と同型。全体の SSH 経路・Ansible 実行ディレクトリの注意は [KB-315](./ansible-deployment.md#kb-315-pi4-fjv-third-kiosk) を参照。
