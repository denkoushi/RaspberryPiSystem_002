...
自主検査の対象部品を資源別に集約した部品カード（進捗バー＋状態バッジ）だけを
**flat page list** でローテーション
（`getRotatingSlideIndex` 共有）。旧 summary と detail/ヒートストリップの別ページは生成しない。
設定: `targetMode`（新規管理UIは `kiosk_active_sessions` / `manual_machine_name`、
`auto_from_leaderboard_status` は legacy wire/runtime 互換、未指定は manual）・
`machineName`（manual 必須）・`resourceCds`（legacy auto 必須・順位ボード相当の資源CD）・
...
`kiosk_active_sessions` はキオスク自主検査の未検索時と同じ、無効化・完了を除く
入力中／判定待ちセッションを更新日時順に最大200件取得し、機種名や資源CDの手入力なしで表示する。
200件を超えた場合も最新200件は表示し、続きがあることを注記する。
...
カード内は項目ラベルを付けず、製番と品名を36px、半角化した機種名を16px基準で表示する。
機種名は製番の右へ2文字分空けて配置し、カードの更新日時（秒なし、27px）は状態バッジの左へ表示する。
資源CDは集約用の内部キーに限定し、画面には資源マスタの日本語名だけを表示する
（複数名は ` / `、未登録は `名称未登録`）。資源名は31px、進捗件数は36pxの白文字とし、
白い進捗バーは資源名の表示領域を確保するため従来比70%の幅で表示する。
...
**References**: [Runbook: pi3-signage-tailscale-recovery](../../runbooks/pi3-signage-tailscale-recovery.md) · [deployment.md §ラズパイ3](../../archive/deployments/legacy-operator-guide-through-2026-07.md#ラズパイ3サイネージの更新) · [tailscale-policy.md §Key expiry](../../security/tailscale-policy.md#key-expiry-常時稼働端末) · `scripts/ops/recover-pi3-signage-remote.sh`
...
- **2026-05-22 キオスク全件無色（方針 A · [KB-297 §全件無色](../KB-297-kiosk-due-management-workflow.md#leader-order-board-seiban-accent-no-color-all-items-2026-05-22)）**: キオスク Web は **OR フィルタ未選択時に左縁無色**。**サイネージ JPEG** は **フィルタ UI がなく常に未完行集合**のため、**`leader-order-seiban-accent-palette.ts` の 24 色ハッシュを維持** — キオスク全件表示と **左縁の有無が異なるのは仕様**。
...
- `apps/api/src/services/signage/leader-order-cards/build-leader-order-cards-svg.ts` / `leader-order-cards-svg-card.ts` / `leader-order-cards-svg-header.ts` / `leader-order-cards-svg-schedule-row.ts` / `leader-order-cards-svg-footer-chips.ts` / `leader-order-seiban-accent-palette.ts` / `leader-board-pure.ts` / `leader-order-cards-data.service.ts` / `leader-order-cards-svg-layout-tokens.ts`
- `apps/web/src/pages/admin/SignageSchedulesPage.tsx`
- `apps/web/src/pages/signage/SignageDisplayPage.tsx`
...
**本番デプロイ（2026-05-21・`main` `a2f9a2c5`・ヘッダ加工機名全文）**: **API のみ**。**対象**: **`raspberrypi5` のみ**。**手順**: [deployment.md](../../archive/deployments/2026-05.md#signage-leader-order-header-full-machine-name-2026-05-21)。**Detach**: `20260521-134013-4448`（**`failed=0`**）。**Phase12**: **43/0/0**（約 **46s**）。**現場**: 加工機名 **`…` なし・1行全文 OK**。

**本番デプロイ（2026-05-22・ブランチ `feat/kiosk-leader-board-manual-order-row-highlight`）**: **手動順位付き行ハイライト**（Web + API SVG）。**対象**: **`raspberrypi5` のみ**。**手順**: [deployment.md](../../archive/deployments/2026-05.md#kiosk-leaderboard-manual-order-row-highlight-2026-05-22)。**代表**: **`3acf4c5a`**。**CI**: **`26281606000` success**。**Detach**: `20260522-192111-31816`（**`ok=134` `changed=4` `failed=0`**·`--follow` 約 **816s**）。**Phase12**: **43/0/0**（約 **96s**）。**Pi4/Pi3**: `no hosts matched`（正）。

**本番デプロイ（2026-05-21・ブランチ `feat/signage-leader-order-cards-5x2-grid-10`）**: **5列×2段・最大/既定10**（API + 管理 Web）。**対象**: **`raspberrypi5` のみ**。**手順**: [deployment.md](../../archive/deployments/2026-05.md#signage-leader-order-cards-5x2-grid-10-2026-05-21)。**代表**: **`0fa2d065`**。**CI**: **`26204469250` success**。**Detach**: `20260521-131417-25249`（**`ok=134` `changed=4` `failed=0`**）。**Phase12**: **43/0/0**（約 **28s**）。**`main` マージ**: [PR #314](https://github.com/denkoushi/RaspberryPiSystem_002/pull/314)（squash **`3e37248f`**）。

**本番デプロイ（2026-05-21・ブランチ `fix/signage-leader-order-due-date-from-prisma-date`）**: **納期 `Date` 正規化**（API のみ）。**対象**: **`raspberrypi5` のみ**。**手順**: [deployment.md](../../archive/deployments/2026-05.md#signage-leader-order-due-date-prisma-date-2026-05-21)。**代表**: **`83501b27`**。**CI**: **`26203259837` success**。**Detach**: `20260521-123106-4810`（**`ok=134` `changed=4` `failed=0`**）。**Phase12**: **43/0/0**（約 **27s**）。**`main` マージ**: [PR #313](https://github.com/denkoushi/RaspberryPiSystem_002/pull/313)（squash **`2ca80f84`**）。

**本番デプロイ（2026-05-21・ブランチ `feat/signage-leader-order-cards-compact-incomplete`）**: **コンパクト化＋未完行のみ表示**（API のみ）。**対象ホスト**: **`raspberrypi5` のみ**（`--limit raspberrypi5`・1 台）。**Pi4 / Pi3**: **`skipping: no hosts matched`**（**Pi3 専用手順は未実施で正**）。**手順**: [deployment.md](../../archive/deployments/2026-05.md#signage-leader-order-cards-compact-incomplete-2026-05-21) どおり **`RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`**・`./scripts/update-all-clients.sh feat/signage-leader-order-cards-compact-incomplete infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`（**`main` マージ後は第2引数を `main`**）。**代表コミット**: **`bdc25afb`**。**CI**: GitHub Actions **`26201804495` success**。**Detach Run ID**: `20260521-114344-436`。**`PLAY RECAP`**: `raspberrypi5` **`ok=134` `changed=4` `failed=0` / `unreachable=0`**・リモート **`exit=0`**・ローカル **`--follow` 約 560s**。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **33s**）。**`main` マージ**: [PR #312](https://github.com/denkoushi/RaspberryPiSystem_002/pull/312)（squash **`2667fc7f`**）。

**本番デプロイ（2026-05-21・ブランチ `feat/signage-leader-order-kiosk-aligned`）**: **キオスク最新レイアウト整合**（API のみ・`leader-order-cards/` 全面更新）。**対象ホスト**: **`raspberrypi5` のみ**（`--limit raspberrypi5`・1 台）。**Pi4 キオスク / Pi3 サイネージ**: Ansible play **`skipping: no hosts matched`**（**Pi3 専用手順は未実施で正**・JPEG は Pi5 から `signage-lite-update` 取得）。**手順**: [deployment.md](../../archive/deployments/2026-05.md#signage-leader-order-kiosk-aligned-2026-05-21) どおり **`RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`**・`./scripts/update-all-clients.sh feat/signage-leader-order-kiosk-aligned infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`（**`main` マージ後は第2引数を `main`**）。**代表コミット**: **`7b54d992`**。**CI（機能先行）**: GitHub Actions **`26199566787` success**。**Detach Run ID**: `20260521-104347-10606`（接頭辞 `ansible-update-`）。**`PLAY RECAP`**: `raspberrypi5` **`ok=134` `changed=4` `failed=0` / `unreachable=0`**・リモート **`exit=0`**・サマリ **`Git: changed`**・ローカル **`--follow` 約 604s（約 10m）**。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **27s**・Tailscale・Pi5 API `100.106.158.2`）。**`main` マージ**: [PR #311](https://github.com/denkoushi/RaspberryPiSystem_002/pull/311)（squash **`6cb3bb1e`**）。

**実機検証（自動・2026-04-08 readability）**: `./scripts/deploy/verify-phase12-real.sh`（Mac / Tailscale）→ **PASS 43 / WARN 0 / FAIL 0**（約 **29s**）。
...
- **カード内行が少ない／空カード**: 資源内が **全完了** → 未完フィルタで **行ゼロ**（仕様）。キオスク **完了フィルタ**と比較。
