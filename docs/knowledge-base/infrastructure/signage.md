...
- **デザインの目安**: 静的プレビュー [signage-leader-order-cards-kiosk-aligned-preview.html](../../design-previews/signage-leader-order-cards-kiosk-aligned-preview.html)（キオスク最新準拠・**2026-05-21 本番反映済み**）。旧レイアウトは [signage-leader-order-cards-preview.html](../../design-previews/signage-leader-order-cards-preview.html)。
- **キオスクと見た目が違う（2026-05-21 以降）**: Pi5 **`api` ref** が **`7b54d992` 以降（またはマージ後 `main` HEAD）**か確認。**Pi4 デプロイは不要**（本変更は API のみ）。Pi3 は **Pi5 未更新**を疑う（上記「JPEG 正本は Pi5」）。**`deviceScopeKey`** はデータスコープのみで、ホストのデプロイ先ではない。
...
**解決状況**: ✅ **実装・本番デプロイ（Pi5 のみ・初回〜4×8〜readability〜キオスク整合 2026-05-21）・Phase12 実機検証（PASS 43/0/0）記録済み**
