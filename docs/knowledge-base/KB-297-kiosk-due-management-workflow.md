...
- **Fix（境界分離）**:
  ...
  - 保持テーブル **`PurchaseOrderLookupRow`**（**2026-04-21 以降**: `sourceCsvDashboardId` + `purchaseOrderNo` + `seiban` + **`purchasePartCodeNormalized`** の複合一意で **upsert** し、Gmail 取り込みで **履歴を上書き全消ししない**。マイグレ適用前に同一キー重複があれば **最新 `updatedAt`（同順位なら `id`）を残して削除**。購買 CSV の `FHINCD` は括弧suffix除去で正規化し、表示は **`purchasePartCodeRaw` を保持**。既存生産日程 winner 行の **`FHINMEI`** と突合。機種名は既存 **`resolveSeibanMachineDisplayNamesBatched`**、未解決は **空文字**。
  - 照会時、生産日程 **`ProductionScheduleOrderSupplement.plannedStartDate`** を **`(FSEIBAN, 正規化FHINCD)` 粒度で最古**（複数行ある場合の最小日付）として合成し、API 行 DTO に **`plannedStartDate`**（`YYYY-MM-DD` または `null`）を付与。Web は **`PurchaseOrderLookupResultList`** で **着手日**を **機種→製番→着手日→品名→品番→個数** の並びで表示。
