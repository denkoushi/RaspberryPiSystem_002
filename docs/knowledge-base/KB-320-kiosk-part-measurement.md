...
| **API 必須** | `createEntry`: 社員/機器タグ必須 · `updateEntry`: 欠落分のみ必須（既存は保持）· `complete`: registration 欠落 entry を **409** |
...
| **新方式の検査記録承認** | キオスク「検査記録確認」で ACTIVE 社員 NFC を承認者証跡として保存。同一 transaction で pending 公差外値を `APPROVED` にし、必要 slot 入力・測定者/測定機器登録が揃っていれば `completedAt` を設定 |
...
| **一覧状態** | `input_incomplete`（required slot 未入力/値不足）· `registration_incomplete`（測定者/測定機器不足）· `approvable` · `approved` |
...
| 症状 | 確認 | 対処 |
|------|------|------|
| 保存が **400 測定者/機器タグ必要** | 右ペイン NFC パネルが未完了か | 両タグをスキャン · 未登録タグは管理画面で社員/計測機器を確認 |
...
- 自主検査入力画面は **測定者/測定機器 NFC 登録必須**（2026-06-09 統合）。保存・完了の契約と制約は [§NFC 登録と手元カメラ実験](#自主検査-nfc-登録-手元カメラ実験-2026-06-09) を正本とする。
