...
Runbook: [§流用導線](../runbooks/kiosk-part-measurement.md#検査図面-流用導線-2026-06-05) · deployment: [§2026-06-05](../archive/deployments/2026-06.md#kiosk-inspection-drawing-reuse-flow-2026-06-05)
...
| 本番 | HEAD **`09a1fe66`** · Detach Pi5/StoneBase **`20260709-223044-17975`** · Pi4×4 **`20260709-224140-20418`** · Phase12 **45/0/0** · [deployment](../archive/deployments/2026-07.md#inspection-drawing-ocr-local-candidates-2026-07-09) |
...
| 本番 | 実装 HEAD **`9811d39a`** / enable HEAD **`ba1d781a`** · Detach enable **`20260710-101808-6154`** · timeout20 **`20260710-102238-19166`** · RapidOCR flag **ON** · [deployment enable](../archive/deployments/2026-07.md#inspection-drawing-ocr-rapidocr-enabled-2026-07-10) |
...
Runbook: [§ボタン活性](../runbooks/kiosk-part-measurement.md#自主検査-セッション操作ボタン活性-2026-06-04) · [deployment §ボタン活性](../archive/deployments/2026-06.md#kiosk-self-inspection-session-button-actions-2026-06-04)
...
Runbook: [§ボタンUI](../runbooks/kiosk-part-measurement.md#自主検査-セッション-ボタンui統一-2026-06-05) · [deployment §ボタンUI](../archive/deployments/2026-06.md#kiosk-self-inspection-session-button-ui-2026-06-05)
...
Runbook: [§ガイドフォーカス](../runbooks/kiosk-part-measurement.md#自主検査-ガイド付きフォーカス-2026-06-04) · [deployment §2026-06-04](../archive/deployments/2026-06.md#kiosk-self-inspection-guided-focus-reset-trial-2026-06-04)
...
Runbook: [§フルリセット・ガイド試行](../runbooks/kiosk-part-measurement.md#自主検査-フルリセット-ガイド試行-2026-06-04) · [deployment §2026-06-04](../archive/deployments/2026-06.md#kiosk-self-inspection-guided-focus-reset-trial-2026-06-04)
...
| 参照デプロイ手順 | [deployment.md §2026-06-03](../archive/deployments/2026-06.md#kiosk-self-inspection-four-modes-and-tolerance-2026-06-03) |
...
- **検証**: Web unit · 一時 Postgres migrate + EXPLAIN（tip 列）。本番は Pi5 + Pi4×5（HEAD **`eb41870a`**）· verify PASS 45 · smoke（`updatedAt` / tip dirty / `onResetZoom` 未配線 / 列ラベル）。詳細は [deployment](../archive/deployments/2026-07.md#inspection-drawing-callout-dirty-zoom-dates-2026-07-10)。
...
- **検証**: API/Web unit · 一時 Postgres migrate + EXPLAIN（persistenceStatus index）。本番は同ブランチで Pi5 + Pi4×5 反映済み（[deployment](../archive/deployments/2026-07.md#self-inspection-autosave-callout-template-lock-2026-07-10) · 後続 fix [deployment](../archive/deployments/2026-07.md#inspection-drawing-callout-dirty-zoom-dates-2026-07-10)）。
...
- **本番**: Pi5 + Pi4×5 · HEAD **`b52931bd`** · Phase12 **PASS 45** · [deployment](../archive/deployments/2026-07.md#self-inspection-confirm-guard-wip-draft-2026-07-11) · PR [#970](https://github.com/denkoushi/RaspberryPiSystem_002/pull/970)。
...
| デプロイ | [deployment §2026-06-05](../archive/deployments/2026-06.md#kiosk-inspection-drawing-point-nudge-2026-06-05) |
...
**正本 Runbook**: [kiosk-part-measurement §フラット band](../runbooks/kiosk-part-measurement.md#検査図面-作成改版ヘッダー-フラット-band-2026-06-04) · **デプロイ**: [deployment.md §2026-06-04](../archive/deployments/2026-06.md#kiosk-inspection-drawing-create-header-flat-layout-2026-06-04)
...
**正本 Runbook**: [kiosk-part-measurement §図面空白](../runbooks/kiosk-part-measurement.md#自主検査セッション図面空白-2026-06-03) · **デプロイ**: [deployment.md §図面空白](../archive/deployments/2026-06.md#kiosk-self-inspection-session-drawing-blank-2026-06-03) · コミット **`9f3f0bac`** · **`main` マージ**
...
- **Pi5 実機 curl**: SSH 先 `127.0.0.1:3000` は Caddy 構成では **000** になりうる。**Tailscale HTTPS** で `/api/...` を叩く（[deployment §PDF プレビュー](../archive/deployments/2026-06.md#kiosk-inspection-drawing-pdf-preview-parity-2026-06-02)）。
...
正本 ADR: [ADR-20260530-kiosk-inspection-drawing-dev-preview-parity.md](../decisions/ADR-20260530-kiosk-inspection-drawing-dev-preview-parity.md)。ExecPlan / Runbook / [deployment §プレビュー](../archive/deployments/2026-05.md#kiosk-inspection-drawing-preview-parity-2026-05-30) と併読。
...
正本 ExecPlan: [kiosk-inspection-drawing-mvp-execplan.md](../plans/kiosk-inspection-drawing-mvp-execplan.md)。Runbook: [kiosk-part-measurement.md §フィルタ overflow](../runbooks/kiosk-part-measurement.md#検査図面-一覧フィルタ-overflow-2026-05-30)。deployment: [deployment.md §2026-05-30](../archive/deployments/2026-05.md#kiosk-inspection-drawing-library-filter-overflow-2026-05-30)。
...
正本 ExecPlan: [kiosk-inspection-drawing-mvp-execplan.md](../plans/kiosk-inspection-drawing-mvp-execplan.md)。Runbook: [kiosk-part-measurement.md §キャンバスズーム](../runbooks/kiosk-part-measurement.md#検査図面-キャンバスズーム-2026-05-30)。deployment: [deployment.md §2026-05-30](../archive/deployments/2026-05.md#kiosk-inspection-drawing-canvas-zoom-2026-05-30)。
...
- **2026-03-30 実績（visual template 本番反映後・`feat/part-measurement-visual-template`）**: [deployment archive](../archive/deployments/legacy-operator-guide-through-2026-07.md) に記録した旧ローリング形式で **Pi5 → Pi4 キオスク 4 台**を一台ずつ更新（**Pi3 除外**）。旧run ID例: `20260330-144026-13597`（`raspberrypi5`）→ `20260330-144741-21698`（`raspberrypi4`）→ `20260330-145303-17447`（`raspi4-robodrill01`）→ `20260330-150059-11671`（`raspi4-fjv60-80`）→ `20260330-150516-1744`（`raspi4-kensaku-stonebase01`）、いずれも **`state: success` / `exitCode: "0"`**。**Phase12**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 37 / WARN 0 / FAIL 0**（約 117s・Mac / Tailscale）。当時の旧run status JSONとMac側historyの不一致を記録したが、現行運用では `--status RUN_ID` を正本にする。**手動残り**: visual付きテンプレで図面Blob表示・`displayMarker`列見出しを現場で目視（Runbook手順）。
...
- **2026-04-04 実績（編集画面上部帯統合・`feat/kiosk-part-measurement-edit-top-strip`）**: Web のみ（`KioskPartMeasurementEditTopStrip` / `KioskPartMeasurementSheetMetaBlock`、`KioskPartMeasurementEditPage` から旧ヘッダセクション撤去）。**デプロイ**: [deployment archive](../archive/deployments/2026-04.md) に記録した旧ローリング形式で **Pi5 → Pi4×4** を一台ずつ更新（Pi3除外）。旧run ID例は `20260404-082321-9627`（`raspberrypi5`）で、Pi4各台も **`PLAY RECAP failed=0`**。**Phase12**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 41 / WARN 0 / FAIL 0**（約106s・Mac / Tailscale・`resolve-ticket`スモーク含む）。**手動残り**: 編集画面で上部帯が窮屈すぎず、DRAFT/FINALIZED・図面ありで操作しやすいかを現場で目視。
...
- **2026-06-02 実績（検査図面 PDF プレビュー整合・`feat/inspection-drawing-pdf-import` → `main` `a3ce2284` PR #382）**: `POST /api/part-measurement/drawings/preview`（副作用なし JPEG）+ Web `usePartMeasurementDrawingLocalPreview` で **表示と保存の同一ラスタ**契約。**デプロイ**: [deployment.md §PDF プレビュー](../archive/deployments/2026-06.md#kiosk-inspection-drawing-pdf-preview-parity-2026-06-02) に従い **`--limit raspberrypi5` のみ**・`RASPI_SERVER_HOST`・`--detach --follow`。Detach Run ID: **`20260602-190538-1780`**（HEAD **`8307c995`** · **`PLAY RECAP failed=0`** · Docker `api`/`web` 再起動）。**Phase12**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 41 / WARN 1 / FAIL 1**（約 **84s** · FAIL は `raspberrypi4` SSH タイムアウトの既知到達性）。**自動**: preview API **401/200** · `pdftoppm` 22.12.0。**Pi5 キオスク目視 OK**（PDF プレビュー · 保存 · 再読込座標一致）。**CI**: **`26812045529`** success。**手動残り**: **`main` マージ後** Pi4×4 順次 · PDF 連打 503 の現場確認（任意）。
...
- アイテムは既存の2つの`<tr>`（上段 **43.3px**、下段 **51px**、合計カード高 **94.3px**）に収める。
  ペイン数、均等分割のgrid、各ペイン／カード／隣接表示領域の外枠寸法、アイテム順、遷移、callback、削除確認は変更しない。
- 上段は製番・機種名・品名・状態を同一行に置く。製番と品名は白色 **21px**、機種名だけ **15.75px** とする。
  製番の直後を起点に機種名を **1ch** 間隔で表示し、状態は品名領域の右端に文字で表示して必要な幅を確保する。
  状態の補足説明（例:「作業者の入力を続けて…」）は表示しない。
- 下段は白色の2行metaとし、各行を一行で省略する。1行目は製造order値・資源CD値・資源名値
  （「資源CD」「資源名」のタイトルは表示しない）、2行目は **更新**・**進捗**・**作業者** とする。
  資源は日本語名を主、CDを補助とし、名称不明時だけCDへfallbackする。
- 機種名を含むすべての長文は折返さずセル境界でellipsisとし、`title`と具体的な`aria-label`で全文を確認できるようにする。
  欠損値は`—`とし、ページ横スクロールは許容しない。
- 最終更新はsessionと生産日程行の`updatedAt`をAsia/Tokyoの分まで表示し、`occurredAt`を代用しない。
- 状態は短い状態名を文字で表示し、中立・注意・危険の限定したテーマ意味を併用する。
  色だけに依存せず、状態の補足説明は表示しない。操作は推奨する次操作を最大1つだけprimary、他をsecondary、削除をdangerとする。
  操作は横並びで必要時のみ折り返し、高さ30.8px、左右4px、上下0、14pxを維持する。
...
- 自主検査測定セッションの上辺は高さ54px固定、折返しなしの3領域とする。左端は1行目 **製造order**、2行目 **FHINCD**。資源CD、品名、方式、必要件数、ガイド状態は隣接する省略可能なメタ情報である。
- 中央は合計2行で、上段に認証済みの **現在の測定者／現在の検査員**、下段に`aria-live="polite"`の通知を表示する。右端の手元カメラ、ズーム、再開、次の測定点、初期化、一覧への操作は44px以上を維持する。
- 通知は **操作エラー → NFC登録エラー → ガイド一時通知 → 保存不可理由 → 完了／記録確認案内** の順で1件だけ表示する。上位通知が消えたら次順位を表示する。公差外未確認は赤、操作／NFCエラーはアンバー、ガイド／完了案内はシアン、その他の保存不可理由は中立色とする。
- 2026-07-28節の「保存不可理由は保存ボタン付近」を本契約で上書きする。保存不可理由、操作エラー、NFCメッセージ、ガイド通知、完了案内を右ペインやヘッダー直下へ重複表示しない。

### 右ペイン密度

- 測定者／検査員バナーは右ペインに置かず、固定ヘッダーだけに表示する。入力件タイトル直後に下書きの保存中／自動保存済／未同期バッジを置く。
- 入力件ボタンと自主検査用百分台キーだけを22px操作の明示的な例外とする。入力件ボタンは左右8px・16px、百分台は10列1行・各22px・16pxとする。ページ送りと他のキオスク操作は44px以上を維持する。
- 直接入力は`box-border w-full min-w-0 max-w-full`で360px右ペインに収める。値入力パネル下の重複するOK／NG等の状態は自主検査セッションだけ非表示にするが、公差外確認操作と測定点カード内の **NG・未確認／NG・確認済み** は残す。
- 測定点一覧は末尾の`min-h-0 flex-1`スクロール領域として回収した高さを受け取る。選択、ガイドblur抑止、状態overrideは従来どおりとする。
