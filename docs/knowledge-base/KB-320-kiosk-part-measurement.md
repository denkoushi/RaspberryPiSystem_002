...
- **API（候補）**: `GET /api/part-measurement/templates/candidates`（`fhincd`・`processGroup`・`resourceCd` 必須・任意 `fhinmei` / `q`）。`matchKind`: `exact_resource`（3要素一致）/ `same_fhincd_other_resource`（2要素・別資源）/ `fhinmei_similar`（1要素・品名ヒント・品番は異なることあり）。**いずれも選択可能**。
