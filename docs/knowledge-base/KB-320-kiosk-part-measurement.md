...
- **2026-04-04 追補（候補の正本定義・`feat/part-measurement-template-scope`・コードのみ）**: DB に `templateScope` / `candidateFhinmei`。1要素候補は **`FHINMEI_ONLY` + 日程 `fhinmei` 照合**（テンプレ `name` の部分一致は廃止）。2要素は **`FHINCD_RESOURCE` または `THREE_KEY` で工程のみ不一致**。`matchKind` は `two_key_fhincd_resource` / `one_key_fhinmei` を追加。デプロイ時は **`prisma migrate deploy`** が必須。
