...
- **管理 `/admin/tools/part-measurement-templates`・テンプレ「編集」（2026-04-05）**: 有効行の **編集** でフォームに読み込み。**登録スコープ・FIHNCD・資源CD・工程・FHINMEI候補キーは変更不可**。**保存**は `POST /api/part-measurement/templates/:id/revise`（**名称・測定項目・図面のみ**。DB 上は **次 `version` の新行**＋同系譜の旧版 `isActive: false`）。**`FHINMEI_ONLY`** も **同じ `resourceCd` 系譜**で版だけ上がる（新規登録のたびにランダム `resourceCd` になる挙動との差）。一覧は既定 **有効版のみ**・**無効版も表示**で旧版の **有効化**が可能。無効版を `revise` すると **409**。
