...
公差外値は「NG を OK に変える」ものではなく、現場リーダーが後で確認・承認する対象として保存する。
...
| **完了 API** | `pendingReviewCount > 0` の間は `complete` を 409 で拒否 |
| **承認済み** | ADMIN/MANAGER が管理画面で承認すると `reviewStatus=APPROVED`、承認者 ID/ユーザー名/日時/コメントを保存し、必要 slot と NFC 登録が揃っていればセッションを完了化 |

追加 API:

- `GET /api/part-measurement/self-inspection/out-of-tolerance-reviews` — 未承認公差外をセッション単位で取得。ADMIN/MANAGER JWT 必須。
- `POST /api/part-measurement/self-inspection/sessions/:id/out-of-tolerance-review/approve` — コメント付き承認。ADMIN/MANAGER JWT 必須。`x-client-key` 単独・VIEWER は不可。

代表ファイル:

- API: `self-inspection.service.ts`、`routes/part-measurement/index.ts`、migration `20260626120000_self_inspection_out_of_tolerance_review`
- Web: `InspectionDrawingValuePanel.tsx`、`selfInspectionDimensionValueInput.ts`、`selfInspectionGuidedFocus.ts`、`KioskSelfInspectionSessionPage.tsx`、`SelfInspectionOutOfToleranceReviewsPage.tsx`
...
- 現場リーダー専用ロール名は未実装。現時点の「現場リーダー確認」は ADMIN/MANAGER の管理ログインで代替する。
- 管理画面 `/admin/part-measurement/self-inspection-reviews` は実装済みだが、実データで「公差外のまま進む」→保存→`review_pending` 表示→承認コメント入力→完了化までの現場通し確認は次回の優先確認項目。
- 通知・エスカレーションは未実装。現仕様は順位ボード、自主検査一覧、機種別ボード、管理レビュー画面で `review_pending` を見せる範囲に留める。
