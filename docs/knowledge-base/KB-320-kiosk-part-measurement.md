...
| 項目 | 内容 |
|------|------|
| **NFC 登録** | 測定機器/測定者を **1行2列**（各 `min-w-0 truncate`）。`nextActionLabel`（スキャン案内）は **非表示**。`registration.message` と保存済みロック文言は維持 |
...
| 項目 | 内容 |
|------|------|
| **登録単位** | `SelfInspectionLotEntry`（入力件）ごと。測定機器 snapshot 列 + 既存測定者 snapshot |
| **API 必須** | 測定者タグは常に必須。測定機器タグは `SelfInspectionRegistrationPolicyConfig.requireMeasuringInstrumentTag` で必須/任意を切替（初期値は任意）。機器タグが送信された場合は従来どおりタグ解決・廃棄済みチェックを行う |
...
| **完了 API** | 新方式セッション（`recordApprovalRequiredAt != null`）は記録承認前の `complete` を 409 で拒否。旧方式は `pendingReviewCount > 0` の間だけ 409 |
| **旧方式の公差外承認** | `recordApprovalRequiredAt=null` の既存セッションのみ、ADMIN/MANAGER が管理画面で承認。`reviewStatus=APPROVED`、承認者 ID/ユーザー名/日時/コメントを保存し、条件が揃えば完了化 |
| **新方式の検査記録承認** | キオスク「検査記録確認」で ACTIVE 社員 NFC を承認者証跡として保存。同一 transaction で pending 公差外値を `APPROVED` にし、必要 slot 入力・測定者登録・ポリシー上必要な測定機器登録が揃っていれば `completedAt` を設定 |

##### 検査記録承認（2026-06-26 追補） {#自主検査-検査記録承認-2026-06-26}

| 区分 | 内容 |
|------|------|
| **対象データ** | `SelfInspectionSession.recordApprovalRequiredAt` が入った新規セッションのみ。migration 既存データは null のまま対象外 |
| **承認単位** | session 単位。公差内/公差外の保存済み検査記録をまとめて確認する |
| **一覧状態** | `input_incomplete`（required slot 未入力/値不足）· `registration_incomplete`（測定者不足、または設定 ON 時の測定機器不足）· `approvable` · `approved` |
| **入口ゲート** | 自主検査トップの「検査記録確認」。納期管理と同じ `2520` 初期パスワードを `x-client-key` 付きで検証し、通過状態は browser session のみ保持 |
...
| **旧 UI/API 境界** | 管理画面 `/admin/part-measurement/self-inspection-reviews` と旧承認 API は `recordApprovalRequiredAt=null` の legacy 用。新方式セッションの旧承認 API は 409 |
...
- API: `self-inspection.service.ts`、`routes/part-measurement/index.ts`、`routes/kiosk/part-measurement-self-inspection-record-approval-auth.ts`、migration `20260626120000_self_inspection_out_of_tolerance_review` / `20260626143000_self_inspection_record_approval`
...
- 入力途中セッションも一覧に出し、`input_incomplete` / `registration_incomplete` / `approvable` で進捗と漏れを見せる。承認操作は `approvable` のみ有効。
...
- 現場実データで「検査記録確認」→ 2520 → 入力途中/登録不足/承認可能の一覧 → ACTIVE 社員 NFC → `completedAt` 設定までを目視記録する。
...
- 新方式の現場通し確認は「検査記録確認」→ 2520 → 入力途中/登録不足/承認可能の一覧 → ACTIVE 社員 NFC → 完了化までを優先する。旧管理画面は `recordApprovalRequiredAt=null` の legacy 用。
...
| 項目 | 内容 |
|------|------|
| **登録ポリシー** | 測定者タグは常に必須。測定機器タグは共有設定で必須/任意を切替。初期値は **任意** |
...
| **Web** | キオスク **自主検査 > 検査記録確認** の上辺メニューに **計測機器タグ必須 ON/OFF** を表示。切替後は一覧・詳細・セッション系 query を再取得 |
| **保存/承認** | OFF のときは社員タグだけで entry 保存・検査記録承認・完了が可能。ON に戻すと機器未登録 entry は再び `registration_incomplete` 扱い |
| **範囲外** | 複数計測機器記録、計測機器持出画面への遷移、持出記録との連携は未実装 |
...
次回再開時は、現仕様を「測定者タグは必須、計測機器タグは共有トグルで必須/任意。初期値は任意」と扱う。NFC タグ未登録の計測機器が残っていても、トグル OFF のままなら社員タグ + 測定値入力で保存・承認できる。

##### 未完了・次回候補

- 複数計測機器を 1 件の自主検査記録へ紐づける機能は未実装。現行 DTO / DB は単一 `measuringInstrumentId` 前提。
- 計測機器タグスキャン後に計測機器持出画面へ遷移し、点検内容確認後に氏名タグで戻る連携は未実装。
- 計測機器タグ必須トグルの変更履歴・監査ログは未実装。現行は共有 row の `updatedBy` / `updatedAt` のみ。
...
| 症状 | 確認 | 対処 |
|------|------|------|
| 保存が **400 測定者/機器タグ必要** | 右ペイン NFC パネルが未完了か · 検査記録確認の **計測機器タグ必須** が ON か | 測定者タグを必ずスキャン。ON 時だけ機器タグもスキャン · 未登録タグは管理画面で社員/計測機器を確認 |
...
- 自主検査入力画面は **測定者 NFC 登録必須**。測定機器 NFC 登録は [§計測機器タグ任意化](#自主検査-計測機器タグ任意化-2026-06-30) の共有設定で必須/任意を切り替える（初期値は任意）。
