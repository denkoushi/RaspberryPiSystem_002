...
| 導線 | 操作 | 備考 |
|------|------|------|
| **既存図面の再利用** | 新規作成画面 → 図面ボタン → **既存から選択** または **新規アップロード** | `visualSource` が単一真実源。`pickExisting` 時は `visualTemplateId` のみで保存可。検索は **API `q` + `limit=80`**（400ms debounce）。専用 API `GET …/inspection-drawing/visual-templates/:id/fhincd` で関連有効テンプレの品番が一意なら品番欄へ自動入力され、テンプレ名未手入力なら `図面名 + 品番` が自動提案される。`none`（テンプレ紐付き 0 件）時は図面名から品番トークンを抽出して自動入力するか、「この図面に紐づく品番が見つかりませんでした。品番を手入力してください。」を表示する（2026-07-08修正） |
...
| 図面新規で品番・テンプレ名に品番が入らない | 専用 API が `unique` / `none` / `multiple` のどれを返すか確認する。`none` かつ図面名に品番トークンがあれば自動入力 + 確認メッセージ。抽出不可なら「品番を手入力してください」メッセージ（2026-07-08修正）。テンプレ紐付き 0 件の図面は DB 上に品番が無いため手入力が必要。複数品番なら不整合扱い、手入力済みなら上書きしない |
...
4. 名称候補に `ネジ穴ピッチ` / `穴ピッチ` があり、既定で寸法公差として扱われること。
5. 管理設定で `幅=幾何公差` に保存後、キオスクを再読込し、`幅` の候補が `0` / `0.001`〜`0.009` に切り替わること。
6. 候補外の手入力値（例: `0.026`）が入力欄に保持され、保存時も従来どおり絶対 `lowerLimit` / `upperLimit` へ変換されること。
...
10. ツールバーは 1 行を維持し、保存状態は保存ボタン右の **文字のみ**、`テスト入力` / `ガイド試行` / `保存済み帳票` / `一覧へ戻る` は右端に並ぶこと。
11. 新規作成で既存図面を選んだとき、同一図面の関連有効テンプレ品番が一意なら品番欄へ自動入力されること。複数品番が紐付く図面では自動入力せずメッセージが出ること。テンプレ紐付き 0 件（`none`）の図面では、図面名フォールバックまたは「品番を手入力してください」メッセージが出ること（2026-07-08修正）。
12. 測定点の **補足** 行で `面`（`正面` / `背面` / `両面`）、`呼び径`（`M3`〜`M30`）、直接入力を併用でき、保存後に `名称 / 呼び径 / 面 / 自由補足` 形式で一覧・テスト入力・帳票へ表示されること。
