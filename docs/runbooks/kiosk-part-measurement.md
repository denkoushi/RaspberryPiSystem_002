...
| 導線 | 操作 | 備考 |
|------|------|------|
| **既存図面の再利用** | 新規作成画面 → 図面ボタン → **既存から選択** または **新規アップロード** | `visualSource` が単一真実源。`pickExisting` 時は `visualTemplateId` のみで保存可。検索は **API `q` + `limit=80`**（400ms debounce）。関連有効テンプレの品番が一意なら品番欄へ自動入力される |
