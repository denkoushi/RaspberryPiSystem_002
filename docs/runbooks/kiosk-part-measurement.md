...
| 導線 | 操作 | 備考 |
|------|------|------|
| **既存図面の再利用** | 新規作成画面 → 図面ボタン → **既存から選択** または **新規アップロード** | `visualSource` が単一真実源。`pickExisting` 時は `visualTemplateId` のみで保存可。検索は **API `q` + `limit=80`**（400ms debounce）。専用 API `GET …/inspection-drawing/visual-templates/:id/fhincd` で関連有効テンプレの品番が一意なら品番欄へ自動入力され、テンプレ名未手入力なら `図面名 + 品番` が自動提案される |
...
| 図面新規で品番・テンプレ名に品番が入らない | 専用 API `GET …/inspection-drawing/visual-templates/:id/fhincd` が `unique` を返すか確認する。複数品番なら不整合扱い、手入力済みなら上書きしない |
...
11. 新規作成で既存図面を選んだとき、同一図面の関連有効テンプレ品番が一意なら品番欄へ自動入力されること。複数品番が紐付く図面では自動入力せずメッセージが出ること。
