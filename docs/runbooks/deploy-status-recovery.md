...
| 納期管理 表面処理別納期（2026-03-13追加） | 右ペインヘッダーに「製番納期」ボタンと「製番内で使用中の表面処理別納期」ボタンが併存すること。表面処理別納期を設定すると該当部品のみ更新され、製番納期更新時に上書き済み表面処理は維持されること。表面処理別納期を解除すると製番納期へフォールバックすること | ボタン表示・上書き保持・解除フォールバック正常 |
