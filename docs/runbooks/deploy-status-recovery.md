...
| 手動順番 overview 割当のみ・カードラベル短縮・下ペイン折りたたみ（2026-03-20追加・2026-03-21デプロイ検証済み） | **API（v2）**: `manual-order-overview?siteKey=...` の各端末 `resources[]` が **資源割当に登録したCDのみ**であること（割当に無い行集計は末尾に付けない）。**Web**: 上ペインカードのロケーション行で、選択工場と重複する `{siteKey} - ` プレフィックスが除去されていること。**UI（実機/VNC・マウス）**: 下ペインのツールバー＋資源帯は、絞り込み無しのとき折りたたみ・上端ホバーで展開；製番検索・工程＋資源など **絞り込みが有効なときは常時表示**。**タッチのみ**ではホバーできず帯が畳んだままになりうる（最上段メニューと同趣旨）。**デプロイ**: ブランチ `feat/manual-order-pane-assignment-label-toolbar`、Pi5 → raspberrypi4 → raspi4-robodrill01 のみ（Pi3 除外）・1台ずつ。Run ID: `20260321-145746-1455` / `20260321-150253-8405` / `20260321-150735-6173`。**自動検証**: `./scripts/deploy/verify-phase12-real.sh` **PASS 28/0/0**（2026-03-21）。**参照**: [KB-297 ペイン polish 節](../knowledge-base/KB-297-kiosk-due-management-workflow.md#manual-order-pane-polish-2026-03-21) / [資源割り当て節](../knowledge-base/KB-297-kiosk-due-management-workflow.md#manual-order-resource-assignment-2026-03-20)。 | Phase12 合格・ラベル／帯は実機目視推奨 |
