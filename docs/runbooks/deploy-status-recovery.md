...
| 手動順番 専用ページ（2026-03-20追加） | **UI（実機/VNC）**: ヘッダーから `/kiosk/production-schedule/manual-order` へ遷移できること。上ペインで端末カード・鉛筆で編集対象切替・編集中バナー/グレーアウト。下ペインで既存生産スケジュールと同様に順番保存（失敗時はカード強調＋通知）。**API**: 専用の新規RESTはなし（overview/order は既存）。**自動検証**: Phase12 に含まれる（overview v2）。Mac 直ブラウザは自己署名で失敗しやすい → 実機推奨（[KB-306](../knowledge-base/frontend.md#kb-306-キオスク進捗一覧-製番フィルタドロップダウン端末別保存) と同趣旨）。**参照**: [KB-297](../knowledge-base/KB-297-kiosk-due-management-workflow.md#手動順番-専用ページキオスク追加2026-03-20) | 遷移・俯瞰・編集・保存フィードバック正常（データ無し環境は表示のみ確認可） |
