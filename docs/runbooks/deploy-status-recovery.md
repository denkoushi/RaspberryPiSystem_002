...
| 生産順序モード拡張（2026-03-19追加） | **API**: `curl -sk "https://100.106.158.2/api/kiosk/production-schedule/due-management/manual-order-overview" -H "x-client-key: client-key-raspberrypi4-kiosk1"` が 200 かつ `actorLocation` / `targetLocation` / `resources` を含むこと。**UI（実機）**: 生産スケジュール画面で (1) 完了/未完カウント右側に「自動順番」「手動順番」トグルが表示されること、(2) デフォルトが手動順番であること、(3) 単一資源CD選択時のみ資源順位ドロップダウンが有効であること、(4) 納期管理画面左ペインに「手動順番 全体像」パネルが表示され、工程別設定件数・乖離・最終更新が確認できること。 | API 200・UI トグル・単一CD時編集可・全体像パネル正常 |
| 代理更新UI連携（案B、2026-03-19追加） | **UI（実機）**: 納期管理画面の対象拠点セレクタが全端末で表示されること。対象拠点を変更した後に生産スケジュール画面で順番更新したとき、許可端末では更新成功すること。非許可端末で他拠点更新時は「この端末では選択中の対象拠点へ手動順番更新できません。」メッセージが表示されること。 | 全端末表示・成功時更新・非許可時403メッセージ表示 |
