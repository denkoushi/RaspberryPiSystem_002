...
| 手動順番 overview 割当のみ・カードラベル短縮・下ペイン折りたたみ（2026-03-20追加） | **API（v2）**: `manual-order-overview?siteKey=...` の各端末 `resources[]` が **資源割当に登録したCDのみ**であること（割当に無い行集計は末尾に付けない）。**Web**: 上ペインカードのロケーション行で、選択工場と重複する `{siteKey} - ` プレフィックスが除去されていること。**UI（実機/VNC・マウス）**: 下ペインのツールバー＋資源帯は、絞り込み無しのとき折りたたみ・上端ホバーで展開；製番検索・工程＋資源など **絞り込みが有効なときは常時表示**。**タッチのみ**ではホバーできず帯が畳んだままになりうる（最上段メニューと同趣旨）。**参照**: [KB-297 資源割り当て節](../knowledge-base/KB-297-kiosk-due-management-workflow.md#manual-order-resource-assignment-2026-03-20) / [手動順番専用ページ節](../knowledge-base/KB-297-kiosk-due-management-workflow.md#手動順番-専用ページキオスク追加2026-03-20)。 | `merge-manual-order-resource-assignments` テスト・実機目視推奨 |
