...
| 手動順番 overview 割当のみ・カードラベル短縮・下ペイン折りたたみ（2026-03-20追加・2026-03-21デプロイ検証済み） | **API（v2）**: `manual-order-overview?siteKey=...` の各端末 `resources[]` が **資源割当に登録したCDのみ**であること（割当に無い行集計は末尾に付けない）。**Web**: 上ペインカードのロケーション行で、選択工場と重複する `{siteKey} - ` プレフィックスが除去されていること。**UI（実機/VNC・マウス）**: 下ペインのツールバー＋資源帯は既定で折りたたみ。**見出し行右端のスライダー型アイコン**へホバーで展開、パネルから離れると遅延後に折りたたみ（`ManualOrderLowerPaneCollapsibleToolbar` + `useTimedHoverReveal`）。**絞り込み有無は帯の開閉に影響しない**（一覧取得・「検索してください」等は従来の `hasScheduleFilterQuery`）。**タッチのみ**ではホバー不可。**デプロイ（割当・ラベル・初回下ペイン折りたたみ）**: ブランチ `feat/manual-order-pane-assignment-label-toolbar`、Pi5 → raspberrypi4 → raspi4-robodrill01 のみ（Pi3 除外）・1台ずつ。Run ID: `20260321-145746-1455` / `20260321-150253-8405` / `20260321-150735-6173`。**下ペイン UI 追従（右端ホバー・開閉と絞り込み分離）**: ブランチ `feat/manual-order-lower-pane-toolbar-hover`。**自動検証**: `./scripts/deploy/verify-phase12-real.sh` **PASS 28/0/0**（2026-03-21）。**参照**: [KB-297 ペイン polish 節](../knowledge-base/KB-297-kiosk-due-management-workflow.md#manual-order-pane-polish-2026-03-21) / [資源割り当て節](../knowledge-base/KB-297-kiosk-due-management-workflow.md#manual-order-resource-assignment-2026-03-20)。 | Phase12 合格・ラベル／帯は実機目視推奨 |
