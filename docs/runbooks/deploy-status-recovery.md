...
| location scope fallback監視（2026-03-15追加） | `ssh denkon5sd02@100.106.158.2 "cd /opt/RaspberryPiSystem_002 && docker compose -f infrastructure/docker/docker-compose.server.yml logs --since=10m api | rg 'Resource category policy resolved via fallback path'"` | 想定外に `legacyString` / `default` が増えていない（移行監視） |
