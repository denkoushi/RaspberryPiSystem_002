...
デプロイ完了後に以下を確認する（2026-03-06 実機検証で実施済み。2026-03-10 全端末共有優先順位デプロイ後、2026-03-11 ロケーション間共有化デプロイ後、2026-03-15 Location Scope Phase7/Phase8/Phase9/Phase0-4 でも同チェックリストで検証済み）:
...
| location scope fallback監視（2026-03-15追加） | `ssh denkon5sd02@100.106.158.2 "cd /opt/RaspberryPiSystem_002 && docker compose -f infrastructure/docker/docker-compose.server.yml logs --since=10m api | grep 'Resource category policy resolved via fallback path'"`（注: Pi5に`rg`は未導入のため`grep`を使用） | 想定外に `legacyString` / `default` が増えていない（移行監視） |
