...
| 手動順番 上ペイン 資源CD割り当て（2026-03-20追加） | **前提**: device-scope v2 有効。**DB**: `prisma migrate deploy` で `ProductionScheduleManualOrderResourceAssignment` が作成されること。**API**: v2 分岐で `./scripts/deploy/verify-phase12-real.sh` が `manual-order-resource-assignments?siteKey=...` の JSON に `"assignments"` を含むことを検証。**UI（実機/VNC）**: `/kiosk/production-schedule/manual-order` でカードに **「資源」** ボタンがあり、モーダルで候補追加・上下・保存後に上ペインの資源順が変わること。未割当カードは **資源未割り当て**。割当のみで行が無い資源は **未設定**。同一資源を二重割当すると **409**。**参照**: [KB-297 資源割り当て節](../knowledge-base/KB-297-kiosk-due-management-workflow.md#manual-order-resource-assignment-2026-03-20)。 | migrate・Phase12（assignments 検証）・モーダル保存で overview 更新 |
