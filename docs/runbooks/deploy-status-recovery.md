...
| 進捗一覧 5列・ラベル削除・presentation 分割（2026-03-22） | **Web のみ**（API 不変）。[`progressOverviewPresentation.ts`](../../apps/web/src/features/kiosk/productionSchedule/progressOverviewPresentation.ts)・[`ProgressOverviewSeibanCard`](../../apps/web/src/components/kiosk/progressOverview/ProgressOverviewSeibanCard.tsx)・[`ProgressOverviewPartRow`](../../apps/web/src/components/kiosk/progressOverview/ProgressOverviewPartRow.tsx)。**UI**: `xl:grid-cols-5`、黄色「納期」「実」削除、部品名・資源CD列に `min-w-0`。**デプロイ**: ブランチ `feat/kiosk-progress-overview-five-cols-layout`、Pi5 → raspberrypi4 → raspi4-robodrill01 のみ（Pi3 除外）・1台ずつ、`RASPI_SERVER_HOST`・`--detach --follow`。**Run ID**: `20260322-084633-25240`（Pi5）/ `20260322-085040-1342`（raspberrypi4）。**raspi4-robodrill01**: 2026-03-22 時点 **未デプロイ**（SSH `100.123.1.113` タイムアウト。復旧後に `--limit "raspi4-robodrill01"` で再実行）。**自動検証**: `./scripts/deploy/verify-phase12-real.sh` **PASS 26 / WARN 1 / FAIL 1**（FAIL=robodrill SSH、WARN=Pi3 可 offline）。**ローカル**: `NODE_ENV=test POSTGRES_PORT=5432 pnpm test:api` 推奨。**参照**: [KB-297 進捗一覧5列節](../knowledge-base/KB-297-kiosk-due-management-workflow.md#progress-overview-five-cols-layout-2026-03-22)。 | Pi5・raspberrypi4 反映済み。robo 復旧後にデプロイと Phase12 再実行 |
