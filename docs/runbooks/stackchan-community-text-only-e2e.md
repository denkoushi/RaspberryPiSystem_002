...
| STT 直後、bridge が `request read timeout after ...s` / `408 REQUEST_TIMEOUT`、実機が read **`errno -11`** 等 | **`STACKCHAN_REQUEST_READ_TIMEOUT_SEC` を正に設定していて狭すぎ**ると **WAV 本文を読み切る前**にソケットが切れうる（**未設定・0 は無制限読取**）。**Pi5 bridge `.env` で 30〜120s 級**へ上げるか **0 に戻す**（KB §2026-05-13・[`private-pi5-stackchan-bridge` README](../../scripts/private-pi5-stackchan-bridge/README.md)）。 |
