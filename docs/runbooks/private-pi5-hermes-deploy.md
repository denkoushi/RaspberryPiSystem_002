...
| `/interest search 今日の天気` | Brave Search API が有効な時だけ一回検索し、結果を番号付き feedback 対象にする |
...
| 操作 | 期待 |
|------|------|
| `/ask 明日の大阪の天気を調べて` | Hermes built-in web で調査し、簡潔な回答と短い出典を返す |
...
11. Web research（D20）:
   - `/ask 明日の大阪の天気を調べて` → Web 調査結果と短い出典が返ること
