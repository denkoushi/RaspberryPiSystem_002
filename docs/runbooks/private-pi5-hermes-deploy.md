...
3. Pi5 で plugin コマンド一覧に `memo` `inbox` `digest` `remind` `recommend` があること
4. Discord 側で Life Pilot コマンド（`/memo` `/inbox` `/digest` `/remind` `/recommend` `/life-reply`）が補完に出ること（token 設定時の Ansible command sync）
5. Discord 受け入れ試験:
