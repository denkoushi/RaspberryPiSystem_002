...
- 長い PDF: スクロール時は **近傍のページ画像のみ** DOM に載る（Pi4 の負荷対策）。オフスクリーンはプレースホルダ＋ `loading="lazy"`。**2026-03-26 以降**: 一覧行ホバー/フォーカスで **詳細 API をデバウンス付きプリフェッチ**、IO 更新は **rAF で間引き**、ページ画像は **ETag + Cache-Control**（`PDF_PAGES_CACHE_CONTROL`）で再利用しやすい（[KB-313](../knowledge-base/KB-313-kiosk-documents.md)）。
