...
- 左: 検索・取込元フィルタ・一覧（**既定は表示**。ツールバー左の **一覧アイコン** で開閉し、表示領域をビューアに譲る）。**2026-03-27 以降**: 各行 **上段**は確定文書番号があればそれを、無ければ表示タイトル系。**下段**は確定要約（[KB-313](../knowledge-base/KB-313-kiosk-documents.md)）。**2026-03-27 追記**: 要約は 2 行で切れるが、**マウスオーバーで `title` ツールチップ**に全文（[KB-313](../knowledge-base/KB-313-kiosk-documents.md)）。
