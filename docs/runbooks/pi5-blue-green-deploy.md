...
The command extracts the full commit SHA from the image tags, allows **new** Prisma migration files only (Expand-only allow-list SQL; modified existing migrations are refused), runs candidate `prisma migrate deploy` and then `migrate status` while the old API is still serving, then verifies the old API again. (`migrate status` exits non-zero while an expected new migration is unapplied, so it is used only as the final verification.) A non-Expand historical migration restored to source control is accepted only when the live DB has one completed, non-rolled-back `_prisma_migrations` row with the exact SHA-256 checksum; the check is read-only. DB rollback is never attempted; every newly applied migration must be Expand-only and compatible with both APIs.
