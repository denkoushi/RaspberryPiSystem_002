...
The command extracts the full commit SHA from the image tags, checks exact OCI labels for the Git SHA and Compose configuration hash, and reuses a matching candidate only after repeating Caddy validation and candidate API health. It pauses only the live signage renderer during candidate work, builds API and Web serially when reuse is impossible, and records pause/resume, reuse, and load samples in release state. Before and after a build, and in the Blue/Green resource guard, it requires three 20-second samples below the Pi5 limit (default `3.00`, 75% of four CPUs) for at most 10 minutes; timeout resumes signage and stops before routing or DB changes. Memory, disk, and JWT checks remain fail-closed.

For migrations, it verifies that **every** completed/non-rolled-back `_prisma_migrations` row has a candidate file with the exact SHA-256, then runs candidate `prisma migrate deploy` and `migrate status` while the old API is still serving before verifying the old API again. A migration file restored to Git after it was already applied is exempt from SQL-shape revalidation only after that full-history checksum check; database lookup failure, a missing file, or checksum mismatch fails closed. DB rollback is never attempted; every migration that can still execute must be Expand-only and compatible with both APIs.
