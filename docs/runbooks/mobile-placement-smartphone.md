...
**2026-04-24（V24 追記・一次元スキャン体感）**: `feat/kiosk-barcode-reader-tuning`（`main`）反映後、Android Chrome で **`/kiosk/mobile-placement` の製造order** と **`/kiosk/purchase-order-lookup` の注番**の **一次元**を読み取ったところ、**両方とも体感速度の改善**が場内確認された（**コア一次元＋`KIOSK_DEFAULT`**／**`PURCHASE_ORDER`＋同 `readerOptions`＋―当時― 2連続一致（`stabilityConfig`）**。[KB-339 §V24](../knowledge-base/KB-339-mobile-placement-barcode-survey.md#v24-barcode-reader-tuning-2026-04-23)）。**2026-04-24 V26 以降**は注番側も **即時確定**（下記）。

**2026-04-24（V26・購買照会即時確定・キオスク標準セッション）**: ブランチ **`fix/kiosk-purchase-order-barcode-instant`**（**`4bc2698f`**）。**仕様**: 購買照会 `/kiosk/purchase-order-lookup` のカメラスキャンは **配膳トップ・パレット可視化と同様に即時確定**（`stabilityConfig` 撤去）。`readerOptions` / `idleTimeoutMs` は **`KIOSK_STANDARD_BARCODE_SCAN_SESSION`** で共通化。**デプロイ（最小）**: **`raspberrypi5` のみ**・Detach **`20260424-102338-6782`**。**Phase12**: **PASS 43 / WARN 0 / FAIL 0**（約 **96s**）。**詳細**: [KB-339 §V26](../knowledge-base/KB-339-mobile-placement-barcode-survey.md#v26-purchase-order-barcode-instant-2026-04-24)。
...
- **`/kiosk/mobile-placement`** は **棚選択・製造 order 登録を優先**。**Zero2W の現在値一覧**は **`/kiosk/mobile-placement/zero2w-status`**（ボタン **「棚番配膳一覧（Zero2W）」**。既定ではメインの選択棚とはフィルタ共有しない・15s ポーリング）。
- Top 上辺の **「Zero2W担当棚」** から **`/kiosk/mobile-placement/zero2w-assignment`** へ遷移し、**`haizenEdgeEnabled` の端末を選ぶ → 棚番を選ぶ → 保存** で担当棚を設定する。
- 専用ページは **棚マスタの登録済み構造化棚**だけを候補にし、一覧・設定対象端末は **`ClientDevice.haizenEdgeEnabled === true`** のみ（**旧データはマイグレーションで `zero2w` 文字列一致をバックフィル**。新規端末は **管理画面「クライアント端末管理」の Zero2W配膳列**または **`PUT /api/clients/:id`**）。
...
- **別のreleaseが実行中と表示される**: run IDを確認して `scripts/update-all-clients.sh --status RUN_ID` を使う。停止が必要なら `--cancel RUN_ID --reason TEXT` を使い、process kill、lock削除、fleet/run state編集は行わない（[deploy-status-recovery.md](./deploy-status-recovery.md)）。
...
- **`haizen-target-devices` が常に空**: `ClientDevice.haizenEdgeEnabled` が **true** か、Pi5 に **`haizenEdgeEnabled` マイグレーション適用済み**か（[KB-368](../knowledge-base/KB-368-zero2w-haizen-placement-tracking.md)）
