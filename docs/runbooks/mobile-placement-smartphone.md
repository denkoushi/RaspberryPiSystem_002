...
最終更新: 2026-04-13（**V18 トップ登録アクション分離（新規登録／棚移動）**・**V17 部品検索最終（AND・登録済みのみ・剪定・`part-search-core` + CI/Docker）**・**V16 部品名検索**・**V15 照合折りたたみ・登録レイアウト**・V14 分配枝・V13 棚番登録 UI・V12 現品票 ROI・Schema 集約・V11 製造orderパーサ・global-filter／注文行除外・V10 本番反映・Pi5 worktree/root ownership・stale lock）
...
**2026-04-13（V18・トップ登録アクション分離: 「新規登録」／「棚移動」・右端単一「登録」廃止）**: ブランチ **`feat/mobile-placement-top-action-buttons`**（コミット **`91e485da`**）。**仕様（Web のみ）**: 下半の確定を **「新規登録」**（`POST …/register-order-placement`）と **「棚移動」**（`PATCH …/order-placement-branches/:id/move`）の **2 主ボタン**に分離。`+` は **棚番登録**（`/kiosk/mobile-placement/shelf-register`）専用。**API / DB 変更なし**。**デプロイ**: [deployment.md](../guides/deployment.md)・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/mobile-placement-top-action-buttons infrastructure/ansible/inventory.yml --limit <host> --detach --follow` を **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** の順に **1 台ずつ**（**Pi3 除外**）。**Detach Run ID**: `20260413-112046-12716` → `20260413-112510-648` → `20260413-112925-12155` → `20260413-113234-2579` → `20260413-113642-29855`（各 **`failed=0` / `unreachable=0`**）。**Phase12**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **29s**）。**ナレッジ**: [KB-339](../knowledge-base/KB-339-mobile-placement-barcode-survey.md) V18。**残作業（手動）**: Android で **新規／移動**の有効条件・送信中の挙動を目視。
...
3. **V14**: **「新規分配を追加」** か **「既存分配を移動」** を選ぶ。新規は **次の `branchNo`** で `POST …/register-order-placement`。移動は **`GET …/order-placement-branches`** で一覧を出し、枝を選んで **`PATCH …/order-placement-branches/:id/move`**。
4. **V18**: 確定は **「新規登録」** または **「棚移動」** のいずれか一方（右端の単一「登録」は廃止）。→ `OrderPlacementEvent` に履歴追記（**工具 `Item` は更新しない**）。現在棚は `OrderPlacementBranchState`（詳細は [api/mobile-placement.md](../api/mobile-placement.md)）
