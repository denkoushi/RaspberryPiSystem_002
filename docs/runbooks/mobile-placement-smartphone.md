...
- **別のreleaseが実行中と表示される**: run IDを確認して `scripts/update-all-clients.sh --status RUN_ID` を使う。停止や復旧が必要な場合は [deploy-status-recovery.md](./deploy-status-recovery.md) の標準手順に従い、process kill、lock削除、運用情報の手編集は行わない。
