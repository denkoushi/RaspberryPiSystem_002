...
相談用SOULと `business-consultation` Skillで、新しい質問を検索目的の候補へ言い換え、
選択後に元の質問と合わせて検索する。既存の `confirmation` / `selection` 契約と画面を使い、
新しい分類器、質問カタログ、検索エンジンは追加しない。
Hermes標準の `clarify` はAPIサーバーの標準toolsetから除外されているため、
この経路では最終回答として確認を表示し、次の利用者入力で会話を再開する。

受入では、初回に業務検索が走らないこと、選択と自由文の補足を引き継ぐこと、
同じ目的を再確認しないこと、原文と回答の一致を確認する。
候補表示までと選択後の完成回答までを別々に計測し、後者の目標を10秒以内とする。
これは最初の質問からの10秒以内を達成したことにはしない。
実機試用結果は未確認。標準releaseの設定バックアップとrollbackで旧SOUL・Skillへ復帰できる。
