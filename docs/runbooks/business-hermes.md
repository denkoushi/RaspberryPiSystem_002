...
### 検索前の目的確認の試用（2026-09-13）

新しい相談の最初の質問では、既存APIが元の質問を保持し、現在の情報源に対応する
「元の質問について記録で確認」「原因と確認ポイント」「公開要領の作業・検査方法」と自由文補足を表示する。
この初回表示と補足待ちではDGX・Hermesを呼ばず、業務検索も行わない。
既存の `confirmation` / `selection` 契約と画面を使う。
Hermes標準の `clarify` はAPIサーバーの標準toolsetから除外されるため、利用者入力を待つ境界はアプリで保持する。

選ばれた目的または自由文補足と元の質問は `confirmedIntent` としてHermesへ渡す。
相談用SOULへ `business-consultation` Skillをテンプレート展開時に含め、検索手順を初回から読込み済みにする。
APIのtoolsetは既存のbusiness_apiのみとし、Skill取得のためのLLM往復を発生させない。
Hermes標準の `agent.max_turns=2` で反復を制限する。上限時の最終要約はHermesが追加で呼ぶため、LLM呼出し総数が必ず2回という意味ではない。
通常は検索結果から短い本文で答え、不足時は条件確認を返す。記録や原文の表示指定がない通常質問では内部IDを生成しない。
初回候補をLLMで自由に生成する段階は含まない。継続中に目的や対象を変えた際の確認はSkillで扱う。

受入では、初回に推論・検索が走らないこと、選択と自由文の補足を引き継ぐこと、
同じ目的を再確認しないこと、原文と回答の一致を確認する。
候補表示までと選択後の完成回答までを別々に計測し、後者の目標を10秒以内とする。
これは最初の質問からの10秒以内を達成したことにはしない。
2026-09-13のSkillだけによる実機試用は、選択前に検索が始まり、候補ボタンも出ず不合格だった。
APIによる初回待機は実機Safariで0.7秒以内に表示できた。一方、旧「似た事例」選択後は86〜94秒かかり、
ID誤記による表示失敗と別症状の候補混入も発生した。番号を自由文で限定した場合は検索1回で原文に一致したが18秒だった。
このため選択肢の範囲保持、Skill事前読込み、標準の反復上限と本文回答へ修正する。修正版の実機結果は確認待ち。
復帰には標準releaseで以前のAPI、Hermes設定、SOUL・Skillを一緒に戻す。
