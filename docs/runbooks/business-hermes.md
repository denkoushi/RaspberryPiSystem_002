...
「過去事例と対策」「原因と確認ポイント」「公開要領の作業・検査方法」と自由文補足を表示する。
...
相談用SOULと `business-consultation` Skillは確認済みの目的で検索・回答する。
...
APIによる初回待機の実機確認は未実施。復帰には標準releaseで以前のAPIとSOUL・Skillを一緒に戻す。
