...
- CD-001: `group_vars/all.yml`の`network_mode`を変更 → 設定変更を検知
  - **目的**: 設定ファイルの変更を正確に検知し、デプロイの必要性を判断する
  - **成功基準**: JSON出力に`config_changes`配列に`network_mode`の変更が含まれ、`changed_keys`に`["network_mode"]`が含まれる
  - **失敗基準**: 変更が検知されない、またはJSON形式が不正
- CD-002: `server_ip`を変更 → 設定変更を検知
  - **目的**: 設定ファイルの変更を正確に検知し、デプロイの必要性を判断する
  - **成功基準**: JSON出力に`config_changes`配列に`server_ip`の変更が含まれ、`changed_keys`に`["server_ip"]`が含まれる
  - **失敗基準**: 変更が検知されない、またはJSON形式が不正
- CD-003: 複数の設定項目を同時に変更 → 全ての設定変更を検知
  - **目的**: 複数の設定変更を漏れなく検知し、全ての変更を記録する
  - **成功基準**: JSON出力に`config_changes`配列に全ての変更が含まれ、`changed_keys`に全ての変更キーが含まれる
  - **失敗基準**: 一部の変更が検知されない、または変更の数が一致しない
- CD-004: `apps/api/src/services/signage/signage.service.ts`を変更 → コード変更を検知、モジュール・エンドポイントを推定
  - **目的**: コードファイルの変更を検知し、影響範囲判定のための情報を提供する
  - **成功基準**: JSON出力に`code_changes`配列に該当ファイルの変更が含まれ、`change_type`が`modified`または`added`または`deleted`である
  - **失敗基準**: 変更が検知されない、またはファイルパスが不正
- CD-005: `apps/web/src/pages/kiosk/KioskReturnPage.tsx`を変更 → コード変更を検知、コンポーネントを推定
  - **目的**: フロントエンドコンポーネントの変更を検知し、影響範囲判定のための情報を提供する
  - **成功基準**: JSON出力に`code_changes`配列に該当ファイルの変更が含まれ、`change_type`が`modified`または`added`または`deleted`である
  - **失敗基準**: 変更が検知されない、またはファイルパスが不正
- CD-006: 複数のファイルを同時に変更 → 全ての変更を検知
  - **目的**: 複数のコード変更を漏れなく検知し、全ての変更を記録する
  - **成功基準**: JSON出力に`code_changes`配列に全ての変更が含まれ、変更の数が一致する
  - **失敗基準**: 一部の変更が検知されない、または変更の数が一致しない
- CD-007: Gitリポジトリがクリーンな状態 → 変更なしを検知、早期終了
  - **目的**: 変更がない場合に無駄な処理を避け、早期終了する
  - **成功基準**: JSON出力に`config_changes`と`code_changes`が空の配列`[]`である
  - **失敗基準**: 変更がないのに変更が検知される、またはJSON形式が不正
- CD-008: 変更がない場合の処理 → 適切なメッセージを出力して終了
  - **目的**: 変更がない場合に適切なメッセージを出力し、後続処理をスキップする
  - **成功基準**: JSON出力に`config_changes`と`code_changes`が空の配列`[]`であり、エラーが発生しない
  - **失敗基準**: エラーが発生する、またはJSON形式が不正

#### impact-analyzer
- IA-001: `network_mode`変更 → 全コンポーネント（server, pi4_kiosk, pi3_signage）を検出
  - **目的**: 設定変更の影響範囲を正確に判定し、適切なデプロイターゲットを決定する
  - **成功基準**: JSON出力に`impact_scope.server == true`、`impact_scope.pi4_kiosk == true`、`impact_scope.pi3_signage == true`が含まれる
  - **失敗基準**: 影響範囲が不完全、またはデプロイターゲットが誤っている
- IA-002: `server_ip`変更 → クライアント側のみを検出
  - **目的**: 設定変更の影響範囲を正確に判定し、サーバー側への不要なデプロイを避ける
  - **成功基準**: JSON出力に`impact_scope.server == false`、`impact_scope.pi4_kiosk == true`、`impact_scope.pi3_signage == true`が含まれる
  - **失敗基準**: サーバー側が影響範囲に含まれる、またはクライアント側が影響範囲に含まれない
- IA-003: `kiosk_url`変更 → Pi4キオスクのみを検出
  - **目的**: 設定変更の影響範囲を正確に判定し、不要なデプロイを避ける
  - **成功基準**: JSON出力に`impact_scope.server == false`、`impact_scope.pi4_kiosk == true`、`impact_scope.pi3_signage == false`が含まれる
  - **失敗基準**: Pi4キオスク以外が影響範囲に含まれる、またはPi4キオスクが影響範囲に含まれない
- IA-004: `SignageService.getContent()`変更 → Pi3サイネージ、管理コンソールを検出
  - **目的**: コード変更の影響範囲を正確に判定し、依存するコンポーネントを検出する
  - **成功基準**: JSON出力に`impact_scope.server == true`（API変更のため）、`impact_scope.pi3_signage == true`が含まれる
  - **失敗基準**: 影響範囲が不完全、または依存関係が考慮されていない
- IA-005: `KioskReturnPage.tsx`変更 → Pi4キオスクを検出
  - **目的**: フロントエンドコンポーネントの変更の影響範囲を正確に判定する
  - **成功基準**: JSON出力に`impact_scope.pi4_kiosk == true`が含まれる
  - **失敗基準**: Pi4キオスクが影響範囲に含まれない、または他のコンポーネントが誤って含まれる
- IA-006: `tools`モジュールのAPI変更 → 依存する`signage`モジュール、`kiosk`モジュールも検出
  - **目的**: モジュール間の依存関係を考慮し、間接的な影響範囲も検出する
  - **成功基準**: JSON出力に`impact_scope.server == true`（API変更のため）、`impact_scope.pi3_signage == true`（signage depends_on tools）、`impact_scope.pi4_kiosk == true`（kiosk depends_on tools）が含まれる
  - **失敗基準**: 依存モジュールが影響範囲に含まれない、または依存関係が考慮されていない
- IA-007: 複数の変更が同時に発生 → 影響範囲を統合して検出
  - **目的**: 複数の変更の影響範囲を統合し、重複を排除してデプロイターゲットを決定する
  - **成功基準**: JSON出力に全ての変更の影響範囲が統合され、`deploy_targets`配列に適切なターゲットが含まれる
  - **失敗基準**: 影響範囲が統合されない、または重複が発生する
- IA-008: 設定ファイルが存在しない場合 → 適切なエラーメッセージを出力
  - **目的**: 設定ファイルが存在しない場合に適切なエラーハンドリングを行う
  - **成功基準**: JSON出力に`overall_status: "error"`が含まれ、適切なエラーメッセージが出力される
  - **失敗基準**: エラーが検知されない、またはエラーメッセージが不適切

#### deploy-executor
- DE-001: サーバー側デプロイを実行 → デプロイが成功
  - **目的**: サーバー側（Pi5）のデプロイを正常に実行し、Docker Composeの再ビルド・再起動を完了する
  - **成功基準**: JSON出力に`results[].status == "success"`が含まれ、実際のデプロイが完了し、Docker Composeサービスが正常に起動している
  - **失敗基準**: デプロイが失敗する、またはDocker Composeサービスが起動しない
- DE-002: Pi4キオスクデプロイを実行 → デプロイが成功
  - **目的**: Pi4キオスクのデプロイを正常に実行し、Ansible Playbookが正常に完了する
  - **成功基準**: JSON出力に`results[].status == "success"`が含まれ、実際のAnsible Playbookが`failed=0`で完了する
  - **失敗基準**: Ansible Playbookが失敗する、または`failed > 0`となる
- DE-003: Pi3サイネージデプロイを実行 → デプロイが成功
  - **目的**: Pi3サイネージのデプロイを正常に実行し、Ansible Playbookが正常に完了する
  - **成功基準**: JSON出力に`results[].status == "success"`が含まれ、実際のAnsible Playbookが`failed=0`で完了する
  - **失敗基準**: Ansible Playbookが失敗する、または`failed > 0`となる
- DE-004: デプロイが既に実行中の場合 → ロックファイルでエラー、適切なメッセージを出力
  - **目的**: 同時デプロイを防止し、ロックファイル機構が正常に動作することを確認する
  - **成功基準**: JSON出力に`overall_status: "error"`が含まれ、適切なエラーメッセージ（ロックファイルが存在する旨）が出力される
  - **失敗基準**: ロックファイルが検知されない、または同時デプロイが実行される

#### verifier
- VF-001: サーバー側のAPIヘルスチェック → APIが正常に応答
  - **目的**: サーバー側APIがデプロイ後に正常に起動し、リクエストに応答できることを確認
  - **成功基準**: HTTP GET `http://{{ server_ip }}:8080/api/system/health` が200を返す
  - **失敗基準**: タイムアウト、接続エラー、または200以外のステータスコード
- VF-003: Pi4キオスクの持出一覧表示機能確認 → キオスクが実際に使用するAPIエンドポイントが正常に動作することを確認
  - **目的**: Pi4キオスクが「持出一覧」を表示できることを確認（キオスクが実際に使用する機能の検証）
  - **検証対象**: `/api/tools/loans/active`エンドポイント（キオスクが実際に使用するエンドポイント）
  - **成功基準**: HTTP GET `http://{{ server_ip }}:8080/api/tools/loans/active` に`x-client-key`ヘッダー付きでリクエストし、200が返り、レスポンスに`loans`配列が含まれる
  - **失敗基準**: タイムアウト、接続エラー、401（認証失敗）、500（サーバーエラー）、または200以外のステータスコード
  - **検証方法**: `x-client-key`ヘッダーに`client-key-raspberrypi4-kiosk1`（または管理コンソールで設定されたキー）を付与してリクエスト
- VF-005: Pi3サイネージの画像取得確認 → 画像が正常に取得できる
  - **目的**: Pi3サイネージがサーバーからコンテンツを取得できることを確認
  - **成功基準**: HTTP GET `http://{{ server_ip }}:8080/api/signage/content` が200を返し、画像データが返される
  - **失敗基準**: タイムアウト、接続エラー、または200以外のステータスコード
- VF-006: サービスが停止している場合 → 検証失敗を検知、適切なエラーメッセージを出力
  - **目的**: systemdサービスが停止している場合に検証失敗を検知する
  - **成功基準**: サービスが`active`の場合、検証成功。サービスが`inactive`または`failed`の場合、検証失敗として適切なエラーメッセージを出力
  - **失敗基準**: 検証ロジック自体がエラーを起こす（SSH接続失敗、コマンド実行失敗等）

#### deploy-all.sh（統合）
- DA-001: 設定変更からデプロイ完了まで一貫実行 → 全モジュールが正常に実行され、検証が成功
  - **目的**: 全モジュール（change-detector → impact-analyzer → deploy-executor → verifier）が正常に連携し、デプロイが完了することを確認する
  - **成功基準**: 全モジュールが正常に実行され、最終的なJSON出力に`verification_results[].overall_status == "passed"`が含まれる
  - **失敗基準**: いずれかのモジュールが失敗する、または検証が失敗する
- DA-002: 設定変更がない場合の処理 → 早期終了し、デプロイを実行しない
  - **目的**: 変更がない場合に無駄な処理を避け、早期終了する
  - **成功基準**: `change-detector.sh`が変更なしを検知し、`deploy-executor.sh`と`verifier.sh`が実行されない
  - **失敗基準**: 変更がないのにデプロイが実行される、または早期終了しない

### 4.3 エラーハンドリングテスト

- ER-001: ネットワーク障害時の処理 → リトライ機能が動作、適切なエラーメッセージを出力
  - **目的**: ネットワーク障害時に適切なエラーハンドリングを行い、リトライ機能が動作することを確認する
  - **成功基準**: ネットワーク障害時に適切なエラーメッセージが出力され、リトライが実行される（実装されている場合）
  - **失敗基準**: エラーが検知されない、またはエラーメッセージが不適切
- ER-002: 権限エラー時の処理 → 適切なエラーメッセージを出力、解決方法を提示
  - **目的**: 権限エラー時に適切なエラーハンドリングを行い、解決方法を提示する
  - **成功基準**: 権限エラー時に適切なエラーメッセージが出力され、解決方法が提示される
  - **失敗基準**: エラーが検知されない、または解決方法が提示されない
- ER-003: ディスク容量不足時の処理 → エラーメッセージとクリーンアップ提案を出力
  - **目的**: ディスク容量不足時に適切なエラーハンドリングを行い、クリーンアップ提案を提示する
  - **成功基準**: ディスク容量不足時に適切なエラーメッセージが出力され、クリーンアップ提案が提示される
  - **失敗基準**: エラーが検知されない、またはクリーンアップ提案が提示されない
- ER-004: Ansible Playbook実行失敗時の処理 → ロールバックが実行される
  - **目的**: Ansible Playbook実行失敗時にロールバックが実行され、サービスが正常に復旧することを確認する
  - **成功基準**: Ansible Playbook実行失敗時にロールバックが実行され、JSON出力に`rollback_status: "success"`が含まれる
  - **失敗基準**: ロールバックが実行されない、またはロールバックが失敗する

### 4.4 実機環境特有のテスト

- RT-001: Pi5からPi3/Pi4へのSSH接続確認 → SSH接続が正常に確立される
  - **目的**: Pi5からPi3/Pi4へのSSH接続が正常に確立されることを確認し、デプロイの前提条件を満たす
  - **成功基準**: SSH接続が正常に確立され、`ssh`コマンドが成功する（exit code 0）
  - **失敗基準**: SSH接続が失敗する、またはタイムアウトする
- RT-002: 実機でのDry-runモード実行 → Dry-runモードが正常に動作、実際のデプロイは実行されない
  - **目的**: Dry-runモードが正常に動作し、実際のデプロイが実行されないことを確認する
  - **成功基準**: Dry-runモードで実行した場合、実際のデプロイが実行されず、JSON出力に`dry_run: true`が含まれる
  - **失敗基準**: Dry-runモードで実際のデプロイが実行される、またはJSON出力が不正
- RT-003: 実機でのロックファイル動作確認 → ロックファイルが正常に作成・削除される
  - **目的**: ロックファイル機構が正常に動作し、同時デプロイを防止することを確認する
  - **成功基準**: デプロイ開始時にロックファイルが作成され、デプロイ完了時に削除される
  - **失敗基準**: ロックファイルが作成されない、または削除されない
- RT-004: 実機でのログ出力確認 → ログファイルが正常に作成され、内容が正しい
  - **目的**: ログファイルが正常に作成され、デプロイ過程が記録されることを確認する
  - **成功基準**: ログファイル（`logs/deploy/*.jsonl`）が作成され、JSON形式のログが記録される
  - **失敗基準**: ログファイルが作成されない、またはログ形式が不正

### 4.5 E2Eテストシナリオ

- E2E-001: `network_mode`を`local`から`tailscale`に変更 → 全コンポーネントが正常に動作
  - **目的**: ネットワークモード変更時のE2Eテストを実行し、全コンポーネントが正常に動作することを確認する
  - **成功基準**: デプロイが成功し、検証が全てパスし、サーバー・Pi3・Pi4が正常に動作する
  - **失敗基準**: デプロイが失敗する、または検証が失敗する、またはコンポーネントが正常に動作しない
- E2E-002: `network_mode`を`tailscale`から`local`に戻す → 全コンポーネントが正常に動作
  - **目的**: ネットワークモード変更の逆方向のE2Eテストを実行し、全コンポーネントが正常に動作することを確認する
  - **成功基準**: デプロイが成功し、検証が全てパスし、サーバー・Pi3・Pi4が正常に動作する
  - **失敗基準**: デプロイが失敗する、または検証が失敗する、またはコンポーネントが正常に動作しない
- E2E-003: `server_ip`を変更 → クライアント側が新しいIPに接続
  - **目的**: サーバーIP変更時のE2Eテストを実行し、クライアント側が新しいIPに接続できることを確認する
  - **成功基準**: デプロイが成功し、Pi3・Pi4が新しいIPに接続できることを検証で確認する
  - **失敗基準**: デプロイが失敗する、またはクライアント側が新しいIPに接続できない
- E2E-004: `SignageService.getContent()`を変更 → Pi3サイネージが正常に動作、管理コンソールも正常
  - **目的**: APIサービス変更時のE2Eテストを実行し、依存するコンポーネントが正常に動作することを確認する
  - **成功基準**: デプロイが成功し、Pi3サイネージが正常に動作し、管理コンソールも正常に動作する
  - **失敗基準**: デプロイが失敗する、またはPi3サイネージ・管理コンソールが正常に動作しない
- E2E-005: `KioskReturnPage.tsx`を変更 → Pi4キオスクが正常に動作
  - **目的**: フロントエンドコンポーネント変更時のE2Eテストを実行し、Pi4キオスクが正常に動作することを確認する
  - **成功基準**: デプロイが成功し、Pi4キオスクが正常に動作し、検証がパスする
  - **失敗基準**: デプロイが失敗する、またはPi4キオスクが正常に動作しない
- E2E-006: `tools`モジュールのAPI変更 → 依存する`signage`モジュール、`kiosk`モジュールも正常に動作
  - **目的**: モジュール間依存関係を考慮したE2Eテストを実行し、間接的な影響範囲も正常に動作することを確認する
  - **成功基準**: デプロイが成功し、サーバー・Pi3・Pi4が正常に動作し、検証が全てパスする
  - **失敗基準**: デプロイが失敗する、または依存モジュールが正常に動作しない
- E2E-007: デプロイ失敗時のロールバック確認 → ロールバックが正常に実行され、サービスが復旧
  - **目的**: デプロイ失敗時のロールバック機能を確認し、サービスが正常に復旧することを確認する
  - **成功基準**: デプロイ失敗時にロールバックが実行され、サービスが正常に復旧し、JSON出力に`rollback_status: "success"`が含まれる
  - **失敗基準**: ロールバックが実行されない、またはサービスが正常に復旧しない
...
- **Phase 5テスト（E2E-001〜E2E-007）**: ✅ 完了（2025-12-06）
  - E2E-001〜E2E-006: 実機デプロイ成功（サーバー側、Pi3、Pi4）
  - E2E-007: ロールバック機能はdry-runで確認済み（実際のロールバックはリスク回避のため未実施）
...
- **詳細検証項目**:
  ...
  - Phase 5: 実際のデプロイ実行、ロールバック確認 → ✅ 完了（2025-12-06、Pi3/Pi4デプロイ成功、ロールバックdry-run確認完了）
...
- **検証項目**:
  ...
  - ✅ エッジケース（変更なし、複数ファイル変更等）が正常に処理されるか（詳細確認完了 - 2025-12-06）
    - 変更なしの場合 → ✅ パス（空の配列を返す、Pi5上で確認済み）
    - 複数ファイル変更の場合 → ✅ パス（設定変更1件、コード変更2件を検知、Pi5上で確認済み）
    - 大規模変更の場合 → ✅ パス（設定変更1件、コード変更3件を検知、Pi5上で確認済み）
    - 設定ファイルのみ変更の場合 → ✅ パス（設定変更のみを検知、Pi5上で確認済み）
    - コードファイルのみ変更の場合 → ✅ パス（コード変更のみを検知、Pi5上で確認済み）
...
- **検証項目**:
  ...
  - ✅ モジュール間依存関係が正しく考慮されるか（詳細確認完了 - 2025-12-06）
    - toolsモジュールのAPI変更 → signage/kioskモジュールへの影響検出 → ✅ パス（Pi5上で確認済み）
    - モジュール間依存関係（signage depends_on tools）が正しく考慮される → ✅ パス（Pi5上で確認済み）
  - ✅ 複数の変更の影響範囲が正しく統合されるか（詳細確認完了 - 2025-12-06）
    - 設定変更とコード変更の同時発生 → ✅ パス（影響範囲が統合される、Pi5上で確認済み）
    - 複数のコード変更の同時発生 → ✅ パス（影響範囲が統合される、Pi5上で確認済み）
...
- **検証項目**:
  ...
  - ✅ エラーハンドリングが正常に動作するか（詳細確認完了 - 2025-12-06）
    - ER-001: 無効なJSON入力の処理 → ✅ パス（適切なエラーメッセージを出力、Pi5上で確認済み）
    - ER-002: 無効なdeploy_targets形式の処理 → ✅ パス（適切なエラーメッセージを出力、Pi5上で確認済み）
    - ER-003: 未知のターゲットの処理 → ✅ 実装済み（DEPLOY_EXECUTOR_ENABLE=1時に"failed"ステータスを返す、リスク回避のため実機検証は未実施）
    - ER-004: デプロイコマンド実行失敗時の処理 → ✅ 実装済み（失敗時に"failed"ステータスを返す、ロールバック機能はdry-runで確認済み）
...
- **検証項目**:
  ...
  - ✅ デプロイ失敗時のロールバックが正常に動作するか（FORCE_DEPLOY_FAILUREでのdry-run確認完了 - 2025-12-06）
    - dry-runモードでロールバック機能の動作を確認
    - 実際のロールバックはリスク回避のため未実施（必要に応じて将来実施）
...
### 7.3 次のステップ
1. **運用パイロットを実施（deploy-all.sh標準手順の試行）** ✅ 完了 (2025-12-07)
   - Pi5で `NETWORK_MODE=tailscale bash scripts/deploy/deploy-all.sh --dry-run` を実行 → 成功（no changes）
   - 変更がないため検証はスキップ、手動で実機確認を実施
2. **Runbook化とドキュメント反映** ✅ 完了 (2025-12-07)
   - `deployment.md` に運用チェックリスト、Tailscale IP一覧、deploy-all.sh使用方法を追加
3. **ブランチ運用ルール** → 次フェーズで実施
   - パイロット作業用に新規ブランチを切る（例: `feature/deploy-all-pilot`）
   - パイロット結果をKBに記録し、標準手順として固定するか判断

### 7.4 実機確認結果 (2025-12-07)

Tailscale経由でリモートアクセスし、全検証項目を確認:

| 項目 | ターゲット | 結果 | 備考 |
|------|-----------|------|------|
| APIヘルス | Pi5 | ✅ 200 OK | `/api/system/health` |
| キオスク用API | Pi5 | ✅ 200 OK | `/api/tools/loans/active` (x-client-key付き) |
| サイネージ用API | Pi5 | ✅ 200 OK | `/api/signage/content` |
| kiosk-browser.service | Pi4 | ✅ active | tools03ユーザーで確認 |
| status-agent.timer | Pi4 | ✅ active | tools03ユーザーで確認 |
| signage-lite.service | Pi3 | ✅ active | systemモードで確認 |
| status-agent.timer | Pi3 | ✅ active | systemモードで確認 |

### 7.5 ブランチ運用ルール（パイロット／本番反映）
- パイロット用ブランチ: `feature/deploy-all-pilot-<yymmdd>-<short>` を作成し、軽微変更で動作確認
- PR必須項目: ドライラン結果(JSON)・本番実行ログ(JSON)・verifier結果、Rollback実施有無
- マージ条件: `deploy-all.sh` を `DEPLOY_EXECUTOR_ENABLE=1 DEPLOY_VERIFIER_ENABLE=1 ROLLBACK_ON_FAIL=1` で1回以上成功させること
- 失敗時: ロールバック結果をPRに追記し、KBに事象/原因/対策を追記（例: Prisma P3009 → KB-088）
- 本番取り込み: パイロット結果を KB に記録後、`main`へマージ

### 7.6 Pi3/Pi4 実機検証計画（影響なし変更での実効性確認）
- 目的: 影響範囲が Pi3/Pi4 に及ぶケースで、`deploy-all.sh` が期待通りにデプロイ・検証・ロールバックを実行することを実機で確認する
- テスト用の軽微変更（機能無影響）
...
- 手順
  ...
  5. ロールバック確認（任意）: `FORCE_DEPLOY_FAILURE=1` で失敗を強制し、`rollback` が実行されることを確認
  6. 後処理: 追加したコメントを削除、`network_mode`を`local`に戻す、Pi3サイネージサービスを再有効化（`systemctl enable + start`）
- 成功基準
  - impact-analyzer が `deploy_targets` に `pi4_kiosk`, `pi3_signage` を含める
  - deploy-executor が Pi4/Pi3 を含むデプロイを `overall_status: success` で完了（Ansible `failed=0`）
...
**現在のデプロイ方法**:
- **サーバー側**: `scripts/server/deploy.sh` → [デプロイメントガイド](../../archive/deployments/legacy-operator-guide-through-2026-07.md#方法1-デプロイスクリプトを使用推奨)
