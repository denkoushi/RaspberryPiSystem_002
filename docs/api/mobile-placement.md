...
口頭問い合わせ向けの **部品名検索**（**現在棚 `OrderPlacementBranchState.scheduleSnapshot` を最優先**し、**生産スケジュール `CsvDashboardRow`（当該ダッシュボードの winner 行）を補助**）。表記ゆれは API 内の **管理された同義語辞書**で展開（例: `アシ` → `脚` / `足` を **同一トークン内**で OR 検索）。**空白区切りの複数語は AND**（例: `テーブル 脚` は `FHINMEI`/`FHINCD` に対し「テーブル」を含み、かつ「脚」系に一致する行）。辞書・正規化ロジックの共有実装は **`@raspi-system/part-search-core`**（`packages/part-search-core`）。
