...
口頭問い合わせ向けの **部品名検索**（**現在棚 `OrderPlacementBranchState.scheduleSnapshot` を最優先**し、**生産スケジュール `CsvDashboardRow`（当該ダッシュボードの winner 行）を補助**）。表記ゆれは API 内の **管理された同義語辞書**で展開（例: `アシ` → `脚` / `足` を OR 検索）。
...
- `scheduleCandidates[]`: `matchSource: "schedule"`。**現在棚に既に紐づく `csvDashboardRowId` は除外**（重複を避ける）。棚未登録の可能性がある旨は UI 側で明示する。
