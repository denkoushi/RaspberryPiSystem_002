...
口頭問い合わせ向けの **部品名検索**（**現在棚 `OrderPlacementBranchState.scheduleSnapshot` を最優先**し、**生産スケジュール `CsvDashboardRow`（当該ダッシュボードの winner 行）を補助**）。表記ゆれは API 内の **管理された同義語辞書**で展開（例: `アシ` → `脚` / `足` を **同一トークン内**で OR 検索）。**空白区切りの複数語は AND**（例: `テーブル 脚` は `FHINMEI`/`FHINCD` に対し「テーブル」を含み、かつ「脚」系に一致する行）。辞書・正規化ロジックの共有実装は **`@raspi-system/part-search-core`**（`packages/part-search-core`）。**かなの表記ゆれ**（ひらがな/カタカナ統一・拗音・促音吸収など）も同パッケージで正規化する。

- **クエリ `q`**: 任意・最大 100 文字。空なら `{ currentPlacements: [], scheduleCandidates: [] }`
- **クエリ `machineName`（任意）**: 最大 200 文字。**空なら無視**。**登録製番ボタン下段の機種名**と同系の **`CsvDashboardRow` 上の MH/SH 行 `FHINMEI` を製番（`FSEIBAN`）単位で集約した文字列**に対し、正規化後の **部分一致**で絞り込み、**`q` の条件と AND**（`scheduleSnapshot` / 日程行の **`FSEIBAN` が一致集合に含まれる行**のみ）。
- **認証**: `x-client-key` 必須
