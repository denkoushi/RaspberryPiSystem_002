...
最終更新: 2026-06-26（復旧必須ディレクトリ・DB gzip・容量不足時自動回復を反映）
...
5. Dropbox向け大容量ファイルはUpload Session（チャンクアップロード）で送信
6. Dropboxで`insufficient_space`が発生した場合、最古バックアップから順次削除して再試行
7. バックアップ実行後、保持期間設定に基づいて古いバックアップを自動削除
...
- 意図的に無効化しているターゲット（例: `photo-storage`）の **enabled 変更はカタログの責務外**（既存 `kind`+`source` があれば `coverage_gap` にならない）。
...
**注意（現行実装の制約）**:
- 自動削除（保持期間のクリーンアップ）は `retention.days` または `retention.maxBackups` のいずれかが設定されている場合に実行されます。
...
### 容量不足時の自動回復（2026-03追加）

- Dropboxバックアップで`insufficient_space`を検知した場合、システムは既存バックアップを**最古順に削除**しながらアップロードを再試行します。
- 削除対象は実行中ターゲットと同じ`kind`/`source`に一致するバックアップです。必要なサイズが空くまで、古いものから順に削除・再試行します。
- 削除したバックアップに対応する履歴は`fileStatus=DELETED`へ更新されます。
- これはバックアップ成功率を優先する動作です。保持優先の運用が必要な場合は、`retention`や実行スケジュールを保守的に設定してください。
- バックアップ完了時は`BackupHistory.backupPath`にも実体パスを保存します。履歴からリストアや削除状態照合を行う場合は、`summary.path`ではなく`backupPath`を正として扱えます。
