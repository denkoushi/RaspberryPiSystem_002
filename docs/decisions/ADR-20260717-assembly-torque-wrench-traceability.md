...
5. Existing templates whose expand-phase traceability column is null are interpreted as `LEGACY`. New templates and revisions saved by the new editor persist `REQUIRED`. Database backfill and NOT NULL tightening are deferred to a separately reviewed contract phase. Legacy transport stays at the API boundary; it does not become part of new domain interfaces.
6. Circle markers are unique and stable within a template. Deletion never renumbers another marker. During the expand phase the application enforces template-wide uniqueness for new/revised templates; historical backfill, duplicate remediation, and the final database unique constraint are deferred. Condition inheritance and range copy update only tightening-condition fields, never identity, coordinates, page, order, callout, or internal tightening ID.
7. Rejected physical inputs are stored as ignored audit records with raw input and a stable reason code, and never advance work. A composite primary key on the newly created agent-event table makes delivery retry idempotent without adding a unique constraint to a populated table.
...
15. Migration deployability is an executable three-layer contract, not an operator checklist. CI/local launch validates the immutable candidate against the deployed baseline, local launch performs a read-only validation against the actual Pi5 ledger before creating a release unit, and the release repeats the ledger validation before candidate work. Any failure precedes checkout, migration, image build, routing change, or terminal mutation.
...
Rejected inputs consume audit storage, and the agent requires a small durable local volume. The expand phase cannot provide the final database marker constraint until historical rows are audited and backfilled. New/revised templates are protected immediately by application validation, while the later contract phase must stop on unresolved duplicates rather than silently change visible numbering.
