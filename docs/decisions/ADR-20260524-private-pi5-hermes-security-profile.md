...
14. **推論**: `model.reasoning_effort: none`（レイテンシ短縮）。
15. **Discord DM**: テンプレ既定 **`require_mention: false`**（`DISCORD_ALLOWED_USERS` で保護）。サーバー運用でメンション必須に戻す場合は inventory で `private_pi5_hermes_discord_require_mention: true`。
16. **`unauthorized_dm_behavior: ignore`**、許可リストは `DISCORD_ALLOWED_USERS`（inventory → template）。
...
- **レイテンシ**は DGX コールドスタート・補助 LLM 呼び出しに依存（keep-warm 未実施）。
