...
### 追加のローカル強化履歴（2026-06-30）

詳細: [security-hardening-history-20260630.md](./security-hardening-history-20260630.md)

- **実機反映は未実施**。既存DB/既存コンテナ/実機設定は変更していない。
- **MFA/2段階認証は未変更**。iPhone/旧スマホ確認完了まで保留。
- **貸出APIの未認証操作を遮断**し、有効な `x-client-key` または管理者系JWTを必須化した。
- **`x-client-key` と `clientId` の不一致を拒否**し、端末キーの横用を防止した。
- **写真/ローカルバックアップのパストラバーサル対策**を追加した。
...
### ✅ 完了した対策（Phase 1-10 + ポートセキュリティ強化）

1. **通常運用（Tailscale）時のセキュリティ**
   - ✅ Tailscale VPN導入（通常運用の標準接続経路）
...
1. **PostgreSQLのパスワードポリシーの強化** ✅ 完了（2025-12-18）
   - ✅ **実装完了**: Docker Compose設定を変更し、環境変数からパスワードを取得するように実装
   - ✅ **実装完了**: `.env.example`に`POSTGRES_PASSWORD`環境変数を追加
   - ⚠️ **本番環境での対応**: 本番環境では強力なパスワードを環境変数で設定する必要がある
   - **参照**: [標準セキュリティチェックリスト監査レポート](./standard-security-checklist-audit.md) / [PostgreSQLパスワードポリシー強化の実装](./postgresql-password-policy-implementation.md)

2. **オフライン保存用USBメディアテスト**
...
**ポートセキュリティ強化（2025-12-18）**: Docker Composeのポートマッピング削除により、Dockerレベルでポートがブロックされ、UFWに依存しない多層防御が実現されました。詳細は [ポートセキュリティ監査レポート](./port-security-audit.md) / [ポートセキュリティ修正後の動作確認手順](./port-security-verification.md) / [ポートセキュリティ修正後の実機検証結果](./port-security-verification-results.md) を参照。

**標準セキュリティチェックリスト監査（2025-12-18）**: IPA「安全なウェブサイトの作り方」、OWASP Top 10 2021、CISベンチマークに基づく監査を実施。主要なセキュリティ対策がほぼ完了していることを確認。未実施項目（CSRF対策、PostgreSQLのSSL/TLS接続強制、パスワードポリシー強化など）と必要性を評価。詳細は [標準セキュリティチェックリスト監査レポート](./standard-security-checklist-audit.md) を参照。
