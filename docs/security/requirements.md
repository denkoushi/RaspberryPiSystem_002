...
### 2.6 キオスクAPIの個人情報最小化・列挙耐性（将来要件）

**背景**:
- キオスクは `x-client-key` 認証のみで利用するエンドポイントが存在する。
- `x-client-key` が漏洩した場合の影響を最小化するため、列挙可能なデータ（例: 社員名簿）を最小限にする必要がある。

**要件**:
- **列挙可能情報の最小化**:
  - キオスク向けの"一覧系/列挙系"APIは、原則として **必要最小限のフィールドのみ** を返す。
  - 例: 従業員一覧は `id`, `displayName`, `department` のみに限定し、`employeeCode`, `contact`, `nfcTagUid` 等は返さない。
- **端末単位のレート制限**:
  - キオスク向けの"列挙系"API（例: 従業員一覧）は、`x-client-key` 単位での簡易レート制限を検討し、スクレイピング/濫用耐性を確保する。
  - 例: 1分あたりN回（Nは運用で調整）。
- **認証の厳格化（存在確認）**:
  - `x-client-key` は必須とし、DB上の `ClientDevice` 存在確認を通過した場合のみレスポンスを返す。
- **ログの秘匿**:
  - `x-client-key` は機密情報として扱い、ログには出力しない（必要なら `[REDACTED]` またはハッシュ化した値のみ）。
  - 外部通知のWebhook URL（Slack等）は **部分文字列も含めてログ出力しない**（漏洩耐性を優先）。

**優先度**: 🟡 中優先度（将来の実装時に考慮）

**関連要件**: [Phase 9/10 詳細仕様書](./phase9-10-specifications.md)の「2. アラートの外部通知（Webhook）」セクションを参照
