...
**対策案**:
- MFA: Userに`mfaEnabled`/`totpSecret`を追加し、ログインAPIでパスワード+TOTP検証
- 監視: AIDE等の軽量整合性チェック＋プロセス・ネットワークの簡易監視を`security-monitor.sh`に統合
- 権限監査: 監査テーブルを追加し、User更新時に履歴保存＋異常パターンでアラート

**優先度**: 最高（MFA）、高（リアルタイム監視・権限監査）
