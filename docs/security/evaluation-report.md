...
1. **ポート露出**: ✅ 期待通り
   - 22/80/443/5900がローカルネットワーク内で待受（ルーターのファイアウォールでインターネットからのアクセスはブロック、UFW設定確認済み）
...
**ルーター設定確認（2026-01-28 追記）**:
- ✅ IODATA UD-LTA/UEのデフォルト設定:
  - SPIファイアウォール: 有効（デフォルト）
  - ポート転送: 設定されていない（デフォルト）
- ✅ **結論**: ルーターのファイアウォールが有効でポート転送が未設定の場合、インターネットからPi5のポート22/80/443/5900に直接アクセスできない
- ✅ **多層防御**: ルーターのファイアウォール + Pi5のUFW + fail2banによる多層防御が実現されている
- ⚠️ **注意**: 「0.0.0.0:22でLISTEN」は「インターネット公開」ではなく「ローカルネットワーク内での待受」という意味。ルーターの設定も含めて評価する必要がある
