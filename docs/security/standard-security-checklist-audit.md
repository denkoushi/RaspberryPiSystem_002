...
1. **PostgreSQLのパスワードポリシーの強化** ✅ 完了（2025-12-18）
   ...
   - **参照**: [PostgreSQLパスワードポリシー強化の実装](./postgresql-password-policy-implementation.md)
...
6. **キオスクAPIの個人情報最小化・列挙耐性の強化**
   - **現状**: キオスク向けAPI（`GET /api/kiosk/employees`等）で、`x-client-key`が漏洩した場合の列挙リスクが存在
   - **対策**: 
     - 列挙可能情報の最小化（必要最小限のフィールドのみ返す）
     - `x-client-key`単位での簡易レート制限の追加（列挙系API）
     - `x-client-key`のログ出力を`[REDACTED]`に変更
     - Webhook URLの部分文字列も含めてログ出力しない
   - **影響**: `x-client-key`漏洩時の影響範囲を最小化、スクレイピング/濫用耐性の向上
   - **実装難易度**: 低〜中（既存コードの修正とレート制限の追加）
   - **参照**: [セキュリティ要件定義](./requirements.md)の「2.6 キオスクAPIの個人情報最小化・列挙耐性（将来要件）」、[Phase 9/10 詳細仕様書](./phase9-10-specifications.md)の「7. リクエストログの機密情報取り扱い（運用ルール）」
