...
**現在の運用方針**:
- **通常運用**: Tailscale経由（常時接続・標準）
- **緊急時のみ**: ローカルネットワーク（Tailscale障害/認証不能時）

**新スキームとの整合性**:
- **推奨**: **Tailscale主運用のままCSV取得**
  - CSV取得時: Tailscale経由でDropboxに接続
  - 取得後: 追加操作不要（Tailscale常時ONが標準）
...
# CSV取得後: 追加操作不要（Tailscale常時ONが標準）
```
...
**現在の運用方針との整合性**:
- **現在**: 通常運用はTailscale（常時接続・標準）
- **新スキーム**: Pi5がDropboxからCSVを取得するため、Tailscale経由の通信を継続

**推奨される運用方針**:
- **通常運用**: Tailscale経由で常時接続
- **CSV取得時**: 通常運用のまま取得（追加の切替不要）

または

- **CSV取得専用のネットワーク**: Tailscale経由でDropboxに接続（通常運用）
...
5. インターネット接続の運用方針を明確化（Tailscale主、localは緊急時のみ）
