...
**検証概要**: バックアップとリストア機能が正常に動作することを確認

**検証手順**: [バックアップ検証チェックリスト](./backup-verification-checklist.md) を参照

**検証頻度**:
- 月次検証: 毎月1回（バックアップ履歴の確認）
...
**6.6.4 キオスク持出一覧・サイネージ（吊具 `idNum` とアイコン削除）**

**確認ポイント**:
- [ ] キオスク持出一覧（`/kiosk/tag` 右ペイン）で計測機器・吊具・工具カードの絵文字アイコン（📏/⚙️/🔧）が表示されないか
- [ ] 吊具カードで **管理番号と同一行**に idNum の**値**が表示されるか（未設定は `-`。**持出一覧では「旧番号:」ラベルは付けない** ― 吊具持出 `/kiosk/rigging/borrow` の詳細ブロックは従来どおり「旧番号」行あり）
- [ ] **返却・取消**ボタンが、カード内の**明細（サムネ＋テキスト）の下段**にあり、長いアイテム名がボタン列と**横並びで幅を奪い合わない**こと（[KB-323](../knowledge-base/KB-323-kiosk-return-card-button-layout.md)）
- [ ] サイネージ持出カードでも吊具のみ旧番号が表示されるか（計測機器・工具は旧番号表示なし）

**検証日時**: _______________
**検証結果**: ☐ 成功 ☐ 失敗（エラー内容: _______________）

**6.6.5 キオスク持出一覧・サイネージ（写真持出ラベル・端末場所）**
...
- [ ] `./scripts/deploy/verify-phase12-real.sh` が **FAIL 0** で完走する（スクリプト内: `POST /api/part-measurement/resolve-ticket` + 有効な `x-client-key` で `"candidates"`、無キーで **401**；**`GET /api/part-measurement/templates/candidates`** も **401**（無キー）・キー付きで **`"candidates"`**）
...
- [ ] （手動）テンプレ未整備時に **`/kiosk/part-measurement/template/pick`** で候補一覧・図面サムネ・**`clone-for-schedule-key` 経由の記録開始**（3要素は直リンク、2要素/1要素は複製後に記録）が期待どおりか。**2026-04-04 以降**: 1要素候補は **`FHINMEI_ONLY` + `candidateFhinmei`** 由来（テンプレ名の部分一致ではない）。**2026-04-05 以降**: 日程 `fhinmei` は正規化後の候補キーを **`includes` すれば一致**（**2 文字未満のキーは不可**）。類似複数時は **より長い候補キーが先**（並びのタイブレーク）。2要素は **`FHINCD_RESOURCE`** または `THREE_KEY` で工程のみ異なる一致。選択後に作成された **正本 active** が日程の3要素キーで解決することを DB または管理画面で確認できるとよい（[KB-320](../knowledge-base/KB-320-kiosk-part-measurement.md)・[ADR-20260404](../decisions/ADR-20260404-part-measurement-template-pick-kiosk.md)）
- [ ] （手動・**検査図面一覧ハブ** · 2026-05-30/2026-07-08）ヘッダー **「検査図面」** → `/kiosk/part-measurement/inspection` で一覧・品番フィルタ（部分一致）・**新規** / **編集** / **履歴**。旧版は保存不可・有効化後に編集可。専用 API: 未認証 `GET /api/part-measurement/inspection-drawing/templates` → **401**、`visualTemplateId` query で図面 ID 完全一致絞り込みできること。品番自動入力用 `GET /api/part-measurement/inspection-drawing/visual-templates/:id/fhincd` が一意品番では `unique`、複数品番では `multiple` を返すこと（[KB-320](../knowledge-base/KB-320-kiosk-part-measurement.md) curl 例）
- [ ] （手動・**検査図面 OCR 候補** · 2026-07-02）`MD100469601 / 7161サドル / 資源CD033` の図面で丸数字 2→`360`、5→`25`、7→`37` が基準値候補 top5 に出ること。候補は人間が選択し、自動確定されないこと（[ADR-20260702](../decisions/ADR-20260702-part-measurement-drawing-ocr-cache.md)）
- [ ] （手動・**検査図面 名称・公差種別設定** · 2026-07-06/2026-07-08）管理コンソール `/admin/tools/part-measurement-templates` の **検査図面 名称・公差種別** で `幅=幾何公差` を保存すると、キオスク検査図面の `幅` が `上限値` 入力 + `合格範囲 0〜上限値` 表示になり、候補が `0` / `0.001`〜`0.009` / `0.01` / `0.015` / `0.020` / `0.030` / `0.050` に切り替わること。既定では `直角度` / `面粗度` が幾何公差、`幅` / `厚み` / `ネジ穴ピッチ` / `穴ピッチ` が寸法公差。候補外手入力値は保持されること（[KB-320 §名称・公差種別設定](../knowledge-base/KB-320-kiosk-part-measurement.md#検査図面-名称-公差種別設定-2026-07-06)）
- [ ] （手動・**検査図面 UI parity** · 2026-05-30/2026-07-08 · `ccacef85` 以降）一覧フィルタが **折り返し**されること。図面ライブラリ新規で一意品番なら品番欄とテンプレ欄が `図面名 + 品番` になること。作成画面で測定点 **名称と基準値/上限値が1行ずつ**、寸法公差は上限/下限2入力、幾何公差は上限値1入力、補足行は `面` / `呼び径` / 直接入力を併設、ツールバーは左に **保存 → 状態文字**、右端に **テスト入力 → ガイド試行 → 保存済み帳票 → 一覧へ戻る** の順、画面下部に旧「図面をタップ…」「一覧プレビューへ」リンクが **ない**こと（[KB-320 §プレビュー](../knowledge-base/KB-320-kiosk-part-measurement.md#検査図面-preview-parity-2026-05-30)）
...
- [x] （Pi 実機 · 2026-06-02 · Pi5）kiosk `client-key` で `POST /api/part-measurement/drawings/preview` → **401**（無キー）/ **200 image/jpeg**（最小 PDF）· `pdftoppm` **22.12.0** 確認済。save · storage GET · PDF 連打 503 は **Pi5 キオスク目視 OK** 後に Pi4 順次で再確認推奨
...
**確認ポイント**（[ADR-20260402](../decisions/ADR-20260402-inference-foundation-phase1.md)、[KB-313](../knowledge-base/KB-313-kiosk-documents.md)、[KB-319](../knowledge-base/KB-319-photo-loan-vlm-tool-label.md)、[KB-340](../knowledge-base/KB-340-api-ocr-vlm-boundary-refactor-deploy.md)）:
...
- [ ] （2026-04-11 追記）OCR/VLM **境界整理のみ**（挙動互換）を全台に載せる場合は [KB-340](../knowledge-base/KB-340-api-ocr-vlm-boundary-refactor-deploy.md) の順序（Pi5→Pi4×4→Pi3）と Detach Run ID 例を参照

**検証日時**: 2026-03-30（`verify-phase12-real.sh` **PASS 37 / WARN 0 / FAIL 0**・Mac / Tailscale・Pi5 に `feat/inference-foundation-phase1` 反映後）／**2026-04-11**（`feat/ocr-vlm-boundary-refactor`・**PASS 43 / WARN 0 / FAIL 0**・Pi5→Pi4×4→Pi3 順次・[KB-340](../knowledge-base/KB-340-api-ocr-vlm-boundary-refactor-deploy.md)）
**検証結果**: ☑ 成功（自動） ☐ 失敗（エラー内容: _______________）

**6.6.12 LocalLLM オンデマンド起停（Pi5→Ubuntu HTTP・任意運用）**
...
**確認ポイント**（[KB-297 §行アクション](../knowledge-base/KB-297-kiosk-due-management-workflow.md#行アクション機種名フォールバック2026-04-02)・[§順位変更キャッシュ高速化](../knowledge-base/KB-297-kiosk-due-management-workflow.md#順位変更キャッシュ高速化leaderboardfastpath2026-04-02)・[§UX polish（leader order board）](../knowledge-base/KB-297-kiosk-due-management-workflow.md#ux-polish-leader-order-board-2026-04-02)）:
...
- [ ] **品名・製番・機種名**: **16.5px 相当**で読みやすいこと。
- [ ] **製番**: **品名の右側**に表示（クラスタ行左端ではない）。
- [ ] **クラスタ行**: **品目コード + 個数**のみ（製番は含まない）。
- [ ] **2 列幅**: 顧客名/製番が **無い行**は **全幅** · **ある行**は左右 **約 50% 分割**（不要な truncate なし）。
- [ ] **品名ホバー全文**: 製番ありで **長い品名が truncate** される行で、**マウスオーバー**により **`title` で全文**が確認できること（Web のみ · タッチ/キーボードは対象外）。
- [ ] **カード外寸**: 資源カード全体のサイズが **従来と同じ**こと。
- [ ] **強制リロード**: 反映直後は **キオスク強制リロード**（§6.6.4）。Pi5 **`web`** が **`05ae1a70` 以降**（または `main` マージ後 HEAD）であること。
...
- [ ] **一括付与**: 未完・**順位未設定**行が複数あるスロットで押下 → **表示順の上から最大 5 行**に **`order-usage` 空き番を昇順**付与（例: 1,2 済 → 3,4,5…）。**既存順位行は変更されない**。
- [ ] **無効条件**: **`listIncomplete` ON** 時はボタン **disabled**。空き番 0 / 対象 0 / 実行中も disabled。
- [ ] **共存**: 押下後も **行ごとの順位ドロップダウン**で個別変更できる。
- [ ] **強制リロード**: 反映直後は **キオスク強制リロード**（§6.6.4）。Pi5 **`web`** が **`b74c54a9` 以降**（または **`main` マージ後 HEAD**）。

**トラブルシュート**: ボタンが出ない → 製番順評価 OFF / Pi5 ref / 強制リロード。付与されない → `listIncomplete`·空き番なし·全行順位済み。飛び番 → [KB-297 §A+α](../knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-order-assignment-auto-release-a-alpha-2026-05-20)（幽霊割当）。
...
4. ヘッダーの「お問い合わせ」ボタンをクリック
5. モーダルが表示されることを確認
6. **送信者ドロップダウン**から社員を選択（必須）
7. **依頼内容ドロップダウン**から「現場まで来てください。」を選択（必須）
8. **打合せ日時**を確認（デフォルトで現在の日時が設定されている）
9. **詳細（任意）**欄に任意のメッセージを入力（任意）
10. 「送信」ボタンをクリック
11. モーダルが閉じ、送信成功が確認できることを確認
12. Slackチャンネルで通知が受信されることを確認（送信者、依頼内容、打合せ日時、詳細が含まれるか）
13. 管理画面の「クライアント」タブ（`/admin/clients`）で、サポートログが記録されていることを確認

**確認ポイント**:
- [x] 「お問い合わせ」ボタンが表示されるか（ヘッダーのナビゲーションメニューに移動）
- [x] モーダルが正しく表示されるか
- [x] **送信者ドロップダウン**が表示され、社員名簿から選択できるか
- [x] **依頼内容ドロップダウン**が表示され、「現場まで来てください。」を選択できるか
- [x] **打合せ日時**フィールドが表示され、デフォルトで現在の日時が設定されているか
- [x] **詳細（任意）**テキストエリアが表示されるか
- [x] 送信者と依頼内容が必須項目として機能するか（未選択時は送信ボタンが無効化されるか）
- [x] 送信後、モーダルが閉じるか
- [x] Slack通知が正しく送信されるか（送信者、依頼内容、打合せ日時、詳細が含まれるか）
- [x] クライアントログに`[SUPPORT]`プレフィックス付きで記録されるか
- [x] レート制限が機能するか（1分に3件まで）
- [x] `x-client-key`なしで401エラーが返るか

**検証日時**: 2026-01-03（基本機能）、2026-01-03（レート制限・認証検証）、2026-01-03（フォームデザイン変更後）
**検証結果**: ✅ 成功（全項目完了）

**検証詳細**:
- **UI確認**: ヘッダーのナビゲーションメニューに「お問い合わせ」ボタンが表示され、クリックでモーダルが開くことを確認
- **フォームデザイン確認**:
  - 送信者ドロップダウンが表示され、社員名簿から選択可能であることを確認
  - 依頼内容ドロップダウンが表示され、「現場まで来てください。」を選択可能であることを確認
  - 打合せ日時フィールドが表示され、デフォルトで現在の日時が設定されていることを確認
  - 詳細（任意）テキストエリアが表示されることを確認
- **メッセージ送信**: 送信者を選択し、依頼内容を「現場まで来てください。」に設定し、打合せ日時を確認（デフォルト値）、詳細を入力して送信を実行
- **Slack通知**: `#general`チャンネルに通知が正常に届き、以下の情報が含まれることを確認:
  - 送信者: 選択した社員名
  - 依頼内容: 「現場まで来てください。」
  - 打合せ日時: 選択した日時（またはデフォルト値）
  - 詳細: 入力したメッセージ
  - クライアントID、端末名、場所、画面、Request ID
- **ClientLog記録**: データベースに以下のログが記録されていることを確認:
  - clientId: クライアントID
  - level: `INFO`
  - message: `[SUPPORT]`プレフィックス付きのメッセージ
  - kind: `kiosk-support`
  - page: 現在のページパス
  - userMessage: 送信者、依頼内容、打合せ日時、詳細を含む完全なメッセージ
- **APIログ**: エラーなし、正常に処理されたことを確認（ステータスコード200）
...
- 打合せ日時は、モーダルが開かれた時点の日時がデフォルト値として設定される
- 送信者と依頼内容は必須項目で、未選択時は送信ボタンが無効化される
