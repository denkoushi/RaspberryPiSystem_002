...
**自動検証（2026-03-25）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 28 / WARN 0 / FAIL 0**（API ヘルス・`deploy-status` 両 Pi4・`/api/tools/loans/active`・Pi5 経由の kiosk/status-agent 等。キオスク **画面レイアウトの目視**は別途実機/VNC で実施）。

**検証日時**: 2026-03-25（自動） / 実機目視: _______________
**検証結果**: ☐ 成功 ☐ 失敗（エラー内容: _______________）
