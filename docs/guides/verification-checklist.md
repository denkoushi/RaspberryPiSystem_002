...
最終更新: 2026-06-24（順位ボード `+人` 人工数表示回復・8H/10H 切替・Gantt 縦バー伸長 §6.6.31 追記）。**2026-06-05 追記**: 資源カード行強調レイアウト §6.6.30。**2026-06-01 追記**: 完了後フッタ工程チップ装飾再同期 §6.6.29。
...
- [ ] （手動・**検査図面一覧ハブ** · 2026-05-30）ヘッダー **「検査図面」** → `/kiosk/part-measurement/inspection` で一覧・品番フィルタ（部分一致）・**新規** / **編集** / **履歴**。旧版は保存不可・有効化後に編集可。専用 API: 未認証 `GET /api/part-measurement/inspection-drawing/templates` → **401**（[KB-320](../knowledge-base/KB-320-kiosk-part-measurement.md) curl 例）
- [ ] （手動・**検査図面 UI parity** · 2026-05-30 · `ccacef85` 以降）一覧フィルタが **折り返し**されること。作成画面で測定点 **基準値/下限/上限が縦並び**、ツールバー保存右に **「一覧へ戻る」**、画面下部に旧「図面をタップ…」「一覧プレビューへ」リンクが **ない**こと（[KB-320 §プレビュー](../knowledge-base/KB-320-kiosk-part-measurement.md#検査図面-preview-parity-2026-05-30)）
...
- [x] （手動・**検査図面 · 測定点位置微調整（十字ボタン）** · 2026-06-05 · **`da9d2675` · Pi5 + stonebase 実機 OK**）作成/改版で **点を配置** + 測定点選択時、右ペイン上部の十字ボタンでマーカーが微調整できること · **テスト入力/ガイド試行では非表示** · 保存後位置維持（[KB-320 §十字ボタン](../knowledge-base/KB-320-kiosk-part-measurement.md#検査図面-測定点位置微調整-十字ボタン-2026-06-05) · [Runbook §十字ボタン](../runbooks/kiosk-part-measurement.md#検査図面-測定点位置微調整-十字ボタン-2026-06-05) · [deployment §2026-06-05](../guides/deployment.md#kiosk-inspection-drawing-point-nudge-2026-06-05)）· **残**: Pi4×3
...
- [ ] **Gantt capacity**: ガント ON で `8H` は `capacityMinutes=480`、`10H` は `capacityMinutes=600` 相当のルーラー/バーになること。同じ所要量で 10H の方が基準帯が長くなる。
- [ ] **`+人` ON**: 人工数がある行で minute label が `machineRequiredMinutes + laborRequiredMinutes` に変わり、Gantt の 8H/10H 縦バーが伸びること。表示速度維持のため、行カード高さ・仮想リスト見積は現行の圧縮/最小高を維持する。
...
**検証日時**: 2026-06-24（`+人` 回復は実機目視·ユーザー確認、8H/10H は local focused tests。Gantt 縦バー伸長分離は local focused tests / build PASS、deploy・現場目視 pending）
**検証結果**: ☑ 成功（自動: 8H/10H focused tests） ☑ 成功（自動: `+人` ON で minute label 400→575、縦バー 480px→575px、行カード高さ維持） ☑ 成功（現場目視: `+人` ON で人工数反映・8H バー伸長回復、速度改善/行維持は維持） ☐ 成功（現場目視: 8H/10H 切替・縦バー伸長分離） ☐ 失敗（エラー内容: _______________）
