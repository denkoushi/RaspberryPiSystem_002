...
**確認ポイント**（[KB-314](../knowledge-base/KB-314-kiosk-loan-card-display-labels.md)）:

- [ ] 写真撮影持出カードの1行目が **「撮影mode」** となり、従来の「写真撮影モード」に戻っていないか（キオスク `/kiosk/tag`）
- [ ] カード内の場所行に **`端末場所:` プレフィックスが無く**、場所の値のみか
- [ ] `curl -sk https://<Pi5>/api/signage/content` の `tools[]` で、写真貸出の `name` が **`撮影mode`**、`clientLocation` がプレフィックス無しか
