...
- [ ] （手動・**検査図面一覧ハブ** · 2026-05-30/2026-07-08）ヘッダー **「検査図面」** → `/kiosk/part-measurement/inspection` で一覧・品番フィルタ（部分一致）・**新規** / **編集** / **履歴**。旧版は保存不可・有効化後に編集可。専用 API: 未認証 `GET /api/part-measurement/inspection-drawing/templates` → **401**、`visualTemplateId` query で図面 ID 完全一致絞り込みできること（[KB-320](../knowledge-base/KB-320-kiosk-part-measurement.md) curl 例）
