...
- [ ] （手動・**検査図面 名称・公差種別設定** · 2026-07-06）管理コンソール `/admin/tools/part-measurement-templates` の **検査図面 名称・公差種別** で `幅=幾何公差` を保存すると、キオスク検査図面の `幅` の上限/下限公差候補が `0.001`〜`0.009` に切り替わること。既定では `直角度` / `面粗度` が幾何公差、`幅` が寸法公差。候補外手入力値は保持されること（[KB-320 §名称・公差種別設定](../knowledge-base/KB-320-kiosk-part-measurement.md#検査図面-名称-公差種別設定-2026-07-06)）
