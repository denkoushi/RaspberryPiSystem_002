...
CI は複数ジョブに分割されている。失敗時は **どのジョブか** を先に特定する。

| ジョブ名 | 役割 |
|----------|------|
| `lint-build-unit` | 監査・lint・共有パッケージ/Web のビルドとユニットテスト（DB なし） |
| `api-db-and-infra` | PostgreSQL 上の API 統合テスト（カバレッジ）とバックアップ/Ansible/監視 |
| `security-docker` | Trivy（fs + イメージ）と Buildx によるイメージビルド |
| `e2e-smoke` / `e2e-tests` | Playwright（スモークは `lint-build-unit` 通過後に API ジョブと並列で開始、フル E2E は API 統合テスト完了後に実行） |
...
**設計背景・audit ゲート・本番非デプロイ時の扱い**: [KB-353](../knowledge-base/ci-cd.md#kb-353-github-actions-のジョブ分割と-composite-action-による-ci-高速化2026-04-18)。
...
2. 失敗したワークフローをクリック
3. 失敗したジョブ（例: `lint-build-unit` / `api-db-and-infra`）をクリック
