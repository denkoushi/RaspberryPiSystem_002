...
**事象**:
- 計測機器を選択して氏名タグをスキャンした際、エラー（タグ未登録、既に貸出中など）が発生すると、持出登録ボタンが無限ループで動作し続ける
- エラーメッセージが長く、ユーザーが理解しにくい

**原因**:
- エラー発生時に`employeeTagUid`がクリアされず、自動送信の`useEffect`が再実行され続けていた
- APIエラーメッセージがそのまま表示され、ユーザーフレンドリーでない

**対処**:
1. エラー発生時に`employeeTagUid`をクリアし、自動再送を防止  
   - 変更箇所: `apps/web/src/pages/kiosk/KioskInstrumentBorrowPage.tsx` の`handleSubmit`関数内の`catch`ブロック
   - `setEmployeeTagUid('')`を追加
2. エラーメッセージを短縮・ユーザーフレンドリーに改善
   - `toShortMessage`関数を追加し、APIエラーメッセージを短縮
   - 「タグ未登録（計測機器）」「タグ未登録（社員）」「既に貸出中です」など、簡潔なメッセージに変換
   - 長いメッセージは「送信に失敗しました。再スキャンしてください。」に統一
3. Pi5でWebを再ビルド・デプロイし、Pi4キオスクブラウザを再起動
4. 確認手順: 計測機器を選択 → 未登録の氏名タグをスキャン → エラーメッセージが表示され、自動再送が止まること → 再スキャンで正常に持出登録できること

**メモ**:
- エラー時は氏名タグをクリアすることで、ユーザーに再スキャンを促す
- エラーメッセージは40文字以内に収め、ユーザーが理解しやすい表現に統一
...
- ✅ NFC/カメラ入力のスコープ分離が正常に動作（計測機器ページからPHOTOモードへの誤発火がないこと）
...
- [NFC/カメラ入力スコープ分離計画](../plans/nfc-stream-isolation-plan.md)
- [KB-091: NFC/カメラ入力のスコープ分離](../knowledge-base/frontend.md#kb-091-nfcカメラ入力のスコープ分離-別ページからのイベント横漏れ防止)
