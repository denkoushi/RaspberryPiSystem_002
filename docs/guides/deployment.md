...
### 補足（2026-05-03: **DGXリソース Phase3（補助起停・`SET_POLICY.applyWorkloadChanges`）**·`feat/dgx-resource-policy-orchestration-phase3`·repo 先行）
...
- **本番反映**: **未実施**（本リポジトリ実装・単体テストのみ。デプロイは通常の `update-all-clients.sh` 手順で別途判断）。
