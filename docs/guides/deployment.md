...
- **仕様（本変更後）**:
  ...
  - **一覧・実効完了 SQL 上の「消滅候補」フラグ**: [`buildFkojunstScheduleCsvDisappearanceEligibleScalarSql`](../../apps/api/src/services/production-schedule/policies/fkojunst-production-schedule-list-visibility.policy.ts) を **`fkmail` ありかつ `statusCode <> 'C'`** に変更（**`S`/`R` に限定しない**・`O`/`P`/その他非C も **消滅完了の対象になり得る**。**`C` はメール完了正本側**で扱う）。
...
- **トラブルシュート**:
  - **外れ値の「完了が付いた／付かない」** → **①** `fkmail.statusCode`（**`C`/`X` はメール完了**・消滅母集団から **`C` は除外**） **②** 対象行の **`occurredAt` が ±3ヶ月窓内か**（窓外は母集団に入らず消滅完了しない） **③** 直近取込が **`empty_schedule_csv` で skip されていないか**（ingestor warn） **④** Pi5 **`api` ref** が **`89086089` 以降（またはマージ後 `main` HEAD）**か。
...
- **問題の文脈（2026-05-09 仕様との関係）**: **消滅母集団**は従来どおり **`fkmail.statusCode <> 'C'` × `occurredAt` ±3ヶ月**（[#schedule-csv-disappearance-nonc-window-2026-05-09](#schedule-csv-disappearance-nonc-window-2026-05-09)）。一方、**差分の「現 winner 側」キー集合**を **`FKOJUNST` メールの JOIN 有無**で間接的に絞り込むと、**メール側に行がまだ載っていない（FK 欠落）本体 winner** を **誤って「CSV から消えた」**とみなし、**消滅完了が早立ち**し得る。
- **仕様（本変更後）**:
  ...
  - **母集団（消滅候補）・`C` 除外・空 winner ガード** は **2026-05-09 項と同一**。**計画ファイル（CSV）は編集しない**——専用サービス経由で **外部完了同期の入力だけ** を整理する。
...
- **トラブルシュート**:
  ...
  - **外部完了が期待と違う** → メール由来は **`C`/`X` のみ**。生産日程CSV「消滅」由来は **2026-05-09 以降** **`fkmail.statusCode <> 'C'` かつ `occurredAt` が UTC ±3ヶ月窓内**の母集団と現 winner の差分（**[§2026-05-09 消滅窓](#schedule-csv-disappearance-nonc-window-2026-05-09)**・[KB-370](../knowledge-base/KB-370-production-schedule-external-completion-triple-source.md)）。
