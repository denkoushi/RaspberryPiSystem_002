...
最終更新: 2026-04-23（**計測機器点検可視化 カード本文タイポ・`計測機器明細` JSON**（ブランチ **`feature/mi-inspection-signage-body-typography`**・代表 **`6a6b190b`**）: **`raspberrypi5` のみ**（**Pi3 は対象外**・JPEG は Pi5 API 生成）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feature/mi-inspection-signage-body-typography infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`・Detach **`20260423-184152-15152`**（**`failed=0` / `unreachable=0` / exit `0`**・所要 **約 11.0 分**）・**後追い Phase12**: **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **78s**・Tailscale）。**仕様**: データソースが **`計測機器明細`** 列（JSON、空エントリ除外）を出し、レンダラーは **`row-instrument-entries.ts`** で JSON 優先→レガシー、**貸出中**は **管理番号行→名称行**（行高 **ベース比 1.5 倍**）・`layout-mi-instrument-body.ts` / `measuring-instrument-loan-inspection-renderer.ts`。**知見**: `apps/api` 差分のため **Docker 再ビルド**。**トラブルシュート**: 複数台は **1 台ずつ**・[KB-200](./knowledge-base/infrastructure/ansible-deployment.md#kb-200-デプロイ標準手順のfail-fastチェック追加とデタッチ実行ログ追尾機能)（未追跡で fail-fast）。**関連**: [KB-347](./knowledge-base/api.md#kb-347-サイネージ可視化の業務日切替jst-翌900自動表示のみ) 追補。**前項** 2026-04-23（**管理コンソール サイネージスケジュール 可視化ダッシュボード選択UX（パレット可視化の発見しやすさ・Web のみ・Pi5 のみ）**（ブランチ **`main`**・代表 **`8e72335e`**）: **`raspberrypi5` のみ**・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh main infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`・Detach **`20260423-173454-25250`**（**`failed=0` / `unreachable=0` / exit `0`**）・**後追い Phase12**: **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **114s**・Tailscale）。**仕様**: `/admin/signage/schedules` の **FULL/SPLIT 可視化スロット**で `VisualizationDashboardGroupedSelect`（**`pallet_visualization_board` は `パレット可視化` `<optgroup>`**・他は **`その他の可視化`**）、オプションラベルに **`パレット可視化` / `計測機器点検` / `renderer:`** 等、**無効ダッシュボード**は **`（無効）`**、一覧取得は **`enabled` 未フィルタ**（選択・保存可否は既存ロジック）。**`pallet_visualization_board` が 0 件**なら **`/admin/visualization-dashboards` へのリンク付き警告**（**一覧 pending / error 時は非表示**）。**知見**: ローカルに **未追跡のプレビュー HTML** 等があると **`update-all-clients.sh` が fail-fast**（[KB-200](./knowledge-base/infrastructure/ansible-deployment.md#kb-200-デプロイ標準手順のfail-fastチェック追加とデタッチ実行ログ追尾機能)）→ **`git stash push -u`**（終了後 **`git stash pop`**）。**トラブルシュート**: パレット可視化が一覧に無いときは **DB に該当ダッシュボード未作成**が先（**パレット可視化プリセット**で作成）。**関連**: [KB-355](./knowledge-base/api.md)。**前項** 2026-04-23（**配膳スマホ パレット可視化 カード同期・表示項目拡張（API+Web）**（ブランチ **`feat/mobile-pallet-viz-card-and-tenkey`**・代表 **`faa6e6db`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01` → `raspberrypi3`** を **1 台ずつ**（**Pi3** は最後に **`--limit raspberrypi3` のみ**・[ラズパイ3（サイネージ）の更新](#ラズパイ3サイネージの更新) 遵守）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/mobile-pallet-viz-card-and-tenkey infrastructure/ansible/inventory.yml --limit <host> --detach --follow`・Detach **`20260423-140752-20500` → `20260423-141809-14243` → `20260423-142223-32575` → `20260423-142535-28478` → `20260423-143027-7377` → `20260423-143404-3250`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **25s**）・**追加スモーク**: `GET https://100.106.158.2/kiosk/mobile-placement/pallet-viz`・`…/kiosk/pallet-visualization` → **各 200**（Mac からの **`curl` は自己署名 TLS のため `curl -k`**）。**仕様**: パレットボード item の **`machineNameDisplay`**・着手日・個数・外寸・`scheduleSnapshot` 拡張、既存行は **`csvDashboardRowId` フォールバック**（`pallet-visualization-query.service.ts` 等）。配膳スマホ **`resolvePalletNoFromTenkeyDigitsImmediate`**・**`palletVizListItemMapping`**・**`usePalletTenkeyNavBusy`**・カード高さ固定・製番ラベル整理・パレット番号は数字のみ表示。**知見**: Pi5 で **`api`/`web` 再ビルド**（差分時）、Pi4 は repo 同期 + **`kiosk-browser` 再起動**。**Pi3 知見**: ヘルス収集で **`signage-lite` が一時 `exit-code` / `activating (auto-restart)`** が出ても **lightdm 復旧 post_tasks 後** **`signage-lite.service is active`**（本記録）。**トラブルシュート**: 複数台は **前 `--detach --follow` 完了後**に次（exit 3・[deploy-status-recovery.md](../runbooks/deploy-status-recovery.md)）。**関連**: [KB-355](../knowledge-base/api.md)。**前項** 2026-04-23（**配膳スマホ パレット可視化 Web モジュール分割（SOLID・Web のみ）**（ブランチ **`feat/mobile-pallet-viz-solid`**・代表 **`25cdbe9b`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**（**Pi3 は対象外**・`/kiosk/mobile-placement/pallet-viz` は配膳スマホ向けのため）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/mobile-pallet-viz-solid infrastructure/ansible/inventory.yml --limit <host> --detach --follow`・Detach **`20260423-130413-22550` → `20260423-130834-5856` → `20260423-131244-3832` → `20260423-131556-13627` → `20260423-132037-22553`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **23s**）・**追加スモーク**: `GET https://100.106.158.2/kiosk/mobile-placement/pallet-viz`・`…/kiosk/pallet-visualization` → **各 200**。**仕様**: `pushPalletTenkeyDigit`・`useKioskMobilePalletDigitBuffer`・`applyMobilePalletOrderScan`・テンキー/ヘッダ/機種セレクトのコンポーネント分割。`KioskMobilePalletVisualizationPage` のテンキー操作時 **`setLocalError(null)` はインライン**（`clearErrorAnd` 廃止で hooks 依存を明確化）。**知見**: Pi5 で **`web` 再ビルド**、Pi4 は repo 同期 + **`kiosk-browser` 再起動**。**トラブルシュート**: 複数台は **前 `--detach --follow` 完了後**に次を起動（exit 3・[deploy-status-recovery.md](../runbooks/deploy-status-recovery.md)）。**関連**: [KB-355](../knowledge-base/api.md)。**前項** 2026-04-23（**スマホ配膳 パレット可視化・機種別 palletCount**（ブランチ **`feat/mobile-pallet-viz-machine-pallet-count`**・代表 **`17c39259`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01` → `raspberrypi3`** を **1 台ずつ**（**Pi3** は最後に **`--limit raspberrypi3` のみ**・[ラズパイ3（サイネージ）の更新](#ラズパイ3サイネージの更新) 遵守）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/mobile-pallet-viz-machine-pallet-count infrastructure/ansible/inventory.yml --limit <host> --detach --follow`・Detach **`20260423-113519-6942` → `20260423-114540-21117` → `20260423-115005-11668` → `20260423-115332-29019` → `20260423-115720-25646` → `20260423-120114-31885`**（Pi3 初回は post_tasks の **`Start signage services after lightdm restore`** で **`signage-lite-update.timer`** が **`Could not find the requested service`** となり **`failed=1`**）→ **再実行** **`20260423-120929-13353`**（**`failed=0`**・**`signage-lite.service is active`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **26s**）・**追加スモーク**: `GET https://100.106.158.2/kiosk/mobile-placement/pallet-viz`・`…/kiosk/pallet-visualization`・`…/admin/pallet-machine-illustrations` → **各 200**。**仕様**: `PalletMachineIllustration.palletCount`（1..60・既存 **10**）・`PATCH /api/tools/.../pallet-count`・**`/kiosk/mobile-placement/pallet-viz`**（テンキー 1/2 桁→カメラスキャン）・`pallet_board` **可変段数**。**知見**: Pi5 **Prisma マイグレ + api/web 再ビルド**。**トラブルシュート**: Pi3 **timer 起動失敗**は **同 host に `--limit raspberrypi3` 再デプロイ**で収束例あり。**関連**: [KB-355](../knowledge-base/api.md)。**前項** 2026-04-23（**Pi4 `barcode-agent`（CDC ACM）+ `useSerialBarcodeStream` + API `lxml` パッチ（Trivy CVE-2026-41066）**（ブランチ **`feat/pallet-serial-barcode-agent`**・代表 **`53fcc704`**）: **`raspberrypi5` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**（**Pi3 は対象外**）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/pallet-serial-barcode-agent infrastructure/ansible/inventory.yml --limit <host> --detach --follow`・Detach **`20260423-091833-30823` → `20260423-093852-25734`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **29s**）・**追加スモーク**: `GET https://100.106.158.2/kiosk/pallet-visualization` → **200**。**仕様**: `docker compose --profile barcode` の **`barcode-agent`**（`ws://localhost:7072/stream`）と **`useKeyboardWedgeScan`** を **`usePalletVisualizationController`** で **同一 `applyBarcode` に合流**（inventory は **StoneBase01 のみ** `barcode_agent_enabled`）。**知見**: Pi5 で **`api` 再ビルド**（`lxml`）・StoneBase で **`restart barcode-agent` handler**。**トラブルシュート**: `/dev/ttyACM*`・ボーレートは **`barcode_agent_serial_*`**・Pi4 上 **`curl -s http://127.0.0.1:7072/api/agent/status`**。**関連**: [KB-355](../knowledge-base/api.md)。**前項** 2026-04-22（**キオスク 持出左ペイン パレット操作行 UX（compact・文言・暗背景視認性・Web のみ）**（ブランチ **`feat/kiosk-pallet-embed-action-row-layout`**・代表 **`825a2f8d`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**（**Pi3 は対象外**）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/kiosk-pallet-embed-action-row-layout infrastructure/ansible/inventory.yml --limit <host> --detach --follow`・Detach **`20260422-212321-19726` → `20260422-212908-2306` → `20260422-213420-32381` → `20260422-213836-18696` → `20260422-214244-25414`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **111s**）・**追加スモーク**: `GET https://100.106.158.2/kiosk/tag`・`…/kiosk/photo`・`…/kiosk/pallet-visualization` → **各 200**。**仕様**: `copy.ts` で **追加/上書/選択削除/全削除**、`PalletVizActionRow` の **`density`**（埋め込み **`compact`**・4 列 1 行・`truncate`）、3・4 番 **`ghostOnDark`**、全削除は **赤系**、`PalletVizEmbeddedPanel` イラスト **`h-32`**。専用 `/kiosk/pallet-visualization` は `density` 省略で **後段ボタン視認性を共有**。**知見**: **Pi5 のみ** Docker **`web` 再ビルド**。**トラブルシュート**: 直列でも **前 `--detach --follow` 完了前**に次を起動すると **Mac ローカルロック** exit 3。**関連**: [KB-355](../knowledge-base/api.md)・[design preview](../design-previews/kiosk-borrow-left-pallet-embed-preview.html)。**前項** 2026-04-22（**キオスク 持出左ペイン パレット併設（`/kiosk/tag`・`/kiosk/photo`・Web のみ）**（ブランチ **`feat/kiosk-borrow-left-pane-pallet-embed`**・代表 **`d189a971`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**（**Pi3 は対象外**・本機能はキオスク持出画面のため）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/kiosk-borrow-left-pane-pallet-embed infrastructure/ansible/inventory.yml --limit <host> --detach --follow`・Detach **`20260422-195055-26766` → `20260422-195523-3733` → `20260422-200022-22643` → `20260422-200451-21847` → `20260422-201046-31258`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **65s**）・**追加スモーク**: `GET https://100.106.158.2/kiosk/tag`・`…/kiosk/photo`・`…/kiosk/pallet-visualization` → **各 200**。**仕様**: `PalletVizEmbeddedPanel`・`usePalletVisualizationController`・左 **`w-80`** 列に compact 番号グリッド・操作行・イラスト・機種名・`PalletVizItemList`。機械データ無し時は **`canClearPallet=false`** でパレット全消去を無効化。**知見**: Pi5 のみ **Docker `web` 再ビルド**。**トラブルシュート**: 直列でも **前 `--detach --follow` 完了前**に次を起動すると **Mac ローカルロック** exit 3。**関連**: [KB-355](../knowledge-base/api.md)・[design preview](../design-previews/kiosk-borrow-left-pallet-embed-preview.html)。**前項** 2026-04-22（**加工機イラスト Docker 永続化（`pallet-machine-illustrations` bind）**（ブランチ **`fix/pallet-machine-illustrations-volume`**・代表 **`937684fd`**）: **`raspberrypi5` のみ**（**Pi3 対象外**）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh fix/pallet-machine-illustrations-volume infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`・Detach **`20260422-185725-32599`**（**`failed=0` / `unreachable=0` / exit `0`**・所要 **約 155s**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **74s**）・**仕様**: `infrastructure/docker/docker-compose.server.yml` の `api` に `pallet-machine-illustrations-storage`（ホスト bind **`/opt/RaspberryPiSystem_002/storage/pallet-machine-illustrations`**）をマウント。`infrastructure/docker/docker-compose.mac-local.override.yml` ではローカル `../../.docker/local/storage/...`。**Ansible** `infrastructure/ansible/roles/server/tasks/main.yml` でホストディレクトリ作成。**根因**: 未マウントのまま `api` 再作成されると、コンテナ内の実ファイルは失われ **デプロイ後にイラスト 404**（DB の `imageRelativeUrl` は残る）。**トラブルシュート**: 修正前に消えたファイルは **再アップロード**が必要。計測機器ジャンル画像（[KB-343](../knowledge-base/infrastructure/ansible-deployment.md#kb-343-measuring-instrument-genre-image-persistence)）と同系。**関連**: [api.md（KB-355 追補）](../knowledge-base/api.md)。**前項** 2026-04-22（**キオスク パレット可視化 UI コンポーネント分割**（ブランチ **`feat/kiosk-pallet-visualization-ui`**・代表 **`029ecc16`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01` → `raspberrypi3`** を **1 台ずつ**（**Pi3** は **最後**に **`--limit raspberrypi3` のみ**・[ラズパイ3（サイネージ）の更新](#ラズパイ3サイネージの更新) 遵守）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/kiosk-pallet-visualization-ui infrastructure/ansible/inventory.yml --limit <host> --detach --follow`・Detach **`20260422-172813-2568` → `20260422-173734-20394` → `20260422-174658-25070` → `20260422-175728-9680` → `20260422-180809-22370` → `20260422-181644-9337`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（最終一巡 約 **148s**）・**仕様**: `KioskPalletVisualizationPage` を `apps/web/src/features/kiosk/pallet-visualization/`（`PalletVizMachineHeader` / `PalletVizActionRow` / `PalletVizItemList` / `copy.ts` 等）へ分割。番号グリッドは **`role="group"` + `aria-label`**、部品行は **`aria-pressed`**。**知見**: **Pi5 のみ** `Rebuild/Restart docker compose services`（Web 再ビルド）。**Pi3**: ヘルス収集時に一時 **`signage-lite` exit-code** が出ても **playbook 完了時点で `signage-lite.service is active`**（本記録）。**トラブルシュート**: **未追跡ファイル**は **`git stash push -u`**（fail-fast 回避）。**関連**: [KB-311](../knowledge-base/KB-311-kiosk-immersive-header-allowlist.md)・[KB-355（api.md）](../knowledge-base/api.md)。**前項** 2026-04-22（**パレット可視化 沉浸式レイアウト（左 aside 独立スクロール復旧）**（ブランチ **`fix/pallet-visualization-immersive-scroll`**・ポリシー **`33d4092f`**・E2E **`019bc752`**）: **`raspberrypi5` のみ**・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh fix/pallet-visualization-immersive-scroll infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`・Detach **`20260422-161223-27548`**（**`failed=0` / `unreachable=0` / exit `0`**）・**仕様**: `usesKioskImmersiveLayout` allowlist（`kioskImmersiveLayoutPolicy.ts`）に **`/kiosk/pallet-visualization`** を追加し **`KioskLayout` が `h-dvh` 系**になる。**根因**: allowlist 漏れで非沉浸式となり左 `aside` の **overflow 分離が効かず**ページ全体が縦スクロールしていた。**回帰**: `e2e/pallet-viz-aside-scroll.spec.ts`。**知見**: Mac 側 **未コミット／未追跡**で fail-fast → **`git stash push -u`**（終了後 **`git stash pop`**）。**関連**: [api.md（KB-355）](../knowledge-base/api.md)・[KB-311](../knowledge-base/KB-311-kiosk-immersive-header-allowlist.md)。**前項** 2026-04-22（**パレット可視化 UI 修正・イラスト公開 GET・キオスク左独立スクロール・ウェッジ入力**（ブランチ **`feat/pallet-visualization-ui-fixes`**・代表 **`d3c2f7b5`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01` → `raspberrypi3`** を **1 台ずつ**（**Pi3** は **最後**に **`--limit raspberrypi3` のみ**・[ラズパイ3（サイネージ）の更新](#ラズパイ3サイネージの更新) 遵守）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/pallet-visualization-ui-fixes infrastructure/ansible/inventory.yml --limit <host> --detach --follow`・Detach **`20260422-150051-25397` → `20260422-152055-3569` → `20260422-152512-6215` → `20260422-152824-13591` → `20260422-153204-7953` → `20260422-153534-30166`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **30s**）。**仕様**: `GET /api/storage/pallet-machine-illustrations/*` は **JWT / x-client-key なし**で配信（`<img src>` 用・UUID ファイル名前提・パスは Fastify ワイルドカード `request.params['*']`）。キオスク `/kiosk/pallet-visualization` は **左ペイン独立スクロール**・**パレット番号 10 列・数字拡大**・**USB キーボードウェッジ**（`useKeyboardWedgeScan`・Enter/アイドル確定・**キー間隔が既定閾値を超えたらバッファ破棄**）。**知見**: Pi5 のみ **Docker 再ビルド**（当該列の先頭ホスト）。**Pi3**: ヘルス収集中に **一時 `signage-lite` exit-code** が出ても **lightdm 復旧後 `active` 化**し得る（長続きは [deploy-status-recovery.md](../runbooks/deploy-status-recovery.md)）。**トラブルシュート**: 直列でも **前ジョブの Mac 側終了前**に次を起動すると **ローカルロック** exit 3。**関連**: [api.md（KB-355 追補）](../knowledge-base/api.md)。**前項** 2026-04-22（**加工機パレット可視化ワークフロー**（キオスク `/kiosk/pallet-visualization`・管理 `/admin/pallet-machine-illustrations`・可視化 `pallet_visualization_board` / `pallet_board`・Prisma `20260422140000_pallet_visualization`・ストレージ `pallet-machine-illustrations`）・ブランチ **`feat/pallet-visualization`**・代表 **`8ea52d09`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01` → `raspberrypi3`** を **1 台ずつ**（**Pi3** は **最後**に **`--limit raspberrypi3` のみ**・[ラズパイ3（サイネージ）の更新](#ラズパイ3サイネージの更新) 遵守）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/pallet-visualization infrastructure/ansible/inventory.yml --limit <host> --detach --follow`・Detach **`20260422-135405-32716` → `20260422-140303-10184` → `20260422-140726-29698` → `20260422-141044-15315` → `20260422-141446-12382` → `20260422-141809-5836`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **24s**）。**追加スモーク**: `GET https://100.106.158.2/kiosk/pallet-visualization`・`…/admin/pallet-machine-illustrations` → **各 200**。**仕様**: 加工機ごとのパレット載せ・おろしイベント記録、管理イラスト差し替え、サイネージで `pallet_board` 表示可能。**知見**: **Pi5 のみ**初回デプロイで **Docker 再ビルド**（API/Web）。**Pi3**: プレフライトで lightdm 停止中は `signage-lite` が一時 **`exit-code`** でも **復旧タスク後**に **`signage-lite.service is active`**（既存 Pi3 補足と同型）。**トラブルシュート**: 直列デプロイで **前ジョブ完全終了前**に次を起動すると **Mac ローカルロック** exit 3。**関連**: [api.md（KB-355）](../knowledge-base/api.md)。**前項** 2026-04-22（**計測機器点検可視化 カード密度・表示順**（`measuring_instrument_loan_inspection`・`row-priority` / `card-layout` / `instrument-name-text`・Vitest）・ブランチ **`feat/mi-inspection-card-density-optimization`**・代表 **`7bbf1077`**）: **`raspberrypi5` のみ**（**Pi3 は対象外**）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/mi-inspection-card-density-optimization infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`・Detach **`20260422-113105-23315`**（**`failed=0` / `unreachable=0` / exit `0`**・所要 **約 10.2 分**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **25s**）。**仕様**: 画像内ヘッダを **タイトル+日付1行**にしカード領域を拡大。**並び**: **貸出あり優先**・**件数降順**・**氏名昇順**。**レイアウト**: 4列固定・**縦可変カード**・1画面超は **警告表示**。**知見**: `apps/api` 変更のため **Docker 再ビルド**が走る。**関連**: [api.md（KB-347 追補）](../knowledge-base/api.md)。**前項** 2026-04-22（**購買照会バーコード読取安定化（誤検出抑止・Web のみ・`feat/purchase-order-scan-stability`・代表 **`b2180b1e`**）: `BARCODE_FORMAT_PRESET_PURCHASE_ORDER`・`useBarcodeScanSession` / `BarcodeScanModal` の任意 **`stabilityConfig` / `readerOptions`**・`barcodeReadStability.ts` + Vitest。購買照会のみ **短時間の同一値2連続一致で確定**、他画面は **未指定で従来どおり即確定**。**対象**: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**（**Pi3 は対象外**）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/purchase-order-scan-stability infrastructure/ansible/inventory.yml --limit <host> --detach --follow`・Detach **`20260422-091400-26049` → `20260422-091829-21827` → `20260422-092256-32471` → `20260422-092617-8016` → `20260422-093223-17140`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **26s**）。**追加スモーク**: `GET https://100.106.158.2/kiosk/purchase-order-lookup` → **200**。**知見**: 共有モーダルを壊さず購買照会だけチューニングするため **オプション化**。**トラブルシュート**: 同一 Pi5 へ `update-all-clients` **並列禁止**・プレフライト失敗後の **stale lock** は [deploy-status-recovery.md](../runbooks/deploy-status-recovery.md) §5.2。ラベル形式が **`BARCODE_FORMAT_PRESET_PURCHASE_ORDER` 外**なら `formatPresets.ts` へ形式追加。**関連**: [KB-297 §FKOBAINO](./knowledge-base/KB-297-kiosk-due-management-workflow.md#fkobaino-purchase-order-lookup-from-gmail-csv-2026-04-20)。**前項** 2026-04-21（**購買照会 FHINCD 照合キー v2（括弧除去 + 末尾数値ハイフン枝番除去）・`PurchaseOrderLookupRow.purchasePartCodeMatchKey`・Prisma マイグレ `20260422120000_purchase_order_lookup_fhincd_match_key`・照会の段階移行フォールバック**（ブランチ **`fix/fhincd-normalization-v2`**・代表 **`65955268`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**（**Pi3 は対象外**）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh fix/fhincd-normalization-v2 infrastructure/ansible/inventory.yml --limit <host> --detach --follow`・Detach **`20260421-211718-16570` → `20260421-213337-30126` → `20260421-213832-8006` → `20260421-214223-235` → `20260421-214721-3204`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **53s**）。**仕様**: upsert 一意は **`purchasePartCodeMatchKey`**（`purchasePartCodeNormalized` は括弧除去のみ・表示互換）。`FHINMEI` / `plannedStartDate` は **照合キー優先**、未取得時は **括弧除去のみ**でフォールバック。**知見**: マイグレで重複行は **最新 `updatedAt`（同順位なら `id`）を残す**。**関連**: [KB-297（FKOBAINO）](./knowledge-base/KB-297-kiosk-due-management-workflow.md#fkobaino-purchase-order-lookup-from-gmail-csv-2026-04-20)。**前項** 2026-04-21（**購買照会: `PurchaseOrderLookupRow` 蓄積型 upsert（`sourceCsvDashboardId` + `FKOBAINO` + `FSEIBAN` + 正規化 `FHINCD`）・生産日程補助の最古着手日（`plannedStartDate`）合成・`Dockerfile.web` Caddy 同梱依存（`pgx`/`puddle`/`smallstep/certificates`/`otel`）ピン留め**（ブランチ **`feat/purchase-order-lookup-history-start-date`**・代表 **`92fd37e4`**）: **`raspberrypi5` のみ**（Pi4/Pi3 未デプロイ）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/purchase-order-lookup-history-start-date infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`・Detach **`20260421-192642-23281`**（**`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **94s**）。**仕様**: Gmail FKOBAINO 取り込みは行単位 **upsert**（同一複合キーは更新）。照会レスポンスに **`plannedStartDate`**（`YYYY-MM-DD`、なしは UI で空表示）。表示順は **機種 → 製番 → 着手日 → 品名 → 品番 → 個数**。**知見**: `pgx` を `replace` しただけでは **`puddle/v2` 非互換で Caddy ビルド失敗** → `puddle` も明示。Trivy は **`smallstep/certificates`・`go.opentelemetry.io/otel/sdk`** も更新対象になり得る。**関連**: [KB-297（FKOBAINO）](./knowledge-base/KB-297-kiosk-due-management-workflow.md#fkobaino-purchase-order-lookup-from-gmail-csv-2026-04-20)・[ci-cd KB-307](./knowledge-base/ci-cd.md#kb-307-trivy-image-web-が-usrbincaddy-の-cve-を検出して-ci-が失敗する)。**前項** 2026-04-21（**計測機器点検可視化: 取消済み `Loan` を貸出中から除外**（`extractLoanIdFromEventRaw`・`loadCancelledLoanIdSet`・`MeasuringInstrumentLoanInspectionDataSource`）・ブランチ **`feat/measuring-instrument-inspection-exclude-cancelled-loans`**・代表 **`41c981b3`**）: **`raspberrypi5` のみ**（Pi4/Pi3 未デプロイ）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/measuring-instrument-inspection-exclude-cancelled-loans infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`・Detach **`20260421-175831-4238`**（**`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **134s**）。**仕様**: NFC 由来の `raw.loanId` が取消済み `Loan` と一致する `持ち出し` は一覧から除外。Gmail CSV は `loanId` が無いことが多く **返却** 行で従来どおりペア消し。**知見**: `Loan.cancel` は `MeasuringInstrumentLoanEvent` に `返却` を追加しないため、今回の突合がないと取消後も「貸出中」に残り得た。**関連**: [api.md（KB-347・2026-04-21 追補）](../knowledge-base/api.md)。**前項** 2026-04-21（**計測機器点検可視化 `measuring_instrument_loan_inspection` の一覧ラベルを `名称 (管理番号)` 形式に**（`formatLoanInspectionInstrumentLabel`・`MeasuringInstrumentLoanInspectionDataSource`）・ブランチ **`feat/measuring-instrument-inspection-display-label`**・代表 **`1c3d5e9b`**）: **`raspberrypi5` のみ**（Pi4/Pi3 未デプロイ・**本変更は API のみ**）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/measuring-instrument-inspection-display-label infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`・Detach **`20260421-143351-15107`**（**`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43 / WARN 0 / FAIL 0**（約 **42s**）。**仕様**: 列名 `計測機器名称一覧` は据え置き、各トークンは **`名称 (管理番号)`**。管理番号欠落時は名称のみ。**知見**: 文言が長くなりカード内省略が早くなり得る（レイアウト調整は別タスク）。**関連**: [api.md（KB-347・2026-04-21 追補）](../knowledge-base/api.md)。**前項** 2026-04-20（**キオスク モバイル注文入力スキャン専用・棚番チップ密度**（購買照会 `/kiosk/purchase-order-lookup`・配膳メイン下半 **製造order** を `readOnly` + `inputMode="none"`・手入力 debounce 廃止・`mobilePlacementKioskTheme` の棚チップ **`grid-cols-3 sm:grid-cols-4`** 等）・ブランチ **`feat/scan-only-order-inputs-and-shelf-chip-mobile`**・代表 **`423e32bb`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**（**`raspi4-fjv60-80`** は **プレフライト SSH timeout**（`100.100.229.95:22`）で **未デプロイ**・**Pi3 は対象外**）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/scan-only-order-inputs-and-shelf-chip-mobile infrastructure/ansible/inventory.yml --limit <host> --detach --follow`・Detach **`20260420-211100-899` → `20260420-211743-28526` → `20260420-212322-14477` → `20260420-213004-29970`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・**知見**: `raspi4-fjv60-80` プレフライト失敗後に Pi5 **`/opt/RaspberryPiSystem_002/logs/.update-all-clients.lock`** が **`runPid: null` の stale** のまま残り（本記録では runId **`20260420-212814-6231`**）、次ホストで **リモートロック取得不能** → [deploy-status-recovery.md](../runbooks/deploy-status-recovery.md) §5.2 で **確認・削除**後に再実行・**Phase12**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 42 / WARN 1 / FAIL 0**。**追加スモーク**: `GET https://100.106.158.2/kiosk/purchase-order-lookup`・`…/kiosk/mobile-placement` → **各 200**。**ナレッジ**: [KB-297 §FKOBAINO](../knowledge-base/KB-297-kiosk-due-management-workflow.md#fkobaino-purchase-order-lookup-from-gmail-csv-2026-04-20)・[KB-339 §V23](../knowledge-base/KB-339-mobile-placement-barcode-survey.md#v23-scan-only-shelf-chip-2026-04-20)・[mobile-placement-smartphone.md](../runbooks/mobile-placement-smartphone.md)。**前項** 2026-04-20（**キオスク購買照会 UX（`/kiosk/purchase-order-lookup`・`usePurchaseOrderLookup`・`buildPurchaseOrderRowLines`・Vitest・ブランチ `feat/kiosk-purchase-order-lookup-ux`・代表 `b0c2e68e`）** **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**（**`raspi4-fjv60-80`** は **プレフライト SSH timeout** で **未デプロイ**・**Pi3 は対象外**）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/kiosk-purchase-order-lookup-ux infrastructure/ansible/inventory.yml --limit <host> --detach --follow`・Detach **`20260420-185803-18736` → `20260420-191222-23450` → `20260420-191818-15715` → `20260420-193055-747`**（各成功 **`failed=0` / `unreachable=0` / exit `0`**）・**知見**: `raspi4-fjv60-80` プレフライト失敗後に Pi5 に **stale lock**（`runPid: null`）が残り、続く **`raspi4-kensaku-stonebase01` でロック取得不能** → [deploy-status-recovery.md](../runbooks/deploy-status-recovery.md) §5.2 の **Python スクリプトでロック削除**後に再実行。**Phase12**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 42 / WARN 1 / FAIL 0**。**追加スモーク**: `GET https://100.106.158.2/kiosk/purchase-order-lookup` → **200**。**ナレッジ**: [KB-297 §FKOBAINO](./knowledge-base/KB-297-kiosk-due-management-workflow.md#fkobaino-purchase-order-lookup-from-gmail-csv-2026-04-20)。**前項** 2026-04-20（**FKOBAINO 購買注文番号照会（Gmail 件名 `FKOBAINO`・`PurchaseOrderLookupRow`・post-ingest 同期・`GET /api/kiosk/purchase-order-lookup/:purchaseOrderNo`・Web `/kiosk/purchase-order-lookup`・固定 Gmail スケジュール `csv-import-purchase-order-fkobaino`）**・ブランチ `feat/fkobaino-purchase-order-lookup`・コミット **`607e71c0`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-kensaku-stonebase01`** を **1 台ずつ** 成功（**`raspi4-fjv60-80`** は **プレフライト SSH で Pi5→`100.100.229.95:22` が timeout** のため **未デプロイ**・**Pi3 は対象外**）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/fkobaino-purchase-order-lookup infrastructure/ansible/inventory.yml --limit <host> --detach --follow`・Detach **`20260420-153825-14948` → `20260420-154924-28306` → `20260420-155406-1101` → `20260420-155936-12366`**（各成功 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 42 / WARN 1 / FAIL 0**（**WARN**: `raspi4-fjv60-80` の kiosk/status-agent SSH・ベースライン同型）。**知見**: プレフライト失敗で Pi5 に **`/opt/RaspberryPiSystem_002/logs/.update-all-clients.lock` が残る**場合は [deploy-status-recovery.md](../runbooks/deploy-status-recovery.md) §5.2 の **`runPid` 死活確認後にロック削除**（本記録では `runPid: null` のため削除可）。**トラブルシュート**: `raspi4-fjv60-80` は **現場で電源・Tailscale・LAN**（`group_vars/all.yml` の `raspi4_fjv60_80_ip`）確認後、**単体 `--limit raspi4-fjv60-80`** で再デプロイ。**ナレッジ**: [KB-297 §FKOBAINO](../knowledge-base/KB-297-kiosk-due-management-workflow.md#fkobaino-purchase-order-lookup-from-gmail-csv-2026-04-20)。**前項** 2026-04-19（**貸出レポート supply 系ツリーマップ復旧（HTML レポート／プレビューへの組み込み・`loan-report` treemap モジュール・デザインプレビュー HTML・Vitest）**・ブランチ `feat/loan-report-supply-treemap-recovery`・デプロイ先端 **`90cc5385`**（機能 **`a8b2f7cf`** + CI ESLint `prefer-const` 追随 **`90cc5385`**））: **`raspberrypi5` のみ**（Pi4/Pi3 未デプロイ・**Pi3 は専用手順のため対象外**・**`--limit` 単一ホスト**）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/loan-report-supply-treemap-recovery infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`・Detach **`20260419-130715-8630`**（**`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 42 / WARN 1 / FAIL 0**（所要は Pi5 単体デプロイの通常レンジ）。**追加スモーク**: 未認証 `GET /api/reports/loan-report/preview?category=rigging` → **`401`**。**WARN**: Pi5 から `raspi4-fjv60-80` への SSH 実チェック不可（ベースライン同様・`deploy-status` は PASS）。**知見**: 貸出レポートの視覚化は **プレビュー HTML と本番レンダラの契約を揃えない**と、ローカル救出物の再統合時にズレやすい（`docs/design-previews` と API 側をペアで見る）。**トラブルシュート**: CI の `lint-build-unit` で ESLint `prefer-const` が出た場合は **本番挙動に影響しない最小修正**（例: `LoanReportEvaluationService` の `const` 化）で main 取り込み前に潰す。**CI**: GitHub Actions Run **`24620066625`** success（PR #170 相当）。**ナレッジ**: [KB-354](../knowledge-base/KB-354-admin-loan-report-gmail-draft-deploy.md) §E。**PR**: [#170](https://github.com/denkoushi/RaspberryPiSystem_002/pull/170)。**前項** 2026-04-19（**貸出レポート API 本番衛生化（開発用ローカル ingest テレメトリ除去・`resolveLoanReportPeriod`/月次アンカー/評価ロジックの Vitest 回帰・プレビュー API 統合テスト）**・`feat/loan-report-hardening`・代表実装コミット **`20c4a765`**）: **`raspberrypi5` のみ**（Pi4/Pi3 未デプロイ・**Pi3 は専用手順のため対象外**）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/loan-report-hardening infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`・Detach **`20260419-081737-4484`**（**`failed=0` / `unreachable=0` / exit `0`**・所要約 **10.5 分**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 42 / WARN 1 / FAIL 0**（約 **65s**）。**追加スモーク**: 未認証 `GET /api/reports/loan-report/preview?category=rigging` → **`401`**。**WARN**: Pi5 から `raspi4-fjv60-80` への SSH 実チェック不可（ベースライン同様・`deploy-status` は PASS）。**知見**: 本番 API に **`127.0.0.1:7426` 等への外向きデバッグ呼び出し**を残さない（性能・セキュリティ・ログ汚染）。**ナレッジ**: [KB-354](../knowledge-base/KB-354-admin-loan-report-gmail-draft-deploy.md) §D。**前項** 2026-04-18（**管理コンソール 貸出レポート 実メトリクス統合・プレビュー幅拡大（各 `loan-analytics` の従業員行に期限超過件数・`LoanReport` 評価から推定配分を排除して借用実データへ・`LoanReportPage` でコンテンツ最大幅をビューポートまで拡張）**・`fix/loan-report-real-metrics-wide-preview`・コミット **`937be20f`**）: **`raspberrypi5` のみ**（Pi4/Pi3 未デプロイ・**Pi3 は専用手順のため対象外**）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh fix/loan-report-real-metrics-wide-preview infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`・Detach **`20260418-204637-27968`**（**`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 42 / WARN 1 / FAIL 0**（約 **62s**）。**WARN**: Pi5 から `raspi4-fjv60-80` への SSH 実チェック不可（ベースライン同様・`deploy-status` は PASS）。**デプロイ（Mac）前**: **未追跡ファイル**（例: `apps/web/public/*preview*.html`）は **`git stash push -u`** で退避しないと fail-fast。**CI**: Run **`24603756892`** success。**ナレッジ**: [KB-354](../knowledge-base/KB-354-admin-loan-report-gmail-draft-deploy.md) §C。**PR**: [#168](https://github.com/denkoushi/RaspberryPiSystem_002/pull/168)。**前項** 2026-04-18（**管理コンソール 貸出レポート Gmail 即時送信・2ペインレイアウト（`POST /api/reports/loan-report/gmail-send`・OAuth **`gmail.send`**・`LoanReportEmailMime` 共通化・`LoanReportPage` 左右分割。プレビュー・下書き API は従来どおり）**・`feat/loan-report-gmail-send-and-layout`・コミット **`d97bdaa7`**）: **`raspberrypi5` のみ**（Pi4/Pi3 未デプロイ・**Pi3 は専用手順のため対象外**）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/loan-report-gmail-send-and-layout infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`・Detach **`20260418-183700-7508`**（**`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 42 / WARN 1 / FAIL 0**。**WARN**: Pi5 から `raspi4-fjv60-80` への SSH 実チェック不可（ベースライン同様・`deploy-status` は PASS）。**追加確認**: 未認証 `GET /api/reports/loan-report/preview?category=rigging` → **`401`**。`POST /api/reports/loan-report/gmail-send`（空 body）→ **`401`**。**`GET /api/system/health`**: デプロイ後 **数十秒経過でも memory 高（例: 95% 台）で `degraded` が続く場合あり**（観測）。**更長の warm-up** で `ok` に戻ることもあるが、**持続時は負荷・コンテナ再作成の有無を確認**。**デプロイ前**: 未追跡ファイル等で fail-fast → **`git stash push -u`**。**CI**: Run **`24601330617`** success。**ナレッジ**: [KB-354](../knowledge-base/KB-354-admin-loan-report-gmail-draft-deploy.md)。**トラブルシュート**: **送信**は **`ADMIN` のみ**。**`gmail.send` は OAuth 再認可が必要**な場合あり。**PR**: [#167](https://github.com/denkoushi/RaspberryPiSystem_002/pull/167)。**前項** 2026-04-18（**管理コンソール 貸出レポート HTML プレビュー・Gmail 下書き（`LoanReportPage`・`GET /api/reports/loan-report/preview`・`POST /api/reports/loan-report/gmail-draft`・OAuth `gmail.compose`・raw MIME base64 行折り返し）**・`feat/admin-loan-report-gmail-draft`・コミット **`6bc00a00`**）: **`raspberrypi5` のみ**（Pi4/Pi3 未デプロイ・**Pi3 は専用手順のため対象外**）・Detach **`20260418-152952-9706`**（**`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 42 / WARN 1 / FAIL 0**（約 **59s**）。**WARN**: Pi5 から `raspi4-fjv60-80` への SSH 実チェック不可（ベースライン同様・`deploy-status` は PASS）。**追加確認**: デプロイ直後 `GET /api/system/health` は **memory 高で `degraded`** になり得るが **約45s warm-up 後 `ok`**。未認証 `GET /api/reports/loan-report/preview?category=rigging` → **`401`**。**デプロイ前**: 未追跡ファイル等で fail-fast → **`git stash push -u`**。**CI**: Run **`24598528235`** success。**ナレッジ**: [KB-354](../knowledge-base/KB-354-admin-loan-report-gmail-draft-deploy.md)。**トラブルシュート**: Gmail 下書きは **`gmail.compose`** と有効な Gmail 接続が必要。**PR**: [#166](https://github.com/denkoushi/RaspberryPiSystem_002/pull/166)。**前項** 2026-04-18（**GitHub Actions CI 高速化・品質ゲート強化（ジョブ分割・`setup-pnpm-monorepo`・PostgreSQL 待機スクリプト・Playwright 並列・audit critical ゲート）**・`feat/ci-speed-quality-improvement`・コミット **`5f4ac44d`**）: **本番 Ansible デプロイは未実施**（変更は `.github/`・CI スクリプト・テスト設定中心）。**実機ベースライン**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 42 / WARN 1 / FAIL 0**（約 **57s**）。**WARN**: Pi5 から `raspi4-fjv60-80` への SSH 実チェック不可（`deploy-status` は PASS）。**CI**: Run **`24596837616`** success。**ナレッジ**: [KB-353](../knowledge-base/ci-cd.md#kb-353-github-actions-のジョブ分割と-composite-action-による-ci-高速化2026-04-18)・[ci-troubleshooting.md](./ci-troubleshooting.md)。**PR**: [#165](https://github.com/denkoushi/RaspberryPiSystem_002/pull/165)（merge **`eba4cb66`**）。**前項** 2026-04-18（**モバイルキオスク viewport（`100dvh`・`viewport-fit=cover`・`apps/web/src/constants/viewportLayout.ts`・`KioskLayout` の非沉浸式 `calc(100vh-6rem)` 廃止と `main` 縦スクロール統一）**・`feat/web-mobile-viewport-dvh`・コミット **`b8650b37`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**・Detach **`20260418-121702-8249` → `20260418-122625-10112` → `20260418-123103-17125` → `20260418-123447-15095` → `20260418-125634-18133`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 42 / WARN 1 / FAIL 0**（約 **59s**）。**WARN**: Pi5 から `raspi4-fjv60-80` への SSH 実チェック不可（`deploy-status` は PASS・スクリプト既定メッセージ。電源・Tailscale・LAN を現場確認）。**Pi3**: 対象外（Pi5 SPA 配信 + Pi4 キオスク。**Pi3 専用手順は今回未実施**）。**ナレッジ**: [KB-352](../knowledge-base/frontend.md#kb-352-モバイルキオスク-viewport-dvh-本番反映)。**トラブルシュート**: 多段デプロイは **`&&` 連結**または **前 `--detach --follow` 完了後**に次を起動（Mac ローカルロック exit 3）。バックグラウンドに残った `update-all-clients.sh` がロックする場合は **PID 終了確認後**に再試行。**PR**: [#164](https://github.com/denkoushi/RaspberryPiSystem_002/pull/164)。**前項** 2026-04-18（**配膳スマホ キオスク UI 高視認テーマ・静的プレビュー整合（`mobilePlacementKioskTheme`・Register/Verify 分割）**・`feature/mobile-placement-contrast-refactor`・コミット **`2d2528ec`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**・Detach **`20260418-111058-3228` → `20260418-111559-19394` → `20260418-112049-24881` → `20260418-112437-2591` → `20260418-113342-26169`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **47s**）。**Pi3**: 対象外（`/kiosk` は Pi5 SPA + Pi4 キオスク。Pi3 専用手順は未実施）。**ナレッジ**: [KB-339 §V22](../knowledge-base/KB-339-mobile-placement-barcode-survey.md)・[mobile-placement-smartphone.md](../runbooks/mobile-placement-smartphone.md) §0。**トラブルシュート**: 5台直列は **`&&` 連結**または前ジョブの Mac 終了を待ってから次を起動（ローカルロック exit 3 回避）。**PR**: [#163](https://github.com/denkoushi/RaspberryPiSystem_002/pull/163)。**前項** 2026-04-17（**キオスク集計 UI バランス調整（レイアウトシェル・パネル枠・TopN トグル・KPI 帯ブレークポイント）**・`feat/kiosk-analytics-ui-balance-refine`・コミット **`f5e58e2e`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**・Detach **`20260417-220620-25346` → `20260417-221119-9068` → `20260417-221607-22959` → `20260417-221948-32509` → `20260417-222428-19933`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **53s**）。**Pi3**: 対象外（Pi5 Web + Pi4 キオスク。Pi3 は専用手順のため未デプロイ）。**CI**: GitHub Actions Run **`24564671757`** success。**ナレッジ**: [KB-334](../knowledge-base/KB-334-kiosk-rigging-loan-analytics-deploy.md)。**トラブルシュート**: 5台直列は **`&&` 連結**で前ジョブ完了後に次を起動（ローカルロック exit 3 回避）。**PR**: [#161](https://github.com/denkoushi/RaspberryPiSystem_002/pull/161)。**前項** 2026-04-17（**キオスク集計 BI ダッシュボード再設計（固定 KPI ストリップ・Top N・当日イベント・ノンスクロール確認）**・`feat/kiosk-analytics-bi-dashboard`・コミット **`9eda66b4`**）: **`raspberrypi5` のみ**・Detach **`20260417-203348-20065`**（**`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **101s**）。**追加確認**: `GET /api/{rigging-gears,tools/items,measuring-instruments}/loan-analytics` が 200、Pi5 `GET /api/system/health` はデプロイ直後いったん **`degraded`**（memory 96.4%）だったが **warm-up 後 `ok`** に復帰。**画面確認**: local Playwright で **`https://100.106.158.2/kiosk/rigging-analytics`** を 1440x900 表示し、**`scrollHeight == clientHeight` / `scrollWidth == clientWidth`**、KPI ストリップ・4パネル・当日イベントを確認。**Pi3**: 対象外（本変更は Pi5 Web / API のみ）。**ナレッジ**: [KB-334](../knowledge-base/KB-334-kiosk-rigging-loan-analytics-deploy.md)。**トラブルシュート**: IDE 内蔵ブラウザが Tailscale 到達不能でも、Mac 側で到達できるなら **Playwright による本番 URL スクリーンショット**でレイアウト確認を代替できる。**さらに前** 2026-04-17（**キオスク順位ボード 資源カードレイアウト（`LeaderOrderResourceCard`・`presentLeaderOrderRow`・クラスタ行・完了ボタン白系・[プレビューHTML](../design-previews/kiosk-rank-board-card-single-preview.html) 整合）**・`feat/kiosk-leader-order-card-layout`・コミット **`d1a06409`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**・Detach **`20260417-191501-20151` → `20260417-192029-29544` → `20260417-192519-8157` → `20260417-192909-2265` → `20260417-193514-20516`**（各 **`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **75s**）。**Pi3**: 対象外（本変更は Pi5 Web + Pi4 キオスク。Pi3 は専用手順のため今回未デプロイ）。**ナレッジ**: [KB-297 §leader-order-card](../knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-resource-card-preview-alignment-2026-04-17)。**トラブルシュート**: Mac から `update-all-clients.sh` 実行時は **`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`**（本記録どおり）を忘れないと Pi5 への接続で失敗し得る。**PR**: [#159](https://github.com/denkoushi/RaspberryPiSystem_002/pull/159)。**前項** 2026-04-17（**生産日程 一覧API `resolvedMachineName` 共通化（順位ボードの `POST …/seiban-machine-names` 100件制限回避）**・`feat/production-schedule-machine-name-common-api`・コミット **`6ed72f83`**）: **`raspberrypi5` → `raspberrypi4`** を **1 台ずつ**・Detach **`20260417-175707-4538`** → **`20260417-180747-13902`**（各 **`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **61s**）。**Pi3**: 対象外。**ナレッジ**: [KB-350](../knowledge-base/KB-350-seiban-machine-name-supplement-fhinmei-mh-sh.md)。**PR**: [#158](https://github.com/denkoushi/RaspberryPiSystem_002/pull/158)（merge **`09bce17c`**）。**前項** 2026-04-17（**Gmail 製番補完: 手動 run 前に固定 `CsvDashboard` ensure（`CsvDashboardImportService.ensureFixedDashboardIfNeeded`）**・`debug/gmail-csv-manual-run-not-trashing`・コミット **`b1d6af9b`**）: **`raspberrypi5` のみ**・初回 Detach `20260417-160004-10564` / `20260417-160131-23680` は **Trivy APT リポジトリの GPG 鍵不整合**で **`server` ロールの `apt update` が `failed=1`**。**Pi5 上の復旧**: 既存 `/usr/share/keyrings/trivy.gpg` を削除し、`curl -fsSL https://aquasecurity.github.io/trivy-repo/deb/public.key | sudo gpg --batch --no-tty --dearmor -o /usr/share/keyrings/trivy.gpg` で再生成 → `sudo apt-get update` 成功を確認してから **同じ** `update-all-clients.sh` を再実行。**成功 Detach**: **`20260417-160328-2759`**（**`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **24s**）。**Pi3**: 対象外。**ナレッジ**: [KB-350](../knowledge-base/KB-350-seiban-machine-name-supplement-fhinmei-mh-sh.md)。**PR**: [#157](https://github.com/denkoushi/RaspberryPiSystem_002/pull/157)。**さらに前** 2026-04-17（**生産日程 製番→機種名補完の固定 Gmail スケジュール保証（既存 `backup.json` 追補）**・`feat/seiban-machine-name-supplement-schedule-ensure`・コミット **`cb88b67b`**）: **`raspberrypi5` のみ**・Detach `20260417-145842-8036`（**`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **31s**）。**内容**: 既存本番 `config/backup.json` に **`csv-import-seiban-machine-name-supplement`** が無いと管理画面一覧に出ないため、`ImportScheduleAdminService` / `CsvImportScheduler` が **固定スケジュールを自動補完して保存**するよう修正。**確認**: Pi5 `backup.json` と `GET /api/imports/schedule` の両方で `csv-import-seiban-machine-name-supplement`（cron `15 6 * * 0`・target `e2f3a4b5-c6d7-4e8f-9a0b-1c2d3e4f5a6b`・`enabled=false`）を確認。**ナレッジ**: [KB-350](../knowledge-base/KB-350-seiban-machine-name-supplement-fhinmei-mh-sh.md)。**前項** 2026-04-17（**生産日程 製番→機種名補完（Gmail `FHINMEI_MH_SH`・`ingestRunId` スコープ同期・`POST …/seiban-machine-names`）**・`feat/seiban-machine-name-supplement-gmail`・コミット **`c770cb9d`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**・Detach `20260417-135407-32471` → `20260417-140318-6669` → `20260417-140736-11875` → `20260417-141050-21814` → `20260417-141730-23351`（各 **`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **29s**）。**Pi3**: 対象外（本変更は Pi5 API + Pi4 キオスク Web）。**ナレッジ**: [KB-350](../knowledge-base/KB-350-seiban-machine-name-supplement-fhinmei-mh-sh.md）。**前項** 2026-04-17（**キオスク サイネージプレビュー対象端末選択（`signagePreviewTargetApiKey`・`/api/kiosk/signage-preview/*`）**・`feat/kiosk-signage-preview-target-selector`・コミット **`2b3871a8`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**・Detach `20260417-121016-12243` → `20260417-122130-4794` → `20260417-122551-22196` → `20260417-122916-30733` → `20260417-123315-12115`（各 **`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **26s**）。**Pi3**: 対象外（本変更は Pi5 API + Pi4 キオスク Web）。**内容**: `ClientDevice.signagePreviewTargetApiKey` とキオスク API で候補取得・保存、`KioskSignagePreviewModal` で `key` 付き `current-image`。**ナレッジ**: [KB-349](../knowledge-base/frontend.md#kb-349-キオスクサイネージプレビューで端末を選択して画像を取得する)・[modules/signage/README.md](../modules/signage/README.md)。**さらに前** 2026-04-17（**管理コンソール サイネージプレビュー 端末選択（`key=` 付き `current-image` 取得）**・`feat/admin-signage-preview-client-select`・コミット **`4dd1165a`**）: **`raspberrypi5` のみ**（Pi4/Pi3 未デプロイ）・Detach **`20260417-111616-21586`**（**`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **27s**）。**内容**: サイネージ用途端末のみセレクトし、`GET /signage/current-image` に **`key=<ClientDevice.apiKey>`** を付与。**ナレッジ**: [KB-348](../knowledge-base/frontend.md#kb-348-管理コンソールサイネージプレビューが端末別レンダ結果とずれるjwtのみでレガシーglobalキャッシュを参照)・[KB-192](../knowledge-base/frontend.md#kb-192-管理コンソールのサイネージプレビュー機能実装とjwt認証問題)。**さらに前** 2026-04-17（**サイネージ 前日スナップショット固定（加工機点検・計測機器持出）**・`feat/signage-snapshot-fix`・コミット **`e1025de2`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01` → `raspberrypi3`** を **1 台ずつ**・Detach `20260417-092831-28647` → `20260417-094134-12068` → `20260417-094603-4958` → `20260417-094927-30738` → `20260417-100845-9831` → `20260417-101235-32147`（各 **`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **29s**）。**内容**: 業務日ラベル（JST 9:00 切替）は維持しつつ、集計は **ラベル日の暦日 0:00〜24:00**、計測機器持出は **前日表示中は当日 0:00 JST までスナップショット固定**、点検結果 **「未使用」は異常にしない**。**ナレッジ**: [KB-347](../knowledge-base/api.md#kb-347-サイネージ可視化の業務日切替jst-翌900自動表示のみ) 追補。**さらに前** 2026-04-16（**FKOJUNST Gmail スケジュール自動補完の本番反映（API のみ・CI 回帰修正）**・`feat/fkojunst-gmail-schedule-guarantee`・コミット **`3a762893`**）: **`raspberrypi5` のみ**（Pi4/Pi3 未デプロイ）・Detach `20260416-222237-18706`（`raspberrypi5`・**`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **56s**）。**内容**: `ImportScheduleAdminService` が注入 `store` 経由で FKOJUNST 固定スケジュールを保証、`BackupConfigLoader.save` 未モック時のテスト互換。**運用**: `backup.json` 自動補完・固定 ID 削除不可は [KB-297 §FKOJUNST](../knowledge-base/KB-297-kiosk-due-management-workflow.md#fkojunst-status-from-gmail-csv-2026-04-16)。**さらに前** 2026-04-16（**生産日程 FKOJUNST（Gmail 工順ステータス）**（`ProductionScheduleFkojunstStatus`・一覧 `rowData.FKOJUNST`）・`feature/fkojunst-gmail-status-import`・コミット **`2ff4da8d`**）: **Pi5→Pi4×4 順次**（Pi3 除外）・Detach `20260416-200358-17426`（`raspberrypi5`）→ `20260416-201513-2763`（`raspberrypi4`）→ `20260416-202019-27635`（`raspi4-robodrill01`）→ `20260416-202436-9992`（`raspi4-fjv60-80`）→ `20260416-202952-27613`（`raspi4-kensaku-stonebase01`）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **89s**）。**Pi3**: 対象外。**運用（初回 FKOJUNST 取り込み時）**: Gmail `targets` に **`csvDashboards` + `9e4f2c1a-8b7d-4e6f-a5c4-1d2e3f4a5b6c`**（件名 `FKOJUNST`）。**ナレッジ**: [KB-297 §FKOJUNST](../knowledge-base/KB-297-kiosk-due-management-workflow.md#fkojunst-status-from-gmail-csv-2026-04-16)。**トラブルシュート**: 同一 Mac から `update-all-clients.sh` を多重起動しない（exit 3・[deploy-status-recovery.md](../runbooks/deploy-status-recovery.md)）。**さらに前** 2026-04-16（**サイネージ可視化の業務日切替 JST 9:00**（加工機点検の日付未指定・計測機器持出 `today_jst` 等）・`feat/signage-business-day-cutover-9am`・コミット **`08d32806`**）: **Pi5→Pi4×4 順次**（Pi3 除外）・Detach `20260416-184654-21455`（`raspberrypi5`）→ `20260416-185919-27958`（`raspberrypi4`）→ `20260416-190415-11118`（`raspi4-robodrill01`）→ `20260416-190813-13486`（`raspi4-fjv60-80`）→ `20260416-191625-26120`（`raspi4-kensaku-stonebase01`）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **59s**）。**Pi3**: 対象外（本変更は Pi5 API の集計境界）。**トラブルシュート**: **直列でも**前ホストの **`--detach --follow` が Mac 終了する前**に次を起動すると **ローカルロック**で `Another update-all-clients.sh process is already running`（exit 3）→ **前ジョブ完了を待つ**か **`cmd1 && cmd2`**。仕様は [KB-347](../knowledge-base/api.md#kb-347-サイネージ可視化の業務日切替jst-翌900自動表示のみ)。**さらに前** 2026-04-15（**キオスク「集計」4パネルダッシュボード・当日イベント・loan-analytics `periodEvents`・Web イメージ Alpine セキュリティ更新**（`feat/kiosk-analytics-four-panel-today-events`・コミット **`323dd9f0`**）: **Pi5→Pi4×4 順次**（Pi3 除外）・Detach `20260415-162422-11542`（`raspberrypi5`）→ `20260415-163600-7918`（`raspberrypi4`）→ `20260415-164041-17295`（`raspi4-robodrill01`）→ `20260415-164408-5423`（`raspi4-fjv60-80`）→ `20260415-164824-29880`（`raspi4-kensaku-stonebase01`）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **27s**）。**Pi3**: 対象外。**トラブルシュート**: 未コミット／未追跡があると `update-all-clients.sh` が fail-fast → **`git stash push -u`**。仕様・知見は [KB-334](../knowledge-base/KB-334-kiosk-rigging-loan-analytics-deploy.md)「2026-04-15」節。**さらに前** 2026-04-14（**キオスク「集計」月選択・タブ別資産フィルタ**（`feat/kiosk-analytics-month-and-asset-filters`・コミット **`8ce1a9da`**）: **Pi5→Pi4×4 順次**（Pi3 除外）・Detach `20260414-211347-29532`（`raspberrypi5`）→ `20260414-212701-15420`（`raspberrypi4`）→ `20260414-213153-7546`（`raspi4-robodrill01`）→ `20260414-213547-9533`（`raspi4-fjv60-80`）→ `20260414-214120-20816`（`raspi4-kensaku-stonebase01`）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **57s**）。**Pi3**: 対象外（キオスク/API 変更のため Pi3 専用手順は未実施）。仕様・知見は [KB-334](../knowledge-base/KB-334-kiosk-rigging-loan-analytics-deploy.md)「2026-04-14」節。**前項** **キオスク統合ローン分析（計測機器 CSV+NFC）**（`feat/unified-loan-analytics`・コミット **`35f5ed4b`**）: **Pi5→Pi4×4 順次**（Pi3 除外）・Detach `20260414-194212-7916`（`raspberrypi5`）→ `20260414-195311-19926`（`raspberrypi4`）→ `20260414-195803-9001`（`raspi4-robodrill01`）→ `20260414-200152-17153`（`raspi4-fjv60-80`）→ `20260414-200700-5820`（`raspi4-kensaku-stonebase01`）・Phase12 **43/0/0**（約 **63s**）。**トラブルシュート**: fail-fast（未 push / 汚いツリー）は [KB-200](../knowledge-base/infrastructure/ansible-deployment.md#kb-200-デプロイ標準手順のfail-fastチェック追加とデタッチ実行ログ追尾機能)。仕様は [KB-344](../knowledge-base/KB-344-unified-loan-analytics.md) / [ui.md](../modules/measuring-instruments/ui.md)。**さらに前** キオスク計測機器持出レイアウト align（`702f7b83`）: Detach `20260414-180107-30581` → …・Phase12 約 **53s**。**さらに前** 計測機器ジャンル画像永続化: **Pi5 のみ**・[KB-343](../knowledge-base/infrastructure/ansible-deployment.md#kb-343-measuring-instrument-genre-image-persistence)。）**前項** 2026-04-18（**管理コンソール 貸出レポート 実メトリクス統合・プレビュー幅拡大（各 `loan-analytics` の従業員行に期限超過件数・`LoanReport` 評価から推定配分を排除して借用実データへ・`LoanReportPage` でコンテンツ最大幅をビューポートまで拡張）**・`fix/loan-report-real-metrics-wide-preview`・コミット **`937be20f`**）: **`raspberrypi5` のみ**（Pi4/Pi3 未デプロイ・**Pi3 は専用手順のため対象外**）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh fix/loan-report-real-metrics-wide-preview infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`・Detach **`20260418-204637-27968`**（**`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 42 / WARN 1 / FAIL 0**（約 **62s**）。**WARN**: Pi5 から `raspi4-fjv60-80` への SSH 実チェック不可（ベースライン同様・`deploy-status` は PASS）。**デプロイ（Mac）前**: **未追跡ファイル**（例: `apps/web/public/*preview*.html`）は **`git stash push -u`** で退避しないと fail-fast。**CI**: Run **`24603756892`** success。**ナレッジ**: [KB-354](../knowledge-base/KB-354-admin-loan-report-gmail-draft-deploy.md) §C。**PR**: [#168](https://github.com/denkoushi/RaspberryPiSystem_002/pull/168)。**前項** 2026-04-18（**管理コンソール 貸出レポート Gmail 即時送信・2ペインレイアウト（`POST /api/reports/loan-report/gmail-send`・OAuth **`gmail.send`**・`LoanReportEmailMime` 共通化・`LoanReportPage` 左右分割。プレビュー・下書き API は従来どおり）**・`feat/loan-report-gmail-send-and-layout`・コミット **`d97bdaa7`**）: **`raspberrypi5` のみ**（Pi4/Pi3 未デプロイ・**Pi3 は専用手順のため対象外**）・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/loan-report-gmail-send-and-layout infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`・Detach **`20260418-183700-7508`**（**`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 42 / WARN 1 / FAIL 0**。**WARN**: Pi5 から `raspi4-fjv60-80` への SSH 実チェック不可（ベースライン同様・`deploy-status` は PASS）。**追加確認**: 未認証 `GET /api/reports/loan-report/preview?category=rigging` → **`401`**。`POST /api/reports/loan-report/gmail-send`（空 body）→ **`401`**。**`GET /api/system/health`**: デプロイ後 **数十秒経過でも memory 高（例: 95% 台）で `degraded` が続く場合あり**（観測）。**更長の warm-up** で `ok` に戻ることもあるが、**持続時は負荷・コンテナ再作成の有無を確認**。**デプロイ前**: 未追跡ファイル等で fail-fast → **`git stash push -u`**。**CI**: Run **`24601330617`** success。**ナレッジ**: [KB-354](../knowledge-base/KB-354-admin-loan-report-gmail-draft-deploy.md)。**トラブルシュート**: **送信**は **`ADMIN` のみ**。**`gmail.send` は OAuth 再認可が必要**な場合あり。**PR**: [#167](https://github.com/denkoushi/RaspberryPiSystem_002/pull/167)。**前項** 2026-04-18（**管理コンソール 貸出レポート HTML プレビュー・Gmail 下書き（`LoanReportPage`・`GET /api/reports/loan-report/preview`・`POST /api/reports/loan-report/gmail-draft`・OAuth `gmail.compose`・raw MIME base64 行折り返し）**・`feat/admin-loan-report-gmail-draft`・コミット **`6bc00a00`**）: **`raspberrypi5` のみ**（Pi4/Pi3 未デプロイ・**Pi3 は専用手順のため対象外**）・Detach **`20260418-152952-9706`**（**`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 42 / WARN 1 / FAIL 0**（約 **59s**）。**WARN**: Pi5 から `raspi4-fjv60-80` への SSH 実チェック不可（ベースライン同様・`deploy-status` は PASS）。**追加確認**: デプロイ直後 `GET /api/system/health` は **memory 高で `degraded`** になり得るが **約45s warm-up 後 `ok`**。未認証 `GET /api/reports/loan-report/preview?category=rigging` → **`401`**。**デプロイ前**: 未追跡ファイル等で fail-fast → **`git stash push -u`**。**CI**: Run **`24598528235`** success。**ナレッジ**: [KB-354](../knowledge-base/KB-354-admin-loan-report-gmail-draft-deploy.md)。**トラブルシュート**: Gmail 下書きは **`gmail.compose`** と有効な Gmail 接続が必要。**PR**: [#166](https://github.com/denkoushi/RaspberryPiSystem_002/pull/166)。**前項** 2026-04-18（**GitHub Actions CI 高速化・品質ゲート強化（ジョブ分割・`setup-pnpm-monorepo`・PostgreSQL 待機スクリプト・Playwright 並列・audit critical ゲート）**・`feat/ci-speed-quality-improvement`・コミット **`5f4ac44d`**）: **本番 Ansible デプロイは未実施**（変更は `.github/`・CI スクリプト・テスト設定中心）。**実機ベースライン**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 42 / WARN 1 / FAIL 0**（約 **57s**）。**WARN**: Pi5 から `raspi4-fjv60-80` への SSH 実チェック不可（`deploy-status` は PASS）。**CI**: Run **`24596837616`** success。**ナレッジ**: [KB-353](../knowledge-base/ci-cd.md#kb-353-github-actions-のジョブ分割と-composite-action-による-ci-高速化2026-04-18)・[ci-troubleshooting.md](./ci-troubleshooting.md)。**PR**: [#165](https://github.com/denkoushi/RaspberryPiSystem_002/pull/165)（merge **`eba4cb66`**）。**前項** 2026-04-18（**モバイルキオスク viewport（`100dvh`・`viewport-fit=cover`・`apps/web/src/constants/viewportLayout.ts`・`KioskLayout` の非沉浸式 `calc(100vh-6rem)` 廃止と `main` 縦スクロール統一）**・`feat/web-mobile-viewport-dvh`・コミット **`b8650b37`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**・Detach **`20260418-121702-8249` → `20260418-122625-10112` → `20260418-123103-17125` → `20260418-123447-15095` → `20260418-125634-18133`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 42 / WARN 1 / FAIL 0**（約 **59s**）。**WARN**: Pi5 から `raspi4-fjv60-80` への SSH 実チェック不可（`deploy-status` は PASS・スクリプト既定メッセージ。電源・Tailscale・LAN を現場確認）。**Pi3**: 対象外（Pi5 SPA 配信 + Pi4 キオスク。**Pi3 専用手順は今回未実施**）。**ナレッジ**: [KB-352](../knowledge-base/frontend.md#kb-352-モバイルキオスク-viewport-dvh-本番反映)。**トラブルシュート**: 多段デプロイは **`&&` 連結**または **前 `--detach --follow` 完了後**に次を起動（Mac ローカルロック exit 3）。バックグラウンドに残った `update-all-clients.sh` がロックする場合は **PID 終了確認後**に再試行。**PR**: [#164](https://github.com/denkoushi/RaspberryPiSystem_002/pull/164)。**前項** 2026-04-18（**配膳スマホ キオスク UI 高視認テーマ・静的プレビュー整合（`mobilePlacementKioskTheme`・Register/Verify 分割）**・`feature/mobile-placement-contrast-refactor`・コミット **`2d2528ec`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**・Detach **`20260418-111058-3228` → `20260418-111559-19394` → `20260418-112049-24881` → `20260418-112437-2591` → `20260418-113342-26169`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **47s**）。**Pi3**: 対象外（`/kiosk` は Pi5 SPA + Pi4 キオスク。Pi3 専用手順は未実施）。**ナレッジ**: [KB-339 §V22](../knowledge-base/KB-339-mobile-placement-barcode-survey.md)・[mobile-placement-smartphone.md](../runbooks/mobile-placement-smartphone.md) §0。**トラブルシュート**: 5台直列は **`&&` 連結**または前ジョブの Mac 終了を待ってから次を起動（ローカルロック exit 3 回避）。**PR**: [#163](https://github.com/denkoushi/RaspberryPiSystem_002/pull/163)。**前項** 2026-04-17（**キオスク集計 UI バランス調整（レイアウトシェル・パネル枠・TopN トグル・KPI 帯ブレークポイント）**・`feat/kiosk-analytics-ui-balance-refine`・コミット **`f5e58e2e`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**・Detach **`20260417-220620-25346` → `20260417-221119-9068` → `20260417-221607-22959` → `20260417-221948-32509` → `20260417-222428-19933`**（各 **`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **53s**）。**Pi3**: 対象外（Pi5 Web + Pi4 キオスク。Pi3 は専用手順のため未デプロイ）。**CI**: GitHub Actions Run **`24564671757`** success。**ナレッジ**: [KB-334](../knowledge-base/KB-334-kiosk-rigging-loan-analytics-deploy.md)。**トラブルシュート**: 5台直列は **`&&` 連結**で前ジョブ完了後に次を起動（ローカルロック exit 3 回避）。**PR**: [#161](https://github.com/denkoushi/RaspberryPiSystem_002/pull/161)。**前項** 2026-04-17（**キオスク集計 BI ダッシュボード再設計（固定 KPI ストリップ・Top N・当日イベント・ノンスクロール確認）**・`feat/kiosk-analytics-bi-dashboard`・コミット **`9eda66b4`**）: **`raspberrypi5` のみ**・Detach **`20260417-203348-20065`**（**`failed=0` / `unreachable=0` / exit `0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **101s**）。**追加確認**: `GET /api/{rigging-gears,tools/items,measuring-instruments}/loan-analytics` が 200、Pi5 `GET /api/system/health` はデプロイ直後いったん **`degraded`**（memory 96.4%）だったが **warm-up 後 `ok`** に復帰。**画面確認**: local Playwright で **`https://100.106.158.2/kiosk/rigging-analytics`** を 1440x900 表示し、**`scrollHeight == clientHeight` / `scrollWidth == clientWidth`**、KPI ストリップ・4パネル・当日イベントを確認。**Pi3**: 対象外（本変更は Pi5 Web / API のみ）。**ナレッジ**: [KB-334](../knowledge-base/KB-334-kiosk-rigging-loan-analytics-deploy.md)。**トラブルシュート**: IDE 内蔵ブラウザが Tailscale 到達不能でも、Mac 側で到達できるなら **Playwright による本番 URL スクリーンショット**でレイアウト確認を代替できる。**さらに前** 2026-04-17（**キオスク順位ボード 資源カードレイアウト（`LeaderOrderResourceCard`・`presentLeaderOrderRow`・クラスタ行・完了ボタン白系・[プレビューHTML](../design-previews/kiosk-rank-board-card-single-preview.html) 整合）**・`feat/kiosk-leader-order-card-layout`・コミット **`d1a06409`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**・Detach **`20260417-191501-20151` → `20260417-192029-29544` → `20260417-192519-8157` → `20260417-192909-2265` → `20260417-193514-20516`**（各 **`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **75s**）。**Pi3**: 対象外（本変更は Pi5 Web + Pi4 キオスク。Pi3 は専用手順のため今回未デプロイ）。**ナレッジ**: [KB-297 §leader-order-card](../knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-resource-card-preview-alignment-2026-04-17)。**トラブルシュート**: Mac から `update-all-clients.sh` 実行時は **`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`**（本記録どおり）を忘れないと Pi5 への接続で失敗し得る。**PR**: [#159](https://github.com/denkoushi/RaspberryPiSystem_002/pull/159)。**前項** 2026-04-17（**生産日程 一覧API `resolvedMachineName` 共通化（順位ボードの `POST …/seiban-machine-names` 100件制限回避）**・`feat/production-schedule-machine-name-common-api`・コミット **`6ed72f83`**）: **`raspberrypi5` → `raspberrypi4`** を **1 台ずつ**・Detach **`20260417-175707-4538`** → **`20260417-180747-13902`**（各 **`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **61s**）。**Pi3**: 対象外。**ナレッジ**: [KB-350](../knowledge-base/KB-350-seiban-machine-name-supplement-fhinmei-mh-sh.md)。**PR**: [#158](https://github.com/denkoushi/RaspberryPiSystem_002/pull/158)（merge **`09bce17c`**）。**前項** 2026-04-17（**Gmail 製番補完: 手動 run 前に固定 `CsvDashboard` ensure（`CsvDashboardImportService.ensureFixedDashboardIfNeeded`）**・`debug/gmail-csv-manual-run-not-trashing`・コミット **`b1d6af9b`**）: **`raspberrypi5` のみ**・初回 Detach `20260417-160004-10564` / `20260417-160131-23680` は **Trivy APT リポジトリの GPG 鍵不整合**で **`server` ロールの `apt update` が `failed=1`**。**Pi5 上の復旧**: 既存 `/usr/share/keyrings/trivy.gpg` を削除し、`curl -fsSL https://aquasecurity.github.io/trivy-repo/deb/public.key | sudo gpg --batch --no-tty --dearmor -o /usr/share/keyrings/trivy.gpg` で再生成 → `sudo apt-get update` 成功を確認してから **同じ** `update-all-clients.sh` を再実行。**成功 Detach**: **`20260417-160328-2759`**（**`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **24s**）。**Pi3**: 対象外。**ナレッジ**: [KB-350](../knowledge-base/KB-350-seiban-machine-name-supplement-fhinmei-mh-sh.md)。**PR**: [#157](https://github.com/denkoushi/RaspberryPiSystem_002/pull/157)。**さらに前** 2026-04-17（**生産日程 製番→機種名補完の固定 Gmail スケジュール保証（既存 `backup.json` 追補）**・`feat/seiban-machine-name-supplement-schedule-ensure`・コミット **`cb88b67b`**）: **`raspberrypi5` のみ**・Detach `20260417-145842-8036`（**`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **31s**）。**内容**: 既存本番 `config/backup.json` に **`csv-import-seiban-machine-name-supplement`** が無いと管理画面一覧に出ないため、`ImportScheduleAdminService` / `CsvImportScheduler` が **固定スケジュールを自動補完して保存**するよう修正。**確認**: Pi5 `backup.json` と `GET /api/imports/schedule` の両方で `csv-import-seiban-machine-name-supplement`（cron `15 6 * * 0`・target `e2f3a4b5-c6d7-4e8f-9a0b-1c2d3e4f5a6b`・`enabled=false`）を確認。**ナレッジ**: [KB-350](../knowledge-base/KB-350-seiban-machine-name-supplement-fhinmei-mh-sh.md)。**前項** 2026-04-17（**生産日程 製番→機種名補完（Gmail `FHINMEI_MH_SH`・`ingestRunId` スコープ同期・`POST …/seiban-machine-names`）**・`feat/seiban-machine-name-supplement-gmail`・コミット **`c770cb9d`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**・Detach `20260417-135407-32471` → `20260417-140318-6669` → `20260417-140736-11875` → `20260417-141050-21814` → `20260417-141730-23351`（各 **`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **29s**）。**Pi3**: 対象外（本変更は Pi5 API + Pi4 キオスク Web）。**ナレッジ**: [KB-350](../knowledge-base/KB-350-seiban-machine-name-supplement-fhinmei-mh-sh.md）。**前項** 2026-04-17（**キオスク サイネージプレビュー対象端末選択（`signagePreviewTargetApiKey`・`/api/kiosk/signage-preview/*`）**・`feat/kiosk-signage-preview-target-selector`・コミット **`2b3871a8`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **1 台ずつ**・Detach `20260417-121016-12243` → `20260417-122130-4794` → `20260417-122551-22196` → `20260417-122916-30733` → `20260417-123315-12115`（各 **`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **26s**）。**Pi3**: 対象外（本変更は Pi5 API + Pi4 キオスク Web）。**内容**: `ClientDevice.signagePreviewTargetApiKey` とキオスク API で候補取得・保存、`KioskSignagePreviewModal` で `key` 付き `current-image`。**ナレッジ**: [KB-349](../knowledge-base/frontend.md#kb-349-キオスクサイネージプレビューで端末を選択して画像を取得する)・[modules/signage/README.md](../modules/signage/README.md)。**さらに前** 2026-04-17（**管理コンソール サイネージプレビュー 端末選択（`key=` 付き `current-image` 取得）**・`feat/admin-signage-preview-client-select`・コミット **`4dd1165a`**）: **`raspberrypi5` のみ**（Pi4/Pi3 未デプロイ）・Detach **`20260417-111616-21586`**（**`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **27s**）。**内容**: サイネージ用途端末のみセレクトし、`GET /signage/current-image` に **`key=<ClientDevice.apiKey>`** を付与。**ナレッジ**: [KB-348](../knowledge-base/frontend.md#kb-348-管理コンソールサイネージプレビューが端末別レンダ結果とずれるjwtのみでレガシーglobalキャッシュを参照)・[KB-192](../knowledge-base/frontend.md#kb-192-管理コンソールのサイネージプレビュー機能実装とjwt認証問題)。**さらに前** 2026-04-17（**サイネージ 前日スナップショット固定（加工機点検・計測機器持出）**・`feat/signage-snapshot-fix`・コミット **`e1025de2`**）: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01` → `raspberrypi3`** を **1 台ずつ**・Detach `20260417-092831-28647` → `20260417-094134-12068` → `20260417-094603-4958` → `20260417-094927-30738` → `20260417-100845-9831` → `20260417-101235-32147`（各 **`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **29s**）。**内容**: 業務日ラベル（JST 9:00 切替）は維持しつつ、集計は **ラベル日の暦日 0:00〜24:00**、計測機器持出は **前日表示中は当日 0:00 JST までスナップショット固定**、点検結果 **「未使用」は異常にしない**。**ナレッジ**: [KB-347](../knowledge-base/api.md#kb-347-サイネージ可視化の業務日切替jst-翌900自動表示のみ) 追補。**さらに前** 2026-04-16（**FKOJUNST Gmail スケジュール自動補完の本番反映（API のみ・CI 回帰修正）**・`feat/fkojunst-gmail-schedule-guarantee`・コミット **`3a762893`**）: **`raspberrypi5` のみ**（Pi4/Pi3 未デプロイ）・Detach `20260416-222237-18706`（`raspberrypi5`・**`failed=0` / `unreachable=0`**）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **56s**）。**内容**: `ImportScheduleAdminService` が注入 `store` 経由で FKOJUNST 固定スケジュールを保証、`BackupConfigLoader.save` 未モック時のテスト互換。**運用**: `backup.json` 自動補完・固定 ID 削除不可は [KB-297 §FKOJUNST](../knowledge-base/KB-297-kiosk-due-management-workflow.md#fkojunst-status-from-gmail-csv-2026-04-16)。**さらに前** 2026-04-16（**生産日程 FKOJUNST（Gmail 工順ステータス）**（`ProductionScheduleFkojunstStatus`・一覧 `rowData.FKOJUNST`）・`feature/fkojunst-gmail-status-import`・コミット **`2ff4da8d`**）: **Pi5→Pi4×4 順次**（Pi3 除外）・Detach `20260416-200358-17426`（`raspberrypi5`）→ `20260416-201513-2763`（`raspberrypi4`）→ `20260416-202019-27635`（`raspi4-robodrill01`）→ `20260416-202436-9992`（`raspi4-fjv60-80`）→ `20260416-202952-27613`（`raspi4-kensaku-stonebase01`）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **89s**）。**Pi3**: 対象外。**運用（初回 FKOJUNST 取り込み時）**: Gmail `targets` に **`csvDashboards` + `9e4f2c1a-8b7d-4e6f-a5c4-1d2e3f4a5b6c`**（件名 `FKOJUNST`）。**ナレッジ**: [KB-297 §FKOJUNST](../knowledge-base/KB-297-kiosk-due-management-workflow.md#fkojunst-status-from-gmail-csv-2026-04-16)。**トラブルシュート**: 同一 Mac から `update-all-clients.sh` を多重起動しない（exit 3・[deploy-status-recovery.md](../runbooks/deploy-status-recovery.md)）。**さらに前** 2026-04-16（**サイネージ可視化の業務日切替 JST 9:00**（加工機点検の日付未指定・計測機器持出 `today_jst` 等）・`feat/signage-business-day-cutover-9am`・コミット **`08d32806`**）: **Pi5→Pi4×4 順次**（Pi3 除外）・Detach `20260416-184654-21455`（`raspberrypi5`）→ `20260416-185919-27958`（`raspberrypi4`）→ `20260416-190415-11118`（`raspi4-robodrill01`）→ `20260416-190813-13486`（`raspi4-fjv60-80`）→ `20260416-191625-26120`（`raspi4-kensaku-stonebase01`）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **59s**）。**Pi3**: 対象外（本変更は Pi5 API の集計境界）。**トラブルシュート**: **直列でも**前ホストの **`--detach --follow` が Mac 終了する前**に次を起動すると **ローカルロック**で `Another update-all-clients.sh process is already running`（exit 3）→ **前ジョブ完了を待つ**か **`cmd1 && cmd2`**。仕様は [KB-347](../knowledge-base/api.md#kb-347-サイネージ可視化の業務日切替jst-翌900自動表示のみ)。**さらに前** 2026-04-15（**キオスク「集計」4パネルダッシュボード・当日イベント・loan-analytics `periodEvents`・Web イメージ Alpine セキュリティ更新**（`feat/kiosk-analytics-four-panel-today-events`・コミット **`323dd9f0`**）: **Pi5→Pi4×4 順次**（Pi3 除外）・Detach `20260415-162422-11542`（`raspberrypi5`）→ `20260415-163600-7918`（`raspberrypi4`）→ `20260415-164041-17295`（`raspi4-robodrill01`）→ `20260415-164408-5423`（`raspi4-fjv60-80`）→ `20260415-164824-29880`（`raspi4-kensaku-stonebase01`）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **27s**）。**Pi3**: 対象外。**トラブルシュート**: 未コミット／未追跡があると `update-all-clients.sh` が fail-fast → **`git stash push -u`**。仕様・知見は [KB-334](../knowledge-base/KB-334-kiosk-rigging-loan-analytics-deploy.md)「2026-04-15」節。**さらに前** 2026-04-14（**キオスク「集計」月選択・タブ別資産フィルタ**（`feat/kiosk-analytics-month-and-asset-filters`・コミット **`8ce1a9da`**）: **Pi5→Pi4×4 順次**（Pi3 除外）・Detach `20260414-211347-29532`（`raspberrypi5`）→ `20260414-212701-15420`（`raspberrypi4`）→ `20260414-213153-7546`（`raspi4-robodrill01`）→ `20260414-213547-9533`（`raspi4-fjv60-80`）→ `20260414-214120-20816`（`raspi4-kensaku-stonebase01`）・Phase12 **`./scripts/deploy/verify-phase12-real.sh` → PASS 43/0/0**（約 **57s**）。**Pi3**: 対象外（キオスク/API 変更のため Pi3 専用手順は未実施）。仕様・知見は [KB-334](../knowledge-base/KB-334-kiosk-rigging-loan-analytics-deploy.md)「2026-04-14」節。**前項** **キオスク統合ローン分析（計測機器 CSV+NFC）**（`feat/unified-loan-analytics`・コミット **`35f5ed4b`**）: **Pi5→Pi4×4 順次**（Pi3 除外）・Detach `20260414-194212-7916`（`raspberrypi5`）→ `20260414-195311-19926`（`raspberrypi4`）→ `20260414-195803-9001`（`raspi4-robodrill01`）→ `20260414-200152-17153`（`raspi4-fjv60-80`）→ `20260414-200700-5820`（`raspi4-kensaku-stonebase01`）・Phase12 **43/0/0**（約 **63s**）。**トラブルシュート**: fail-fast（未 push / 汚いツリー）は [KB-200](../knowledge-base/infrastructure/ansible-deployment.md#kb-200-デプロイ標準手順のfail-fastチェック追加とデタッチ実行ログ追尾機能)。仕様は [KB-344](../knowledge-base/KB-344-unified-loan-analytics.md) / [ui.md](../modules/measuring-instruments/ui.md)。**さらに前** キオスク計測機器持出レイアウト align（`702f7b83`）: Detach `20260414-180107-30581` → …・Phase12 約 **53s**。**さらに前** 計測機器ジャンル画像永続化: **Pi5 のみ**・[KB-343](../knowledge-base/infrastructure/ansible-deployment.md#kb-343-measuring-instrument-genre-image-persistence)。）
