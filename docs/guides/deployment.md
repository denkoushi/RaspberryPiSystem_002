...
- 2026-07-12にPi4全5台とPi3全1台のローリング本番受入れ、およびPi5 Blue/Greenの候補作成・切替・5分監視・cleanupを成功完了した。2026-07-13にはcleanup handoff復旧、StoneBase01カナリアの明示承認、全端末更新を標準経路で再確認した。通常更新はこの経路を使う。実装と実績の正本は [rolling terminal deployment plan](../plans/rolling-terminal-bluegreen-deploy.md) と [Pi5 Blue/Green deployment runbook](../runbooks/pi5-blue-green-deploy.md)、障害・復旧の正本は [KB-400](../knowledge-base/KB-400-pi5-bluegreen-cleanup-monitor-lock.md)。設計判断は [ADR-20260712-rolling-terminal-release-orchestration](../decisions/ADR-20260712-rolling-terminal-release-orchestration.md) と [ADR-20260712-deploy-target-minimization-canary-hold](../decisions/ADR-20260712-deploy-target-minimization-canary-hold.md)。
...
- 2段階導入では、初回の互換性リリースを先に全Pi4へ配備する。旧ブラウザは `notice` 状態を描画・ACKできないため、このリリースではコーディネータの通知送信を無効にする。全5台の `kiosk-browser.service` 再起動・通常画面復帰・release success を確認した後の有効化リリースで、通常更新の通知を必須化する。
...
- Phase 2はPhase 3を利用できない緊急時のフォールバックとして保持する。Phase 3 stateがliveのPi5では、通常のPhase 2 api/web更新はfail-closedである。
...
### Pi5 Blue/Greenデプロイ（Phase 3） {#pi5-blue-green-phase3}
