...
### 補足（2026-05-08 · **FKOJUNST メール同期・winner 行解決の複合3キー `IN` チャンク（PostgreSQL 32767）**·**API のみ**·**Pi5 のみ**） {#fkojunst-mail-winner-triple-tuple-in-chunk-2026-05-08}

- **変更概要**: [`fkojunst-status-mail-sync.pipeline.ts`](../../apps/api/src/services/production-schedule/fkojunst-status-mail-sync.pipeline.ts) の **メール行→幕 winner `CsvDashboardRow.id` 解決**が、**3 本の巨大 `IN (...)` と `$queryRaw`** に載る形だったため、候補数が **約 1 万件超級**などで **バインド変数総数が PostgreSQL の prepared statement 上限（典型 32767）**を超え、**`too many bind variables` / `received 32768` 級**で失敗し得た。**対策**: [`postgres-prepared-statement-bind-limit.ts`](../../apps/api/src/lib/postgres-prepared-statement-bind-limit.ts) の **`maxTuplePlaceholdersPerQuery`** で **タプル `IN` の安全チャンク幅**を求め、[`fkojunst-mail-winner-by-triple.reader.ts`](../../apps/api/src/services/production-schedule/fkojunst-mail-winner-by-triple.reader.ts) の **`findFkojunstMailWinnerIdsByMailTriples`** が **`(fkojun, fkoteicd, fsezono)` を複数回 `$queryRaw`** して **`Map` マージ**。論理キーは [`fkojunst-mail-status-key.ts`](../../apps/api/src/services/production-schedule/fkojunst-mail-status-key.ts)。**新規マイグレーションなし**（[KB-371](../knowledge-base/KB-371-csv-dashboard-dedup-postgres-bind-limit.md) の `dataHash` チャンクと同系統）。
- **対象ホスト**: **`raspberrypi5` のみ**（`--limit raspberrypi5`）。Pi4／Pi3 play は **no hosts matched**（**Pi3 専用手順は不要**）。
- **標準コマンド**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh fix/fkojunst-mail-winner-triple-chunk infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`（**`main` 取り込み後はブランチ引数を `main`**）。
- **本番デプロイ（先行反映・実績）**: **Detach Run ID**（接頭辞 `ansible-update-`）: **`20260508-211407-25543`**（**`PLAY RECAP` `ok=134` `changed=4` `failed=0` / `unreachable=0`**・リモート **`exit` `0`**・ローカル **`--follow` 約 1063s（約 17.7 分）**）。**`Run prisma migrate deploy` / `prisma migrate status`**: **成功**。
- **実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（本記録 **約 167s**・Tailscale・Pi5 API `100.106.158.2`）。
- **トラブルシュート**:
