...
- **対象ホスト（実績・ユーザー指定）**: **`raspberrypi5`** → **`raspi4-kensaku-stonebase01`**（各 **`--limit` で 1 台ずつ**）。**未反映**: `raspberrypi4` / `raspi4-robodrill01` / `raspi4-fjv60-80`。
...
- **推奨残デプロイ**: **`raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80`**（本改訂の **120s-only put / 操作ロック** を未反映 3 台へ·1 台ずつ）。
- **トラブルシュート**:
