...
3. **ディスク使用量**: 段階的な閾値で監視（10年運用対応）
   - **警告（WARNING）**: 70%を超えている場合
   - **アラート（ALERT）**: 80%を超えている場合 → `storage-usage-high`アラートを生成
   - **クリティカル（ERROR）**: 90%を超えている場合 → `storage-usage-high`アラートを生成
...
### ディスク使用量の段階的監視（10年運用対応）

`monitor.sh`はディスク使用量を段階的に監視し、閾値を超えた場合にファイルベースアラートを生成します：

- **70%超**: 警告ログを出力（アラートは生成しない）
- **80%超**: `storage-usage-high`アラートを生成（管理コンソールに表示）
- **90%超**: `storage-usage-high`アラートを生成（管理コンソールに表示）、エラーとして終了

### アラートの確認方法

生成されたアラートは`/opt/RaspberryPiSystem_002/alerts/alert-*.json`に保存され、管理コンソールのダッシュボード（`/admin`）で確認できます。

**アラートの種類**:
- `storage-usage-high`: ディスク使用量が80%または90%を超えた場合
- `storage-maintenance-failed`: ストレージメンテナンスが失敗した場合

詳細は [ローカル環境対応の通知機能ガイド](./local-alerts.md) を参照してください。
