...
3. 同意画面の設定（初回の場合）:
   ...
   - 「保存して次へ」をクリック
   - **スコープ**: 以下のスコープを追加（「スコープを追加」ボタンをクリック）:
     - `https://www.googleapis.com/auth/gmail.readonly`（Gmailの読み取り）
     - `https://www.googleapis.com/auth/gmail.modify`（Gmailの変更）
     - `https://mail.google.com/`（**重要**: ゴミ箱からの恒久削除に必要）
   - ⚠️ **注意**: `https://mail.google.com/`スコープは、ゴミ箱自動削除機能を使用する場合に必須です。このスコープがないと、ゴミ箱内のメールを恒久削除できません。
   - 「保存して次へ」をクリック
...
4. 「許可」をクリックして、Pi5がGmailにアクセスすることを承認
   - ⚠️ **重要**: ゴミ箱自動削除機能を使用する場合、認証画面で「View and manage your mail」スコープが表示されていることを確認してください。このスコープがない場合、ゴミ箱からの恒久削除ができません。
5. 認証が完了すると、自動的にコールバックURLにリダイレクトされます
6. 認証成功メッセージが表示されます

**スコープ追加後の再認証**:
- Google Cloud ConsoleでOAuthスコープを追加した場合、既存のトークンには新しいスコープが含まれません
- スコープ追加後は、必ず管理コンソール → Gmail設定 → 「OAuth認証」ボタンから再認証を実行してください
- 再認証後、新しいトークンに追加したスコープが含まれ、ゴミ箱自動削除機能が正常に動作します
...
**⚠️ 重要: OAuthスコープ要件**:
- ゴミ箱自動削除機能を使用するには、`https://mail.google.com/`スコープが必要です
- Google Cloud ConsoleのOAuth同意画面に`https://mail.google.com/`スコープを追加してください（手順は「1.3 OAuth 2.0認証情報の作成」を参照）
- スコープ追加後は、必ず再認証を実行してください（手順は「2.2 OAuth認証の実行」を参照）
- スコープが不足している場合、`Request had insufficient authentication scopes.`エラーが発生し、削除処理が失敗します
...
**症状**: ゴミ箱自動削除機能が実行されても、メールが削除されない。または`Request had insufficient authentication scopes.`エラーが発生する

**原因と対処法**:
1. **OAuthスコープ不足**
   - Google Cloud ConsoleのOAuth同意画面に`https://mail.google.com/`スコープが追加されているか確認
   - スコープが追加されていない場合、手順「1.3 OAuth 2.0認証情報の作成」に従ってスコープを追加
   - スコープ追加後、必ず再認証を実行してください（手順「2.2 OAuth認証の実行」を参照）

2. **再認証が未実行**
   - スコープ追加後、既存のトークンには新しいスコープが含まれません
   - 管理コンソール → Gmail設定 → 「OAuth認証」ボタンから再認証を実行
   - 再認証後、新しいトークンに`https://mail.google.com/`スコープが含まれます

3. **ラベルが付与されていない**
   - 削除対象のメールに`rps_processed`ラベルが付与されているか確認
   - ラベルが付与されていない場合、CSV取り込み処理が正常に完了していない可能性があります
