...
- **`FKOJUNST_Status` Gmail 取込タイムアウト（2026-06-18）**: advisory lock 修正（KB-391・PR #452）後、完了トランザクション内の **大量 `csvDashboardRow.update`（500件並列 × 多数チャンク）** が Prisma **P2028 / `Transaction already closed`** を起こす。**修正**: 完了 tx 内で advisory lock 取得後、**`UPDATE ... FROM (VALUES ...)` のバッチ更新**で行内容と source 公開を**原子化**（完了前の行更新は行わない）。詳細は [KB-391 §FKOJUNST completion timeout follow-up](../knowledge-base/KB-391-gmail-csv-import-reliability.md#fkojunst-completion-timeout-follow-up-2026-06-18)。修正デプロイ後に **`csv-import-productionschedule-fkojunst-status-mail` を手動実行**する。
