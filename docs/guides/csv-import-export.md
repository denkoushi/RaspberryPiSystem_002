...
- **CustomerSCAW（製番→顧客名）**: Gmail 件名 **`CustomerSCAW`**・列 **`Customer`** / **`FANKENMEI`**。MH/SH winner 行の **`FHINMEI`** と `FANKENMEI` を正規化照合し `ProductionScheduleFseibanCustomerScaw` を **取込ソースごと全置換**。生産日程一覧・`responseProfile=leaderboard` とも **トップレベル `customerName`** を返す。固定スケジュール **`csv-import-productionschedule-customer-scaw`**（**日曜 5:31 JST 相当 `31 5 * * 0`・既定 enabled**）。詳細は [KB-361](../knowledge-base/KB-361-customer-scaw-gmail-csv.md)。
- **削除ルール（生産スケジュールのみ）**:
