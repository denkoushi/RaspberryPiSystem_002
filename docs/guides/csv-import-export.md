...
- **Gmail csvDashboards スケジュール衝突（2026-06-17）**: 有効な Gmail csvDashboards スケジュールが **同じ分・時・曜日**で重なると、`GmailImportOrchestrator` は先に走ったサイクル実行中に **後続を履歴なしでスキップ**し得る。管理コンソールの CSV 取込タブは **`GET/POST/PUT /api/imports/schedule` の `warnings`** で衝突候補を表示する（判定不能な cron 形状も warning のみ）。**本番既存設定はコードで自動変更しない**。衝突が出た場合は cron をずらす（例: `FHINMEI_MH_SH` を `15 6 * * 0` → `18 6 * * 0` または `20 6 * * 0`）。**デプロイ後の復旧**: 管理コンソールで cron を修正 → **`csv-import-seiban-machine-name-supplement` を手動実行**して未処理 Gmail を再取込。
- **`FKOJUNST_Status` Gmail 取込タイムアウト（2026-06-18）**: advisory lock 修正（KB-391・PR #452）後、完了トランザクション内の **大量 `csvDashboardRow.update`（500件並列 × 多数チャンク）** が Prisma **P2028 / `Transaction already closed`** を起こす。**修正**: 完了 tx 内で advisory lock 取得後、**`UPDATE ... FROM (VALUES ...)` のバッチ更新**で行内容と source 公開を**原子化**（完了前の行更新は行わない）。詳細・本番実機検証（管理画面手動取込 **COMPLETED**）は [KB-391](../knowledge-base/KB-391-gmail-csv-import-reliability.md#admin-console-manual-verification-2026-06-18)。
...
- **詳細**: [KB-370 §2CSV 運用](../knowledge-base/KB-370-production-schedule-external-completion-triple-source.md#kb-370-2csv-schedule-operational-pairing)·[deployment §2026-05-17](../archive/deployments/2026-05.md#schedule-csv-disappearance-2csv-intersection-2026-05-17)。
...
| 補助同期で **`Unique constraint failed`（`csvDashboardRowId`）**／`createMany` が `P2002` | **同一 winner 行 ID** に既に補助行があるが、DB 上の **3キー**（`ProductNo`/`FSIGENCD`/`FKOJUN`）が **補助CSVと不一致**で新規 create に落ちていた（2026-05-06 修正: winner 行に紐づく既存は **update でキー整合**） | [KB-328 §P2002](../knowledge-base/KB-328-production-schedule-supplement-key-mismatch-investigation.md#order-supplement-sync-p2002-csv-dashboard-row-id) |
...
- **取得件数**:
  - `users.messages.list` を1回実行し、**最大N件**（`GMAIL_MAX_MESSAGES_PER_BATCH`、デフォルト30件）のメッセージIDを取得
  - 全件ページングは行わない（検索API呼び出しを抑制）
- **どのメールを取るか**: 検索結果の先頭から最大N通を順次処理
- **どの添付を取るか**: 各メールの **「最初に見つかった添付」**（multipartを再帰探索して最初の `attachmentId` を採用）
- **処理後の扱い（csvDashboards）**:
  - 成功時: `UNREAD` ラベルを外して既読化し、その後ゴミ箱へ移動
  - 失敗時: メールを残す（次回実行で再試行）

#### 高頻度更新（1時間に10回など）の注意

PowerAutomateが「追加/変更のたびにメール送信」だと、Gmail側に同件名メールが短時間に多数たまり得ます。現行仕様では **1回の実行で最大N通（デフォルト30通）**を処理するため、次が起き得ます。

- **滞留**: 到着頻度が処理能力（N通/実行）を上回ると、未読が徐々にたまる
...
- 本システム側のスケジュールを **送信頻度に合わせて調整**する（過剰に短くしない）
- `GMAIL_MAX_MESSAGES_PER_BATCH` を段階的に調整する（例: 10→20→30）
- 「同件名の未読が溜まる」状況を避ける（失敗原因のCSVは早めに修正）
...
- `GET /api/kiosk/production-schedule`: 生産スケジュールデータを取得（各行に`note`（備考）を含む）
  ...
  - **補助予定情報**: `plannedQuantity` / `plannedStartDate` / `plannedEndDate` を含む（件名 `部品納期個数` から照合された場合のみ）
...
Gmail APIの429（rate limit）を早期に検知するため、対象スケジュールの履歴を定期確認します。

1. 管理コンソールの「インポート履歴」で対象スケジュールをフィルタ
2. 以下の兆候が続く場合は設定を見直す
   - `FAILED` が連続し、`errorMessage` に `Gmail API is rate limited; deferred until ...` が出る
   - `COMPLETED` 比率が急落する
3. 対応の優先順
   - `GMAIL_MAX_MESSAGES_PER_BATCH` を下げる（例: 30→20→10）
   - スケジュール間隔を広げる（例: 10分→15分）
   - 失敗し続けるCSVの原因（ヘッダー不一致など）を先に解消する
