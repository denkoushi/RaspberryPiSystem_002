...
- **Gmail csvDashboards スケジュール衝突（2026-06-17）**: 有効な Gmail csvDashboards スケジュールが **同じ分・時・曜日**で重なると、`GmailImportOrchestrator` は先に走ったサイクル実行中に **後続を履歴なしでスキップ**し得る。管理コンソールの CSV 取込タブは **`GET/POST/PUT /api/imports/schedule` の `warnings`** で衝突候補を表示する（判定不能な cron 形状も warning のみ）。**本番既存設定はコードで自動変更しない**。衝突が出た場合は cron をずらす（例: `FHINMEI_MH_SH` を `15 6 * * 0` → `18 6 * * 0` または `20 6 * * 0`）。**デプロイ後の復旧**: 管理コンソールで cron を修正 → **`csv-import-seiban-machine-name-supplement` を手動実行**して未処理 Gmail を再取込。**`FKOJUNST_Status` の DB 取込失敗**（`Failed to deserialize column of type 'void'`）は advisory lock を `$executeRaw` で取る API 修正が必要。修正デプロイ後に **`csv-import-productionschedule-fkojunst-status-mail` を手動実行**する。
