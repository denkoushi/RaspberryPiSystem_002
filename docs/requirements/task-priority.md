...
- [ ] サービス層のユニットテスト追加
  ...
  - [ ] `ItemService`のテスト
  - [ ] `LoanService`のテスト（持出・返却・アクティブ一覧）
...
**次のアクション**: `LoanService`のテストから開始（最も重要なビジネスロジック）。
...
**タスク**:
- [ ] 認証・認可の強化（JWT有効期限の短縮、リフレッシュトークンの実装）
- [ ] APIレート制限の実装
- [ ] 入力バリデーションの強化
- [ ] SQLインジェクション対策の確認（Prisma使用により既に対策済み）
- [ ] XSS対策の確認
- [ ] HTTPSの強制（本番環境）

**推定工数**: 3-4日

**次のアクション**: 認証・認可の強化から開始。
