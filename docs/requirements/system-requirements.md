...
### FR-009: 写真撮影持出機能（新規追加）
- **説明**: 従業員タグのみスキャンで撮影＋持出を記録できる
- **詳細**:
  - 既存の2タグスキャン機能（FR-004）は維持
  - 従業員タグスキャンがシャッターを押す役割も兼ねる
  - スキャン前にItemをカメラの前に置く
  - ラズパイ4にGPIO接続（またはUSB接続）のカメラでItemを撮影
  - 撮影した写真をデータに保存（従業員IDと写真のみ、Item情報は保存しない）
  - 持出一覧にサムネイルでItem写真を表示
  - 返却画面でも写真サムネイルを表示してユーザーがItemを認識できる
  - 写真の保存期間: 撮影年の2年後の1月中に削除（1月中に毎日チェック）
  - 写真サイズ: 元画像800x600px（JPEG品質80%）、サムネイル150x150px（JPEG品質70%）
  - 写真ファイルサイズ: 元画像は100KB程度（転送効率を考慮）
  - 写真ファイルサイズ: 元画像は100KB程度（転送効率を考慮）
  - 撮影失敗時: 3回までリトライ、それでも失敗したらエラー（写真は必須）
  - 撮影場所: Raspberry Pi 4（クライアント端末）で撮影し、画像データをAPIサーバー（Raspberry Pi 5）に送信
  - カメラ機能はモジュール化して今後他の追加機能でも利用可能にする
  - クライアント端末ごとに初期表示画面を設定可能（データベース + 管理画面）
- **技術要件**:
  - データ保存: `Loan`テーブルに`photoUrl`、`photoTakenAt`カラムを追加
  - `ClientDevice`テーブルに`defaultMode`カラムを追加（PHOTO/TAG）
  - 写真保存先: ラズパイ5の1TB SSD（`/opt/RaspberryPiSystem_002/storage/photos/`）
  - Dockerボリュームでマウント
  - サムネイル: Caddyで静的ファイル配信
  - 元画像: API経由で認証制御
- **将来拡張**:
  - 画像認識でItemを自動特定する機能（将来的に実装予定）
- **検証**: Validation 9（写真撮影持出）**← 未実施**
- **関連ドキュメント**: 
  - [工具管理モジュール](../modules/tools/README.md): モジュール仕様
  - [検証チェックリスト](../guides/verification-checklist.md): 検証手順
...
### NFR-006: 視認性・アクセシビリティ
- **説明**: 工場現場での視認性とアクセシビリティを確保すること
- **詳細**:
  - **コントラスト比**: WCAG AAA準拠（コントラスト比7:1以上、推奨21:1）
  - **フォントサイズ**: 最小14px（現状の12pxから拡大）
  - **アイテム種別の識別**: 色だけでなくアイコンでも識別可能（色覚多様性への配慮）
  - **カラーパレット**: 提案3（工場現場特化・高視認性テーマ）を採用
    - 工具: `bg-blue-500` + `text-white` + 🔧 アイコン
    - 計測機器: `bg-purple-600` + `text-white` + 📏 アイコン
    - 吊具: `bg-orange-500` + `text-white` + ⚙️ アイコン
  - **適用範囲**: 管理コンソール、キオスク、サイネージのすべての画面
- **実装状況**: ⏳ 実装予定（2025-12-17要件定義完了）
- **関連ドキュメント**:
  - [UI視認性向上カラーテーマ要件定義](./ui-visibility-color-theme.md): カラーテーマ要件の詳細
  - [工具管理モジュール](../modules/tools/README.md): UIデザイン要件
  - [計測機器管理 UI設計メモ](../modules/measuring-instruments/ui.md): カラーテーマ要件
  - [デジタルサイネージモジュール](../modules/signage/README.md): カラーテーマ要件
...
### Validation 9: 写真撮影持出機能
- **状態**: ✅ **完了**（2025-12-01）
- **内容**: 従業員タグのみスキャンで撮影＋持出記録の作成、写真の保存・表示、返却画面での写真表示
- **検証結果**: 実機環境で正常に動作することを確認。ただし、勝手にスキャンされる事象の改善を依頼済み（経過観察中）。
- **関連ドキュメント**: [検証チェックリスト](../guides/verification-checklist.md), [工具管理モジュール](../modules/tools/README.md)
...
2. **ログ出力の最適化**
   ...
   - ログローテーションの設定
   - ログ分析ツールの導入検討
   - **デバッグログの環境変数制御**: ✅ 実装済み（2025-11-27）
     - `VITE_ENABLE_DEBUG_LOGS`環境変数でデバッグログの出力を制御
     - デフォルトは常にログを出力（開発中に便利）
     - 本番環境でログを無効化したい場合は`VITE_ENABLE_DEBUG_LOGS=false`を設定
     - エラーログは常に出力（問題の特定に必要）
     - **目的**: 365日24時間動作する環境でのメモリ使用量削減
