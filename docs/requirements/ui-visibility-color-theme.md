...
1. **キオスクの計測機器・吊具持出画面の更新**
   - `KioskInstrumentBorrowPage.tsx`: フォームのテキスト色を更新（12箇所）
   - `KioskRiggingBorrowPage.tsx`: フォームのテキスト色を更新（8箇所）

2. **吊具一覧ページの更新**
   - `RiggingGearsPage.tsx`: テーブルのテキスト色を更新（21箇所）

#### 優先度: 中
3. **管理コンソールのその他のページの更新**
   - Cardコンポーネントを使用しているページのカード内テキスト色を更新（107箇所）
   - フォーム要素のスタイルを更新

4. **工具管理のその他のページの更新**
   - Cardコンポーネントを使用しているページのカード内テキスト色を更新（69箇所）
...
#### ⚠️ 一部未適用
- REQ-UI-001: セカンダリ背景（`bg-slate-100`は一部未適用）
- REQ-UI-002: フォントサイズ（一部のページで`text-xs`（12px）が残っている）
- REQ-UI-005: 適用範囲（計測機器・吊具持出画面、吊具一覧ページが未更新）
