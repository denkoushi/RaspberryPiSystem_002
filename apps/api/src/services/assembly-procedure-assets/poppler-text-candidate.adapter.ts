...
import type {
  AssemblyProcedurePdfTextCandidateInput,
  AssemblyProcedurePdfTextCandidatePort,
  AssemblyProcedureTextCandidate,
  AssemblyProcedureTextCandidateBounds,
...
const FULL_PAGE_ROI: AssemblyProcedureTextCandidateBounds = {
  xRatio: 0,
  yRatio: 0,
  widthRatio: 1,
  heightRatio: 1,
...
function normalizeRoi(
  value: AssemblyProcedureTextCandidateBounds | undefined,
): AssemblyProcedureTextCandidateBounds | null {
...
): AssemblyProcedureTextCandidate | null {
...
/**
 * Parse the XML-ish output emitted by `pdftotext -bbox-layout`.
 * Poppler's bbox y coordinates use the same top-left orientation as the
 * source-page ratios used by the assembly editor.
 */
export function parsePopplerBboxLayout(
  ...
  const candidates: AssemblyProcedureTextCandidate[] = [];
...
export class PopplerBboxLayoutTextCandidateAdapter
  implements AssemblyProcedureTextCandidatePort, AssemblyProcedurePdfTextCandidatePort
{
  ...
  constructor(options: PopplerBboxLayoutTextCandidateAdapterOptions = {}) {
    ...
    this.maxOutputBytes = Number.isInteger(options.maxOutputBytes) && (options.maxOutputBytes ?? 0) > 0
      ? options.maxOutputBytes!
      : DEFAULT_POPPLER_TEXT_MAX_OUTPUT_BYTES;
    this.runCommand = options.runCommand ?? defaultRunCommand;
  }

  async extractPdfCandidates(
...
/**
 * Compatibility facade retaining the previous constructor while the new
 * default path uses bbox-layout. Passing a DocumentTextExtractorPort enables
 * the old text-only behavior for existing integrations; options use the new
 * coordinate-aware command.
 */
