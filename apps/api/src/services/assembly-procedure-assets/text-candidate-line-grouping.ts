...
type PositionedCandidate = {
  candidate: AssemblyProcedureTextCandidate;
  bounds: AssemblyProcedureTextCandidateBounds;
  inputIndex: number;
};

type CandidateLine = {
  anchor: AssemblyProcedureTextCandidateBounds;
  candidates: PositionedCandidate[];
};

function right(bounds: AssemblyProcedureTextCandidateBounds): number {
  return bounds.xRatio + bounds.widthRatio;
}

function bottom(bounds: AssemblyProcedureTextCandidateBounds): number {
  return bounds.yRatio + bounds.heightRatio;
}

function centerY(bounds: AssemblyProcedureTextCandidateBounds): number {
  return bounds.yRatio + bounds.heightRatio / 2;
}

function verticalOverlapRatio(
  left: AssemblyProcedureTextCandidateBounds,
  rightBounds: AssemblyProcedureTextCandidateBounds,
): number {
...
/**
 * Builds deterministic horizontal text candidates from coordinate OCR fragments.
 * Inputs that cannot be grouped safely are returned unchanged.
 */
export function groupAssemblyProcedureTextCandidates(
  ...
  const positioned = candidates
    .map((candidate, inputIndex): PositionedCandidate => ({
      candidate,
      bounds: candidate.bounds as AssemblyProcedureTextCandidateBounds,
      inputIndex,
    }))
    .sort(
      (left, rightValue) =>
  ...
  const segments = lines
    ...
    .flatMap((line) => {
      ...
      const representativeHeightInXUnits =
        representativeHeight / aspectRatio;
