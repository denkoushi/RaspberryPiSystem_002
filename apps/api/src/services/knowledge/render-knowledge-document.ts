import { knowledgeSourceSchema, KNOWLEDGE_TOPIC, validateOrganizedNote, type ReadyKnowledgeSource } from './knowledge-source.js';
...
export function renderKnowledgeDocument(records: readonly ReadyKnowledgeSource[]): KnowledgeDocument {
  if (records.length === 0 || records.length > 20) throw new Error('Knowledge requires 1–20 sources');
  ...
  const content = [`# ${KNOWLEDGE_TOPIC.title}`, 'AIが整理した内容です。判断の際は元のメモと写真を確認してください。', ...blocks].join('\n\n');
  return {
    markdown: content,
    report: {
      formatVersion: 1, topicId: KNOWLEDGE_TOPIC.id, title: KNOWLEDGE_TOPIC.title,
