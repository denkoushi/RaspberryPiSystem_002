...
export function prepareSourceFact(detail: unknown, ref: Ref, now: string): FactCase | null {
  ...
  if (r.kind !== ref.kind || r.id !== ref.id) return null;
  let scope: string, subject: string, answer: string;
  if (r.kind === 'nonconformity') {
    if ((r.partNumber !== null && !label(r.partNumber)) || !label(r.nonconformityNo) || (r.provenance as { activeLatest?: boolean } | undefined)?.activeLatest !== true) return null;
    ...
    scope = `不適合記録${r.nonconformityNo}` + (r.partNumber === null ? '' : `・図番${r.partNumber}`);
    if (r.partNumber === null) fields.unshift(['partNumber', '図番']);
    subject = '記録内容';
    answer = `${scope}\n過去記録の引用であり、現在の作業指示ではありません。\n` + fields.map(([key, name]) => `${name}：${text(r[key!]) ? r[key!] : '未記録'}`).join('\n');
  } else if (r.kind === 'work_instruction') {
    if (!label(r.partNumber) || r.public !== true || !label(r.shootingTarget) || !Array.isArray(r.rows) || r.rows.length !== 1) return null;
