...
import { findProductionSchedulePalletSnapshot } from '../production-schedule/production-schedule-snapshot.service.js';
import { formatPlannedStartDateForPalletDisplay } from './pallet-visualization-display-fields.js';
...
export async function resolveScheduleSnapshotForPalletItem(
  ...
  const snapshot = await findProductionSchedulePalletSnapshot(primary.rowId);
  if (!snapshot) {
    throw new ApiError(404, 'スケジュール行の取得に失敗しました', undefined, 'PALLET_SCHEDULE_ROW_MISSING');
  }

  const plannedQuantity = snapshot.plannedQuantity;
  const plannedStartDateDisplay = formatPlannedStartDateForPalletDisplay(snapshot.plannedStartDate);
  const outsideDimensionsDisplay = snapshot.outsideDimensionsDisplay;
  ...
  const scheduleSnapshot: Record<string, unknown> = {
    ...
    FSIGENCD: snapshot.resourceCode,
    plannedQuantity,
  ...
  return {
    ...
    fhincd: String(snapshot.partCode ?? primary.fhincd ?? '').trim(),
    fhinmei: String(snapshot.partName ?? primary.fhinmei ?? '').trim(),
    fseiban: String(snapshot.seiban ?? primary.fseiban ?? '').trim(),
    machineName,
