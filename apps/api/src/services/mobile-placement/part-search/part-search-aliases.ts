 ...
 */
export const PART_SEARCH_ALIAS_GROUPS: readonly (readonly string[])[] = [
  ['アシ', '脚', '足'] as const
];

/**
 * クエリに同義語グループが含まれる場合、グループ内の語を OR 検索に展開する。
 */
export function expandSearchTerms(normalizedQuery: string): {
  terms: string[];
  ...
  let terms = new Set<string>([normalizedQuery]);
  const matchedGroups: string[] = [];
  for (const group of PART_SEARCH_ALIAS_GROUPS) {
    const matchedTokens = group.filter((g) => normalizedQuery.includes(g));
