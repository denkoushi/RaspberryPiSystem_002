...
/**
 * 既存の日程照会契約。公開契約と内部の検索・保存形式は生産日程が所有する。
 * rowId は CsvDashboardRow.id であり、独立した業務IDではない。
 * fseiban 等のフィールドは既存の意味を維持し、業務用語への変換は別工程で行う。
 */
export type ProductionScheduleLookupRow = {
  rowId: string;
  fseiban: string;
  productNo: string;
  fhincd: string;
 ...
 */
export async function listScheduleRowsByProductNo(productNo: string): Promise<ProductionScheduleLookupRow[]> {
 ...
 */
export async function listScheduleRowsByFseiban(fseiban: string): Promise<ProductionScheduleLookupRow[]> {
