...
export interface LeaderboardShellSnapshotStore {
   ...
   * `appendedRowIds.length === 0` または `Math.floor(chunkSize) > appendedRowIds.length` のとき
   * 並び全体が終端に達したとみなし `partialOrdering` を `false` にする。
   */
...
export function createInMemoryLeaderboardShellSnapshotStore(
  ...
  return {
    ...
    appendSnapshotOrderingChunk(snapshotId, appendedRowIds, chunkSize) {
      gc();
      const rec = map.get(snapshotId);
      if (!rec) return;
      const nChunk = Math.max(1, Math.floor(chunkSize));
      const appended = [...appendedRowIds];
      const nextIds =
        appended.length > 0
          ? ([...rec.orderedRowIds, ...appended] as string[])
          : ([...rec.orderedRowIds] as string[]);
      const exhausted = appended.length === 0 || appended.length < nChunk;
      map.set(snapshotId, {
        ...rec,
        orderedRowIds: Object.freeze(nextIds) as readonly string[],
        partialOrdering: exhausted ? false : true
      });
    },
    get(snapshotId) {
      gc();
