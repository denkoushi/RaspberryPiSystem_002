...
async function createFixture(): Promise<{ siteKey: string; rows: SummaryRow[]; rowIds: string[]; splitIds: string[] }> {
  ...
  const splitParent = rows.find((row) => row.data.FSEIBAN.endsWith('-D'))!;
  for (const [index, splitQuantity] of splitParent.splitQuantities.entries()) {
    const split = await db().productionScheduleOrderSplit.create({
      data: {
        csvDashboardId: PRODUCTION_SCHEDULE_DASHBOARD_ID,
        parentCsvDashboardRowId: splitParent.id,
        splitNo: index + 1,
        splitQuantity
      },
      select: { id: true }
  ...
  const fixture = { siteKey, rows, rowIds: rows.map((row) => row.id), splitIds };
...
function projectionInput(fixture: { rows: SummaryRow[]; splitIds: string[] }): {
  ...
  const splitParent = fixture.rows.find((row) => row.data.FSEIBAN.endsWith('-D'))!;
  ...
  const details = new Map(fixture.rows.map((row) => [row.id, {
    ...
    orderSplits: row === splitParent
      ? fixture.splitIds.map((id, index) => ({ id, splitQuantity: row.splitQuantities[index]!, dueDate: null, updatedAt: splitUpdatedAt }))
      : []
  }] as [string, GrindingPlanningBoardProjectionRowDetail]));
  const movedParent = fixture.rows.find((row) => row.data.FSEIBAN.endsWith('-A'))!;
...
describeIntegration('grinding planning board lightweight load summary integration', () => {
  ...
  it('reads minimal source columns, applies override/split/completion rules, and equals full projection load', async () => {
    ...
    const full = projectGrindingPlanningBoard({
      ...
      category: 'grinding',
      splitEnabled: true,
      seibanOrder: fixture.rows.map((row) => row.data.FSEIBAN),
      isResourceInCategory: (resourceCd, category) => category === 'grinding' && resourceCd.startsWith('G-')
    });
    ...
    expect(summary.load.find((entry) => entry.resourceCd === 'G-01')).toMatchObject({
      originalItemCount: 5,
      alternateItemCount: 3,
      originalRequiredMinutes: 200,
      alternateRequiredMinutes: 60
    });
    expect(summary.load.find((entry) => entry.resourceCd === 'G-02')).toMatchObject({
      originalItemCount: 1,
      alternateItemCount: 3,
      originalRequiredMinutes: 40,
      alternateRequiredMinutes: 180
    });
    expect(summary.unknownRequiredMinutesCount).toBe(2);
  });
});
