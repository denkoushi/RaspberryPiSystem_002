...
describe('business Hermes environment shape', () => {
  ...
  it('keeps the existing guide deadline and gives chat its independent 180s investigation default', () => {
    const parsed = schema.parse({});

    expect(parsed.BUSINESS_HERMES_TIMEOUT_MS).toBe(8_000);
    expect(parsed.BUSINESS_HERMES_CHAT_TIMEOUT_MS).toBe(180_000);
    expect(schema.parse({
      BUSINESS_HERMES_TIMEOUT_MS: '30000',
      BUSINESS_HERMES_CHAT_TIMEOUT_MS: '300000'
    })).toMatchObject({ BUSINESS_HERMES_TIMEOUT_MS: 30_000, BUSINESS_HERMES_CHAT_TIMEOUT_MS: 300_000 });
  });

  it('rejects timeout values above each contract bound', () => {
    expect(() => schema.parse({ BUSINESS_HERMES_TIMEOUT_MS: '30001' })).toThrow();
    expect(() => schema.parse({ BUSINESS_HERMES_CHAT_TIMEOUT_MS: '300001' })).toThrow();
  });
});
