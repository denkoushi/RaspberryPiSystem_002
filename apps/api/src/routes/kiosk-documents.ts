...
import { AssemblyProcedureReferenceService } from '../services/assembly/assembly-procedure-reference.service.js';
...
export function registerKioskDocumentRoutes(app: FastifyInstance): void {
  ...
  const assemblyProcedureReferenceService = new AssemblyProcedureReferenceService();
  ...
  app.delete('/kiosk-documents/:id', { preHandler: [canManage] }, async (request) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(request.params);
    const assemblyReferenceCount =
      await assemblyProcedureReferenceService.countKioskDocumentReferences(id);
    if (assemblyReferenceCount > 0) {
      throw new ApiError(
        409,
        '組立テンプレートの手順書列で使用中の要領書は削除できません',
        undefined,
