  ...
  return (
    ...
    >
      <div className="mt-4 grid max-h-[min(50vh,24rem)] gap-2 overflow-auto" role="listbox" aria-label="文章候補">
        {candidates.map((candidate, index) => (
          <Button
            ...
            variant="ghost"
            role="option"
            className="flex min-h-11 items-start justify-between gap-3 border border-slate-200 bg-slate-50 !px-3 !py-2 text-left text-slate-900 hover:bg-slate-100 hover:text-slate-900"
      ...
      <div className="mt-4 flex flex-wrap justify-end gap-2">
        ...
        <Button type="button" variant="ghost" className="min-h-11 text-slate-800" onClick={onClose}>
          キャンセル
        </Button>
      </div>
    </Dialog>
