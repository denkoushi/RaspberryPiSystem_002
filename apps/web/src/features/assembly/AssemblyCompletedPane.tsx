      ...
      {sessions.length === 0 ? (
        ...
        <div className="grid min-h-0 flex-1 content-start grid-cols-1 gap-2 overflow-y-auto p-2 sm:grid-cols-2 2xl:grid-cols-3">
          {sessions.map((session) => (
            <div
              key={session.id}
              className="flex min-h-11 min-w-0 flex-col gap-2 rounded border border-white/10 bg-slate-900/55 px-3 py-2.5 text-white"
            >
              <div className="flex min-w-0 items-start justify-between gap-2">
                ...
                <span className="flex min-h-11 shrink-0 items-center justify-center rounded border border-cyan-300/30 bg-cyan-500/15 px-3 text-xs font-bold text-cyan-100">
                  完了
                </span>
              </div>
              <span className="min-w-0">
              ...
              <div className="flex min-w-0 items-end justify-between gap-2">
                <span className="min-w-0">
                  ...
                  <span className="mt-0.5 block truncate text-xs font-semibold text-white/55">作業者</span>
                </span>
