...
export function ManualOrderDeviceCard({
  ...
  return (
    <article
      ...
      className={clsx(
        'rounded border bg-slate-900/60 p-4 transition-all',
    ...
    >
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="truncate text-2xl font-semibold leading-tight text-cyan-200" title={locationLine}>
            {locationLine}
          </p>
        </div>
        <button
          ...
          className="shrink-0 rounded border border-white/20 bg-white/10 p-2 text-amber-300 hover:bg-white/20"
      ...
      {status === 'saving' ? <p className="mb-2 text-xl text-amber-200">保存中…</p> : null}
      {status === 'error' ? <p className="mb-2 text-xl text-rose-200">保存に失敗しました</p> : null}

      <div className="space-y-2">
        {resources.length === 0 ? (
          <p className="rounded bg-slate-800 px-3 py-2 text-lg text-white/60">手動順番は未設定です</p>
        ) : (
          resources.map((resource) => {
            const rows = resource.rows ?? [];
            return (
              <div key={resource.resourceCd} className="rounded bg-slate-800/80 px-3 py-2">
                <div className="mb-2 flex items-center justify-between gap-2 text-xl">
                ...
                {rows.length > 0 ? (
                  ...
                  <p className="text-lg text-white/45">行データを取得できませんでした</p>
                )}
              </div>
            );
          })
