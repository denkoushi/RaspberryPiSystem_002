            ...
            <tbody>
              {pdfsQuery.data.map((pdf: SignagePdf) => (
                <tr key={pdf.id} className="border-b border-white/5">
                  ...
                  <td className="px-4 py-2">{pdf.displayMode === 'SLIDESHOW' ? 'スライドショー' : '単一表示'}</td>
                  ...
                  <td className="px-4 py-2">
                    <Button onClick={() => handleDelete(pdf.id)} variant="ghost" className="px-3 py-1 text-sm text-red-400">
                      削除
                    </Button>
                  </td>
                </tr>
