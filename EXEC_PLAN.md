...
- [ ] (2025-11-25) **Phase 1: 429エラー・404エラーの根本原因特定と修正**（最優先・他の機能にも影響）
  ...
  - **手順1**: レート制限プラグインの動作確認（実環境でレート制限が実際に適用されているか、`config: { rateLimit: false }`が正しく認識されているか、サブルーターの`config`が親アプリのプラグインで認識されるか）
  - **手順2**: ルーティングの確認（フロントエンドが実際に呼んでいるエンドポイント、バックエンドのルーティング設定、Caddyのリバースプロキシ設定）
  - **手順3**: 修正実装（レート制限プラグインをルート登録前に登録するか、サブルーター内で登録する。ルーティング不一致を修正）
  - **検証**: ダッシュボード・履歴ページで429エラー・404エラーが発生しないことを確認
