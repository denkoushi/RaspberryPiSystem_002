...
- [x] (2026-03-07) **納期管理トリアージ（B第1段階）実装完了・デプロイ完了・実機検証完了**: リーダーが「選ぶだけ」で計画対象を絞れるよう、納期管理画面にトリアージ導線を追加。`ProductionScheduleTriageSelection` テーブルで拠点共有の選択済み製番を永続化し、`GET /api/kiosk/production-schedule/due-management/triage` と `PUT /api/kiosk/production-schedule/due-management/triage/selection` を追加。判定は納期基準（danger/caution/safe）を主軸に、高件数でエスカレーション、理由（`DUE_DATE_*`, `LARGE_*`, `SURFACE_PRIORITY`）を返却。納期管理左ペインに「今日判断候補（トリアージ）」パネル、選択/解除、選択済みのみトグルを実装。**デプロイ**: Run ID `20260307-154428-18368`、`state: success`、Pi5+Pi4×2（`--limit "server:kiosk"`）。**実機検証**: APIヘルス（degraded/メモリ95.2%）、deploy-status（両Pi4で `isMaintenance: false`）、キオスクAPI（loans/active・production-schedule・due-management/summary・**due-management/triage**・**PUT triage/selection**）、マイグレーション（39件）、`ProductionScheduleTriageSelection` テーブル存在、backup.json、Pi4サービス（kiosk-browser・status-agent.timer ともに active）を確認。詳細は [KB-297](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md) / [production-schedule-kiosk-execplan.md](./docs/plans/production-schedule-kiosk-execplan.md) を参照。
...
変更履歴: 2026-03-07 — 納期管理トリアージ（B第1段階）デプロイ・実機検証完了を反映。Run ID `20260307-154428-18368`、Pi5+Pi4×2（`--limit "server:kiosk"`）、実機検証（APIヘルス・deploy-status・トリアージAPI・マイグレーション39件・ProductionScheduleTriageSelection・backup.json・Pi4サービス）を記録。KB-297 に B第1段階デプロイ・実機検証セクションを追加。deploy-status-recovery.md にトリアージAPI検証項目を追加。
