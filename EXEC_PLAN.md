...
- [x] (2026-04-14) **計測機器ジャンル画像ストレージ永続化（compose bind + Ansible best-effort 退避）実装**（**本番デプロイ・`main` マージ・Phase12 は未実施**）: ブランチ **`fix/measuring-instrument-genre-image-persistence`**。**原因**: `measuring-instrument-genres` が compose 未マウントのため `api` 再作成で実ファイル消失。**対策**: [`infrastructure/docker/docker-compose.server.yml`](./infrastructure/docker/docker-compose.server.yml) に `measuring-instrument-genres-storage`、[`docker-compose.mac-local.override.yml`](./infrastructure/docker/docker-compose.mac-local.override.yml) にローカル bind、[`infrastructure/ansible/roles/server/tasks/main.yml`](./infrastructure/ansible/roles/server/tasks/main.yml) でホストディレクトリ作成と **api 再作成前**の `docker compose ps -q api` 起点の救出（ホスト既存ファイルは上書きしない）。**検証**: `docker compose -f infrastructure/docker/docker-compose.server.yml config` OK、変更 TS ファイル eslint OK。**ローカル永続化スモーク**: `.docker/local/storage/measuring-instrument-genres` にテストファイルを置き、同一パスを `-v` でマウントした一時コンテナを `docker rm -f` 後に再マウントしても内容が読めることを確認（API 経由のアップロード→`api` 再作成のフル E2E は未実施）。**参照**: [deployment.md](./docs/guides/deployment.md)・[ui.md](./docs/modules/measuring-instruments/ui.md)。**次**: PR → `main` マージ → Pi5 へ標準デプロイ → 管理画面ジャンル画像・キオスク表示確認。
