...
- [x] (2026-05-02) **順位ボード `leaderboard` 一覧 GET の過大 `pageSize` を API 側でクランプ（旧 SPA キャッシュ吸収）+ 計測 ingest 除去**·`fix/leaderboard-pagesize-server-cap`·**`0865cc56`**（[PR #236](https://github.com/denkoushi/RaspberryPiSystem_002/pull/236)）·[`list.ts`](./apps/api/src/routes/kiosk/production-schedule/list.ts)·[deployment.md §pageSize](./docs/guides/deployment.md)·[KB-297 §ページサイズ上限制御](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-leaderboard-pagesize-server-cap-2026-05-02): **`responseProfile=leaderboard` のとき `pageSize = min(requested, 900)`**。`127.0.0.1:7426` への開発用 **`fetch`** を **API/Web 本番経路から除去**。**対象ホスト**: **`raspberrypi5` のみ**（ユーザー方針）·**Pi3 除外**。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh main infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**: （本セッションの `ansible-update-*` で追記）·**自動実機**: `./scripts/deploy/verify-phase12-real.sh`。**知見**: Caddy **`/var/log/caddy/access.log`** の JSON `uri` に **`pageSize=1240` が混在**すると再発の兆し。
