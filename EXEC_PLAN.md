...
**候補タスク**:
1. **管理コンソール UI**: `ADMIN` / `MANAGER` が status・試用チャットを行える画面（レート制限・監査は [ADR-20260329](./docs/decisions/ADR-20260329-local-llm-pi5-api-operations.md) に沿って方針固定推奨）。
2. ~~**可観測性**~~: **ログ基準は ADR + pino 実装で一次完了**（Prometheus 等のメトリクスは必要になったら別 ADR）。
3. ~~**運用ポリシー（ログ・ロール境界）**~~: **ADR-20260329 で固定**（共有トークン手順の Runbook 追記は継続可）。
