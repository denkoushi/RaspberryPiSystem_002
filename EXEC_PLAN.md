...
- [x] (2026-04-11) **現品票 OCR パイプライン専用化（用途別 `tesseract.js`・前処理・`ocrPreviewSafe`）実装・ドキュメント同期・ブランチ `feat/mobile-placement-ocr-pipeline-hardening`**: `ImageOcrProfile`（`services/ocr/image-ocr-profiles.ts`）・`TesseractJsImageOcrAdapter`（`jpn+eng` ラベルパス + `eng` 数字／英数字 whitelist）・`actual-slip-image-ocr.service` の 3 パス直列と結合パース・`POST …/parse-actual-slip-image` 応答に **`ocrPreviewSafe`**。Web はプレビューに `ocrPreviewSafe` を優先。**参照**: [api/mobile-placement.md](./docs/api/mobile-placement.md)・[KB-339](./docs/knowledge-base/KB-339-mobile-placement-barcode-survey.md)・[mobile-placement-smartphone.md](./docs/runbooks/mobile-placement-smartphone.md)・[docs/INDEX.md](./docs/INDEX.md)。**デプロイ・`main` マージは未**（コミット／プッシュは別作業）。
...
### 配膳スマホ運用フォロー（V2 既定・V4 登録済み棚・**V5 現品票画像 OCR**・2026-04-11 本番反映後）
...
1. **Android 実機（V5）**: 現品票の **画像OCR**（`parse-actual-slip-image`）で **製造order（10桁）/ FSEIBAN** 候補が出ること・**注文番号（9桁）の誤採用がない**ことを目視確認。初回のみ **tesseract ワーカ起動**で数十秒かかり得る（[Runbook](./docs/runbooks/mobile-placement-smartphone.md) §5）。
