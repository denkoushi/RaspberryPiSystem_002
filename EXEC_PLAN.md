...
3. **並行**: [キオスク要領書 PDF（2026-03-25）](#キオスク要領書-pdf2026-03-25) の Gmail 本番試験を優先度に応じて進める。写真持出 VLM **フェーズ1** は [PR #56](https://github.com/denkoushi/RaspberryPiSystem_002/pull/56) で `main` 統合済み。**類似候補ギャラリー（pgvector）** は [PR #57](https://github.com/denkoushi/RaspberryPiSystem_002/pull/57) で `main` 統合済み（2026-03-29、CI 成功確認済み）。~~**埋め込み本番配線・バックフィル・Runbook**~~: `feat/photo-tool-embedding-rollout-shadow-eval` を `main` に統合済み（2026-03-29）。**次の作業（優先度順の目安）**: **vault で `vault_photo_tool_embedding_*` を有効化**（未到達なら埋め込みは既定 OFF のまま）、**`pnpm backfill:photo-tool-gallery:prod`**（埋め込み ON 後の既存 GOOD 再投入）、**シャドー補助のログ評価**（[ADR-20260331](./docs/decisions/ADR-20260331-photo-tool-label-good-assist-shadow.md)・`PHOTO_TOOL_LABEL_ASSIST_SHADOW_ENABLED=true` を**意図的に**有効化した Pi5 のみ）、**`verify-phase12-real.sh` への軽量 candidates 401 チェック（任意）** — 手順は [photo-tool-similarity-gallery.md](./docs/runbooks/photo-tool-similarity-gallery.md) / [KB-319](./docs/knowledge-base/KB-319-photo-loan-vlm-tool-label.md)。**残りアイデア**: VLM **フェーズ2**（Item マスタ照合・誤認補正ワークフロー拡張）と **`assistedLabel` の active 化**は別 ADR/ExecPlan で切り出す。
