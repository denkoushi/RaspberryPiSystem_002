...
- [x] (2026-03-15) **Location Scope Phase9（compat呼び出し棚卸し・公開面縮小）実装・品質ゲート・受け入れ検証完了**: `feat/location-scope-phase9-compat-callsite-audit` ブランチで `kiosk/shared.ts` の互換公開面を整理し、未使用の `resolveLocationKey` / `resolveCompatLocationScopeContext`（および `CompatLocationScopeContext` 再エクスポート）を削除して標準契約中心へ集約。`csv-import-execution.service.ts` のローカル関数 `resolveLocationKey` は `resolveImportMetadataLocationKey` に改名し、責務名を明確化。API/DB契約は不変。**検証**: `@raspi-system/api` lint・対象テスト（resolver / resource-category-policy / adapter / triage / scoring / learning）・build、`@raspi-system/web` lint・build を通過。**受け入れ確認**: health（`status: degraded`, memory warning 95.4% の既知警告）/ deploy-status 両Pi4（`isMaintenance:false`）/ 納期管理API（triage・daily-plan・global-rank・proposal・learning-report・actual-hours/stats 200）/ Mac向け `targetLocation=%E7%AC%AC2%E5%B7%A5%E5%A0%B4&rankingScope=globalShared` 応答確認 / signage API 200 / backup.json 存在 / migration up to date / Pi3 signage + Pi4 kiosk/status-agent active を確認。
...
### Location Scope Phase9（compat呼び出し棚卸し・公開面縮小）実装完了後の次のタスク（2026-03-15）

**概要**: `feat/location-scope-phase9-compat-callsite-audit` で互換公開面の縮小と命名衝突解消を実施し、標準契約中心の境界へ整理。品質ゲートと実機受け入れ確認を完了。
