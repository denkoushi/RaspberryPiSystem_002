...
- [x] (2026-03-25) **吊具 idNum 周辺: 3 台順次デプロイ成功（Pi5 + raspberrypi4 + raspi4-robodrill01）・Phase12 実機検証 PASS 28/0/0・ドキュメント反映（KB-312・verification-checklist 6.6.4・deploy-status-recovery・INDEX・本ファイル）**: `update-all-clients.sh` `main` を `--limit` 1 台ずつ。未追跡ファイルは **`git stash push -u`** で退避が必要な場合あり。**自動検証**: `./scripts/deploy/verify-phase12-real.sh`。**知見**: Mac→Pi4 直 SSH は timeout しうるが Pi5 経由は可（ACL）。**手動**: `/kiosk/tag` レイアウト・サイネージ見た目は実機/VNC 推奨。**参照**: [KB-312](./docs/knowledge-base/KB-312-rigging-idnum-deploy-verification.md) / [deploy-status-recovery.md](./docs/runbooks/deploy-status-recovery.md)。
