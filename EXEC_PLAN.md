...
- [x] (2026-05-02) **順位ボード 行フッター工程チップを `responseProfile=leaderboard` 一覧応答へ内包（progress-overview の二重取得撤去・完了後整合 invalidate 拡張）**·`feat/kiosk-leaderboard-footer-contract`·（**実装のみ・マージ／デプロイは未実施**・依頼により **コミット／プッシュ未実施**）·`leaderboardFooterChipsByPartKey`（[`leaderboard-part-footer-processes.service.ts`](./apps/api/src/services/production-schedule/leaderboard/leaderboard-part-footer-processes.service.ts)・[`production-schedule-query.service.ts`](./apps/api/src/services/production-schedule/production-schedule-query.service.ts)）·Web [`ProductionScheduleLeaderOrderBoardPage.tsx`](./apps/web/src/pages/kiosk/ProductionScheduleLeaderOrderBoardPage.tsx)·[`hooks.ts`](./apps/web/src/api/hooks.ts) `useCompleteKioskProductionScheduleRow` が **history-progress / progress-overview** も invalidate。**検証**: API `production-schedule-query.service.test`・Web `collectLeaderBoardFooterResourceChips.test`・`pnpm --filter web exec tsc -b` **PASS**。キオスク `kiosk-production-schedule.integration` は **PostgreSQL が要る環境**で再実行。
