...
### サイネージ: キオスク進捗一覧スロット（`kiosk_progress_overview`）運用フォロー（2026-04-01）

**概要**: `feature/signage-kiosk-progress-overview` を本番へ反映済み（Pi5→Pi4×4→Pi3 順次）。`./scripts/deploy/verify-phase12-real.sh` は **PASS 38 / WARN 0 / FAIL 0**（**`GET /api/signage/current-image` + Pi3 `x-client-key`** 含む）。
...
**概要**: 実装と本番有効化は完了。`./scripts/deploy/verify-phase12-real.sh` は **PASS 37 / WARN 0 / FAIL 0**、Pi5→Ubuntu **`/start` / `/stop` 200**、ComfyUI 既存生成 OK、アイドル時 `llama-server` 不在まで確認済み。
