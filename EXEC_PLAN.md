...
- [x] (2026-05-06) **CsvDashboard Gmail 取込: NON_RETRIABLE 廃棄成功時は最終失敗集計から除外（同一バッチ内の正常メールで全体を落とさない）**·ブランチ **`cursor/mishima-nonretriable-dispose-fix`**·[PR #259](https://github.com/denkoushi/RaspberryPiSystem_002/pull/259)（**`main` へマージ後**は **`main` 先端の squash コミット**を正とする）。**知見（三島研削）**: PowerAutomate 由来で添付が **UTF-8 BOM のみ級**になることがあり **`CSV_HEADER_MISMATCH` → NON_RETRIABLE → ゴミ箱**は想定どおり（part 選択ミスではない観測）。**実装**: [`csv-dashboard-import.service.ts`](./apps/api/src/services/csv-dashboard/csv-dashboard-import.service.ts)·単体 [`csv-dashboard-import.service.ingest-behavior.test.ts`](./apps/api/src/services/csv-dashboard/__tests__/csv-dashboard-import.service.ingest-behavior.test.ts)。**衛生**: 調査用 **localhost ingest** は **`gmail-api-client` / ingestor / import service から除去**。**ナレッジ**: [KB-297 §空BOM・廃棄](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#mishima-grinding-empty-csv-bom-nonretriable-2026-05-06)·[csv-import-export.md](./docs/guides/csv-import-export.md)·[INDEX.md](./docs/INDEX.md)·[knowledge-base/index.md](./docs/knowledge-base/index.md)。**本番**: マージ後 **Pi5 API のみ** [deployment.md](./docs/guides/deployment.md)（`./scripts/update-all-clients.sh main … --limit raspberrypi5`）。
...
### 三島研削 CSV 取込修正（PR #259）の Pi5 本番反映と運用確認（2026-05-06）

**概要**: [PR #259](https://github.com/denkoushi/RaspberryPiSystem_002/pull/259)（`cursor/mishima-nonretriable-dispose-fix`）は **API のみ**。`main` 取り込み後、[deployment.md](./docs/guides/deployment.md) 標準で **`raspberrypi5` のみ** `./scripts/update-all-clients.sh main infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。
