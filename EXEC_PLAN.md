...
- [x] (2026-03-10) **納期管理 B第7段階（実績工数CSV連携 + 全体ランキング連携）実装完了・デプロイ完了・実機検証完了**: `ProductionScheduleActualHoursRaw` / `ProductionScheduleActualHoursFeature` テーブルを追加し、`productionActualHours` ターゲットで Gmail CSV 取込をスケジューラに統合。取込後に `FHINCD × FSIGENCD` で中央値・件数・p75 を再集約（除外条件: 直近30日、0工数、外れ値）し、`due-management-scoring.service.ts` に `actualHoursScore` を追加して全体ランキングへ反映。手動検証導線として `POST /api/kiosk/production-schedule/due-management/actual-hours/import` と `GET /api/kiosk/production-schedule/due-management/actual-hours/stats` を追加。**デプロイ**: Pi5 → raspberrypi4 → raspi4-robodrill01 の順に `--limit` で1台ずつ実行（Run ID `20260310-154937-13894` / `20260310-155444-12790` / `20260310-155947-3485`）。推奨運用は [deployment.md](./docs/guides/deployment.md) の「1台ずつ順番デプロイ」を参照。**実機検証**: APIヘルス、deploy-status（両Pi4で `isMaintenance: false`）、キオスクAPI、納期管理API（triage・daily-plan・global-rank・global-rank/proposal・global-rank/learning-report・**actual-hours/stats**）、サイネージAPI、backup.json、マイグレーション（44件）、Pi4/Pi3サービス稼働を確認。`actual-hours/stats` は未取込時 `totalRawRows: 0`, `totalFeatureKeys: 0`, `topFeatures: []` を返す（想定どおり）。**追補（Canonical差分化（2026-03-10 実装完了））**: `ProductionScheduleActualHoursCanonical` を導入し、Raw append-only + Canonical winner選定 + Feature再集約へ責務分離。`ActualHoursImportOrchestratorService` で手動APIとスケジューラを統合。既存Rawからのバックフィルは [actual-hours-canonical-backfill.md](./docs/runbooks/actual-hours-canonical-backfill.md) を参照。実行順序は「コミット→CI→Deploy→バックフィル」。詳細は [KB-297](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#b第7段階実績工数csv連携--全体ランキング連携2026-03-10) / [deploy-status-recovery.md](./docs/runbooks/deploy-status-recovery.md) を参照。
