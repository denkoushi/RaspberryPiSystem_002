...
- [ ] (2025-11-25) **課題**: 実環境（ラズパイ5/4）で以下の不具合が発生している。
  - **429エラー**: ダッシュボード・履歴ページで429エラーが発生。レート制限を無効化（max=100000）したが解決していない。
  - **404エラー**: ダッシュボード・履歴ページで404エラーが発生。ルーティング不一致の可能性。
  - **削除機能**: 返却済みの貸出記録があっても削除できない。1件だけ削除できたが、他の従業員・アイテムは削除できない。
  - **インポート機能**: USBメモリからのCSVインポートでP2002エラー（nfcTagUidの重複）が発生。エラーメッセージは改善されたが、根本原因は解決していない。
- [ ] (2025-11-25) **課題**: GitHub Actions CIテストが直近50件くらい全て失敗している。ローカルでは84テストが成功するが、CI環境では失敗している。テストの実効性に問題がある。
- [x] (2025-11-25) **Phase 1: 429エラー・404エラーの根本原因特定と修正**（最優先・他の機能にも影響）
  ...
  - **根本原因**: ✅ レート制限プラグインが3箇所で重複登録されていたことが判明（`app.ts`, `routes/index.ts`, `routes/tools/index.ts`）
  - **手順1**: ✅ レート制限プラグインの動作確認完了。サブルーターの`config`が親アプリのプラグインで認識されていないことを確認。
  - **手順2**: ✅ ルーティングの確認完了。フロントエンドとバックエンドのエンドポイントは一致していることを確認。
  - **手順3**: ✅ 修正実装完了。レート制限プラグインの重複登録を解消（`app.ts`と`routes/tools/index.ts`から削除、`routes/index.ts`のみで登録）。
  - **ナレッジベース**: ✅ `docs/knowledge-base/troubleshooting-knowledge.md`に試行内容を記録。
  - **検証**: ⏳ 実環境での検証待ち（ダッシュボード・履歴ページで429エラー・404エラーが発生しないことを確認）
- [ ] (2025-11-25) **Phase 2: 削除機能の修正**（独立・データベース制約の問題）
  - **現状**: 返却済みの貸出記録があっても削除できない。1件だけ削除できたが、他の従業員・アイテムは削除できない。データベースの外部キー制約が正しく適用されていない可能性。
  - **手順1**: データベース制約の確認（`ON DELETE SET NULL`が正しく適用されているか、マイグレーションが正しく適用されているか）
  - **手順2**: 削除ロジックの確認（削除前にチェックしている条件、エラーハンドリング）
  - **手順3**: 修正実装（データベース制約が正しく適用されていない場合はマイグレーションを再適用、削除ロジックを修正）
  - **検証**: 返却済み貸出記録がある従業員・アイテムを削除できることを確認
- [ ] (2025-11-25) **Phase 3: インポート機能の修正**（独立・データ整合性の問題）
  - **現状**: USBメモリからのCSVインポートでP2002エラー（nfcTagUidの重複）が発生。エラーメッセージは改善されたが、根本原因は解決していない。nfcTagUidの重複チェックが正しく動作していない可能性。
  - **手順1**: 重複チェックロジックの確認（CSV内の重複チェック、既存データとの重複チェック、クロスエンティティ（従業員とアイテム間）の重複チェック）
  - **手順2**: 修正実装（重複チェックロジックを修正、エラーメッセージを改善）
  - **検証**: nfcTagUidが重複しているCSVをインポートした場合、適切なエラーメッセージが表示されることを確認
- [ ] (2025-11-25) **Phase 4: CIテストの修正**（独立・テスト環境の問題）
  - **現状**: GitHub Actions CIテストが直近50件くらい全て失敗している。ローカルでは84テストが成功するが、CI環境では失敗している。テストの実効性に問題がある。
  - **手順1**: CIテストの失敗原因を特定（ローカルとCI環境の差異、テストの実効性）
  - **手順2**: 修正実装（CI環境の設定を修正、テストを修正）
  - **検証**: GitHub Actions CIテストが正常に動作することを確認
- [ ] (2025-11-25) **次のマイルストーン**: Validation 6（オフライン耐性）とValidation 7（USB一括登録）を実施（Milestone 5の残タスク）。
