...
### 部品測定テンプレ `templateScope` / `candidateFhinmei` 運用フォロー（2026-04-04）

**概要**: [KB-320](./docs/knowledge-base/KB-320-kiosk-part-measurement.md) Phase12 節の **Detach Run ID**・**Phase12 43 基準**を正とする。Pi3 は今回デプロイ対象外（機能は Pi5 API + Pi4 キオスク）。

**候補タスク**:

1. **現場目視（推奨）**: `/kiosk/part-measurement/template/pick` で **`two_key_fhincd_resource` / `one_key_fhinmei`** 表示・**`clone-for-schedule-key` 後**の記録開始・管理 `/admin/tools/part-measurement-templates` のスコープ欄。
2. **トラブルシュート**: 候補が空／migrate 失敗 → Pi5 の `prisma migrate status`・API ログ・[KB-320](./docs/knowledge-base/KB-320-kiosk-part-measurement.md) の図面・api `Created` 節。
