...
- [ ] (2025-11-25) **次のタスク: 非機能要件の実機検証と運用マニュアル作成**（優先度: 高、Phase 5完了後に実施）
  ...
  - **作業内容**:
    ...
    - **【CI検証】タスク2-1**: 🔄 監視・アラート機能のCIテスト（CI実行結果確認中）
      - `scripts/test/monitor.test.sh`がCIワークフローで`pnpm test:monitor`として実行されている
      - CI #215〜#218のログを確認して、監視テストが実行されて成功したか検証が必要
    - **【CI検証】タスク3-1**: 🔄 パフォーマンステストのCI追加（CI実行結果確認中）
      - `apps/api/src/routes/__tests__/performance.test.ts`が`pnpm test`で統合テストの一部として実行されるはず
      - CI #215〜#218のログを確認して、パフォーマンステストが実行されて成功したか検証が必要
