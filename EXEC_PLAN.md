...
- [ ] (2025-11-28) **Milestone 7: デジタルサイネージ機能の実装**（新規機能）
  ...
  - **機能要件**:
    - 工具管理データ（サムネイル付き）の表示
    - PDFファイルの表示（スライドショー形式または1ページ表示形式）
    - スケジュール自動切り替え（曜日・時間帯・優先順位）
    - 手動切り替え
    - 表示パターン（工具管理データのみ、PDFのみ、左右分割表示）
    - 緊急表示機能（最優先表示）
  - **技術要件**:
    - クライアント端末: Raspberry Pi 3/ZERO2W（Raspberry Pi OS、Chromiumキオスクモード）
    - データ取得: ポーリング方式（30秒〜1分間隔）
    - 設定画面: ラズパイ5の管理画面に統合
    - モニター: 1920x1080（Full HD）を基準に設計
  - **実装計画**:
...
**注意事項**:
- PDFページ生成機能（サーバー側でPDFを画像に変換）は未実装（Phase 4で実装予定）
