...
- [x] (2026-03-10) **納期管理 B第7段階（実績工数CSV連携 + 全体ランキング連携）実装完了**: `ProductionScheduleActualHoursRaw` / `ProductionScheduleActualHoursFeature` テーブルを追加し、`productionActualHours` ターゲットで Gmail CSV 取込をスケジューラに統合。取込後に `FHINCD × FSIGENCD` で中央値・件数・p75 を再集約（除外条件: 直近30日、0工数、外れ値）し、`due-management-scoring.service.ts` に `actualHoursScore` を追加して全体ランキングへ反映。手動検証導線として `POST /api/kiosk/production-schedule/due-management/actual-hours/import` と `GET /api/kiosk/production-schedule/due-management/actual-hours/stats` を追加。検証: `apps/api` lint 成功、実績工数 import/aggregate ユニットテスト追加・成功。
