...
### 配膳スマホ運用フォロー（V2 既定・V4 登録済み棚 UI 含む・2026-04-11 本番反映後）
...
1. **Android 実機（V2 + V4）**: Tailscale・`clientKey` 付きで `/kiosk/mobile-placement` を開き、**照合 OK/NG**・**仮棚 + 製造order登録**（**404 `ORDER_PLACEMENT_SCHEDULE_NOT_FOUND`** の逆テストを含む）・**登録済み棚フィルタ／候補選択**（履歴投入後に候補が増えること）。
