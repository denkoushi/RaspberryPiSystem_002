...
- [x] (2026-04-09) **キオスク「集計」: DADS 準拠 UI・ヘッダ「集計」・持出返却アイテムタブ（`GET /api/tools/items/loan-analytics`）・ViewModel リファクタ・本番 Pi5 のみデプロイ・Phase12・ドキュメント・`main` 同期**: 実装は `main`（マイグレーションなし・アイテム Loan は `itemId` のみ・吊具/計測器列は NULL 条件）。**デプロイ**: [deployment.md](./docs/guides/deployment.md)・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh main infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**: `20260409-213409-15007`（**`PLAY RECAP` `failed=0` / `unreachable=0`**・リモート exit **`0`**・所要約 **17 分**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **108s**・Mac / Tailscale）。**スモーク**: `GET /api/tools/items/loan-analytics` + kiosk `x-client-key` → **200**。**Pi4 Ansible**: 不要（SPA は Pi5 `web`）。**ナレッジ**: [KB-334](./docs/knowledge-base/KB-334-kiosk-rigging-loan-analytics-deploy.md)。**ドキュメントコミット**: 本セッションで KB-334 / INDEX / knowledge-base 索引 / 本ファイルを更新 → **`main` にマージ・push**、GitHub Actions 成功を確認。
