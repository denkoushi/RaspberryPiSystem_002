...
- [ ] (2025-11-25) **課題**: 実環境（ラズパイ5/4）で以下の不具合が発生している。**ナレッジベース**: [KB-001](#kb-001-429エラーレート制限エラーが発生する), [KB-002](#kb-002-404エラーが発生する), [KB-003](#kb-003-p2002エラーnfctaguidの重複が発生する), [KB-004](#kb-004-削除機能が動作しない)
  - **429エラー** ([KB-001](docs/knowledge-base/troubleshooting-knowledge.md#kb-001-429エラーレート制限エラーが発生する)): ダッシュボード・履歴ページで429エラーが発生。レート制限を無効化（max=100000）したが解決していない。
  - **404エラー** ([KB-002](docs/knowledge-base/troubleshooting-knowledge.md#kb-002-404エラーが発生する)): ダッシュボード・履歴ページで404エラーが発生。ルーティング不一致の可能性。
...
- [ ] (2025-11-25) **課題**: GitHub Actions CIテストが直近50件くらい全て失敗している。**ナレッジベース**: [KB-005](#kb-005-ciテストが失敗する)
...
- [x] (2025-11-25) **Phase 1: 429エラー・404エラーの根本原因特定と修正**（最優先・削除機能とインポート機能を動作させるための前提条件）**ナレッジベース**: [KB-001](#kb-001-429エラーレート制限エラーが発生する), [KB-002](#kb-002-404エラーが発生する)
  ...
  - **手順4**: ✅ `allowList`関数を実装。特定のパス（ダッシュボード・履歴ページ・キオスクエンドポイント）をレート制限から除外する実装を追加。→ [KB-001](docs/knowledge-base/troubleshooting-knowledge.md#kb-001-429エラーレート制限エラーが発生する)に記録
  - **ナレッジベース**: ✅ `docs/knowledge-base/troubleshooting-knowledge.md`に試行内容を記録（KB-001, KB-002）。
  - **検証**: ⏳ 実環境での検証待ち（**削除機能とインポート機能が動作することを確認**。429エラー・404エラーが発生しないことを確認）
- [ ] (2025-11-25) **Phase 2: 削除機能の修正**（Phase 1完了後に実施・データベース制約の問題）**ナレッジベース**: [KB-004](#kb-004-削除機能が動作しない)
  ...
  - **手順1**: Phase 1完了後、削除機能が動作するか確認。動作しない場合のみ以下を実施。
  - **手順2**: データベース制約の確認（`ON DELETE SET NULL`が正しく適用されているか、マイグレーションが正しく適用されているか）
  - **手順3**: 削除ロジックの確認（削除前にチェックしている条件、エラーハンドリング）
  - **手順4**: 修正実装（データベース制約が正しく適用されていない場合はマイグレーションを再適用、削除ロジックを修正）
  - **検証**: 返却済み貸出記録がある従業員・アイテムを削除できることを確認
- [ ] (2025-11-25) **Phase 3: インポート機能の修正**（Phase 1完了後に実施・データ整合性の問題）**ナレッジベース**: [KB-003](#kb-003-p2002エラーnfctaguidの重複が発生する)
  ...
  - **手順1**: Phase 1完了後、インポート機能が動作するか確認。動作しない場合のみ以下を実施。
  - **手順2**: 重複チェックロジックの確認（CSV内の重複チェック、既存データとの重複チェック、クロスエンティティ（従業員とアイテム間）の重複チェック）
  - **手順3**: 修正実装（重複チェックロジックを修正、エラーメッセージを改善）
  - **検証**: nfcTagUidが重複しているCSVをインポートした場合、適切なエラーメッセージが表示されることを確認。正常なCSVをインポートできることを確認。
- [ ] (2025-11-25) **Phase 4: CIテストの修正**（独立・テスト環境の問題）**ナレッジベース**: [KB-005](#kb-005-ciテストが失敗する)
