...
- [x] (2026-04-04) **部品測定テンプレ: 登録スコープ（`THREE_KEY` / `FHINCD_RESOURCE` / `FHINMEI_ONLY`）・`candidateFhinmei`・候補 matchKind 整理・`feat/part-measurement-template-scope`（実装・テスト・ドキュメント。デプロイ・`main` マージは別途）**: Prisma マイグレーション `20260404100000_part_measurement_template_scope`（`PartMeasurementTemplateScope`・`CANDIDATE_FHINCD_RESOURCE` / `CANDIDATE_FHINMEI_ONLY`）。候補APIはスコープ別クエリ＋`classifyCandidateMatch`。複製は非正本でも工程不一致で落とさない。Web は管理／キオスク新規登録でスコープ選択。**参照**: [ADR-20260404](./docs/decisions/ADR-20260404-part-measurement-template-pick-kiosk.md) / [KB-320](./docs/knowledge-base/KB-320-kiosk-part-measurement.md) / [verification-checklist.md](./docs/guides/verification-checklist.md) §6.6.9。
