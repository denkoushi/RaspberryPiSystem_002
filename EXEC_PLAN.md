...
- [x] (2026-03-23) **実績基準時間 推定式見直し（`p75優先`→`縮小中央値`）・共通読取コンテキスト導入・回帰テスト追加・ドキュメント反映完了**: API のみ。`actual-hours` 読取を [`actual-hours-read-context.service.ts`](./apps/api/src/services/production-schedule/actual-hours/actual-hours-read-context.service.ts) へ集約し、`production-schedule-query` / `due-management-query` / `due-management-scoring` の解決経路を統一。`actual-hours-feature-resolver` は戦略差し替え可能化（既定 `shrinkedMedianV1`, 互換 `legacyP75`）し、`w = n/(n+3)` による縮小中央値を適用。Feature 集約は lookback 365日を既定化。**検証**: `pnpm --filter @raspi-system/api lint` / `build`、`actual-hours-feature-resolver.service.test.ts` / `production-schedule-query.service.test.ts` / `due-management-scoring.service.test.ts` が全件成功。**ドキュメント**: [KB-297 推定式見直し節](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#実績基準時間-推定式見直し2026-03-23) / [ADR-20260323-actual-hours-baseline-estimation](./docs/decisions/ADR-20260323-actual-hours-baseline-estimation.md) / [docs/INDEX.md](./docs/INDEX.md) / [docs/knowledge-base/index.md](./docs/knowledge-base/index.md) を更新。
