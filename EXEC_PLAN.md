...
- [ ] (2026-05-04) **キオスク順位ボード・製番順評価モード（端末ローカルのみ）**·ブランチ **`feat/kiosk-seiban-priority-eval-mode`**（**未 push / 未コミット指示**）。**仕様**: 左ペイン **製番順評価 ON/OFF** と **登録製番の上下移動**（共有 `sharedHistory` は変更せず、`localStorage` は **工場 + 対象端末スコープ**）。**ON 時のみ** 各資源列内の行ソートを **製番ローカル順優先** → 既存 **`processingOrder`** 優先ロジックへ委譲。**検証**: `apps/web` Vitest（`buildLeaderBoardViewModel` / `sortLeaderBoardRowsForSeibanEvalDisplay` / `seibanPriority.pure`）**11 tests passed**。
