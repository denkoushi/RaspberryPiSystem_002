...
- [x] (2026-05-04) **Zero2W 担当棚設定（Android キオスク専用ページ・対象 Zero2W 指定 API）**·ブランチ **`feat/mobile-placement-zero2w-assignment`**·代表 **`153af161`**。**仕様**: `/kiosk/mobile-placement` 上辺に **4つ目ボタン** **`Zero2W担当棚`** を追加し、**`/kiosk/mobile-placement/zero2w-assignment`** で **Zero2W 候補端末**へ **棚マスタの登録済み構造化棚**を割り当てる。**API**: `GET /api/mobile-placement/haizen-target-devices`・`PUT /api/mobile-placement/haizen-target-devices/:clientDeviceId/preset-shelf`（応答の候補一覧は **`apiKey` を含めない**。誤設定抑制のため **対象 Zero2W を明示指定**）。**既存互換**: self 用 `PATCH /api/mobile-placement/haizen-preset-shelf` は維持、Top の **「棚番配膳（Zero2W）」** パネルは表示専用のまま。**単体/CI**: Vitest・integration・smoke。**本番デプロイ（Pi5 のみ）**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/mobile-placement-zero2w-assignment infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**: **`20260504-183939-27983`**（**`PLAY RECAP` `ok=134` `changed=4` `failed=0` / `unreachable=0`**・exit **`0`**・`--follow` 約 **721s**）。Pi4／Pi3 play は **no hosts matched**（**意図どおり・Pi3 専用手順は不要**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **95s**）。**`main` 取り込み後はブランチ引数を `main`**。**ナレッジ**: [KB-368](./docs/knowledge-base/KB-368-zero2w-haizen-placement-tracking.md)·[deployment.md](./docs/guides/deployment.md)（2026-05-04 evening 項）·[mobile-placement-smartphone.md](./docs/runbooks/mobile-placement-smartphone.md)。
