...
### LocalLLM オンデマンド・運用フォロー（2026-03-30）

**概要**: 実装は `main` へ統合後、Pi5+Pi4×4 へ順次載せ替え済みなら `./scripts/deploy/verify-phase12-real.sh` で回帰確認（**PASS 37 / WARN 0 / FAIL 0** 実測・約 100s）。**本番既定は `LOCAL_LLM_RUNTIME_MODE=always_on`** のため、VRAM 解放は **`on_demand`** と Ubuntu 側制御層の配線後に初めて効く。

**候補タスク**:

1. **Ubuntu**: [control-server.mjs](./scripts/ubuntu-local-llm-runtime/control-server.mjs) を配置し systemd 化、nginx で `POST /start`・`POST /stop` を Tailnet 経由で Pi5（`tag:server`）からのみ到達可能にする。Compose の `llama-server` 初期状態は Runbook（[local-llm-tailscale-sidecar.md](./docs/runbooks/local-llm-tailscale-sidecar.md)）と整合させる。
2. **Pi5（Ansible `docker.env.j2` / vault）**: 運用で切り替えるときのみ `LOCAL_LLM_RUNTIME_MODE=on_demand` と `LOCAL_LLM_RUNTIME_CONTROL_START_URL` / `STOP_URL` / `TOKEN`（および必要なら `HEALTH_BASE_URL`）を設定。未設定時はコードが **no-op にフォールバック**し警告ログ（upstream 常駐前提）。
3. **実働確認**: 写真持出 1 件の前後で Pi5 ログの **`component: localLlmRuntimeControl`** と **`component: inference`**（`useCase: photo_label`）、Ubuntu の **`nvidia-smi`**（`/app/llama-server` の有無）を確認。ComfyUI OOM の背景は [KB-319](./docs/knowledge-base/KB-319-photo-loan-vlm-tool-label.md)。
