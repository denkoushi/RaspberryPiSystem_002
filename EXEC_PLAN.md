...
**概要**: 代理エンドポイント・Ubuntu sidecar・Ansible 配線（[KB-318](./docs/knowledge-base/infrastructure/ansible-deployment.md#kb-318-pi5-local-llm-via-docker-env)）・本番疎通まで完了（[KB-317](./docs/knowledge-base/infrastructure/security.md#kb-317-ubuntu-localllm-を-tailscale-sidecar--tagllm-で分離公開する)）。

**候補タスク**:
1. **管理コンソール UI**: `ADMIN` / `MANAGER` が status・試用チャットを行える画面（レート制限・監査は ADR で方針固定推奨）。
2. **可観測性**: upstream レイテンシ・失敗率のメトリクス、または構造化ログの基準化。
3. **運用ポリシー**: 共有トークン再発行手順、キオスク等からの直叩き禁止の明示（現状は管理系 API 想定）。

**参照**: [local-llm-tailscale-sidecar.md](./docs/runbooks/local-llm-tailscale-sidecar.md) / [ADR-20260328](./docs/decisions/ADR-20260328-ubuntu-local-llm-tailnet-sidecar.md)
