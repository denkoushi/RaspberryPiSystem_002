...
- [ ] (2025-11-25) **次のマイルストーン**: 実環境（ラズパイ5/4）での動作確認を実施し、実際にどの機能が動作していないかを特定する。429エラー、404エラー、削除機能、インポート機能の問題が解決されているかを検証する。
- [ ] (2025-11-25) **次のマイルストーン**: GitHub Actions CIテストの失敗原因を特定し、修正する。ローカルでは84テストが成功するが、CI環境では失敗している原因を調査する。
