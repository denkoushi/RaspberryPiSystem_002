...
### キオスク部品測定記録・ロールアウト残（2026-03-29）

**概要**: `part-measurement` は Pi5 + `raspi4-kensaku-stonebase01` でカナリア反映済み。`./scripts/deploy/verify-phase12-real.sh` に `resolve-ticket` スモークを組み込み済み（**PASS 37 / WARN 0 / FAIL 0** 基準）。

**候補タスク**:

1. **残りキオスク Pi4**: [deployment.md](./docs/guides/deployment.md) に従い `raspberrypi4` / `raspi4-robodrill01` / `raspi4-fjv60-80` を **1 台ずつ** `--limit` でデプロイする（**Pi3** はリソース僅少のため専用手順のみ）。
2. **運用**: 品番×工程のテンプレ先行登録、現場で `/kiosk/part-measurement` の実票スキャン・確定まで目視確認（[kiosk-part-measurement.md](./docs/runbooks/kiosk-part-measurement.md)）。
3. **任意**: 確定データの帳票化・CSV エクスポート等の要件が出たら別 ADR / ExecPlan で切り出す。

**参照**: [KB-320](./docs/knowledge-base/KB-320-kiosk-part-measurement.md) / [ADR-20260329](./docs/decisions/ADR-20260329-part-measurement-kiosk-record.md) / [verification-checklist.md](./docs/guides/verification-checklist.md) 6.6.9
