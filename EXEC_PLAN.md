...
### DGX Spark LocalLLM: 一区切り後の将来課題（2026-04-27 追補: 本番制御層反映・ACL 手順の文書化後）

**概要**: DGX `system-prod-primary` の単一 endpoint（green: llama.cpp + mmproj）で text + image を扱う構造、Pi5 本番の Spark 一本化、`active assist` による改善、Phase12 回帰確認までは完了した。加えて **blue（`vLLM` + `Qwen3.6-27B-NVFP4`）** は repo 上で **到達**しており、**cold start が長大**（実測 ~12 分規模）なこと、Pi5 側 **ready timeout 上限 / keep-warm / `reasoning` フォールバック** まで**底堅化**済み。2026-04-27: **`runtime_stop_policy` 分離版 `control-server.py` の本番投入**、Tailscale **一時 `tcp:22` の閉じ戻し**、到達経路・トラブルシュートを [KB-357](./docs/knowledge-base/infrastructure/security.md) / [Runbook](./docs/runbooks/dgx-system-prod-local-llm.md) / [tailscale-policy.md](./docs/security/tailscale-policy.md) に集約。**未完了**: `ACTIVE_LLM_BACKEND=blue` を**本番既定**にするかの**運用判断**、Pi5/Ansible への**正規 deploy 収束**、一部 **VLM/画像 400** の**残チューニング**（[ExecPlan](./docs/plans/dgx-spark-local-llm-migration-execplan.md#immediate-next-steps) の Immediate Next Steps 参照）。現時点では `photo_label` の生の初期判定をさらに追い込むことは優先しない。
...
6. **リポジトリと Pi5 の収束**: 関連差分の **`main` マージ**後、Pi5 を **`update-all-clients` 等で正規追従**し、手動 `rsync` 例外を減らす（[deployment.md](./docs/guides/deployment.md)）。
