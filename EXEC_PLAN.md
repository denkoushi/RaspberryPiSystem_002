...
- [x] (2026-03-22) **進捗一覧 5列グリッド・「納期」「実」ラベル削除・`progressOverviewPresentation` / `ProgressOverviewSeibanCard` / `ProgressOverviewPartRow` 分割・デプロイ（Pi5・raspberrypi4）・実機検証（部分）・ドキュメント反映・main マージ**: Web のみ（API 不変）。`PROGRESS_OVERVIEW_CARD_GRID_CLASS` で `xl:grid-cols-5`、部品名・資源CD列に `min-w-0`。**デプロイ**: ブランチ `feat/kiosk-progress-overview-five-cols-layout`、`RASPI_SERVER_HOST`・`--limit` 1台ずつ・`--detach --follow`。**Run ID**: `20260322-084633-25240`（Pi5）/ `20260322-085040-1342`（raspberrypi4）。**raspi4-robodrill01**: SSH `100.123.1.113` タイムアウトのため **未デプロイ**（復旧後に `--limit "raspi4-robodrill01"` で再実行）。**実機検証**: `./scripts/deploy/verify-phase12-real.sh` **PASS 26 / WARN 1 / FAIL 1**（FAIL=RoboDrill Pi4 SSH、WARN=Pi3 offline 可）。**ローカル**: `NODE_ENV=test POSTGRES_PORT=5432 pnpm test:api` 推奨（`NODE_ENV=production` 残留・`POSTGRES_PORT` 自動切替に注意）。**参照**: [KB-297 progress-overview-five-cols](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#progress-overview-five-cols-layout-2026-03-22) / [deploy-status-recovery.md](./docs/runbooks/deploy-status-recovery.md)。
