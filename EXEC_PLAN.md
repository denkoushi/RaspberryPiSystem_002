...
- [x] (2026-03-19) **手動順番 deviceScopeKey 正規化（siteKey / Mac targetDeviceScopeKey / overview devices[]）実装**: `ProductionScheduleOrderAssignment.siteKey` 追加マイグレーション、`KIOSK_MANUAL_ORDER_DEVICE_SCOPE_V2_ENABLED` で v1/v2 切替、Mac 代理は `targetDeviceScopeKey` 必須・登録端末検証、overview は `siteKey` + `devices[]` + `__legacy_site__`、`GET .../search-state` に `locationScope` 追加、Web は `VITE_KIOSK_MANUAL_ORDER_DEVICE_SCOPE_V2_ENABLED` で整合。**デプロイ・実機検証**は既存 runbook 手順で実施。ADR: [ADR-20260319-manual-order-device-scope-v2](./docs/decisions/ADR-20260319-manual-order-device-scope-v2.md)。
