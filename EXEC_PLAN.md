...
**情報の分類軸**:
- **軸1: 情報の性質（What）**: アーキテクチャ、機能仕様、運用、開発、トラブルシューティング
- **軸2: 情報のライフサイクル（When/How）**: 静的（変更が少ない）、動的（頻繁に更新）、参照（参照のみ）
- **軸3: 情報の参照頻度（Who/Why）**: 高頻度、中頻度、低頻度
...
**エントロピー増加を防ぐルール**:
1. **ファイルサイズ制限**: 1ファイルは300行以内を推奨、300行を超える場合は分割を検討
2. **重複排除**: 同じ情報は1箇所にのみ記載、他のドキュメントからは参照リンクで参照
3. **命名規則**: `README.md`（ディレクトリ概要）、`overview.md`（カテゴリ全体の概要）、`index.md`（索引・目次）
4. **更新頻度の管理**: Frontmatterで`last-verified`を記録、6ヶ月以上更新されていないドキュメントはレビュー対象
5. **索引の維持**: `INDEX.md`は常に最新の状態を維持、新しいドキュメント追加時は必ず`INDEX.md`を更新

**推奨アクション（優先順位）**:
1. **INDEX.md作成**: 低コスト・高効果、今すぐ実施可能
2. **ナレッジベース分割**: 964行は既に長い、分割効果大
3. **Frontmatter導入**: 新規作成時から適用、段階的に展開
4. **既存ドキュメントの整理**: 重複排除、参照リンク化
