...
- [ ] (2025-11-25) **次のマイルストーン**: 上記の不具合を修正する。
  - 429エラーの根本原因を特定し、レート制限設定を修正する。
  - 404エラーの原因を特定し、ルーティングを修正する。
  - 削除機能の不具合を修正し、返却済み貸出記録があっても削除できるようにする。
  - インポート機能のP2002エラーを修正し、nfcTagUidの重複チェックを正しく動作させる。
- [ ] (2025-11-25) **次のマイルストーン**: GitHub Actions CIテストの失敗原因を特定し、修正する。テストの実効性を向上させる。
