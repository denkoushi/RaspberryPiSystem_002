...
- [x] (2026-05-04) **Zero2W 担当棚設定（Android キオスク専用ページ・対象 Zero2W 指定 API）**·ブランチ **`feat/mobile-placement-zero2w-assignment`**。**仕様**: `/kiosk/mobile-placement` 上辺に **4つ目ボタン** **`Zero2W担当棚`** を追加し、**`/kiosk/mobile-placement/zero2w-assignment`** で **Zero2W 候補端末**へ **棚マスタの登録済み構造化棚**を割り当てる。**API**: `GET /api/mobile-placement/haizen-target-devices`・`PUT /api/mobile-placement/haizen-target-devices/:clientDeviceId/preset-shelf`。**既存互換**: self 用 `PATCH /api/mobile-placement/haizen-preset-shelf` は維持、Top の **「棚番配膳（Zero2W）」** パネルは表示専用のまま。**検証**: API/Vitest・キオスク smoke を追加。**運用**: コミット / プッシュ / デプロイは未実施。
