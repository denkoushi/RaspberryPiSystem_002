...
- [ ] (2025-12-08) **実機検証準備完了**: featureブランチでのデプロイ手順を検証手順書に追加。デプロイスクリプト（`scripts/server/deploy.sh`）でfeatureブランチ指定可能であることを確認。シードデータに計測機器テストデータ（MI-001, MI-002, MI-003）が含まれていることを確認。**次のステップ**: ラズパイ5でfeatureブランチをデプロイし、シードデータ投入後に実機検証を実施。
