...
3. **Android 実機**: Chrome で **`/kiosk/purchase-order-lookup`** を開き、**バーコードスキャン**と **`x-client-key`** 付き API 応答を目視。
