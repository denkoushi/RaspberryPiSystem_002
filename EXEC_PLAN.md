...
- [x] (2026-05-06) **キオスク順位ボード `responseProfile=leaderboard`・可視行 COUNT と行 SELECT の並列化（内部レイテンシ・仕様同一）**·ブランチ **`fix/leaderboard-internal-query-latency`**·実装代表 **`35629338`**。**実装**: [`production-schedule-list-count.service.ts`](./apps/api/src/services/production-schedule/production-schedule-list-count.service.ts)・[`production-schedule-query.service.ts`](./apps/api/src/services/production-schedule/production-schedule-query.service.ts)。**Vitest**: [`production-schedule-query.service.test.ts`](./apps/api/src/services/production-schedule/__tests__/production-schedule-query.service.test.ts)。**本番**: **`raspberrypi5` のみ**（`--limit raspberrypi5`·**1 台**）。Pi4／Pi3 play **no hosts matched**（**Pi3 専用手順不要**）。**標準**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh fix/leaderboard-internal-query-latency infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`（**`main` 取り込み後はブランチ引数を `main`**）。**Detach Run ID**（接頭辞 `ansible-update-`）: **`20260506-103441-24679`**（**`PLAY RECAP` `ok=134` `changed=4` `failed=0` / `unreachable=0`**・リモート exit **`0`**・ローカル **`--follow` 約 649s**）。**実機**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（**約 80s**·Tailscale）。**トラブルシュート**: Pi5 **`api` コンテナ ref**・detach **`PLAY RECAP` 正本**・キオスク **強制リロード**（[verification-checklist.md](./docs/guides/verification-checklist.md) §6.6.4）。**ナレッジ**: [KB-369](./docs/knowledge-base/KB-369-leader-order-board-api-internal-latency.md)·[KB-297 §COUNT 並列化](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-api-count-parallel-2026-05-06)·[deployment.md](./docs/guides/deployment.md)（2026-05-06 leaderboard COUNT 並列化項）。**（`main` マージ後に本項へ PR 番号と squash 先端 SHA を追記すること）**
