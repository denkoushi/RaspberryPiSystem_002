...
- [x] (2026-04-13) **配膳スマホ V18（トップ登録アクション分離: 「新規登録」／「棚移動」・Web のみ・API/DB 変更なし）本番・Pi5→Pi4×4 順次（Pi3 除外）・Phase12・ドキュメント・`main` マージ（PR）**: ブランチ **`feat/mobile-placement-top-action-buttons`**・コミット **`91e485da`**（`useMobilePlacementPageState`・`MobilePlacementRegisterSection`・`types.ts`・`navigationState`）。**デプロイ**: [deployment.md](./docs/guides/deployment.md)・`export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・`./scripts/update-all-clients.sh feat/mobile-placement-top-action-buttons infrastructure/ansible/inventory.yml --limit <host> --detach --follow` を **1 台ずつ**。**Detach Run ID**: `20260413-112046-12716` → `20260413-112510-648` → `20260413-112925-12155` → `20260413-113234-2579` → `20260413-113642-29855`、各 **`failed=0` / `unreachable=0`**。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **29s**）。**ナレッジ**: [KB-339](./docs/knowledge-base/KB-339-mobile-placement-barcode-survey.md) V18・[mobile-placement-smartphone.md](./docs/runbooks/mobile-placement-smartphone.md) §0。**残作業（手動）**: Android で **新規登録／棚移動**の有効条件・送信中挙動を目視。
