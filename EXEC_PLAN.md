...
- [x] (2025-11-25) **Phase 1: 429エラー・404エラーの根本原因特定と修正**（最優先・他の機能にも影響）
  ...
  - **手順3**: ✅ 修正実装完了。レート制限プラグインをサブルーター内で登録することで、ルートの`config: { rateLimit: false }`が正しく認識されるように修正。`apps/api/src/routes/index.ts`と`apps/api/src/routes/tools/index.ts`でレート制限プラグインを登録。
