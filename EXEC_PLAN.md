...
- [x] (2026-03-19) **代理更新UI連携（案B）実装完了（未デプロイ）**: 生産スケジュール順番更新で `targetLocation` を送れるようにし、納期管理で選んだ対象拠点を共通状態として再利用可能化。`useKioskTargetLocation` を追加して `localStorage` 依存を画面から分離。納期管理画面の対象拠点セレクタは全端末表示へ変更。生産スケジュール画面では順番更新時に `targetLocation` を payload 注入し、非許可端末の `403 TARGET_LOCATION_FORBIDDEN` を画面内メッセージで明示。**検証**: `@raspi-system/web` lint/test/build 成功（66 tests passed）。**ドキュメント**: KB-297 / deploy-status-recovery を更新。
