...
- [x] (2026-05-07) **Mobile Placement Zero2W hardening（ブランチ `feat/mobile-placement-zero2w-hardening`・ユーザー指示によりコミット/プッシュは未実施）**: メイン **`/kiosk/mobile-placement`** から Zero2W 常設一覧を分離し **`/kiosk/mobile-placement/zero2w-status`**（ボタン「棚番配膳一覧（Zero2W）」）を追加。下半 **`MobilePlacementRegisterSection`** の **`flex-1 overflow-hidden`** を撤して **`KioskLayout` の `main` 縦スクロール**へ寄せる。**Prisma** **`ClientDevice.haizenEdgeEnabled`**（マイグレ **`20260507190000_client_device_haizen_edge_enabled`**・`apiKey`/`name` に `zero2w` を含む既存行をバックフィル）。**API**: `GET …/haizen-target-devices` は **`haizenEdgeEnabled === true` のみ**。**管理画面**: クライアント端末管理に **Zero2W配膳** チェック・**`PUT /api/clients/:id`** に **`haizenEdgeEnabled`**。**エージェント**: **`HAIZEN_DISTRIBUTION_MODE`**（既定 **`legacy_short_numeric`**／誤認抑止は **`prefixed_dist`** + **`DIST:<整数>`**）。**検証**: `apps/api` Vitest（haizen・telemetry）・`apps/web` Vitest smoke・`clients/haizen-agent` pytest。**ナレッジ**: [KB-368](./docs/knowledge-base/KB-368-zero2w-haizen-placement-tracking.md)·[mobile-placement-smartphone.md](./docs/runbooks/mobile-placement-smartphone.md)·[api/mobile-placement.md](./docs/api/mobile-placement.md)·[zero2w-tanaban-edge-setup.md](./docs/runbooks/zero2w-tanaban-edge-setup.md)。**運用上の残**: 本番 DB への **`prisma migrate deploy`**・Pi5 **API/Web デプロイ**・キオスク **強制リロード**（[verification-checklist.md](./docs/guides/verification-checklist.md) §6.6.4）。

- [x] (2026-05-06) **生産日程CSV DEDUP 取込の空 winner ガード（誤った一括消滅完了の防止）**·**Web `axios` ^1.16.0（Trivy fs HIGH 回避）**·ブランチ **`fix/schedule-csv-empty-guard`**·**`main`**: [PR #264](https://github.com/denkoushi/RaspberryPiSystem_002/pull/264) **squash merge**・**`main` 先端** **`f9b1683e`** を正とする。**実装**: [`production-schedule-csv-ingest-external-completion-sync.service.ts`](./apps/api/src/services/production-schedule/external-completion/production-schedule-csv-ingest-external-completion-sync.service.ts)·[`csv-dashboard-ingestor.ts`](./apps/api/src/services/csv-dashboard/csv-dashboard-ingestor.ts)·[`apps/web/package.json`](./apps/web/package.json)·Vitest [`production-schedule-csv-ingest-external-completion-sync.service.test.ts`](./apps/api/src/services/production-schedule/external-completion/__tests__/production-schedule-csv-ingest-external-completion-sync.service.test.ts)。**仕様**: 現 winner **論理キー 0 件**時は **CSV 由来の消滅差分・外部完了同期・スナップショット更新をスキップ**（`empty_schedule_csv`）·ingestor **warn**。**本番**: **`raspberrypi5` のみ**（`--limit raspberrypi5`）。**Detach Run ID** **`20260506-171017-29269`**（**`PLAY RECAP` `ok=134` `changed=4` `failed=0` / `unreachable=0`**・リモート **`exit` `0`**・ローカル **`--follow` 約 18 分規模**）。**実機**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（**約 146s**）。**ナレッジ**: [KB-370](./docs/knowledge-base/KB-370-production-schedule-external-completion-triple-source.md)·[deployment.md](./docs/guides/deployment.md)（2026-05-06 · 空 winner ガード 項）。

- [x] (2026-05-06) **生産スケジュール「実効完了」の3系統OR統合（手動・FKOJUNST メール消滅＋`C`/`P`/`X`/`O`・生産日程CSV winner 論理キー消滅）**·ブランチ **`feat/completion-triple-source-unification`**·**`main`**: [PR #263](https://github.com/denkoushi/RaspberryPiSystem_002/pull/263) **squash merge**・**`main` 先端** **`4af94e05`** を正とする（実装代表 **`2b8c8427`**）·**Prisma** **`20260506150000_triple_source_external_completion`**（**由来別3列**＋**`ProductionScheduleCsvIngestLogicalKeySnapshot`**）。**実装**: [`fkojunst-external-completion-sync.repository.ts`](./apps/api/src/services/production-schedule/external-completion/fkojunst-external-completion-sync.repository.ts)·[`production-schedule-csv-ingest-external-completion-sync.service.ts`](./apps/api/src/services/production-schedule/external-completion/production-schedule-csv-ingest-external-completion-sync.service.ts)·[`csv-dashboard-ingestor.ts`](./apps/api/src/services/csv-dashboard/csv-dashboard-ingestor.ts)·[`production-schedule-effective-completion.sql.ts`](./apps/api/src/services/production-schedule/production-schedule-effective-completion.sql.ts)。**ナレッジ**: [KB-370](./docs/knowledge-base/KB-370-production-schedule-external-completion-triple-source.md)·[deployment.md](./docs/guides/deployment.md)（2026-05-06 実効完了3系統OR 項）。**本番**: **`raspberrypi5` のみ**（`--limit raspberrypi5`）。Pi4／Pi3 play **no hosts matched**（**Pi3 専用手順不要**）。**Detach Run ID**（`ansible-update-`）: **`20260506-152049-17895`**（**`PLAY RECAP` `ok=134` `changed=4` `failed=0` / `unreachable=0`**・リモート **`exit` `0`**・ローカル **`--follow` 約 803s**）。**実機**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（**約 138s**）。**以降のデプロイ**: **`main`**。

- [x] (2026-05-06) **キオスク順位ボード `responseProfile=leaderboard`・可視行 COUNT と行 SELECT の並列化（内部レイテンシ・仕様同一）**·ブランチ **`fix/leaderboard-internal-query-latency`**·実装代表 **`35629338`**（**`main`**: [PR #261](https://github.com/denkoushi/RaspberryPiSystem_002/pull/261) **squash merge**・**`main` 先端** **`ac12066e`** を正とする）。**実装**: [`production-schedule-list-count.service.ts`](./apps/api/src/services/production-schedule/production-schedule-list-count.service.ts)・[`production-schedule-query.service.ts`](./apps/api/src/services/production-schedule/production-schedule-query.service.ts)。**Vitest**: [`production-schedule-query.service.test.ts`](./apps/api/src/services/production-schedule/__tests__/production-schedule-query.service.test.ts)。**本番**: **`raspberrypi5` のみ**（`--limit raspberrypi5`·**1 台**）。Pi4／Pi3 play **no hosts matched**（**Pi3 専用手順不要**）。**標準**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh main infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`（**以降の標準デプロイは `main`**）。**Detach Run ID**（接頭辞 `ansible-update-`）: **`20260506-103441-24679`**（**`PLAY RECAP` `ok=134` `changed=4` `failed=0` / `unreachable=0`**・リモート exit **`0`**・ローカル **`--follow` 約 649s**）。**実機**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（**約 80s**·Tailscale）。**トラブルシュート**: Pi5 **`api` コンテナ ref**・detach **`PLAY RECAP` 正本**・キオスク **強制リロード**（[verification-checklist.md](./docs/guides/verification-checklist.md) §6.6.4）。**ナレッジ**: [KB-369](./docs/knowledge-base/KB-369-leader-order-board-api-internal-latency.md)·[KB-297 §COUNT 並列化](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-api-count-parallel-2026-05-06)·[deployment.md](./docs/guides/deployment.md)（2026-05-06 leaderboard COUNT 並列化項）。

- [x] (2026-05-06) **キオスク順位ボード・段階取得（leaderboard-shell／leaderboard-total／leaderboard-decorations）・初回体感短縮（API+Web・並び再利用・再ソートしないマージ）**·ブランチ **`feat/leaderboard-phased-fetch-2s`**·実装代表 **`cd751a2a`**（**`main`**: [PR #262](https://github.com/denkoushi/RaspberryPiSystem_002/pull/262) **squash merge**・**`main` 先端**は **マージ完了後の `origin/main` HEAD** を正とする）。**実装**: [`leaderboard-phased-read.ts`](./apps/api/src/routes/kiosk/production-schedule/leaderboard-phased-read.ts)·[`leaderboard-shell-hydrate.service.ts`](./apps/api/src/services/production-schedule/leaderboard/leaderboard-shell-hydrate.service.ts)·[`production-schedule-query.service.ts`](./apps/api/src/services/production-schedule/production-schedule-query.service.ts)·Web [`ProductionScheduleLeaderOrderBoardPage.tsx`](./apps/web/src/pages/kiosk/ProductionScheduleLeaderOrderBoardPage.tsx)。**Vitest**: [`kiosk-production-schedule.integration.test.ts`](./apps/api/src/routes/__tests__/kiosk-production-schedule.integration.test.ts)（phased）。**本番**: **`raspberrypi5` のみ**。**Detach Run ID** **`20260506-113443-32585`**（**`PLAY RECAP` `ok=134` `changed=4` `failed=0` / `unreachable=0`**・exit **`0`**・`**--follow` 約 849s**）。**実機**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（**約 74s**）。**知見**: 装飾 hydrate の **`Prisma.join`**・**UUID 順序は `text` + `array_position`**・**Fkojunst 可視 WHERE の前置 `AND` 増殖防止** → [KB-369](./docs/knowledge-base/KB-369-leader-order-board-api-internal-latency.md)。**ナレッジ**: [KB-369](./docs/knowledge-base/KB-369-leader-order-board-api-internal-latency.md)·[KB-297 §段階取得](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-leaderboard-phased-fetch-2026-05-06)·[deployment.md](./docs/guides/deployment.md)（2026-05-06 段階取得項）。

- [x] (2026-05-06) **キオスク順位ボード・ProductNo winner の materialization（`leaderboard` / `leaderboard-shell`・相関除去・仕様同一）**·**`main`**: [PR #265](https://github.com/denkoushi/RaspberryPiSystem_002/pull/265) **squash merge**・**`main` 先端** **`ae5f938a`**（ブランチ **`fix/leaderboard-shell-winner-materialization`**・実装代表 **`b05baa5f`**）。**実装**: [`max-product-no-winner-spec.ts`](./apps/api/src/services/production-schedule/row-resolver/max-product-no-winner-spec.ts)·[`max-product-no-winner-materialization.ts`](./apps/api/src/services/production-schedule/row-resolver/max-product-no-winner-materialization.ts)·[`leaderboard-row-selection.service.ts`](./apps/api/src/services/production-schedule/leaderboard/leaderboard-row-selection.service.ts)·[`production-schedule-query.service.ts`](./apps/api/src/services/production-schedule/production-schedule-query.service.ts)·[`leaderboard-shell-hydrate.service.ts`](./apps/api/src/services/production-schedule/leaderboard/leaderboard-shell-hydrate.service.ts)。**本番**: **`raspberrypi5` のみ**（`--limit raspberrypi5`）。Pi4／Pi3 **no hosts matched**（**Pi3 専用手順不要**）。**Detach Run ID**: **`20260506-190944-2060`**（**`PLAY RECAP` `ok=134` `changed=4` `failed=0` / `unreachable=0`**·リモート **`exit` `0`**·`--follow` **約 888s**）。**実機**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（**約 132s**）。**ナレッジ**: [KB-369](./docs/knowledge-base/KB-369-leader-order-board-api-internal-latency.md)·[deployment.md](./docs/guides/deployment.md)（2026-05-06 winner materialization 項）。**`main` マージ後**は **`main`** をデプロイ引数に統一。

- [x] (2026-05-06) **DGX `control-server` 単一アクティブ運用ガード（`POST /start` 前の非アクティブ側実 stop・green/blue 双方の stop 解決の起動検証）**·ブランチ **`feat/dgx-single-active-backend-guard`**·実装代表 **`d4a59800`**（**`main`**: [PR #260](https://github.com/denkoushi/RaspberryPiSystem_002/pull/260) **squash merge**・**`main` 先端** **`7469d5dc`** を正とする）。**対象ホスト**: **DGX Spark（`system-prod`）のみ**（Pi5／Pi4／Pi3 **未実施で正**）。**配置**: repo の **`scripts/dgx-local-llm-system/control-server.py`**・**`dgx_llm_single_active_guard.py`**（**`control-server.py` と同一ディレクトリに必ず同梱**）・**`stop-llama-server.sh`**・**`stop-trtllm-server.sh`** を **`scp`** で **`ubudgxkoushi@100.118.82.72:/srv/dgx/system-prod/bin/`**。**再起動**: **`start-control-server.sh` は PID ファイルのプロセスが生存していると新コードを読まず `exit 0`** のため、**`/srv/dgx/system-prod/logs/control-server.pid` の PID を `kill` したうえで** **`bash /srv/dgx/system-prod/bin/start-control-server.sh`**（gateway は変更なしのため **再起動不要**）。**実機検証**: Runbook の **4 点チェック**・`127.0.0.1:39090` **401**・`127.0.0.1:38081/healthz` **200**・`/v1/models` で **`system-prod-primary`**（本記録は **`ACTIVE_LLM_BACKEND=blue`**・**`38082` 非 listen**・**`llama-server` プロセス無し**）。**ナレッジ**: [dgx-system-prod-local-llm.md](./docs/runbooks/dgx-system-prod-local-llm.md)·[deployment.md](./docs/guides/deployment.md)（2026-05-06 DGX control-server 項）·[KB-365 §Phase12](./docs/knowledge-base/KB-365-dgx-resource-phase3-workload-orchestration.md#phase-12-dgx-control-server-single-active-guard-2026-05-06)·[ADR-20260428](./docs/decisions/ADR-20260428-dgx-active-backend-prod-default.md)。
...
- [x] (2026-05-06) **CsvDashboard Gmail 取込: NON_RETRIABLE 廃棄成功時は最終失敗集計から除外（同一バッチ内の正常メールで全体を落とさない）**·**`main`**: [PR #259](https://github.com/denkoushi/RaspberryPiSystem_002/pull/259) **squash merge**・代表コミット **`e47ad84c`** を正とする（ブランチ **`cursor/mishima-nonretriable-dispose-fix`**）。**知見（三島研削）**: PowerAutomate 由来で添付が **UTF-8 BOM のみ級**になることがあり **`CSV_HEADER_MISMATCH` → NON_RETRIABLE → ゴミ箱**は想定どおり（part 選択ミスではない観測）。**実装**: [`csv-dashboard-import.service.ts`](./apps/api/src/services/csv-dashboard/csv-dashboard-import.service.ts)·単体 [`csv-dashboard-import.service.ingest-behavior.test.ts`](./apps/api/src/services/csv-dashboard/__tests__/csv-dashboard-import.service.ingest-behavior.test.ts)。**衛生**: 調査用 **localhost ingest** は本系コードから **含めない**。**ナレッジ**: [KB-297 §空BOM・廃棄](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#mishima-grinding-empty-csv-bom-nonretriable-2026-05-06)·[csv-import-export.md](./docs/guides/csv-import-export.md)·[INDEX.md](./docs/INDEX.md)·[knowledge-base/index.md](./docs/knowledge-base/index.md)。**本番**: **Pi5 API のみ** [deployment.md](./docs/guides/deployment.md)（`./scripts/update-all-clients.sh main … --limit raspberrypi5`・**未デプロイ**）。
...
- [x] (2026-05-05) **`FKOJUNST_Status` CSV 由来外部完了を「直前成功同期との dedupe キー消失差分」へ変更**·ブランチ **`feat/fkojunst-status-disappearance-external-completion`**·代表 **`6d9c3549`**（**`main`**: [PR #250](https://github.com/denkoushi/RaspberryPiSystem_002/pull/250) **squash merge**・**`main` 先端**を正とする）。**仕様**: **`previousKeys − currentKeys`** を CSV 完了キーとし **winner の `rowKey` が消失集合に含まれる行のみ** `isExternallyCompleted=true`。**スナップショット**: **`ProductionScheduleFkojunstStatusMailDedupeKeySnapshot`**（マイグレーション **`20260504220000_fkojunst_status_mail_dedupe_key_snapshot`**）。**ガード**: dedupe 後 **0 キー**は同期スキップ・初回は完了無し。**実効完了 OR**: [`production-schedule-effective-completion.sql.ts`](./apps/api/src/services/production-schedule/production-schedule-effective-completion.sql.ts) 維持。**本番**: **`raspberrypi5` のみ**（`--limit raspberrypi5`・**1 台ずつ**）。Pi4／Pi3 **no hosts matched**。**標準**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/fkojunst-status-disappearance-external-completion infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`（**`main` 取り込み後は** `main`）。**Detach Run ID**: **`20260505-072811-487`**（**`PLAY RECAP` `ok=134` `changed=4` `failed=0` / `unreachable=0`**・exit **`0`**・`--follow` **約 617s**）。**マイグレ**: Ansible **`Run prisma migrate deploy`** 成功（**`20260504220000`** 適用）。**実機**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（**約 82s**）。**ナレッジ**: [KB-297 §外部完了](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#fkojunst-status-external-completion-b-2026-05-02)·[deployment.md](./docs/guides/deployment.md)（2026-05-04 / **2026-05-05 本番反映** 項）·[INDEX.md](./docs/INDEX.md)·[knowledge-base/index.md](./docs/knowledge-base/index.md)。

- [x] (2026-05-04) **キオスク順位ボード・製番順評価 ON 時の登録製番ランクピッカー（↑↓ 廃止）**·ブランチ **`feat/leader-board-seiban-rank-picker`**·代表 **`d4d6160c`**。**仕様**: 左ペインで **順位番号タップ → 1…N**（[`LeaderBoardSeibanRankPicker`](./apps/web/src/features/kiosk/leaderOrderBoard/LeaderBoardSeibanRankPicker.tsx)）・純関数 [`reorderSeibanToRank.ts`](./apps/web/src/features/kiosk/leaderOrderBoard/seibanPriority/reorderSeibanToRank.ts)・[`usePersistedLeaderBoardSeibanEval.ts`](./apps/web/src/features/kiosk/leaderOrderBoard/usePersistedLeaderBoardSeibanEval.ts) **`moveRegisteredSeibanToRank`**・[`AnchoredDropdownPortal`](./apps/web/src/components/kiosk/AnchoredDropdownPortal.tsx) **`fixedZIndex`**・[`kioskRevealUi.ts`](./apps/web/src/hooks/kioskRevealUi.ts) **`KIOSK_RANK_PICKER_Z_ABOVE_LEFT_STACK`**。**API/DB 不変**。**本番**: **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`**（**`--limit` 順次**）·**Pi3 除外**。**Detach Run ID**: **`20260504-211859-16303`** / **`20260504-212412-27756`** / **`20260504-212945-9891`** / **`20260504-213330-19891`** / **`20260504-213745-19344`**（各 **`failed=0` / `unreachable=0`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **60s**）。**ナレッジ**: [KB-297 §ランクピッカー](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-seiban-rank-picker-2026-05-04)·[deployment.md](./docs/guides/deployment.md)（2026-05-04 late 項）·本セッションで **`main` merge + push**（ローカル **`main` 同期**）。

- [x] (2026-05-04) **キオスク順位ボード・製番順評価モード（端末ローカルのみ）**·ブランチ **`feat/kiosk-seiban-priority-eval-mode`**·代表コミット **`ffe250cb`**（**`main` へマージ後**は **`main` 先端**を正とする）。**仕様**: 左ペイン **製番順評価 ON/OFF**。**登録製番の順序調整**は同一日の **`feat/leader-board-seiban-rank-picker`** で **ランクピッカー**へ（評価モード単体リリース時は ↑↓）。共有 `sharedHistory` / DB / API は変更せず、**`localStorage` は工場 + 対象端末スコープ**。**ON 時のみ** 各資源列内の行ソートを **製番ローカル順（ランクマップ）最優先** → 以降は既存 **`compareLeaderBoardRowsForDisplay`**（納期・`processingOrder` 等）へ委譲。**OFF 時**は左ペイン製番一覧は **共有履歴順**、ボードソートは既存どおり。**単体**: `apps/web` Vitest（`buildLeaderBoardViewModel` / `sortLeaderBoardRowsForSeibanEvalDisplay` / `seibanPriority` 純粋関数）等。**本番**: **`raspberrypi5` のみ**（`--limit raspberrypi5`・**1 台ずつ**）。Pi4／Pi3 play **no hosts matched**（**Pi3 専用手順不要**）。**標準コマンド**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/kiosk-seiban-priority-eval-mode infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`（**`main` 取り込み後は** `main`）。**Detach Run ID**: **`20260504-203034-22339`**（**`PLAY RECAP` `ok=134` `changed=4` `failed=0` / `unreachable=0`**・exit **`0`**・`--follow` 約 **397s**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **156s**）。**トラブルシュート**: **評価 UI が無い／ソートが変わらない** → Pi5 **`web` イメージ**が当該コミットか・[verification-checklist.md](./docs/guides/verification-checklist.md) §6.6.4 **強制リロード**。**知見**: **husky pre-commit** で `import/order` 違反（`ProductionScheduleLeaderOrderBoardPage.tsx` の `buildSeibanRankMap` の import 位置）→ **`eslint-plugin-import` 規約どおり並べ替え**。**ナレッジ**: [KB-297 §製番順評価](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-seiban-priority-eval-mode-2026-05-04)·[KB-297 §ランクピッカー](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-seiban-rank-picker-2026-05-04)·[deployment.md](./docs/guides/deployment.md)（2026-05-04 製番順評価項）。

- [x] (2026-05-04) **Zero2W 担当棚設定（Android キオスク専用ページ・対象 Zero2W 指定 API）**·**`main` へ fast-forward**（先端 **`8c2dfbf4`**；実装 **`153af161`**）·代表 **`feat/mobile-placement-zero2w-assignment`** 相当。**仕様**: `/kiosk/mobile-placement` 上辺に **4つ目ボタン** **`Zero2W担当棚`** を追加し、**`/kiosk/mobile-placement/zero2w-assignment`** で **Zero2W 候補端末**へ **棚マスタの登録済み構造化棚**を割り当てる。**API**: `GET /api/mobile-placement/haizen-target-devices`・`PUT /api/mobile-placement/haizen-target-devices/:clientDeviceId/preset-shelf`（応答の候補一覧は **`apiKey` を含めない**。誤設定抑制のため **対象 Zero2W を明示指定**）。**既存互換**: self 用 `PATCH /api/mobile-placement/haizen-preset-shelf` は維持、Top の **「棚番配膳（Zero2W）」** パネルは表示専用のまま。**単体/CI**: Vitest・integration・smoke。**本番デプロイ（Pi5 のみ）**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh <ref> infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`（運用標準 **`main`**）。**Detach Run ID**: **`20260504-183939-27983`**（**`PLAY RECAP` `ok=134` `changed=4` `failed=0` / `unreachable=0`**・exit **`0`**・`--follow` 約 **721s**）。Pi4／Pi3 play は **no hosts matched**（**意図どおり・Pi3 専用手順は不要**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **95s**）。**ナレッジ**: [KB-368](./docs/knowledge-base/KB-368-zero2w-haizen-placement-tracking.md)·[deployment.md](./docs/guides/deployment.md) evening 項·[mobile-placement-smartphone.md](./docs/runbooks/mobile-placement-smartphone.md)。

- [x] (2026-05-04) **Zero 2 W 棚番エッジ + 配膳追跡（`status-agent`・`haizen-*` API・`haizen-agent`・実機 E2E）**·ブランチ **`feat/zero2w-haizen-tracking`**（**`main` へマージ後ドキュメント追随**）。**SSH / Ansible**: Pi5 踏み台・`zero2w-edge-setup.yml` + **`inventory-zero2w-edge-fragment.yml`**（実 IP は `.gitignore`）·環境変数 **`ANSIBLE_REPO_VERSION`** で Zero の **`git reset` 追従**（`clients/haizen-agent` は **該当機能が載った ref 必須**）。**sudo**: **`ansible_become_password`** または Zero の **限定 NOPASSWD**（[KB-367](./docs/knowledge-base/KB-367-zero2w-tanaban-edge-tailscale-ansible.md)）。**実機検証**: Pi5 上 **`curl`** で **`PATCH /api/mobile-placement/haizen-preset-shelf`** → **`POST …/haizen-scans`** → **`GET …/haizen-current`**（**`UNRESOLVED` 応答は日程未一致で正常**の場合あり）、Zero で **`haizen-agent.service` enabled/active**・**`/etc/raspi-haizen-agent.conf` は `640` + 実行ユーザ GID**。**ナレッジ**: [KB-368](./docs/knowledge-base/KB-368-zero2w-haizen-placement-tracking.md)·[zero2w-tanaban-edge-setup.md](./docs/runbooks/zero2w-tanaban-edge-setup.md)·[mobile-placement.md](./docs/api/mobile-placement.md)。**残**: **`haizen-agent` の playbook への組込み**（ユニット・conf・`python3-evdev`）·TLS 信頼鎖整備後の **`TLS_SKIP_VERIFY` 復帰**。
...
- [x] (2026-05-03) **Pi5 deploy 再発防止（self-SSH preflight／`authorized_keys` 冪等管理／orphan bootstrap lock）**·ブランチ **`feat/deploy-preflight-selfssh-lock-guard`**·代表 **`31baa662`**·**対象ホスト**: **`raspberrypi5` のみ**（Pi4／Pi3 play **no hosts matched**。**Pi3 個別デプロイ不要**）。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/deploy-preflight-selfssh-lock-guard infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**: **`20260503-190010-8105`**（**`PLAY RECAP` `ok=131` `changed=3` `failed=0` / `unreachable=0` / exit `0`**・`--follow` 約 **194s**）。**実機**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **103s**）。**仕様**: preflight 直前 **loopback SSH チェック**（`UPDATE_ALL_CLIENTS_SELF_SSH_OK`）·**server** `ssh-loopback-authorized.yml`·**bootstrap lock** の **EXIT 解放**·**`REMOTE_BOOTSTRAP_ORPHAN_SECONDS`** による **孤立 lock 掃除**。**ナレッジ**: [KB-366](./docs/knowledge-base/infrastructure/ansible-deployment.md#kb-366-pi5-self-ssh-preflight-and-orphan-lock)·[deployment.md](./docs/guides/deployment.md)（前提節 + 補足「Pi5 deploy 再発防止」）·**`main` 取り込み後はブランチ引数を `main`**。

- [x] (2026-05-03) **DGX リソース Phase8（KPI 先頭・説明削減・全文可読）**·ブランチ **`feat/dgx-resource-dashboard-ui-phase8`**·代表 **`89f65a7c`**·**対象ホスト**: **`raspberrypi5` のみ**（Pi4／Pi3 play **no hosts matched**。**Pi3 個別デプロイ不要**）。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/dgx-resource-dashboard-ui-phase8 infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**: **`20260503-181600-946`**（**`PLAY RECAP` `ok=130` `changed=4` `failed=0` / `unreachable=0` / exit `0`**・`--follow` 約 **666s**）。**実機**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **118s**）。**仕様**: `/admin/tools/dgx-resource` を **KPI 先頭**へ戻し、読み込み完了後の **`h1` / 補助説明**とシナリオ絵文字を削除、Spark / シナリオ / KPI の長文は **省略せず折り返し**。**知見**: Pi5 標準デプロイ preflight は **Pi5→Pi5 self-SSH** を前提とするため、**Pi5 自身の公開鍵が `authorized_keys` に無いと `Permission denied (publickey)`** で失敗し、**`runner=bootstrap` / `runPid=null`** の orphan lock が残り得る。**ナレッジ**: [KB-365 §Phase8](./docs/knowledge-base/KB-365-dgx-resource-phase3-workload-orchestration.md#phase-8kpi-先頭説明削減全文可読web-のみ本番反映)·[deployment.md](./docs/guides/deployment.md) Phase8 項·[dgx-system-prod-local-llm.md](./docs/runbooks/dgx-system-prod-local-llm.md)·[ansible-deployment KB](./docs/knowledge-base/infrastructure/ansible-deployment.md)。
...
- [x] (2026-05-03) **DGXリソース Phase5（運用者コンソール `overview.operator`・ワークロード遷移分離）**·ブランチ **`feat/dgx-resource-operator-console`**·代表 **`e88d9206`**·**実装**: `dgx-resource.operator-overview.ts`・`dgx-resource.workload-transition.ts`・Web `DgxResourceOperatorConsole`・`scenarioExecute.outcomeKind`。**対象ホスト**: **`raspberrypi5` のみ**（Pi4/Pi3 play **no hosts matched**。**Pi3 個体への Ansible 適用はしない**）。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/dgx-resource-operator-console infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**: **`20260503-115446-2532`**（`PLAY RECAP` **`ok=130` `changed=4` `failed=0` / `unreachable=0`**・exit **`0`**・ローカル `--follow` 完了まで **約 826s**）。**実機**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **97s**）。**ナレッジ**: [KB-365](./docs/knowledge-base/KB-365-dgx-resource-phase3-workload-orchestration.md#phase-5-本番反映記録)·[ADR-20260503](./docs/decisions/ADR-20260503-dgx-resource-operator-console.md)·[deployment.md](./docs/guides/deployment.md) Phase5 節·[dgx-system-prod-local-llm.md](./docs/runbooks/dgx-system-prod-local-llm.md)。**知見**: Phase12 は Pi5 のみ更新でも **90–120s 規模**になり得る（広域 API + 他ホスト疎通）。**UI**: ポリシー変更でシナリオが無効化された場合は **主要シナリオへ自動フォールバック**。

- [x] (2026-05-03) **DGXリソース Phase4（ガイド付き複合運用：`overview.monitoring`・複合シナリオ Preview/Execute）**·ブランチ **`feat/dgx-resource-guided-orchestration-monitoring`**·代表実装 **`522ec93a`**·[PR #241](https://github.com/denkoushi/RaspberryPiSystem_002/pull/241)·**`main`**: squash **`a2fa0510`**·**実装**: `dgx-resource.monitoring-overview.ts`・`dgx-resource.scenario-planner.ts`・`PREVIEW_ORCHESTRATION_SCENARIO` / `EXECUTE_ORCHESTRATION_SCENARIO`・Web `DgxResourceMonitoringPanel` / `DgxResourceOrchestrationPanel`・Vitest。**対象ホスト**: **`raspberrypi5` のみ**。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/dgx-resource-guided-orchestration-monitoring infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**: **`20260503-102936-930`**（`PLAY RECAP` **`ok=130` `changed=4` `failed=0` / `unreachable=0`**・exit **`0`**）。**実機**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**。**ナレッジ**: [KB-365](./docs/knowledge-base/KB-365-dgx-resource-phase3-workload-orchestration.md) §Phase4·[deployment.md](./docs/guides/deployment.md) Phase4 節·`docs/knowledge-base/api.md`。**知見**: `--detach --follow` は **`PLAY RECAP` の後でも**ローカルが **終了処理待ちで長く見える**ことがある → **`PLAY RECAP` / `summary.json` / `*.exit` を正本**。
...
- [x] (2026-05-02) **`FKOJUNST_Status` CSV 不在の外部完了（別テーブル・manual OR external）**·`feature/fkojunst-external-completion-b`·実装 **`a83c5439`**・記録 **`bfc4ac0d`**·[PR #239](https://github.com/denkoushi/RaspberryPiSystem_002/pull/239): [`fkojunst-external-completion-sync.service.ts`](./apps/api/src/services/production-schedule/external-completion/fkojunst-external-completion-sync.service.ts)·[`production-schedule-effective-completion.sql.ts`](./apps/api/src/services/production-schedule/production-schedule-effective-completion.sql.ts)·[`csv-dashboard-post-ingest.service.ts`](./apps/api/src/services/csv-dashboard/csv-dashboard-post-ingest.service.ts)·Prisma **`20260502103000_add_production_schedule_external_completion`**。**対象**: **`raspberrypi5` のみ**（Pi4/Pi3 **不要**）。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feature/fkojunst-external-completion-b infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`（**`main` 取り込み後は `main`**）。**Detach Run ID**（`ansible-update-`）: **`20260502-215033-1769`**（**`PLAY RECAP` `ok=130` `changed=4` `failed=0` / `unreachable=0` / exit `0`**・所要 **約 1445s**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **139s**）。**知見**: **`Rebuild/Restart docker compose services` が無出力のまま長時間**になり得る・完了は **`PLAY RECAP` / `summary.json` / `*.exit`**。本体 CSV 取込後も **Status を読み直して**外部完了を再同期。**ナレッジ**: [deployment.md](./docs/guides/deployment.md)·[KB-297 §外部完了](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#fkojunst-status-external-completion-b-2026-05-02)·[INDEX.md](./docs/INDEX.md)。**CI**: 先行デプロイ検証のため **緑化待ちせずマージ判断（ユーザー合意）**。
...
- [x] (2026-05-02) **順位ボード `leaderboard` 一覧 GET の過大 `pageSize` を API 側でクランプ（旧 SPA キャッシュ吸収）+ 計測 ingest 除去**·`fix/leaderboard-pagesize-server-cap`·**`0865cc56`**（[PR #236](https://github.com/denkoushi/RaspberryPiSystem_002/pull/236)）+ ドキュメント追記 **`e4605dcb`**·[`list.ts`](./apps/api/src/routes/kiosk/production-schedule/list.ts)·[deployment.md §pageSize](./docs/guides/deployment.md)·[KB-297 §ページサイズ上限制御](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-leaderboard-pagesize-server-cap-2026-05-02): **`responseProfile=leaderboard` のとき `pageSize = min(requested, 900)`**。`127.0.0.1:7426` への開発用 **`fetch`** を **API/Web 本番経路から除去**。**対象ホスト**: **`raspberrypi5` のみ**·**Pi3 除外**。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh main infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260502-175955-7198`**（**`PLAY RECAP` `failed=0` / `unreachable=0` / exit `0`**）。**自動実機**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **141s**・Tailscale）。**知見**: Caddy **`/var/log/caddy/access.log`** の JSON `uri` に **`pageSize=1240` 級が混在**すると再発の兆し。

- [x] (2026-05-02) **順位ボード 行フッター工程チップを `responseProfile=leaderboard` 一覧応答へ内包（progress-overview の二重取得撤去・完了後整合 invalidate 拡張）**·`feat/kiosk-leaderboard-footer-contract`·代表実装コミット **`a1be93a4`**·本番 **`raspberrypi5` のみ**（**`--limit raspberrypi5`**・Pi4/Pi3 play **no hosts matched**）·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-297 §leaderboard フッター契約](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-leaderboard-footer-chips-contract-2026-05-02): [`leaderboard-part-footer-processes.service.ts`](./apps/api/src/services/production-schedule/leaderboard/leaderboard-part-footer-processes.service.ts)·[`production-schedule-query.service.ts`](./apps/api/src/services/production-schedule/production-schedule-query.service.ts)·Web [`ProductionScheduleLeaderOrderBoardPage.tsx`](./apps/web/src/pages/kiosk/ProductionScheduleLeaderOrderBoardPage.tsx)·[`hooks.ts`](./apps/web/src/api/hooks.ts)。**Prisma マイグレーションなし**。**Pi3**: **除外**（本変更の必須対象外）。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/kiosk-leaderboard-footer-contract infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260502-142341-11156`**（**`PLAY RECAP` `failed=0` / `unreachable=0` / exit `0`**・**`ok=130` `changed=4`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（所要 **約 127s**・Tailscale）。**CI**（feature push）: GitHub Actions **`25244446489`** **success**。**知見**: 順位ボードの行下チップは **一覧応答に同梱**すると **progress-overview 全件 fetch** を不要にでき、完了後は **history-progress / progress-overview** の **invalidate** で他画面とのズレを抑えられる。**性能追補**: コミット **`1da74f2a`**（Pi5 Detach **`20260502-150239-22559`**）でフッター集約SQLを最適化し、Pi5 localhost `pageSize=400` の実測を **`leaderboard` 4.28–4.81s → 1.40–1.43s** に改善（`full` 1.63–1.68s / 1.61–1.82s）。**`main`**: [PR #235](https://github.com/denkoushi/RaspberryPiSystem_002/pull/235) squash merge（**`6bb6c38c`**）・GitHub Actions CI **`25245773927`** success・ローカル／`origin/main` 同期済み。

- [x] (2026-05-02) **順位ボード資源チップ・progress-overview 部品行キー整合・進捗一覧 `part.processes` 復元**·`fix/leaderboard-resource-chips-join-and-scope`·代表 **`44aea2d9`**·本番 **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`**（**`--limit` 順次**）·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-297 §結合粒度](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-resource-chips-part-key-overview-join-2026-05-02): [`progress-overview-query.service.ts`](./apps/api/src/services/production-schedule/progress-overview-query.service.ts)·[`progressOverviewPresentation.ts`](./apps/web/src/features/kiosk/productionSchedule/progressOverviewPresentation.ts)·[`ProgressOverviewPartRow.tsx`](./apps/web/src/components/kiosk/progressOverview/ProgressOverviewPartRow.tsx)·[`buildLeaderBoardPartResourceProcessKey.ts`](./apps/web/src/features/kiosk/leaderOrderBoard/buildLeaderBoardPartResourceProcessKey.ts)·[`collectLeaderBoardFooterResourceChips.ts`](./apps/web/src/features/kiosk/leaderOrderBoard/collectLeaderBoardFooterResourceChips.ts)。**Pi3**: **除外**。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·ホストごと `./scripts/update-all-clients.sh fix/leaderboard-resource-chips-join-and-scope infrastructure/ansible/inventory.yml --limit <host> --detach --follow`。**Detach Run ID**: **`20260502-125430-31676`** / **`20260502-130426-1173`** / **`20260502-131032-13145`** / **`20260502-131507-16128`** / **`20260502-132009-12509`**。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **152s**）。**知見**: チップの正本は **部品行**単位。**トラブルシュート**: 手元 `git fetch` 失敗時は **ネット到達を確認**。**`main`**: [PR #233](https://github.com/denkoushi/RaspberryPiSystem_002/pull/233) squash merge（**`6045b202`**）・GitHub Actions CI **`25243785120`** success・ローカル／`origin/main` 同期済み。

- [x] (2026-05-02) **キオスク順位ボード 各行下辺・製番単位の資源進捗チップ帯（overview 集約の共有）**·`fix/leaderboard-row-footer-resource-chips`·代表 **`16911165`**·本番 **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`**（**`--limit` 順次**）·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-297 §行下辺チップ](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-row-footer-resource-chips-2026-05-02): [`collectLeaderBoardFooterResourceChips.ts`](./apps/web/src/features/kiosk/leaderOrderBoard/collectLeaderBoardFooterResourceChips.ts)·[`LeaderOrderResourceRow.tsx`](./apps/web/src/features/kiosk/leaderOrderBoard/LeaderOrderResourceRow.tsx)·[`LeaderBoardGrid.tsx`](./apps/web/src/features/kiosk/leaderOrderBoard/LeaderBoardGrid.tsx)。**API / DB 変更なし**。**Pi3**: **除外**。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·ホストごと `./scripts/update-all-clients.sh fix/leaderboard-row-footer-resource-chips infrastructure/ansible/inventory.yml --limit <host> --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260502-105130-11663`** / **`20260502-105758-25070`** / **`20260502-110434-28709`** / **`20260502-110923-18185`** / **`20260502-111424-3838`**（各 **`failed=0` / `unreachable=0` / exit `0`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **130s**）。**知見**: チップ帯は **カードではなく行側**に載せ、**進捗一覧と同じ集約純関数**に寄せる。**ドキュメント追補済み**。**`main`**: PR マージ後 **ローカル `main` を `origin/main` に同期**。

- [x] (2026-05-02) **キオスク順位ボード製番アクセント（フィルタ空でも製番ハッシュ着色）／進捗一覧製番カードの資源CD集約チップ帯**·`feat/kiosk-seiban-accent-and-progress-resource-strip`·代表 **`924a2ff4`**·本番 **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`**（**`--limit` 順次**）·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-297 §2026-05-02](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-seiban-accent-always-progress-resource-strip-2026-05-02): [`seibanAccentPalette.ts`](./apps/web/src/features/kiosk/leaderOrderBoard/seibanAccentPalette.ts)·[`collectAggregatedProgressOverviewResourceProcesses.ts`](./apps/web/src/features/kiosk/productionSchedule/collectAggregatedProgressOverviewResourceProcesses.ts)·[`ProgressOverviewSeibanCard.tsx`](./apps/web/src/components/kiosk/progressOverview/ProgressOverviewSeibanCard.tsx)。**API / DB 変更なし**。**Pi3**: **除外**。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・ホストごと `./scripts/update-all-clients.sh feat/kiosk-seiban-accent-and-progress-resource-strip infrastructure/ansible/inventory.yml --limit <host> --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260502-094331-28033`** / **`20260502-094916-31090`** / **`20260502-095506-23348`** / **`20260502-095947-26960`** / **`20260502-100443-16279`**（各 **`failed=0` / `unreachable=0` / exit `0`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **114s**）。**知見**: **StoneBase** で barcode-agent **`Wait …` が一時 RETRYING**でも playbook は **`failed=0` で完走し得る**（Pi4 performance  rollout 項と同系）。**ドキュメント追補済み**。**`main`**: PR squash merge 後 **ローカル `main` を `origin/main` に同期（本セッション）**。

- [x] (2026-05-01) **DGX リソース管理コンソール（Pi5 API `/system/dgx-resource/*`・管理 Web `/admin/tools/dgx-resource`）**·`feature/dgx-resource-ui-phase1`·代表 **`5eb78001`**·本番 **`raspberrypi5` のみ**·Phase12·[deployment.md](./docs/guides/deployment.md)·[dgx-system-prod-local-llm.md](./docs/runbooks/dgx-system-prod-local-llm.md): API `dgx-resource` 境界・`LOCAL_LLM_RUNTIME_STOP_REQUEST_TIMEOUT_MS` を停止に使用・任意 `DGX_RESOURCE_*`。**Prisma マイグレーションなし**。**CI**: GitHub Actions Run **`25214297856`** success。**デプロイ（先行反映）**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feature/dgx-resource-ui-phase1 infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260501-214011-6943`**（**`ok=130` `changed=4`**・**exit `0`**・Pi4/Pi3 **no hosts matched**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **54s**）。**知見**: デプロイスクリプトは **未コミット変更で fail-fast** → **stash/commit**（[KB-200](./docs/knowledge-base/infrastructure/ansible-deployment.md#kb-200-デプロイ標準手順のfail-fastチェック追加とデタッチ実行ログ追尾機能)）。**Pi3**: **除外**。**`main`**: [PR #229](https://github.com/denkoushi/RaspberryPiSystem_002/pull/229) squash merge（**`2f7461af`**）・ローカル／`origin/main` 同期済み。

- [x] (2026-05-01) **キオスク順位ボード UX（製番視認性・資源進捗列・左ペイン2列・製番フィルタ行アクセント）**・`feat/kiosk-leader-order-board-ux`・代表 **`84abca0b`**・本番 **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`**（**`--limit` 順次**）・Phase12・[deployment.md](./docs/guides/deployment.md)・[KB-297 §UX](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-ux-seiban-accent-2026-05-01): [`KioskResourceProcessChips.tsx`](./apps/web/src/components/kiosk/resourceProgress/KioskResourceProcessChips.tsx)・[`ProgressOverviewPartRow.tsx`](./apps/web/src/components/kiosk/progressOverview/ProgressOverviewPartRow.tsx)・[`LeaderBoardDueAssistPanel.tsx`](./apps/web/src/features/kiosk/leaderOrderBoard/LeaderBoardDueAssistPanel.tsx)・[`LeaderBoardLeftToolStack.tsx`](./apps/web/src/features/kiosk/leaderOrderBoard/LeaderBoardLeftToolStack.tsx)・[`LeaderBoardGrid.tsx`](./apps/web/src/features/kiosk/leaderOrderBoard/LeaderBoardGrid.tsx)・[`seibanAccentPalette.ts`](./apps/web/src/features/kiosk/leaderOrderBoard/seibanAccentPalette.ts)・[`ProductionScheduleLeaderOrderBoardPage.tsx`](./apps/web/src/pages/kiosk/ProductionScheduleLeaderOrderBoardPage.tsx)。**API / DB 変更なし**。**Pi3**: **除外**。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`・ホストごと `./scripts/update-all-clients.sh feat/kiosk-leader-order-board-ux infrastructure/ansible/inventory.yml --limit <host> --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260501-224248-30928`** / **`20260501-224814-26947`** / **`20260501-225329-28559`** / **`20260501-225740-4207`** / **`20260501-230236-8738`**（各 **`failed=0` / `unreachable=0` / exit `0`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **67s**）。**知見**: 進捗チップは **単一コンポーネント**、`activeQueries` は **ページ→行**への明示伝播。**`main`**: [PR #230](https://github.com/denkoushi/RaspberryPiSystem_002/pull/230) squash merge（**`3b9ef317`**）・ローカル／`origin/main` 同期済み（本セッション）。

- [x] (2026-05-01) **ドキュメント: 三島研削 `ProductionSchedule_Mishima_Grinding`—上流 CSV に実日付列が無いことの明示**·**実装・デプロイなし**（手動取込の型・キー確認のナレッジ化）。固定 `CsvDashboard` ID **`3f2f6b0e-6a1e-4c0b-9d0b-1a4f3f0d2a01`**。取込 run 例 **`a1f180aa-9aba-4e3a-87c4-92ea0a83d26a`**。**`registeredAt`/`updatedAt` は CSV 空**→**`occurredAt` フォールバックは仕様通り**。[KB-297 §三島研削](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#mishima-grinding-csv-no-date-2026-05-01)·[csv-import-export.md](./docs/guides/csv-import-export.md)·[docs/INDEX.md](./docs/INDEX.md)·[docs/knowledge-base/index.md](./docs/knowledge-base/index.md)。**`main`**: PR マージ後ローカル `main` を `origin/main` に同期。
- [x] (2026-05-01) **Gmail CSV 日時の PowerAutomate 互換（`FKOJUNST_Status`・一般 `CsvDashboard`）**·`fix/csv-datetime-compat-powerautomate`·代表 **`a9ce2f1b`**·本番 **`raspberrypi5` のみ**·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-297 §PowerAutomate 日時互換](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#powerautomate-csv-datetime-compat-2026-05-01)·[csv-import-export.md](./docs/guides/csv-import-export.md): [`csv-dashboard-datetime-parse.ts`](./apps/api/src/services/csv-dashboard/csv-dashboard-datetime-parse.ts)·[`fkojunst-status-mail-sync.pipeline.ts`](./apps/api/src/services/production-schedule/fkojunst-status-mail-sync.pipeline.ts)·[`csv-dashboard-ingestor.ts`](./apps/api/src/services/csv-dashboard/csv-dashboard-ingestor.ts)。**ISO8601（`…T…Z`）** を **Status `FUPDTEDT`** および **ダッシュボード日付列 `occurredAt`** に追加受理（従来形式も維持）。**マイグレーションなし**。**CI**: GitHub Actions Run **`25202971192`** success（`lint-build-unit`・`e2e-smoke`・`e2e-tests`）。**デプロイ（先行反映）**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh fix/csv-datetime-compat-powerautomate infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260501-141453-4379`**（**`ok=130` `changed=4`**・**exit `0`**・Pi4/Pi3 **no hosts matched**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **26s**）。**知見**: パース失敗は **`[CsvDashboardIngestor]`** の **warn**（`dashboardId` / `dateColumnName`）で追える。**`main`**: [PR #227](https://github.com/denkoushi/RaspberryPiSystem_002/pull/227) squash merge（**`df914a90`**）・ローカル／`origin/main` 同期済み。
- [x] (2026-05-01) **補助 `plannedEndDate` 更新時の空値維持・デプロイ後バックフィル導線**·`feat/order-supplement-planned-enddate-retain-and-backfill`·実装ベース **`46acf99c`**·本番 **`raspberrypi5` のみ**·Phase12·[deployment.md](./docs/guides/deployment.md)·[csv-import-export.md](./docs/guides/csv-import-export.md#order-supplement-planned-end-date-backfill)·[KB-297 §空値維持](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#order-supplement-planned-end-date-retain-2026-05-01)·[KB-328](./docs/knowledge-base/KB-328-production-schedule-supplement-key-mismatch-investigation.md): **update** で CSV の `plannedEndDate` が空（パース null）なら **既存計画納期を保持**。1 回バックフィル `pnpm backfill:order-supplement-planned-end-date(:prod)`。**マイグレーションなし**。**デプロイ（`main` 取り込み前の先行反映）**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/order-supplement-planned-enddate-retain-and-backfill infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**: **`20260501-131827-4551`**（**`ok=130` `changed=4`**・**exit `0`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **28s**）。**知見**: Pi4/Pi3 **個別デプロイ不要**（API のみ）。**`main`**: [PR #226](https://github.com/denkoushi/RaspberryPiSystem_002/pull/226) squash merge（**`1882d13b`**）・ローカル／`origin/main` 同期済み。
- [x] (2026-05-01) **補助 `plannedEndDate` 字句拡張（ISO datetime・順位ボード納期 `-` 修正）**·`main`·代表 **`0356b304`**·本番 **`raspberrypi5` のみ**·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-297 §字句拡張](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#order-supplement-planned-end-date-parse-2026-05-01)·[KB-328](./docs/knowledge-base/KB-328-production-schedule-supplement-key-mismatch-investigation.md): [`order-supplement-sync.pipeline.ts`](./apps/api/src/services/production-schedule/order-supplement-sync.pipeline.ts)·[`order-supplement-sync.service.test.ts`](./apps/api/src/services/production-schedule/__tests__/order-supplement-sync.service.test.ts)。**原因**: `parsePlannedDate` が **`YYYY-MM-DDTHH:mm:ss`** 等を受理せず **`plannedEndDate` null**。**Fix**: ISO 接頭辞＋時刻・`YYYY/M/D` 等。**マイグレーションなし**。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh main infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260501-122119-30686`**（**`ok=130` `changed=4`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **28s**）。**知見**: 既存 null 行は **再同期**。**ドキュメント**: [deployment.md](./docs/guides/deployment.md)・KB-297・KB-328・`docs/INDEX.md` へ本番デプロイ・Phase12 実績を追記し、本変更を **`main` へマージ（PR）**。
- [x] (2026-05-01) **着手日同期を差分反映へ移行（全削除置換廃止・手動補正保護・1年超過 prune）**·`feat/order-supplement-incremental-sync`·代表 **`58dfe0ee`**·本番 **`raspberrypi5` のみ**·Phase12·[deployment.md](./docs/guides/deployment.md) 補足（2026-05-01）·[KB-297 §着手日補助の差分同期](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#order-supplement-incremental-sync-2026-05-01)·[KB-328](./docs/knowledge-base/KB-328-production-schedule-supplement-key-mismatch-investigation.md): `ProductionScheduleOrderSupplement` の同期を **`deleteMany -> createMany` 全置換**から **create/update 差分反映**へ変更し、`plannedStartDateManuallySet` / `lastSeenAt` を Prisma へ追加（migration **`20260501015000_order_supplement_incremental_sync`**）。CSV 側が空でも既存非 null 着手日を維持し、**手動補正フラグ=true** の着手日は同期で上書きしない。1年以上前の自動着手日は prune。**ローカル検証**: 対象 Vitest / `lint` / `build`。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/order-supplement-incremental-sync infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260501-111010-10961`**（**`ok=130` `changed=4`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**。**計画**: [order-supplement-incremental-sync-execplan.md](./docs/plans/order-supplement-incremental-sync-execplan.md)。**`main`**: [PR #224](https://github.com/denkoushi/RaspberryPiSystem_002/pull/224) squash merge（**`6c16c32d`**）・GitHub Actions **CI**（push `25199657157`）success・ローカル／`origin/main` 同期済み。
...
- [x] (2026-04-30) **CustomerSCAW `FANKENYMD` 近傍選定（同一 `FANKENMEI`・複数顧客）**·`feat/customer-scaw-fankenymd-proximity`·`8d95c2dd`·本番 **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **`--limit` 1 台ずつ**·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-361](./docs/knowledge-base/KB-361-customer-scaw-gmail-csv.md)·[csv-import-export.md](./docs/guides/csv-import-export.md): `customer-scaw-fankenymd.ts`・`customer-scaw-candidates.ts`・`customer-scaw-sync.pipeline.ts`（`ProductionScheduleOrderSupplement` **LEFT JOIN**）・`customer-scaw-dashboard.definition.ts`（任意列 **`FANKENYMD`**）。**Prisma マイグレーションなし**。**仕様**: `FANKENYMD`（UTC 日）と補助 **`plannedStartDate`** の最短距離・同距離は **`FANKENYMD <= 着手日`** 優先・同率 **CSV 後勝ち**・日付欠損は **後勝ち**。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/customer-scaw-fankenymd-proximity infrastructure/ansible/inventory.yml --limit <host> --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260430-104715-27689`** / **`20260430-105722-30608`** / **`20260430-110145-20682`** / **`20260430-110452-23417`** / **`20260430-110808-28659`**（各 **`failed=0` / `unreachable=0` / exit `0`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **28s**）。**Pi3**: **除外**。**`main`**: merged + pushed（[PR #220](https://github.com/denkoushi/RaspberryPiSystem_002/pull/220)・merge **`d34a3b54`**）（ローカル／`origin/main` 同期済み）。

- [x] (2026-04-30) **CustomerSCAW 近傍選定の着手日（製番 `MIN(plannedStartDate)`・`FANKENYMD` パース拡張）**·`fix/customer-scaw-seiban-start-date`·`0ca15b5c`·本番 **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **`--limit` 1 台ずつ**·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-361](./docs/knowledge-base/KB-361-customer-scaw-gmail-csv.md)·[csv-import-export.md](./docs/guides/csv-import-export.md): `customer-scaw-sync.pipeline.ts`（製番集約サブクエリ **`src` + `buildMaxProductNoWinnerCondition('src')`**）・`customer-scaw-fankenymd.ts`（日付表記拡張）・`customer-scaw-candidates.test.ts`。**Prisma マイグレーションなし**。**仕様**: 着手日は **製番単位 MIN(`plannedStartDate`)** を近傍判定に使用（MH 行直行 JOIN の null 多発を回避）。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh fix/customer-scaw-seiban-start-date infrastructure/ansible/inventory.yml --limit <host> --detach --follow`（**`main` 取り込み後は `main`**）。**Detach Run ID**（`ansible-update-`）: **`20260430-121103-24992`** / **`20260430-122148-22710`** / **`20260430-122624-28383`** / **`20260430-122940-2356`** / **`20260430-123309-8603`**（各 **`failed=0` / `unreachable=0` / exit `0`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **25s**）。**Pi3**: **除外**。**`main`**: merged + pushed（[PR #221](https://github.com/denkoushi/RaspberryPiSystem_002/pull/221)・merge **`b8b60584`**）（ローカル／`origin/main` 同期済み）。

- [x] (2026-04-30) **CustomerSCAW（製番→顧客名・生産日程一覧/順位ボード）**·`feat/customer-scaw-fseiban-customer-link`·`31c7985c`·本番 **`raspberrypi5` → `raspberrypi4` → `raspi4-robodrill01` → `raspi4-fjv60-80` → `raspi4-kensaku-stonebase01`** を **`--limit` 1 台ずつ**·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-361](./docs/knowledge-base/KB-361-customer-scaw-gmail-csv.md)·[csv-import-export.md](./docs/guides/csv-import-export.md): Prisma `ProductionScheduleFseibanCustomerScaw`・`customer-scaw-sync.pipeline`・一覧 enrich・`customerName`（API トップレベル / `leaderboard`）・[`LeaderOrderResourceRow`](./apps/web/src/features/kiosk/leaderOrderBoard/LeaderOrderResourceRow.tsx) 表示。**仕様**: Gmail 件名 **`CustomerSCAW`**・`FANKENMEI`↔MH/SH winner の `FHINMEI` 照合・**source 単位全置換**・固定スケジュール **`csv-import-productionschedule-customer-scaw`**。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/customer-scaw-fseiban-customer-link infrastructure/ansible/inventory.yml --limit <host> --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260430-092747-16`** / **`20260430-093831-8790`** / **`20260430-094303-29499`** / **`20260430-094627-7368`** / **`20260430-094955-16246`**（各 **`failed=0` / `unreachable=0` / exit `0`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **25s**）。**Pi3**: **除外**（ユーザー指定・リソース僅少のため未デプロイ）。**`main`**: merged + pushed（[PR #218](https://github.com/denkoushi/RaspberryPiSystem_002/pull/218)・merge **`cb132f12`**）（ローカル／`origin/main` 同期済み）。
- [x] (2026-04-29) **キオスク順位ボード Pi4 向け再レンダー抑制（order-usage 波及削減・Virtualization / transition 局所化）**·`feat/kiosk-leaderboard-pi4-followup`·`7902f5ac`·本番 **`raspberrypi5` → Pi4 キオスク 4 台**（`raspberrypi4` / `raspi4-robodrill01` / `raspi4-fjv60-80` / `raspi4-kensaku-stonebase01`）を **`--limit` 1 台ずつ**·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-297 §Pi4 performance](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-pi4-performance-2026-04-24): [`LeaderBoardGrid.tsx`](./apps/web/src/features/kiosk/leaderOrderBoard/LeaderBoardGrid.tsx)·[`LeaderOrderResourceCard.tsx`](./apps/web/src/features/kiosk/leaderOrderBoard/LeaderOrderResourceCard.tsx)·[`LeaderOrderResourceRow.tsx`](./apps/web/src/features/kiosk/leaderOrderBoard/LeaderOrderResourceRow.tsx)·[`buildLeaderBoardViewModel.ts`](./apps/web/src/features/kiosk/leaderOrderBoard/buildLeaderBoardViewModel.ts)·[`apps/web/src/api/client.ts`](./apps/web/src/api/client.ts)（`leaderboard` JSDoc 整合）。**仕様**: **15 秒 `order-usage`** 更新で **全カードへ同一マップ参照が波及しにくい**よう **資源ごとの使用順位配列**のみ子へ。**行キー `row.id`**・**overscan 抑制**・**`transition-all` 撤去**（transform/opacity のみ）。**API 契約変更なし**。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·各ホスト毎 `./scripts/update-all-clients.sh feat/kiosk-leaderboard-pi4-followup infrastructure/ansible/inventory.yml --limit <host> --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260429-214453-13263`** / **`20260429-215053-15127`** / **`20260429-215805-17537`** / **`20260429-220418-1032`** / **`20260429-221048-28118`**（いずれも **`failed=0` / `unreachable=0`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **64s**）。**Pi3**: 除外。**`main`**: merged + pushed（[PR #217](https://github.com/denkoushi/RaspberryPiSystem_002/pull/217)・merge **`0b2c7a2f`**）（ローカル／`origin/main` 同期済み）。

- [x] (2026-04-29) **キオスク順位ボード 製番一覧パネル（接頭辞UI改修・末尾削除／全解除・3列・9桁表示枠・登録済みグレーアウト維持）**·`feat/leaderboard-seiban-panel-layout-and-prefix-controls`·`74d360b6`·本番 **`raspberrypi5` のみ**·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-297 §製番一覧パネル](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-board-seiban-list-panel-2026-04-29): [`LeaderBoardSeibanListPanel.tsx`](./apps/web/src/features/kiosk/leaderOrderBoard/LeaderBoardSeibanListPanel.tsx)·[`leaderBoardSeibanPrefixFilterActions.ts`](./apps/web/src/features/kiosk/leaderOrderBoard/leaderBoardSeibanPrefixFilterActions.ts)。**仕様**: **表示は最大9桁相当の固定幅**（フィルタ値は非パディング）・**末尾削除**・**全解除**・**`sm` 以上3列グリッド**・接頭辞操作と次文字ボタンを **視覚ブロック分割**。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/leaderboard-seiban-panel-layout-and-prefix-controls infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260429-210617-3239`**（**`failed=0` / `unreachable=0` / exit `0`**・**`ok=130` `changed=4`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **139s**）。**Pi3/Pi4**: 不要（Pi5 `web` のみ）。**`main`**: merged + pushed（[PR #216](https://github.com/denkoushi/RaspberryPiSystem_002/pull/216)・merge **`0f1f9448`**）（ローカル／`origin/main` 同期済み）。

- [x] (2026-04-29) **キオスク順位ボード 製番一覧パネル（接頭辞フィルタ・共有履歴優先ソート・コントラスト・横幅）**·`feat/leaderboard-seiban-panel-prefix-filter`·`900cb141`·本番 **`raspberrypi5` のみ**·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-297 §製番一覧パネル](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-board-seiban-list-panel-2026-04-29): [`LeaderBoardSeibanListPanel.tsx`](./apps/web/src/features/kiosk/leaderOrderBoard/LeaderBoardSeibanListPanel.tsx)·[`sortVisibleSeibanEntriesForDisplay.ts`](./apps/web/src/features/kiosk/leaderOrderBoard/sortVisibleSeibanEntriesForDisplay.ts)·[`collectSeibanPrefixCharset.ts`](./apps/web/src/features/kiosk/leaderOrderBoard/collectSeibanPrefixCharset.ts)。**仕様**: **約2倍幅**・**共有履歴登録済みを先頭表示**・**接頭辞 `startsWith` の段階フィルタ**（**次に続けられる文字**のみボタン表示）・**解除**。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/leaderboard-seiban-panel-prefix-filter infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260429-202355-27582`**（**`failed=0` / `unreachable=0` / exit `0`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **106s**）。**Pi3/Pi4**: 不要（Pi5 `web` のみ）。**`main`**: merged + pushed（[PR #215](https://github.com/denkoushi/RaspberryPiSystem_002/pull/215)・merge **`dee788f7`**）（ローカル／`origin/main` 同期済み）。

- [x] (2026-04-29) **キオスク順位ボード 表示中製番一覧パネル（共有履歴トグル）**·`feat/leaderboard-seiban-list-panel`·`f544a45c`·本番 **`raspberrypi5` のみ**·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-297 §製番一覧パネル](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-board-seiban-list-panel-2026-04-29): [`LeaderBoardSeibanListPanel.tsx`](./apps/web/src/features/kiosk/leaderOrderBoard/LeaderBoardSeibanListPanel.tsx)·[`deriveVisibleSeibanEntries.ts`](./apps/web/src/features/kiosk/leaderOrderBoard/deriveVisibleSeibanEntries.ts)·[`LeaderBoardLeftToolStack.tsx`](./apps/web/src/features/kiosk/leaderOrderBoard/LeaderBoardLeftToolStack.tsx)·[`useLeaderBoardDueAssist.ts`](./apps/web/src/features/kiosk/leaderOrderBoard/useLeaderBoardDueAssist.ts)。**仕様**: 「製番一覧」→右半画面で **表示中製番を一意に一覧**・**登録済みグレーアウト**・**押下で共有履歴へ登録／再押下で解除**（**`toggleSeibanInSharedHistory`**）。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/leaderboard-seiban-list-panel infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260429-193317-26767`**（**`failed=0` / `unreachable=0` / exit `0`**・所要 **約 436s**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **88s**）。**Pi3/Pi4**: 不要（Pi5 `web` のみ）。**`main`**: merged + pushed（ローカル／`origin/main` 同期済み）。

- [x] (2026-04-29) **キオスク順位ボード 備考モーダルから製番登録（共有履歴）**·`feat/leaderboard-seiban-register-modal-close`·`a3265139`·本番 **`raspberrypi5` のみ**·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-297 §備考モーダル製番登録](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-note-modal-seiban-register-2026-04-29): [`KioskNoteModal.tsx`](./apps/web/src/components/kiosk/KioskNoteModal.tsx)·[`ProductionScheduleLeaderOrderBoardPage.tsx`](./apps/web/src/pages/kiosk/ProductionScheduleLeaderOrderBoardPage.tsx)·[`useLeaderBoardDueAssist.ts`](./apps/web/src/features/kiosk/leaderOrderBoard/useLeaderBoardDueAssist.ts)·[`KioskNoteModal.test.tsx`](./apps/web/src/components/kiosk/KioskNoteModal.test.tsx)。**仕様**: 鉛筆→備考モーダル→**「製番登録」**→ 共有 **`search-state.history`** へ追加・成功時 **`closeNoteModal()`**（備考保存は別ボタン）。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/leaderboard-seiban-register-modal-close infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260429-184211-20335`**（**`failed=0` / `unreachable=0` / exit `0`**・所要 **約 405s**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **90s**）。**Pi3/Pi4**: 不要（Pi5 `web` のみ）。**`main`**: merged + pushed（ローカル／`origin/main` 同期済み）。

- [x] (2026-04-29) **加工機点検状況 KPI とカード配色基準の統一（正常/異常件数）**·`fix/uninspected-kpi-card-alignment`·`48cfb6c2`·本番 **`raspberrypi5` のみ**·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-360](./docs/knowledge-base/api.md#kb-360-加工機点検状況のkpiをカード配色基準と統一正常異常件数)·[`daily-inspection-kpi.ts`](./apps/api/src/services/tools/daily-inspection-kpi.ts)·[`machine.service.ts`](./apps/api/src/services/tools/machine.service.ts)。**仕様**: KPI「点検済み/未点検」＝カード青/赤条件（`normalCount>0 || abnormalCount>0`）。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh fix/uninspected-kpi-card-alignment infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260429-174518-6203`**（**`failed=0` / `unreachable=0` / exit `0`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **148s**）。**知見**: Mac 側 `--follow` が Docker 再作成中に **SSH timeout** してもリモートデタッチは完走し得る → Pi5 `*.exit` で確認。**Pi3/Pi4**: 不要。**`main`**: merge + push（ローカル `main` 同期）。

- [x] (2026-04-29) **キオスク持出一覧 本文末尾揃え・サムネ108px・カード外寸固定**·`feat/kiosk-active-loan-card-right-align-thumb-15x-fixed-size`·`d1c6abe7`·本番 **`raspberrypi5` のみ**·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-323 追補](./docs/knowledge-base/KB-323-kiosk-return-card-button-layout.md): [`kioskActiveLoanCardLayout.ts`](./apps/web/src/components/kiosk/kioskActiveLoanCardLayout.ts)·[`KioskActiveLoanCard.tsx`](./apps/web/src/components/kiosk/KioskActiveLoanCard.tsx)·[`KioskReturnPage.tsx`](./apps/web/src/pages/kiosk/KioskReturnPage.tsx)·[`KioskActiveLoanCard.test.tsx`](./apps/web/src/components/kiosk/__tests__/KioskActiveLoanCard.test.tsx)。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/kiosk-active-loan-card-right-align-thumb-15x-fixed-size infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260429-163457-5210`**（**`failed=0` / `unreachable=0` / exit `0`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**。**Pi3/Pi4**: 不要。**`main`**: merge + push（ローカル `main` 同期）。

- [x] (2026-04-29) **キオスク持出一覧 貸出日時フォーマット（秒なし・Asia/Tokyo・24時間）**·`feat/kiosk-active-loan-borrowed-at-format`·`5d83816f`·本番 **`raspberrypi5` のみ**·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-323 追補](./docs/knowledge-base/KB-323-kiosk-return-card-button-layout.md): [`formatKioskActiveLoanBorrowedAt.ts`](./apps/web/src/features/kiosk/formatKioskActiveLoanBorrowedAt.ts)·[`KioskReturnPage.tsx`](./apps/web/src/pages/kiosk/KioskReturnPage.tsx)。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/kiosk-active-loan-borrowed-at-format infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260429-151939-18182`**（**`failed=0` / `unreachable=0` / exit `0`**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 42 / WARN 1 / FAIL 0**。**Pi3/Pi4**: 不要（Pi5 `web` のみ）。**`main`**: **merge + push**（ローカル `main` 同期）。
...
- [x] (2026-04-29) **DGX Spark private ComfyUI コンテナ雛形（repo・Tailscale 前提）**: ブランチ **`feat/dgx-private-comfyui-tailscale`**。`scripts/dgx-private-comfyui/` に Compose / Dockerfile 例・起停スクリプト・[dgx-private-comfyui.md](./docs/runbooks/dgx-private-comfyui.md)。**private-personal** のみマウント・**system-prod 非マウント**・ホストは **127.0.0.1:8188** のみ公開。**未実施（運用者実機）**: DGX での `docker compose build/up`、Mac ブラウザでのワークフロー編集・実行。**コミット/プッシュ**: 依頼により未実施。

- [x] (2026-05-01) **DGX private ComfyUI 外出先運用の接続復旧と workflow エラー対処**: `ubudgxkoushi@100.118.82.72` で **ComfyUI 稼働 (`dgx-private-comfyui Up` / `HTTP 200`)** を再確認。外出先で AI 実行環境のみ `Permission denied (publickey,password)` となる事象は、**非対話 SSH 前提（BatchMode）で公開鍵未登録**が原因と確定し、DGX `authorized_keys` へ公開鍵追加後に復旧。運用上の注意として、`ssh -N -L ...` は **Mac 側**で実行（DGX 側で実行すると `Address already in use`）、`-N` は**無出力が正常**を確認。ComfyUI ログの workflow 失敗主因（`mid_layer_scale='linear'`、`UNET/CLIP` 名不一致、`Note` 欠落）に対して、DGX 実機へ **互換ホットフィックス**（`mid_layer_scale` 文字列互換、`Note` 互換ノード追加、`fp8/bf16` alias 追加）を適用。実体モデルは **bf16**（`flux-2-klein-base-9b-bf16.safetensors` / `qwen_3_8b_bf16.safetensors`）。Runbook/KB/Index に反映。
...
- [x] (2026-04-29) **キオスク順位ボード 製番チップ・複数選択（OR）・製番OR検索全解除**·`feat/leaderboard-seiban-multiselect-or-clear`·`d26b50d3`·本番 **`raspberrypi5` のみ**·Phase12·[deployment.md](./docs/guides/deployment.md)·[KB-297 §製番OR検索](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-board-seiban-or-filter-2026-04-29): [`leaderBoardSeibanFilterModel.ts`](./apps/web/src/features/kiosk/leaderOrderBoard/leaderBoardSeibanFilterModel.ts)·[`useLeaderBoardDueAssist.ts`](./apps/web/src/features/kiosk/leaderOrderBoard/useLeaderBoardDueAssist.ts)·[`LeaderBoardLeftToolStack.tsx`](./apps/web/src/features/kiosk/leaderOrderBoard/LeaderBoardLeftToolStack.tsx)·[`ProductionScheduleLeaderOrderBoardPage.tsx`](./apps/web/src/pages/kiosk/ProductionScheduleLeaderOrderBoardPage.tsx)。**デプロイ**: `export RASPI_SERVER_HOST="denkon5sd02@100.106.158.2"`·`./scripts/update-all-clients.sh feat/leaderboard-seiban-multiselect-or-clear infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。**Detach Run ID**（`ansible-update-`）: **`20260429-123438-30137`**（**`failed=0` / `unreachable=0` / exit `0`**・約 **348s**）。**実機（自動）**: `./scripts/deploy/verify-phase12-real.sh` → **PASS 43 / WARN 0 / FAIL 0**（約 **23s**）。**Pi3/Pi4**: 不要（Pi5 `web` のみ）。**`main`**: 本セッションで `feat/leaderboard-seiban-multiselect-or-clear` を `main` に **merge + `origin/main` push**（ローカル `main` を最新化）。
...
### 三島研削 CSV 取込修正（`main` **`e47ad84c`**）の Pi5 本番反映と運用確認（2026-05-06）

**概要**: [PR #259](https://github.com/denkoushi/RaspberryPiSystem_002/pull/259) は **`main` にマージ済み**（**`e47ad84c`**）。**API のみ**。[deployment.md](./docs/guides/deployment.md) 標準で **`raspberrypi5` のみ** `./scripts/update-all-clients.sh main infrastructure/ansible/inventory.yml --limit raspberrypi5 --detach --follow`。

**候補タスク**:

1. **Detach Run ID**・**`PLAY RECAP`**・**Phase12** `./scripts/deploy/verify-phase12-real.sh` を [deployment.md](./docs/guides/deployment.md)／[KB-297 §空BOM](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#mishima-grinding-empty-csv-bom-nonretriable-2026-05-06) に追記する（実施後）。
2. 管理コンソールから **三島研削**対象の Gmail 取込を **手動実行**し、**極小添付＋正常添付が同一バッチ**のとき **全体が COMPLETED 相当**になること・**`debug.disposedMessageIdSuffixes`** を確認する。
3. **`FHINMEI_MH_SH`** スケジュールの鮮度・未取得製番の有無を平常監視する（別系統。必要なら [KB-350](./docs/knowledge-base/KB-350-seiban-machine-name-supplement-fhinmei-mh-sh.md)）。
...
5. **`sudo_nopasswd_commands` の実機運用確認（2026-05-06 ドキュメント反映済み）**: サンプル断片どおり適用後、Pi5 から **`ansible_become_password` なし**で `zero2w-edge-setup.yml` が通ることを **運用チェックリスト**へ 1 行で載せる（`sudo -n true`・`ansible … -m ping -b`）。**HID ケーブル／リーダー交換時**は `ls /dev/input/by-id/*-event-kbd` で **`haizen_agent_hid_device` を更新**してから playbook を再実行する手順を **Runbook の「動作確認」近傍**に追記する余地あり。
...
### DGX リソース管理コンソール: Phase4（ガイド付き複合運用モニター）場内スモーク（2026-05-03）

**概要**: Pi5 へ **`feat/dgx-resource-guided-orchestration-monitoring`** を反映済み（Detach **`20260503-102936-930`**・Phase12 **43/0/0**）。**`overview.monitoring`** と **複合シナリオ**（**`PREVIEW_ORCHESTRATION_SCENARIO` / `EXECUTE_ORCHESTRATION_SCENARIO`**）は **環境状態・権限前提**であり、自動検証は **公開 API 経路のみ**。**運用ログ**: [KB-365 Phase 4 節](./docs/knowledge-base/KB-365-dgx-resource-phase3-workload-orchestration.md#phase-4半自動オーケストレーションoverview-運用ヒント実機反映)·[deployment.md](./docs/guides/deployment.md)（**2026-05-03** の **Phase4**・`feat/dgx-resource-guided-orchestration-monitoring` 補足）。

**候補タスク**:

1. `/admin/tools/dgx-resource` で **運用ヒントパネル**（**Monitoring**）が **`overview`** / **前提 ENV** と矛盾なく表示されること。**強制リロード**: [verification-checklist.md](./docs/guides/verification-checklist.md) §6.6.4。
2. **Orchestration** で **Preview** が **順序込みステップ一覧**として返り、対象ワークロードに対してのみ **`EXECUTE` が有効**になること。**Execute** は意図した順で **順次**試行される（失敗時のロールバック方針は [KB-365](./docs/knowledge-base/KB-365-dgx-resource-phase3-workload-orchestration.md)）。
3. 既存 **Phase3**（補助起停・調停）と併せ、**競合状態**で UI が **`policy` / targets / monitoring** と食い違って見えないこと（Stale ログはイベント・再取得で収束）。
...
**概要**: ブランチ **`feat/kiosk-seiban-accent-and-progress-resource-strip`**（**`924a2ff4`**）は **Pi5 SPA 配信**のため自動検証では **PASS 43/0/0** 済み。**残りは場内での表示確認**（[verification-checklist.md](./docs/guides/verification-checklist.md) §6.6.4 強制リロード）。

**候補タスク**:

1. **順位ボード** `/kiosk/production-schedule/leader-order-board`: 製番フィルタクエリ無しでも、登録済み製番を持つ行に **左縁アクセント色**があること。**製番空行**には縁強調が付かないこと。
2. **進捗一覧** `/kiosk/production-schedule/progress-overview`: 製番カード先頭に **資源CD集約チップ帯**（AND 完了）が並び、`resourceCd` 昇順になっていること（データ依存・目視）。
3. **Pi4**: SPA は Pi5 由来。**キャッシュ**が残るときは強制リロード。

詳細ナレッジ: [KB-297 §2026-05-02](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-seiban-accent-always-progress-resource-strip-2026-05-02)。
...
4. **備考モーダル製番登録（`feat/leaderboard-seiban-register-modal-close`·`a3265139`）**: Phase12 **PASS 43/0/0** 済み。**場内確認**: `/kiosk/production-schedule/leader-order-board` で任意行の鉛筆→**「製番登録」**→ モーダルが閉じ、左パネル共有製番履歴に当該 **`fseiban`** が現れること（**備考本文は別途「保存」が必要**）。**Network**: **`PUT …/search-state`** が **`428`/`409`** で詰まっていないこと。
5. **`historyWriting` が長く `disabled` のまま**: **`PUT …/search-state` の競合連続**を疑い **ページ再読込**（[KB-297 §備考モーダル製番登録](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-order-board-note-modal-seiban-register-2026-04-29)）。
6. **表示中製番一覧パネル（`feat/leaderboard-seiban-list-panel`·`f544a45c`）**: Phase12 **PASS 43/0/0** 済み。**場内確認**: `/kiosk/production-schedule/leader-order-board` で **製番一覧**→ 表示中行がボタンで並ぶこと・**共有履歴登録済みはグレーアウト**・押下で **登録／解除**・**Esc**／背景で閉じること。**Network**: **`PUT …/search-state`** が **`428`/`409`** で詰まっていないこと（[KB-297 §製番一覧パネル](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#leader-board-seiban-list-panel-2026-04-29)）。
