...
3. **並行**: [キオスク要領書 PDF（2026-03-25）](#キオスク要領書-pdf2026-03-25) の Gmail 本番試験を優先度に応じて進める。写真持出 VLM **フェーズ1** は [PR #56](https://github.com/denkoushi/RaspberryPiSystem_002/pull/56) で `main` 統合済み。**類似候補ギャラリー（pgvector）** は [PR #57](https://github.com/denkoushi/RaspberryPiSystem_002/pull/57) で `main` 統合済み（2026-03-29、CI 成功確認済み）。**次の作業（優先度順の目安）**: **埋め込み本番配線**（`PHOTO_TOOL_EMBEDDING_*` を [docker.env.j2](./infrastructure/ansible/templates/docker.env.j2) 等へ）、**GOOD 既存データのバックフィル**（Runbook 化）、**`verify-phase12-real.sh` への軽量 candidates チェック（任意）** — 詳細は [KB-319](./docs/knowledge-base/KB-319-photo-loan-vlm-tool-label.md) / [ADR-20260330](./docs/decisions/ADR-20260330-photo-tool-similarity-gallery-pgvector.md)。**残りアイデア**: VLM **フェーズ2**（Item マスタ照合・誤認補正ワークフロー拡張）は別 ExecPlan/ADR で切り出す。
