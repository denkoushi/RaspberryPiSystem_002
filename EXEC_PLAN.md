...
- [x] (2026-03-11) **FSIGENマスタ導入（資源CD→資源名）とホバー表示対応**: `ProductionScheduleResourceMaster` を追加し、`resourceCd + resourceName` ユニークで資源名称を保持。`resourceClassCd` / `resourceGroupCd` も保存。初回投入用に `apps/api/prisma/seeds/dataSIGEN.csv` を追加し、`prisma/seed.ts` で upsert 取り込みを実装。`GET /api/kiosk/production-schedule/resources` は後方互換のまま `resourceNameMap` を返却するよう拡張し、納期管理詳細 `parts[].processes[]` に `resourceNames` を追加。Webは既存備考ホバー方式を流用し、生産スケジュール資源CDボタン・納期管理工程バッジへ `title` / `aria-label` を追加。**検証**: `production-schedule-query.service.test.ts` 更新、`kiosk-production-schedule.integration.test.ts` 更新（※ローカルDB未起動で統合テストは未実行）、`pnpm --filter @raspi-system/api prisma:generate`、`pnpm --filter @raspi-system/api build`、`pnpm --filter @raspi-system/web build` を確認。詳細は [KB-297](./docs/knowledge-base/KB-297-kiosk-due-management-workflow.md#資源cd名称マスタ導入とホバー表示2026-03-11) を参照。
