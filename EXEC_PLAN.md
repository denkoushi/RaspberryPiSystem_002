...
- [x] (2026-04-14) **Unified Loan Analytics（計測機器 CSV+NFC 統合・キオスク集計3タブ化・月次フィルタ統一）実装完了（ローカル作業ツリー、未コミット）**: ブランチ `feat/unified-loan-analytics` 上で、API `GET /api/measuring-instruments/loan-analytics`、NFC イベントミラー（`MeasuringInstrumentLoanEvent`）、重複統合（管理番号+動作+5分窓・NFC優先）、取消除外、shared-types / web client / hook 拡張、`KioskRiggingAnalyticsPage` の `計測機器` タブ追加と `対象月` フィルタ統一を実装。回帰として API Vitest（service/repository）と Web Vitest（page）を追加。参照: [KB-344](./docs/knowledge-base/KB-344-unified-loan-analytics.md) / [ui.md](./docs/modules/measuring-instruments/ui.md)。
