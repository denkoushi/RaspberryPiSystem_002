...
### DGX Spark LocalLLM: 一区切り後の将来課題（2026-04-26）

**概要**: DGX `system-prod-primary` の単一 endpoint で text + image を扱う構造、Pi5 本番の Spark 一本化、`active assist` による改善、Phase12 回帰確認までは完了した。現時点では、`photo_label` の生の初期判定精度をさらに追い込むことは優先しない。以後は通常運用で人レビュー `GOOD` gallery を育て、困りごとが再浮上した時だけ掘り直す。

**候補タスク**:

1. **通常運用**: 人レビュー `GOOD` gallery を継続蓄積し、active assist 前提の実運用精度を徐々に上げる。
2. **運用観測**: DGX 一本化後の `component: inference` / `useCase: photo_label` と `component: localLlmRuntimeControl` を数回分 spot check し、不要な start/stop やエラー再発がないかを見る。
3. **将来課題**: `ねじゲージ` / `金属棒` / `てこ式ダイヤルゲージ` の hard case は、実運用上の痛みが再度強くなった時点で再開する。
4. **将来置換**: 必要になった時点で、同じ alias / 同じ入口のまま `Qwen3.6` 系へ 1:1 置換するか判断する。
