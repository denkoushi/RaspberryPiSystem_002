...
- [x] (2025-11-26) **Phase 5: CI/テストアーキテクチャ整備**（優先度: 最高・完了）**ナレッジベース**: [KB-024](docs/knowledge-base/troubleshooting-knowledge.md#kb-024-ciテストアーキテクチャの設計不足)
  ...
  - **背景分析（2025-11-26）**:
    ...
    - **外部レビュー結果**: 「空DB作成 → Prisma Migrate 適用 → テストデータ投入 → pg_dump フルバックアップ → 空DBにリストア → データ検証」の流れ自体は、PostgreSQLの論理バックアップ（pg_dump）を使った災害復旧テストとして妥当であり、やり過ぎでもないと評価された（Level 2〜3 相当のテストとして位置付け）。
  ...
  - **作業内容**:
    ...
    - **ステップ3**: ✅ バックアップ/リストアテストの再設計（完了・CI成功）
      ...
      - **修正完了**: `docker exec`に`-i`フラグを追加してヒアドキュメントを受け取れるように修正
    ...
    - **ステップ4**: ✅ E2Eテストの安定化（完了・現状確認済み）
      - **現状**: E2Eテストは既に「CI環境で確実に動作する範囲に限定」されている
      - **実行されるテスト**: 画面表示確認、ナビゲーション確認、フォーム表示確認
      - **CI環境でスキップされるテスト**: 認証フローの詳細（タイミング問題）、NFCスキャン（物理デバイス必要）
      - **代替テスト**: 認証ロジックは統合テスト、NFCスキャンはユニットテストと統合テストでカバー
      - **ドキュメント化完了**: KB-024にE2Eテストの安定化の現状と今後の改善計画を統合（2025-11-26）
  - **成功基準**: CIパイプラインで全テストが成功する（特にバックアップ/リストアテストが安定して通る）
  - **今後の検証・改良が必要な項目**:
    - ✅ **ステップ3完了**: バックアップ/リストアテストの再設計完了、CIで1回成功を確認（2025-11-26）
    - ✅ **ドキュメント整備完了**: test-architecture.mdにCIワークフローの説明を追加、KB-024から参照を追加（2025-11-26）
    - ✅ **CIの継続的な成功確認**: 4回連続で成功を確認（2025-11-26）
      - CI #215〜#218でバックアップ/リストアテスト、APIテスト、E2Eテストがすべて成功
      - バックアップ/リストアテストが安定して動作することを確認
      - 目標（連続5回以上）には未達だが、4回連続成功で一定の安定性を確認
      - 詳細: KB-024の「CIの継続的な成功確認」セクションを参照
    - ✅ **ステップ4完了**: E2Eテストの安定化完了。CI環境で確実に動作する範囲に限定済み
...
- [ ] (2025-11-25) **次のタスク: 非機能要件の実機検証と運用マニュアル作成**（優先度: 高、Phase 5完了後に実施）
  ...
  - **作業内容**:
    ...
    - **【CI検証】タスク2-1**: ✅ 監視・アラート機能のCIテスト（完了・CI #221, #222で成功）
      - `scripts/test/monitor.test.sh`がCIワークフローで`pnpm test:monitor`として実行されている
      - APIヘルスチェック、メトリクスエンドポイント、監視スクリプトの関数テストが実行されて成功
...
**1. EXEC_PLAN.mdの肥大化自体は問題ではない**
- **現状**: 1129行、完了タスク41件、未完了タスク3件
- **AIの能力**: 1129行程度のドキュメントは問題なく処理可能。検索・参照・更新も可能。
- **人間の読者**: マークダウンの見出し構造（28個の`##`セクション、10個の`###`サブセクション）により、必要な箇所に素早くアクセス可能。
- **検索性**: `grep`やIDEの検索機能で、特定のタスクや決定事項を素早く見つけられる。
- **結論**: 2000-3000行程度までは、適切な構造化により問題なく管理可能。

**2. 肥大化が問題になるケース**
- **5000行超**: 検索性が低下し、必要な情報を見つけるのに時間がかかる
- **セクション構造が不明確**: 見出しが適切でない場合、情報の所在が分からなくなる
- **重複情報の蓄積**: 同じ情報が複数箇所に散在している場合、更新漏れが発生する

**3. Outcomes & Retrospectiveに要約することで補完できる理由**
- **一覧性の確保**: マイルストーン完了時に、Outcomes & Retrospectiveに「何が達成されたか」「何が残っているか」「学んだこと」を要約することで、Progressセクションの詳細を見る前に全体像を把握できる。
- **時系列の整理**: マイルストーン単位で要約することで、プロジェクトの進化の流れが明確になる。
- **検索の効率化**: 詳細な情報はProgressセクションに残し、概要はOutcomes & Retrospectiveに集約することで、2段階の検索が可能になる。
  - 第1段階: Outcomes & Retrospectiveで概要を把握
  - 第2段階: 必要に応じてProgressセクションの詳細を確認

**4. PLANS.mdの「生きたドキュメント」要件との整合性**
- **PLANS.mdの要求**: 「生きたドキュメント」として常に更新が必要。Progressセクションは「実際の現在の状態を常に反映する」必要がある。
- **案3の利点**: 完了タスクを削除せずに残すことで、PLANS.mdの要件を満たす。Outcomes & Retrospectiveに要約を追加することで、一覧性も確保できる。
- **他の案との比較**:
  - 案1（アーカイブ化）: Progress Archiveセクションを追加することで、PLANS.mdのスケルトンから外れる可能性がある
  - 案2（要約化）: 詳細を削除することで、PLANS.mdの「自己完結性」要件に反する可能性がある
  - 案4（別ファイル分離）: PLANS.mdの「自己完結性」要件に明確に反する

**5. 具体的な補完方法**
- **マイルストーン完了時**: Outcomes & Retrospectiveに以下の形式で要約を追加
  ```markdown
  ...
  **詳細**: Progressセクションの「Milestone X」を参照
  ```
- **定期的な更新**: 四半期ごとなど、定期的にOutcomes & Retrospectiveを更新し、プロジェクト全体の進捗を要約する。

**6. INDEX.mdとの連携**
- **INDEX.mdの役割**: 各種ドキュメント（docs/）の「入口」として機能
- **EXEC_PLAN.mdとの関係**: EXEC_PLAN.mdはプロジェクト管理ドキュメントとして、INDEX.mdから参照される
- **補完関係**: INDEX.mdで「プロジェクトの進捗を確認したい」→ EXEC_PLAN.md → Outcomes & Retrospectiveで概要を把握 → Progressセクションで詳細を確認

**結論**: EXEC_PLAN.mdの肥大化自体は問題ではなく、Outcomes & Retrospectiveに要約を追加することで、一覧性と詳細性の両立が可能になる。

- **実装方針**:
  - マイルストーン完了時に、Outcomes & Retrospectiveに要約を追加
  - Progressセクションは完了タスクも含めて維持（PLANS.mdの要件を満たす）
  - 定期的（四半期ごとなど）にOutcomes & Retrospectiveを更新し、プロジェクト全体の進捗を要約
  - INDEX.mdからEXEC_PLAN.mdへの参照を維持し、検索性を確保
...
**アイデア1: ナレッジベースの分割（カテゴリ別）**
- **現状**: `troubleshooting-knowledge.md` に全ナレッジを集約（964行）
- **提案**: カテゴリ別に分割
  - 理由: 1ファイル200-300行程度に収まり、見通しが良い。問題発生時にカテゴリから絞り込める。新規追加時も適切な場所が明確。
- **推奨度**: ⭐⭐⭐⭐⭐（高）- 今すぐ実施可能、効果が高い

**アイデア2: インデックス駆動型構成**
- **現状**: 各ディレクトリにREADME.mdがあるが、全体の索引がない
- **提案**: ルートに統一インデックスを作成
  - 理由: ドキュメントが増えても「入口」が1つで迷わない。目的から逆引きできる。新規参加者のオンボーディングが容易。
- **推奨度**: ⭐⭐⭐⭐⭐（高）- 低コストで高効果

**アイデア3: モジュール単位の自己完結ドキュメント**
- **現状**: `docs/modules/` に一部あるが、不完全
- **提案**: 各モジュールに必要なドキュメントを自己完結させる
  - 理由: コードとドキュメントが近い場所にあり、更新漏れが減る。モジュール単位で理解できる。マイクロサービス化した場合にも対応可能。
- **推奨度**: ⭐⭐⭐（中）- 大規模リファクタリングが必要、現時点では過剰かも

**アイデア4: タグ/メタデータシステム**
- **現状**: ファイル名とディレクトリ構造のみで分類
- **提案**: 各ドキュメントにFrontmatterでメタデータを追加
  - 理由: 複数の軸で検索可能（タグ、対象者、更新日）。静的サイトジェネレータ（Docusaurus等）への移行が容易。陳腐化したドキュメントを発見しやすい。
- **推奨度**: ⭐⭐⭐⭐（高）- 今すぐ全ファイルに適用は大変だが、新規作成時から導入可能。将来のドキュメントサイト化の布石。

**アイデア5: ドキュメントサイト化（Docusaurus/VitePress）**
- **現状**: GitHubでMarkdownを直接閲覧
- **提案**: 静的サイトジェネレータでドキュメントサイトを構築
  - 理由: 全文検索が可能。サイドバーナビゲーションで階層構造が見やすい。バージョン管理が可能。
- **推奨度**: ⭐⭐（低〜中）- 導入コストが高い、ドキュメント量が50ファイル超えてから検討

#### 推奨アクション（優先順位）

| 優先度 | アイデア | 理由 |
|--------|----------|------|
| 1 | **INDEX.md作成** | 低コスト・高効果、今すぐ実施可能 |
| 2 | **ナレッジベース分割** | 964行は既に長い、分割効果大 |
| 3 | **Frontmatter導入** | 新規作成時から適用、段階的に展開 |
| 4 | モジュール自己完結 | 将来の拡張時に検討 |
| 5 | ドキュメントサイト化 | 50ファイル超えたら検討 |

**関連タスク**: タスク4（運用マニュアル作成）、タスク5（共通基盤ドキュメント作成）と合わせて実施予定。
