...
- [ ] (2025-11-25) **次のタスク: Validation 7（USB一括登録）の実機検証**（Milestone 5の残タスク）
  - **目的**: CSVインポート機能の実機環境での動作確認
  - **前提条件**: Phase 3（インポート機能の修正）が完了していること
  - **現状**: CSVインポート機能は実装済み（従業員2件、工具3件のインポートに成功）。実機環境での検証が未実施。
  - **手順1**: USBメモリに`employees.csv`と`items.csv`を配置
  - **手順2**: Pi5管理UIの「一括登録」画面からCSVファイルをアップロード
  - **手順3**: 成功ダイアログの件数を確認
  - **手順4**: `import_jobs`テーブルでジョブの状態を確認
  - **手順5**: 従業員・アイテム一覧に反映されていることを確認
  - **検証**: 実機環境でCSVインポートが正常に動作することを確認
