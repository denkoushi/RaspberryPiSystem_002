...
- **`runtimeProfile`**: profile 別の運用予算。vLLM は `vllm.gpuMemoryUtilization` / `maxModelLen` / `maxNumSeqs`、llama.cpp は `llamaCpp.ctxSize` / `parallel` / `nGpuLayers` を持つ。`business_qwen36_27b_nvfp4` の `0.65` はこの profile の known-good default であり、グローバル値ではない。
