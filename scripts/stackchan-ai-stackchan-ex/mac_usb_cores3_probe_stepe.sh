#!/usr/bin/env bash
# CoreS3 Step E — single-task display / Face draw probe (build/upload/monitor).
set -euo pipefail

PROBE_DIR="$(cd "$(dirname "$0")/cores3-probe-stepe" && pwd)"
PIO_ENV="${STACKCHAN_PROBE_PIO_ENV:-m5stack-cores3-probe-stepe}"
...
Environment:
  STACKCHAN_USB_PORT
  STACKCHAN_AI_STACKCHAN_EX   (default: \$HOME/AI_StackChan_Ex)
EOF
}

if [[ ! -f "${AVATAR_LIB}/library.json" ]]; then
  echo "Step E requires: ${AVATAR_LIB}" >&2
  exit 1
fi
