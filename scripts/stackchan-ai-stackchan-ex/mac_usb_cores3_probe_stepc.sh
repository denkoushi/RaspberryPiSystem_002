#!/usr/bin/env bash
# CoreS3 Step C — Avatar-only probe (build/upload/monitor).
set -euo pipefail

PROBE_DIR="$(cd "$(dirname "$0")/cores3-probe-stepc" && pwd)"
PIO_ENV="${STACKCHAN_PROBE_PIO_ENV:-m5stack-cores3-probe-stepc}"
