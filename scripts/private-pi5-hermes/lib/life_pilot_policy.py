#!/usr/bin/env python3
"""Policy for the private daily-life Hermes pilot (D6-life)."""
...
class LifePilotPolicy:
    """Allow/deny rules for private life notes before worker automation exists."""
    ...
    @classmethod
    def from_mapping(cls, data: dict[str, Any]) -> LifePilotPolicy:
        ...
        phase = _str("phase")
        if phase != "life_pilot_v0":
            raise ValueError("phase must be life_pilot_v0")

        output_mode = _str("output_mode")
        if output_mode != "private_life_log":
            raise ValueError("output_mode must be private_life_log for D6-life")
        ...
        return cls(
            ...
            storage_root=_str("storage_root"),
            ...
            external_web_enabled=_bool("external_web_enabled"),
            homeassistant_enabled=_bool("homeassistant_enabled"),
...
@dataclass(frozen=True)
class LifePromptValidationResult:
    ok: bool
    reason: str = ""


def validate_life_pilot_document(data: dict[str, Any]) -> list[str]:
    """Return human-readable errors; empty means valid."""
    errors: list[str] = []
    try:
        policy = LifePilotPolicy.from_mapping(data)
    ...
    hard_gates = {
        ...
        "external_web_enabled": policy.external_web_enabled,
        "homeassistant_enabled": policy.homeassistant_enabled,
    }
    for key, value in hard_gates.items():
        if value:
            errors.append(f"{key} must be false for D6-life")
    ...
    if policy.storage_root != "/home/hermes/.hermes-life":
        errors.append("storage_root must be exactly /home/hermes/.hermes-life")

    return errors


def validate_life_prompt(
    prompt: str,
    policy: LifePilotPolicy,
    *,
    allow_empty: bool = False,
) -> LifePromptValidationResult:
    """Reject prompts that try to turn the life pilot into an executor."""
    text = (prompt or "").strip()
    if not text:
        if allow_empty:
            return LifePromptValidationResult(True, "ok")
        return LifePromptValidationResult(False, "empty prompt")
    if len(text) > policy.max_prompt_chars:
        return LifePromptValidationResult(
            False,
    ...
    for needle in policy.deny_prompt_substrings:
        if needle.lower() in lowered:
            return LifePromptValidationResult(False, f"prompt contains forbidden phrase: {needle!r}")
