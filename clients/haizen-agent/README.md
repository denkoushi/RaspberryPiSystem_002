...
## 分配番号 QR（製造 order と別スキャン）

設定 **`HAIZEN_DISTRIBUTION_MODE`** で認識ルールを切り替えます。

| 値 | 説明 |
|------|------|
| **`legacy_short_numeric`**（省略時既定） | 値が **1〜999 の整数**のみ（1〜3桁、先頭0なし）のスキャンを分配番号として扱う。**製造 order と誤認しうる** |
| **`prefixed_dist`**（別名 `prefixed` / `dist_prefix`） | **`DIST:` + 整数** のみを分配とみなす（例 `DIST:3`、`dist:12`。空白は `:` の後のみ許容）。短い数字のみのスキャンは **製造 order** |

その直後の製造 order スキャンにだけ `distributionNumber` として付与され、**消費後は未設定に戻ります**。

現場で誤認が問題になる場合は **`prefixed_dist`** とし、分配用 QR は **`DIST:<番号>`** に統一してください。
